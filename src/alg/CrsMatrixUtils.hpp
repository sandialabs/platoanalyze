...
template <typename Ordinal>
void print_matrix_to_file(typename Plato::CrsMatrix<Ordinal>::RowMapVectorT aRowBegin,
                          typename Plato::CrsMatrix<Ordinal>::OrdinalVectorT aColumns,
                          typename Plato::CrsMatrix<Ordinal>::ScalarVectorT aValues,
                          const std::string& aFileName);

template <typename Ordinal>
void write_matrix_to_binary_file(typename Plato::CrsMatrix<Ordinal>::RowMapVectorT aRowBegin,
                                 typename Plato::CrsMatrix<Ordinal>::OrdinalVectorT aColumns,
                                 typename Plato::CrsMatrix<Ordinal>::ScalarVectorT aValues,
                                 const std::string& aFileName);

template <typename Ordinal>
bool has_symmetric_sparsity_pattern(const typename Plato::CrsMatrix<Ordinal>::RowMapVectorT& aRowBegin,
...
template <typename Ordinal, typename Callable>
bool for_each_row_column(const RowVectorOnHost<Ordinal>& aRowBegin,
                         const OrdinalVectorOnHost<Ordinal>& aColumns,
                         const Callable& aCallable)
{
    static_assert(std::is_same<std::result_of_t<Callable(Ordinal, Ordinal)>, bool>::value,
                  "aCallable must be a function returning bool");
    for (int i = 0; i < aRowBegin.size() - 1; ++i)
    {
        const Ordinal tColsBegin = aRowBegin(i);
        const Ordinal tColsEnd = aRowBegin(i + 1);
...
CrsRowsColumnsValues<Ordinal> crs_matrix_non_block_form(const CrsMatrix<Ordinal>& aMatrix)
{
    ...
    {
        // if there were a version of this function that only fills values, could avoid copies of indices on subsequent
        // calls
                          ...
                          const std::string& aFileName)
{
    ...
    tFileStream << "rows = [";
    detail::for_each_row_column<Ordinal>(
        tRowBeginOnHost, tColumnsOnHost, tValuesOnHost,
    ...
    detail::for_each_row_column<Ordinal>(tRowBeginOnHost, tColumnsOnHost, tValuesOnHost,
                                         [&tFileStream](const Ordinal /*aRow*/, const Ordinal /*aColumn*/,
                                                        const Plato::Scalar aValue) { tFileStream << aValue << '\n'; });
    tFileStream << "];\n";
    tFileStream.close();
}
                                    ...
                                    const typename Plato::CrsMatrix<Ordinal>::OrdinalVectorT& aColumns)
{
    ...
    detail::for_each_row_column<Ordinal>(
        tRowBeginOnHost, tColumnsOnHost,
    ...
    // If tFoundEntries isn't empty, there was at least one upper triangular entry not matching a lower triangular
    // entry.
