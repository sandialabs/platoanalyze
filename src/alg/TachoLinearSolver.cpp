...
tachoSolver<SX>::tachoSolver(const int *solverParams) : m_numRows(0), m_Solver()
{
    setSolverParameters(solverParams);
}

template <typename SX>
tachoSolver<SX>::~tachoSolver()
{
    m_Solver.release();
}
...
template <typename SX>
void tachoSolver<SX>::Initialize(const int numRows,
...
void tachoSolver<SX>::Initialize(
    int numRows, size_type_array rowBegin, ordinal_type_array columns, value_type_array ax, const bool printTimings)
{
    m_numRows = numRows;
    if (m_numRows == 0) return;
...
template <typename SX>
void tachoSolver<SX>::MySolve(int NRHS, value_type_matrix b, value_type_matrix x)
{
    if (m_numRows == 0) return;
    ...
    {  // this workspace is allocated when NRHS is changed
        ...
        {
            m_TempRhs = value_type_array(Kokkos::ViewAllocateWithoutInitializing("m_TempRhs"), rhs_span);
        }
    }
    ...
    if (aSolverParams.isType<std::string>("Factorization Type"))
    {
        ...
        else
        {
            ANALYZE_THROWERR("Unknown factorization type " + factorizationType + "for Tacho solver." +
                             "Supported types are: 'Cholesky' (symmetric positive definite), 'LDLT' (symmetric "
                             "indefinite), and 'SymLU' (symmetric sparsity pattern but non-symmetric entries).");
        }
    }
...
TachoLinearSolver::TachoLinearSolver(const Teuchos::ParameterList &aSolverParams,
    ...
    : Plato::AbstractSolver(aSolverParams, aMPCs), mSolver(constructSolverFromParameterList(aSolverParams, aType))
{
}

void TachoLinearSolver::innerSolve(Plato::CrsMatrix<int> aA, Plato::ScalarVector aX, Plato::ScalarVector aB)
{
    ...
    if (!Plato::has_symmetric_sparsity_pattern<CrsOrdinal>(tRowBegin, tColumns))
    {
        throw std::runtime_error(
            "Tacho was given a matrix with a non-symmetric sparsity pattern.\n"
    ...
    {
        static int iter = 0;
        Plato::print_matrix_to_file<CrsOrdinal>(tRowBegin, tColumns, tValues,
