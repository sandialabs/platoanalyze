...
class WeightedLocalScalarFunction : public Plato::LocalScalarFunctionInc
{
   private:
    ...
    static constexpr auto mNumConfigDofsPerCell =
        mNumSpatialDims * mNumNodesPerCell; /*!< number of configuration degrees of freedom per element */
    static constexpr auto mNumGlobalDofsPerCell =
        PhysicsT::SimplexT::mNumDofsPerCell; /*!< number of global degrees of freedom per element */
    static constexpr auto mNumLocalDofsPerCell =
        PhysicsT::SimplexT::mNumLocalDofsPerCell; /*!< number of local degrees of freedom per element */
    ...
    std::vector<std::shared_ptr<Plato::LocalScalarFunctionInc>>
        mLocalScalarFunctionContainer; /*!< Vector of LocalScalarFunctionInc objects */
    ...
    void initialize(Teuchos::ParameterList &aInputParams)
    {
        if (aInputParams.sublist("Criteria").isSublist(mWeigthedSumFunctionName) == false)
        {
            const auto tError = std::string("UNKNOWN USER DEFINED SCALAR FUNCTION SUBLIST '") +
                                mWeigthedSumFunctionName + "'. USER DEFINED SCALAR FUNCTION SUBLIST '" +
                                mWeigthedSumFunctionName + "' IS NOT DEFINED IN THE INPUT FILE.";
        ...
        for (Plato::OrdinalType tFunctionIndex = 0; tFunctionIndex < tFunctionNames.size(); ++tFunctionIndex)
        {
            ...
            auto tScalarFunction =
                tFactory.create(mSpatialModel, mDataMap, aInputParams, tFunctionNames[tFunctionIndex]);
   ...
   public:
     ...
     **********************************************************************************/
    WeightedLocalScalarFunction(const Plato::SpatialModel &aSpatialModel,
     ...
     * \param [in] aDataMap  output database
     **********************************************************************************/
    WeightedLocalScalarFunction(const Plato::SpatialModel &aSpatialModel, Plato::DataMap &aDataMap)
     ...
     **********************************************************************************/
    void writeDiagnostics() { mWriteDiagnostics = true; }
     ...
     **********************************************************************************/
    void appendFunctionName(const std::string &aName) { mFunctionNames.push_back(aName); }
     ...
     **********************************************************************************/
    void appendFunctionWeight(const Plato::Scalar &aWeight) { mFunctionWeights.push_back(aWeight); }
     ...
     **********************************************************************************/
    std::string name() const override { return (mWeigthedSumFunctionName); }
                       ...
                       const Plato::TimeData &aTimeData) const override
    {
        ...
        for (Plato::OrdinalType tFunctionIndex = 0; tFunctionIndex < mLocalScalarFunctionContainer.size();
             tFunctionIndex++)
        {
                        ...
                        const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            ANALYZE_THROWERR(
                "DIMENSION MISMATCH: NUMBER OF LOCAL SCALAR FUNCTIONS DOES NOT MATCH THE NUMBER OF WEIGHTS")
        }
             ...
             tFunctionIndex++)
        {
            ...
            const auto tFunctionValue = mLocalScalarFunctionContainer[tFunctionIndex]->value(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
                                        ...
                                        const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            const auto tFunctionWeight = mFunctionWeights[tFunctionIndex];
            auto tFunctionGradZ = mLocalScalarFunctionContainer[tFunctionIndex]->gradient_z(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
                                        ...
                                        const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            const auto tFunctionWeight = mFunctionWeights[tFunctionIndex];
            auto tFunctionGradX = mLocalScalarFunctionContainer[tFunctionIndex]->gradient_x(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
                                        ...
                                        const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            const auto tFunctionWeight = mFunctionWeights[tFunctionIndex];
            auto tFunctionGradCurrentGlobalState = mLocalScalarFunctionContainer[tFunctionIndex]->gradient_u(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
            Plato::blas2::update(tFunctionWeight, tFunctionGradCurrentGlobalState, static_cast<Plato::Scalar>(1.0),
                                 tOutput);
        }
        return tOutput;
    }
                                         ...
                                         const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            const auto tFunctionWeight = mFunctionWeights[tFunctionIndex];
            auto tFunctionGradPreviousGlobalState = mLocalScalarFunctionContainer[tFunctionIndex]->gradient_up(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
            Plato::blas2::update(tFunctionWeight, tFunctionGradPreviousGlobalState, static_cast<Plato::Scalar>(1.0),
                                 tOutput);
        }
        return tOutput;
    }
                                        ...
                                        const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            const auto tFunctionWeight = mFunctionWeights[tFunctionIndex];
            auto tFunctionGradCurrentLocalState = mLocalScalarFunctionContainer[tFunctionIndex]->gradient_c(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
            Plato::blas2::update(tFunctionWeight, tFunctionGradCurrentLocalState, static_cast<Plato::Scalar>(1.0),
                                 tOutput);
        }
        return tOutput;
    }
                                         ...
                                         const Plato::TimeData &aTimeData) const override
    {
        ...
        {
            const auto tFunctionWeight = mFunctionWeights[tFunctionIndex];
            auto tFunctionGradPreviousLocalState = mLocalScalarFunctionContainer[tFunctionIndex]->gradient_cp(
                aCurrentGlobalState, aPreviousGlobalState, aCurrentLocalState, aPreviousLocalState, aControls,
                aTimeData);
            Plato::blas2::update(tFunctionWeight, tFunctionGradPreviousLocalState, static_cast<Plato::Scalar>(1.0),
                                 tOutput);
        }
        return tOutput;
    }
