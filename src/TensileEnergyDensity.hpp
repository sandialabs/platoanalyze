   ...
   public:
                                           ...
                                           const Plato::ScalarVectorT<ResultType>& aTensileEnergyDensity) const
    {
        ...
        for (Plato::OrdinalType tDim = 0; tDim < mNumSpatialDims; ++tDim)
        {
            ...
            {
                tTensileEnergyDensity += (aPrincipalStrains(tDim) * aPrincipalStrains(tDim) * aLameMu);
            }
        }
        StrainType tStrainTraceTensile = (tStrainTrace >= 0.0) ? tStrainTrace : static_cast<Plato::Scalar>(0.0);
        tTensileEnergyDensity +=
            (aLameLambda * tStrainTraceTensile * tStrainTraceTensile * static_cast<Plato::Scalar>(0.5));
