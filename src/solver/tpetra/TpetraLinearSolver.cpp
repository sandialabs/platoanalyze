...
**********************************************************************************/
TpetraSystem::TpetraSystem(int aNumNodes, Comm::Machine aMachine, int aDofsPerNode)
 ...
 **********************************************************************************/
Teuchos::RCP<Tpetra_Matrix> TpetraSystem::fromMatrix(Plato::CrsMatrix<Plato::OrdinalType> aInMatrix) const
{
 ...
 **********************************************************************************/
Teuchos::RCP<Tpetra_MultiVector> TpetraSystem::fromVector(const Plato::ScalarVector tInVector) const
{
 ...
 **********************************************************************************/
void TpetraSystem::toVector(Plato::ScalarVector& tOutVector, const Teuchos::RCP<Tpetra_MultiVector> tInVector) const
{
    Teuchos::TimeMonitor LocalTimer(*mVectorConversionTimer);
...
void TpetraLinearSolver::initialize()
{
    setupSolverOptions();
...
TpetraLinearSolver::TpetraLinearSolver(const Teuchos::ParameterList& aSolverParams,
                                       int aNumNodes,
...
template <typename T>
inline void TpetraLinearSolver::addDefaultToParameterList(Teuchos::ParameterList& aParams,
                                                          const std::string& aEntryName,
                                                          const T& aDefaultValue)
{
    if (!aParams.isType<T>(aEntryName)) aParams.set(aEntryName, aDefaultValue);
}

void TpetraLinearSolver::setupSolverOptions()
{
    ...
    if (mSolver == "gmres")
    {
        ...
        REPORT(
            "Tpetra using 'Pseudoblock GMRES' solver instead of user-specified 'GMRES' since matrix has block "
            "structure.")
    }
    else if (mSolver == "cg")
    {
    ...
    if (mSolverParams.isType<int>("Iterations")) tMaxIterations = mSolverParams.get<int>("Iterations");
    if (mSolverParams.isType<double>("Tolerance")) tTolerance = mSolverParams.get<double>("Tolerance");
    ...
    if (mSolver == "pseudoblock gmres")
        this->addDefaultToParameterList(mSolverOptions, "Num Blocks",
                                        tMaxIterations);  // This is the number of iterations between restarts
...
void TpetraLinearSolver::setupPreconditionerOptions()
{
    ...
    this->addDefaultToParameterList(mPreconditionerOptions, "number of equations",
                                    mDofsPerNode);  // Same as 'Block Size' above in solver options
...
void TpetraLinearSolver::belosSolve(Teuchos::RCP<const OP> A,
                                    Teuchos::RCP<MV> X,
 ...
 **********************************************************************************/
void TpetraLinearSolver::innerSolve(Plato::CrsMatrix<Plato::OrdinalType> aA,
