...
struct CurrentStates
{
    ...
    explicit CurrentStates(const std::shared_ptr<Plato::TimeData>& aInputTimeData)
        : mCurrentStepIndex(0), mTimeData(aInputTimeData)
    {
    }
    ...
    inline void print(const char my_string[]) const
    {
        printf("Printing CS %s Step %d : CPPG %10.4e , CG %10.4e , PG %10.4e , CL %10.4e , PL %10.4e\n\n", my_string,
               mCurrentStepIndex, Plato::blas1::norm(mProjectedPressGrad), Plato::blas1::norm(mCurrentGlobalState),
               Plato::blas1::norm(mPreviousGlobalState), Plato::blas1::norm(mCurrentLocalState),
...
inline Plato::NewtonRaphson::measure_t newton_raphson_stopping_criterion(const std::string& aInput)
{
    ...
    std::for_each(tCopy.begin(), tCopy.end(), [](char& aChar) { aChar = std::toupper(aChar); });
    ...
    else
    {
        ...
        tMsg << "Newton-Raphson Stopping Criterion '" << aInput.c_str()
             << "' is not defined as a valid stopping criterion. "
 ...
 **********************************************************************************/
inline void print_newton_raphson_stop_criterion(const Plato::NewtonRaphsonOutputData& aOutputData,
                                                std::ofstream& aOutputFile)
{
    ...
    switch (aOutputData.mStopingCriterion)
    {
        ...
        {
            aOutputFile << "\n\n****** Newton-Raphson solver stopping due to exceeding the maximum number of "
                           "iterations. ******\n\n";
            break;
        }
        case Plato::NewtonRaphson::NORM_MEASURE_TOLERANCE:
        {
            aOutputFile << "\n\n******  Newton-Raphson algorithm stopping due to the norm of the stopping measure "
                           "tolerance being met. ******\n\n";
            break;
        }
        ...
        {
            aOutputFile << "\n\n******  MAJOR FAILURE: Newton-Raphson algorithm stopping due to NaN norm values "
                           "detected. ******\n\n";
            break;
        }
 ...
 * \param [in,out] aOutputFile Newton-Raphson solver output file
 **********************************************************************************/
inline void print_newton_raphson_diagnostics(const Plato::NewtonRaphsonOutputData& aOutputData,
                                             std::ofstream& aOutputFile)
{
    if (aOutputData.mWriteOutput == false)
    {
 ...
 * \param [in,out] aOutputFile Newton-Raphson solver output file
 **********************************************************************************/
inline void print_newton_raphson_diagnostics_header(const Plato::NewtonRaphsonOutputData& aOutputData,
                                                    std::ofstream& aOutputFile)
{
    if (aOutputData.mWriteOutput == false)
    {
    ...
    aOutputFile << std::scientific << std::setprecision(6) << std::right << "Iter" << std::setw(13) << "Norm"
                << std::setw(22)
                << "Relative"
                   "\n"
                << std::flush;
...
inline void compute_relative_residual_norm_error(Plato::NewtonRaphsonOutputData& aOutputData)
{
    ...
    {
        aOutputData.mNormMeasure =
            std::abs(aOutputData.mReferenceNorm - aOutputData.mCurrentNorm) / std::abs(aOutputData.mCurrentNorm);
        aOutputData.mReferenceNorm = aOutputData.mCurrentNorm;
    }
}
