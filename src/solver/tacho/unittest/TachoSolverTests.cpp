...
double solutionError(
    const int numRows, const SX* rhs, const SX* sol, const int* rowBegin, const int* columns, const double* values)
{
...
template <class SX>
void makeNonSymmetric(const int numRows, const int* rowBegin, const int* columns, SX* values)
{
    for (int i = 0; i < numRows; i++)
    {
        for (int j = rowBegin[i]; j < rowBegin[i + 1]; j++)
...
template <class SX>
void sortColumns(const int numRows, const int* rowBegin, int* columns, SX* values)
{
...
template <class SX>
bool solveLinearSystems(
    tacho::tachoSolver<SX>& solver, Kokkos::Timer& timer, const int numRows, int* rowBegin, int* columns, SX* values)
{
    ...
    {  // randomize rhs
        ...
        srand(seed);
        for (int i = 0; i < numRows; ++i) rhs[i] = rand() / ((double)RAND_MAX + 1.0);
    }
    ...
    Kokkos::deep_copy(Kokkos::subview(b, Kokkos::ALL(), 0), Kokkos::View<SX*, tacho::device_type>(rhs.data(), numRows));
#else
    ...
    {
        std::cout << "tacho FAILS (rel Error = " << relativeError << ", tol = " << tol << std::endl;
    }
    else
    {
...
template <class SX>
bool testTachoSolver(int numRows, int* rowBegin, int* columns, SX* values, const int sym)
{
    Kokkos::Timer timer;
    //  Tacho options
