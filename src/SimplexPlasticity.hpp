...
class SimplexPlasticity : public Plato::Simplex<SpaceDim>
{
   public:
    ...
    static constexpr auto mNumDofsPerNode =
        mNumSpatialDims + 1; /*!< number of global degrees of freedom per node { disp_x, disp_y, disp_z, pressure} */
    ...
    static constexpr auto mNumDofsPerCell =
        mNumDofsPerNode * mNumNodesPerCell; /*!< number of global degrees of freedom per cell */
    static constexpr Plato::OrdinalType mNumLocalDofsPerCell =
        (SpaceDim == 3)
            ? 14
            : ((SpaceDim == 2)
                   ? 10
                   : (((SpaceDim == 1) ? 4 : 0))); /*!< number of local degrees of freedom per cell for J2-plasticity*/
    ...
    static constexpr auto mNumNodeStatePerNode =
        mNumSpatialDims; /*!< number of node states, i.e. pressure gradient, dofs per node */
    static constexpr auto mNumNodeStatePerCell =
        mNumNodeStatePerNode * mNumNodesPerCell; /*!< number of node states, i.e. pressure gradient, dofs per cell */
};
