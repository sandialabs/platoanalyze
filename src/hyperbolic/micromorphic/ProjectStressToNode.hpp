...
class ProjectStressToNode : ElementType
{
   private:
    ...
    Plato::Scalar
        mSkewScale[mNumFullTerms]; /*!< matrix with indices to scalr factors for skew terms with Voigt storage */
   ...
   public:
    ...
    {
        this->addSymmetricStressAtNodes(aCellOrdinal, aGpOrdinal, aOutput, aSymmetricMicroStress, aBasisFunctions,
                                        aVolume, 1.0);
        this->addSymmetricStressAtNodes(aCellOrdinal, aGpOrdinal, aOutput, aSymmetricMesoStress, aBasisFunctions,
                                        aVolume, -1.0);
   ...
   private:
    ...
    template <typename ProjectedScalarType, typename StressScalarType, typename VolumeScalarType>
    KOKKOS_INLINE_FUNCTION void addSymmetricStressAtNodes(
        ...
        Plato::Scalar aScale) const
    {
        for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < mNumNodesPerCell; tNodeIndex++)
        {
            for (Plato::OrdinalType tDofIndex = 0; tDofIndex < mNumFullTerms; tDofIndex++)
            {
                ...
                ProjectedScalarType tResult = aScale * aVolume(aCellOrdinal, aGpOrdinal) *
    ...
    template <typename ProjectedScalarType, typename StressScalarType, typename VolumeScalarType>
    KOKKOS_INLINE_FUNCTION void addSkewStressAtNodes(
        Plato::OrdinalType aCellOrdinal,
        Plato::OrdinalType aGpOrdinal,
        Plato::ScalarMultiVectorT<ProjectedScalarType> aOutput,
        const Plato::ScalarArray3DT<StressScalarType>& aStress,
        ...
        Plato::Scalar aScale) const
    {
        for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < mNumNodesPerCell; tNodeIndex++)
        {
            ...
            {
                Plato::OrdinalType tLocalOrdinal = tNodeIndex * mNumDofsPerNode + tDofIndex + DofOffset;
                ProjectedScalarType tResult = aScale * mSkewScale[tDofIndex] * aVolume(aCellOrdinal, aGpOrdinal) *
                                              aStress(aCellOrdinal, aGpOrdinal, mVoigtMap[tDofIndex]) *
                                              aBasisFunctions(tNodeIndex);
                Kokkos::atomic_add(&aOutput(aCellOrdinal, tLocalOrdinal), tResult);
            }
