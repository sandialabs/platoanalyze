...
class CubicLinearElasticMaterial : public MaterialModel<SpatialDim>
{
   ...
   private:
    void setRank4Tensor(const Teuchos::ParameterList& aParamList, const std::string& aName, const MatrixType aType)
    {
        ...
        else if (aParamList.isSublist(tExpressionName))
        {
            ...
            ANALYZE_THROWERR("ParameterList for Cubic Micromorphic Elastic Material must have name " + tListName +
                             " or " + tExpressionName)
    }
};
