...
class NewmarkIntegrator
{
   ...
   public:
    explicit NewmarkIntegrator(const Teuchos::ParameterList& aParams, Plato::Scalar aMaxEigenvalue)
    ...
    bool isConditionallyStable() const { return mBeta < mGamma / 2.0; }

    decltype(mNumSteps) getNumSteps() const { return mNumSteps; }

    decltype(mTimeStep) getTimeStep() const { return mTimeStep * mTimeStepScale; }
    ...
    virtual Plato::ScalarVector u_value(const Plato::ScalarVector& aU,
    ...
    virtual Plato::ScalarVector v_value(const Plato::ScalarVector& aU,
                                        const Plato::ScalarVector& aU_prev,
                                        const Plato::ScalarVector& aV,
                                        const Plato::ScalarVector& aV_prev,
                                        const Plato::ScalarVector& aA,
    ...
    virtual Plato::ScalarVector a_value(const Plato::ScalarVector& aU,
                                        const Plato::ScalarVector& aU_prev,
                                        const Plato::ScalarVector& aV,
                                        const Plato::ScalarVector& aV_prev,
                                        const Plato::ScalarVector& aA,
...
class NewmarkIntegratorUForm : public NewmarkIntegrator
{
   ...
   public:
    explicit NewmarkIntegratorUForm(const Teuchos::ParameterList& aParams, Plato::Scalar aMaxEigenvalue)
    ...
    Plato::Scalar v_grad_u(Plato::Scalar aTimeStep) override { return -mGamma / (mBeta * aTimeStep); }

    Plato::Scalar v_grad_u_prev(Plato::Scalar aTimeStep) override { return mGamma / (mBeta * aTimeStep); }

    Plato::Scalar v_grad_v_prev(Plato::Scalar aTimeStep) override { return mGamma / mBeta - 1.0; }

    Plato::Scalar v_grad_a_prev(Plato::Scalar aTimeStep) override { return (mGamma / (2.0 * mBeta) - 1.0) * aTimeStep; }

    Plato::Scalar a_grad_u(Plato::Scalar aTimeStep) override { return -1.0 / (mBeta * aTimeStep * aTimeStep); }

    Plato::Scalar a_grad_u_prev(Plato::Scalar aTimeStep) override { return 1.0 / (mBeta * aTimeStep * aTimeStep); }

    Plato::Scalar a_grad_v_prev(Plato::Scalar aTimeStep) override { return 1.0 / (mBeta * aTimeStep); }

    Plato::Scalar a_grad_a_prev(Plato::Scalar aTimeStep) override { return 1.0 / (2.0 * mBeta) - 1.0; }

    Plato::ScalarVector u_value(const Plato::ScalarVector& aU,
                                const Plato::ScalarVector& aU_prev,
                                const Plato::ScalarVector& aV,
                                const Plato::ScalarVector& aV_prev,
                                const Plato::ScalarVector& aA,
    ...
    {
        return Plato::ScalarVector();
    }

    Plato::ScalarVector v_value(const Plato::ScalarVector& aU,
                                const Plato::ScalarVector& aU_prev,
                                const Plato::ScalarVector& aV,
                                const Plato::ScalarVector& aV_prev,
                                const Plato::ScalarVector& aA,
                                ...
                                Plato::Scalar dt) override
    {
        ...
        auto tBeta = mBeta;
        Kokkos::parallel_for(
            "Velocity residual value", Kokkos::RangePolicy<>(0, tNumData),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
                Plato::Scalar tPredV = aV_prev(aOrdinal) + (1.0 - tGamma) * dt * aA_prev(aOrdinal);
                Plato::Scalar tPredU = aU_prev(aOrdinal) + dt * aV_prev(aOrdinal) +
                                       dt * dt / 2.0 * (1.0 - 2.0 * tBeta) * aA_prev(aOrdinal);
    ...
    Plato::ScalarVector a_value(const Plato::ScalarVector& aU,
                                const Plato::ScalarVector& aU_prev,
                                const Plato::ScalarVector& aV,
                                const Plato::ScalarVector& aV_prev,
                                const Plato::ScalarVector& aA,
...
class NewmarkIntegratorAForm : public NewmarkIntegrator
{
   ...
   public:
    explicit NewmarkIntegratorAForm(const Teuchos::ParameterList& aParams, Plato::Scalar aMaxEigenvalue)
        : NewmarkIntegrator(aParams, aMaxEigenvalue)
    {
    }
    ...
    Plato::Scalar v_grad_a(Plato::Scalar aTimeStep) override { return -mGamma * aTimeStep; }

    Plato::Scalar u_grad_a(Plato::Scalar aTimeStep) override { return -mBeta * aTimeStep * aTimeStep; }
                                ...
                                Plato::Scalar dt) override
    {
        ...
        auto tBeta = mBeta;
        Kokkos::parallel_for(
            "Displacement residual value", Kokkos::RangePolicy<>(0, tNumData),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
                Plato::Scalar tPredU = aU_prev(aOrdinal) + dt * aV_prev(aOrdinal) +
                                       dt * dt / 2.0 * (1.0 - 2.0 * tBeta) * aA_prev(aOrdinal);
