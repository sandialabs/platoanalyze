...
void ContactSurface::initialize(const Teuchos::ParameterList& aParams, Plato::Mesh aMesh)
{
    ...
    if (!aParams.isType<std::string>("Parent Block")) ANALYZE_THROWERR("Parent Block was not provided in contact pair")

    mParentBlock = aParams.get<std::string>("Parent Block");
}

void ContactSurface::addParentData(const Plato::OrdinalVector& aParentElements,
...
Plato::OrdinalVector ContactSurface::parentElements() const
{
    if (!mHasParentData)
...
Plato::OrdinalVector ContactSurface::elementWiseChildMap() const
{
    if (!mHasParentData)
        ANALYZE_THROWERR("In ContactSurface class: Attempting to access parent data before it is assigned.")
...
Plato::ScalarMultiVector ContactSurface::mappedChildNodeLocations() const
{
    if (!mHasParentData)
        ANALYZE_THROWERR("In ContactSurface class: Attempting to access parent data before it is assigned.")
...
Plato::OrdinalType count_total_child_nodes(const std::vector<ContactPair>& aPairs)
{
    ...
    for (auto tPair : aPairs) tNum += tPair.surfaceA.childNodes().size() + tPair.surfaceB.childNodes().size();
                                  ...
                                  Plato::OrdinalVector& aParentElements)
{
    ...
    for (auto tPair : aPairs)
    {
        ...
        Kokkos::parallel_for(
            "store child nodes and parent elements", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumNodes),
...
void check_for_repeated_child_nodes(const Plato::OrdinalVector& aChildNodes, Plato::OrdinalType aNumMeshNodes)
{
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, aNumMeshNodes),
