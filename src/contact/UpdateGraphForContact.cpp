...
void UpdateGraphForContact::createNodeNodeGraph(const Plato::OrdinalVector& aChildNodes,
...
void UpdateGraphForContact::NodeNodeGraph(Plato::OrdinalVector& aOffsetMap, Plato::OrdinalVector& aNodeOrds) const
{
    aOffsetMap = mFullOffsetMap;
    aNodeOrds = mFullNodeOrds;
}

void UpdateGraphForContact::NodeNodeGraphTranspose(Plato::OrdinalVector& aOffsetMap,
...
Plato::OrdinalType UpdateGraphForContact::extractChildNodeOffsets(const Plato::OrdinalVector& aChildNodes)
{
    ...
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<>(0, tNumChildNodes),
...
void UpdateGraphForContact::storeUniqueParentNodeContributions(const Plato::OrdinalVector& aChildNodes,
                                                               const Plato::OrdinalVector& aParentElements)
{
    auto tNumChildNodes = aChildNodes.size();
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumChildNodes), KOKKOS_LAMBDA(Plato::OrdinalType iChildNode) {
            ...
            for (Plato::OrdinalType iOrd = tFrom; iOrd < tTo; iOrd++)
            {
                ...
                if (tOutput >= 0)
                {
                    ...
                    for (Plato::OrdinalType tElemLocalNodeOrd = 0; tElemLocalNodeOrd < tNumNodesPerElement;
                         tElemLocalNodeOrd++)
                    {
                        ...
                        {
                            if (tAllGraphOrdinals(tFatGraphOffset + tIndex) == tNodeOrd) isUnique = false;
                        }
                        if (isUnique)
                        {
...
Plato::OrdinalType UpdateGraphForContact::updateOffsetMap()
{
...
void UpdateGraphForContact::updateNodeOrds()
{
    auto tNumTotalNodes = mFullOffsetMap.size() - 1;
    auto tNumNodesPerElement = mNumNodesPerElement;
    auto tOffsetMap = mOffsetMap;
    ...
    Kokkos::parallel_for(
        "node ordinals accounting for contact", Kokkos::RangePolicy<>(0, tNumTotalNodes),
...
void UpdateGraphForContact::countNonzerosForTranspose(Plato::OrdinalVector& aOffsetMap) const
{
    auto tFullOffsetMap = mFullOffsetMap;
    auto tFullNodeOrds = mFullNodeOrds;
    ...
    Kokkos::parallel_for(
        "nonzeros", Kokkos::RangePolicy<OrdinalType>(0, tNumTotalNodes), KOKKOS_LAMBDA(OrdinalType iNodeOrdinal) {
...
Plato::OrdinalType UpdateGraphForContact::constructTransposeOffsetMap(Plato::OrdinalVector& aOffsetMap) const
{
    Plato::OrdinalType tNumTotalNodes = aOffsetMap.size() - 1;
    Plato::OrdinalType tNumEntries(0);
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<OrdinalType>(0, tNumTotalNodes + 1),
...
void UpdateGraphForContact::constructTransposeNodeOrds(const Plato::OrdinalVector& aOffsetMap,
                                                       Plato::OrdinalVector& aNodeOrds) const
{
    auto tFullOffsetMap = mFullOffsetMap;
    auto tFullNodeOrds = mFullNodeOrds;
    ...
    Kokkos::parallel_for(
        "node ords", Kokkos::RangePolicy<OrdinalType>(0, tNumTotalNodes), KOKKOS_LAMBDA(OrdinalType iNodeOrdinal) {
            auto tFrom = tFullOffsetMap(iNodeOrdinal);
            auto tTo = tFullOffsetMap(iNodeOrdinal + 1);
