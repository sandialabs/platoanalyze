...
class IntegrateContactForce
{
   ...
   public:
                    ...
                    Plato::Scalar aTimeStep) const
    {
        ...
        Plato::ScalarArray3DT<StateScalarType> tSurfaceDisplacement("displacement on contact surface", tNumFaces,
                                                                    tNumPoints, NumDofsPerNode);
        (*mComputeSurfaceDisplacement)(mElementOrds, aState, tSurfaceDisplacement);

        Plato::ScalarArray3DT<ResultScalarType> tContactForce("contact force at cubature points", tNumFaces, tNumPoints,
                                                              NumDofsPerNode);
                ...
                for (Plato::OrdinalType tNode = 0; tNode < ElementType::mNumNodesPerFace; tNode++)
                {
                    for (Plato::OrdinalType tDof = 0; tDof < NumDofsPerNode; tDof++)
                    {
                        ...
                        ResultScalarType tResult =
                            tSurfaceArea * tBasisValues(tNode) * tContactForce(iCellOrdinal, iGPOrdinal, tDof);
