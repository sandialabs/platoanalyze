 ...
 **********************************************************************************/
inline void print_standard_vector_1D(const std::vector<Plato::Scalar>& aInput, std::string aName = "Data")
{
...
KOKKOS_INLINE_FUNCTION void print_array_1D_device(const ArrayT& aInput, const char* aName)
{
    printf("BEGIN PRINT: %s\n", aName);
    Plato::OrdinalType tSize = aInput.size();
    for (decltype(tSize) tIndex = 0; tIndex < tSize; tIndex++)
 ...
 **********************************************************************************/
template <typename ArrayT>
KOKKOS_INLINE_FUNCTION void print_array_2D_device(const Plato::OrdinalType& aLeadOrdinal,
                                                  const ArrayT& aInput,
                                                  const char* aName)
{
 ...
 **********************************************************************************/
template <typename ArrayT>
KOKKOS_INLINE_FUNCTION void print_array_3D_device(const Plato::OrdinalType& aLeadOrdinal,
                                                  const ArrayT& aInput,
                                                  const char* aName)
{
...
inline void print_array_ordinals_1D(const Plato::OrdinalVector& aInput, std::string aName = "")
{
    ...
    Plato::OrdinalType tSize = aInput.size();
    Kokkos::parallel_for(
        "print array ordinals 1D", Kokkos::RangePolicy<>(0, tSize), KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
#ifdef PLATOANALYZE_LONG_LONG_ORDINALTYPE
 ...
 **********************************************************************************/
inline void print_sparse_matrix_to_file(Teuchos::RCP<Plato::CrsMatrixType> aInMatrix,
                                        std::string aFilename = "matrix.txt")
{
    FILE* tOutputFile;
        ...
        for (auto iColMapEntryIndex = tFrom; iColMapEntryIndex < tTo; iColMapEntryIndex++)
        {
            ...
            for (Plato::OrdinalType iLocalRowIndex = 0; iLocalRowIndex < tNumRowsPerBlock; iLocalRowIndex++)
            {
                ...
                for (Plato::OrdinalType iLocalColIndex = 0; iLocalColIndex < tNumColsPerBlock; iLocalColIndex++)
                {
                    ...
                    auto tSparseIndex =
                        iColMapEntryIndex * tBlockSize + iLocalRowIndex * tNumColsPerBlock + iLocalColIndex;
#ifdef PLATOANALYZE_LONG_LONG_ORDINALTYPE
...
inline void print(const ArrayT& aInput, std::string aName = "")
{
    ...
    Plato::OrdinalType tSize = aInput.size();
    Kokkos::parallel_for(
        "print 1D array", Kokkos::RangePolicy<>(0, tSize), KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
#ifdef PLATOANALYZE_LONG_LONG_ORDINALTYPE
            printf("X[%lld] = %e\n", aIndex, aInput(aIndex));
#else
        printf("X[%d] = %e\n", aIndex, aInput(aIndex));
...
inline void print_array_2D(const ArrayT& aInput, const std::string& aName)
{
    ...
    Kokkos::parallel_for(
        "print 2D array", Kokkos::RangePolicy<>(0, tNumRows), KOKKOS_LAMBDA(const Plato::OrdinalType& aRow) {
                               ...
                               std::string aName = "")
{
    printf("\nBEGIN PRINT: %s\n", aName.c_str());
    Kokkos::parallel_for(
        "print 2D array Fad", Kokkos::RangePolicy<>(0, aNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& aCell) {
...
inline void print_array_3D(const ArrayT& aInput, const std::string& aName)
{
    ...
    Kokkos::parallel_for(
        "print 3D array", Kokkos::RangePolicy<>(0, tNumMatrices), KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
...
inline void print_fad_val_values(const Plato::ScalarMultiVectorT<ViewType>& aInput, const std::string& aName)
{
    std::cout << "\nSTART: Print ScalarMultiVectorT '" << aName << "'.\n";
    const auto tLenghtDim1 = aInput.extent(0);
    const auto tLenghtDim2 = aInput.extent(1);
    Kokkos::parallel_for(
        "print_fad_val_values", Kokkos::RangePolicy<>(0, tLenghtDim1),
 ...
 **********************************************************************************/
template <typename ViewType>
inline void print_fad_val_values(const Plato::ScalarVectorT<ViewType>& aInput, const std::string& aName)
{
    std::cout << "\nStart: Print ScalarVector '" << aName << "'.\n";
    const auto tLength = aInput.extent(0);
    Kokkos::parallel_for(
        "print_fad_val_values", Kokkos::RangePolicy<>(0, tLength), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
 ...
 * \param [in] aName  name used to identify 1D view
 **********************************************************************************/
template <Plato::OrdinalType NumNodesPerCell, Plato::OrdinalType NumDofsPerNode, typename ViewType>
inline void print_fad_dx_values(const Plato::ScalarVectorT<ViewType>& aInput, const std::string& aName)
{
    std::cout << "\nStart: Print ScalarVector '" << aName << "'.\n";
    const auto tLength = aInput.extent(0);
    Kokkos::parallel_for(
        "print_fad_dx_values", Kokkos::RangePolicy<>(0, tLength), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
            for (Plato::OrdinalType tNode = 0; tNode < NumNodesPerCell; tNode++)
            {
                for (Plato::OrdinalType tDof = 0; tDof < NumDofsPerNode; tDof++)
                {
                    printf("Input(Cell=%d,Node=%d,Dof=%d) = %f\n", aOrdinal, tNode, tDof,
                           aInput(aOrdinal).dx(tNode * NumDofsPerNode + tDof));
                }
            }
        });
    std::cout << "End: Print ScalarVector '" << aName << "'.\n";
