...
{
    Kokkos::parallel_for(
        "PlatoDriver::copy", Kokkos::RangePolicy<>(0, aNumVertices), KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
...
inline void copy_2Dview_to_write(const Plato::ScalarMultiVector& aInput, Omega_h::Write<Omega_h::Real>& aOutput)
{
    ...
    Kokkos::parallel_for(
        "PlatoDriver::compress_copy_2Dview_to_write", Kokkos::RangePolicy<>(0, tNumMajorEntries),
        KOKKOS_LAMBDA(const Plato::OrdinalType& tMajorIndex) {
...
inline void copy_1Dview_to_write(const Plato::ScalarVector& aInput, Omega_h::Write<Omega_h::Real>& aOutput)
{
    ...
    Kokkos::parallel_for(
        "PlatoDriver::compress_copy_1Dview_to_write", Kokkos::RangePolicy<>(0, tNumEntries),
        KOKKOS_LAMBDA(const Plato::OrdinalType& tIndex) { aOutput[tIndex] = aInput(tIndex); });
}
...
inline Omega_h::LOs copy(const ScalarVectorT<ViewType>& aInput)
{
    ...
    Kokkos::parallel_for(
        "copy", Kokkos::RangePolicy<>(0, tLength),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { tWrite[aOrdinal] = aInput(aOrdinal); });
...
inline ScalarVectorT<ViewType> copy(const Omega_h::LOs& aInput)
{
    ...
    Kokkos::parallel_for(
        "copy", Kokkos::RangePolicy<>(0, tLength),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { tOutput(aOrdinal) = aInput[aOrdinal]; });
    return tOutput;
}
 ...
 **********************************************************************************/
template <typename ViewType>
void print(const ViewType& aInput, const std::string& aName)
{
    std::cout << "Start Printing Array with Name '" << aName << "'\n";
    auto tLength = aInput.size();
    Kokkos::parallel_for(
        "print", Kokkos::RangePolicy<>(0, tLength),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { printf("Array(%d)=%d\n", aOrdinal, aInput[aOrdinal]); });
 ...
 **********************************************************************************/
inline std::string get_entity_name(const Omega_h::Int aEntityDim)
{
    std::unordered_map<Omega_h::Int, std::string> tMap = {{0, "VERTEX"}, {1, "EDGE"}, {2, "FACE"}, {3, "REGION"}};
    auto tItr = tMap.find(aEntityDim);
    if (tItr == tMap.end())
    {
        ANALYZE_THROWERR(
            std::string("Entity dimension '") + std::to_string(aEntityDim) + "' is not supported. " +
 ...
 * \return field data
 **********************************************************************************/
inline Plato::ScalarVector read_metadata_from_mesh(const Omega_h::Mesh& aMesh,
                                                   ...
                                                   const std::string& aTagName)
{
    if (aMesh.has_tag(aEntityDim, aTagName) == false)
    {
        ...
        ANALYZE_THROWERR(std::string("Tag '") + aTagName + "' with entity dimension '" + tEntityName +
                         "' is not defined in mesh.")
    }
    ...
    Kokkos::parallel_for(
        "copy read array into output array", Kokkos::RangePolicy<>(0, tSize),
        KOKKOS_LAMBDA(const Plato::OrdinalType& tIndex) { tOutput(tIndex) = tData[tIndex]; });
    return tOutput;
}
 ...
 **********************************************************************************/
inline std::vector<Omega_h::filesystem::path> read_pvtu_file_paths(const std::string& aPvdDir)
{
...
inline Omega_h::LOs entity_ordinals(const Omega_h::MeshSets& aMeshSets, const std::string& aSetName, bool aThrow = true)
{
    ...
    {
        ANALYZE_THROWERR(std::string("DID NOT FIND NODE SET WITH NAME '") + aSetName + "'. NODE SET '" + aSetName +
                         "' IS NOT DEFINED IN INPUT MESH FILE, I.E. INPUT EXODUS FILE");
    }
 ...
 **********************************************************************************/
template <Omega_h::SetType EntitySet>
inline bool is_entity_set_defined(const Omega_h::MeshSets& aMeshSets, const std::string& aSetName)
{
 ...
 **********************************************************************************/
template <Omega_h::SetType EntitySet>
inline Omega_h::LOs get_entity_ordinals(const Omega_h::MeshSets& aMeshSets,
 ...
 **********************************************************************************/
inline Plato::OrdinalType get_num_entities(const Omega_h::Int aEntityDim, const Omega_h::Mesh& aMesh)
{
    ...
    {
        ANALYZE_THROWERR(
            std::string("Entity with dimension id '") + std::to_string(aEntityDim) + " is not supported. " +
 ...
 **********************************************************************************/
template <Omega_h::Int EntityDim, Omega_h::SetType EntitySet>
inline Omega_h::LOs find_entities_on_non_prescribed_boundary(const std::vector<std::string>& aEntitySetNames,
                                                             Omega_h::Mesh& aMesh,
                                                             Omega_h::MeshSets& aMeshSets)
{
    ...
    for (auto& tEntitySetName : aEntitySetNames)
    {
        ...
        tEntitiesAreOnNonPrescribedBoundary =
            Omega_h::land_each(tEntitiesAreOnNonPrescribedBoundary, tEntitiesAreNotOnPrescribedBoundary);
    }
 ...
 **********************************************************************************/
template <Omega_h::Int EntityDim, Omega_h::SetType EntitySet>
inline Omega_h::LOs get_boundary_entities(const std::vector<std::string>& aEntitySetNames,
                                          Omega_h::Mesh& aMesh,
                                          Omega_h::MeshSets& aMeshSets)
{
    ...
    auto tBoundaryEntities = Plato::omega_h::find_entities_on_non_prescribed_boundary<EntityDim, EntitySet>(
        aEntitySetNames, aMesh, aMeshSets);
    return tBoundaryEntities;
}
...
inline void read_fields(const Omega_h::Mesh& aMesh,
 ...
 * \param [in] aFaceOrdinal  face ordinal, i.e. \f$ i\in\{0,n_{f}\} \f$, where \f$ n_f \f$ is the number of faces on the
 *element. \param [in] aCoords       node coordinates
 *
 * \return unit normal vector
 *
 **********************************************************************************/
KOKKOS_INLINE_FUNCTION Omega_h::Vector<1> unit_normal_vector(const Plato::OrdinalType& aCellOrdinal,
 ...
 *
 **********************************************************************************/
KOKKOS_INLINE_FUNCTION Omega_h::Vector<2> unit_normal_vector(const Plato::OrdinalType& aCellOrdinal,
                                                             const Plato::OrdinalType& aFaceOrdinal,
 ...
 *
 **********************************************************************************/
KOKKOS_INLINE_FUNCTION Omega_h::Vector<3> unit_normal_vector(const Plato::OrdinalType& aCellOrdinal,
                                                             const Plato::OrdinalType& aFaceOrdinal,
 ...
 **********************************************************************************/
template <class Type>
Omega_h::Write<Type> create_omega_h_write_array(std::string aName, Plato::OrdinalType aEntryCount)
{
...
inline void output_node_state_to_viz_file(const Plato::ScalarMultiVector& aField,
                                          ...
                                          Omega_h::vtk::Writer& aWriter)
{
    for (Plato::OrdinalType tSnapshot = 0; tSnapshot < aField.extent(0); tSnapshot++)
    {
        ...
        aMesh.add_tag(Omega_h::VERT, aFieldName.c_str(), NumDofsPerNode,
                      Omega_h::Reals(Omega_h::Write<Omega_h::Real>(tSubView)));
...
inline void output_element_state_to_viz_file(const Plato::OrdinalType& aNumSnapShots,
