...
class GeometryMisfit : public EvaluationType::ElementType,
...
/******************************************************************************/
{
   ...
   public:
    ...
    GeometryMisfit(const plato::domain::SpatialDomain& aSpatialDomain,
        ...
        : FunctionBaseType(aSpatialDomain, aDataMap, aFunctionParams, aFunctionName), mPointCloudName("")
    /**************************************************************************/
    {
    ...
    /**************************************************************************/
    void evaluate_conditional(const Plato::ScalarMultiVectorT<ControlScalarType>& aControl,
    ...
    /**************************************************************************/
    {
        ...
        Kokkos::parallel_for(
            "compute misfit", Kokkos::RangePolicy<>(0, tNumPoints),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aPointOrdinal) {
                // get face index
        ...
        Kokkos::parallel_for(
            "divide and apply weight", Kokkos::RangePolicy<>(0, tNumFaces),
   ...
   public:
    void faceCentroids(Plato::Mesh aMesh, Plato::ScalarMultiVector aCentroids)
    {
        ...
        Kokkos::parallel_for(
            "compute centroids", Kokkos::RangePolicy<>(0, tNumFaces),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aFaceOrdinal) {
                for (Plato::OrdinalType tNodeI = 0; tNodeI < mNumNodesPerFace; tNodeI++)
                {
                    ...
                    auto tMeshLocalVertexOrdinal =
                        tConnectivity(tElementOrds(aFaceOrdinal) * cNodesPerElement + tElemLocalVertexOrdinal);
                    for (Plato::OrdinalType tDimI = 0; tDimI < mNumSpatialDims; tDimI++)
                    {
                        aCentroids(tDimI, aFaceOrdinal) +=
                            tCoords(tMeshLocalVertexOrdinal * mNumSpatialDims + tDimI) / mNumNodesPerFace;
                    }
                }
            });
    }

    void createPointGraph(Plato::Mesh aMesh)
    {
        ...
        if (mDataMap.ordinalVectors.count(mPointCloudRowMapName) == 0)
        {
            ...
            ArborX::BVH<Plato::MemSpace> bvh{
                tExecSpace, Plato::GMF::Points{prim_x.data(), prim_y.data(), prim_z.data(), tNumPrimitives}};
            ...
            ArborX::query(bvh, tExecSpace,
                          Plato::GMF::Points{pred_x.data(), pred_y.data(), pred_z.data(), tNumPredicates}, tIndices,
                          tOffsets);
    ...
    void writeSearchResults(const Plato::ScalarMultiVector& aPoints,
                            ...
                            const Plato::ScalarVectorT<Plato::OrdinalType>& aIndices)
    {
        ...
        Kokkos::parallel_for(
            "vectors", Kokkos::RangePolicy<>(0, tNumPoints), KOKKOS_LAMBDA(const Plato::OrdinalType& aPointOrdinal) {
    ...
    void writeVTKHeader(std::ofstream& aOutFile)
    {
    ...
    void toVTK(std::string aBaseName, const Plato::ScalarMultiVector& aPoints)
    {
    ...
    void writeToStream(std::ostream& aOutFile, const Plato::ScalarMultiVector& aEntries)
    {
    ...
    void toVTK(std::string aBaseName, const Plato::ScalarMultiVector& aPoints, const Plato::ScalarMultiVector& aVectors)
    {
        std::ofstream tOutFile;
        tOutFile.open(aBaseName + ".vtk");
    ...
    void parsePointCloud(Teuchos::ParameterList& aFunctionParams)
    {
    ...
    inline std::vector<Plato::Scalar> readPoint(std::string tLineIn)
    {
    ...
    std::vector<std::vector<Plato::Scalar>> readPointsFromFile(std::string aFileName)
    {
    ...
    Plato::ScalarMultiVector scalarMultiVectorFromData(std::vector<std::vector<Plato::Scalar>> aPoints)
    {
