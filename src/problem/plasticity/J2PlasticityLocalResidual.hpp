...
class J2PlasticityLocalResidual : public Plato::AbstractLocalVectorFunctionInc<EvaluationType>
{
   ...
   public:
                          ...
                          const Plato::TimeData& aTimeData) const
    {
        ...
        Kokkos::parallel_for(
            "Compute cell local residuals", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                tThermoPlasticityUtils.computeElasticStrain(aCellOrdinal, aGlobalState, aLocalState, tBasisFunctions,
                                                            tTotalStrain, tElasticStrain);
                ...
                // compute eta = (deviatoric_stress - backstress) ... and its norm ... the normalized version is the
                // yield surface normal
                tJ2PlasticityUtils.computeDeviatoricStressMinusBackstressNormalized(
                    aCellOrdinal, tDeviatoricStress, aLocalState, tYieldSurfaceNormal, tDevStressMinusBackstressNorm);
                    ...
                    0.0)  // -> yielding (assumes local state already updated)
                {
                    ...
                    tJ2PlasticityUtils.fillBackstressTensorResidualPlasticStep(
                        aCellOrdinal, tPenalizedHardeningModulusKinematic, aLocalState, aPrevLocalState,
                                  ...
                                  const Plato::TimeData& aTimeData) const
    {
        ...
        Kokkos::parallel_for(
            "Update local state dofs", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                tComputeGradient(aCellOrdinal, tGradient, aConfig, tCellVolume);
                ...
                Plato::Scalar tYieldStress =
                    tPenalizedInitialYieldStress + tPenalizedHardeningModulusIsotropic * aLocalState(aCellOrdinal, 0);

                // compute the yield function at the trial state
                Plato::Scalar tTrialStateYieldFunction =
                    tSqrt3Over2 * tDevStressMinusBackstressNorm(aCellOrdinal) - tYieldStress;
                ...
                if (tTrialStateYieldFunction > static_cast<Plato::Scalar>(1.0e-10))  // plastic step
                {
                    ...
                    aLocalState(aCellOrdinal, 1) =
                        ...
                        (static_cast<Plato::Scalar>(3.0) * tPenalizedShearModulus +
                    ...
                    tJ2PlasticityUtils.updatePlasticStrainAndBackstressPlasticStep(
                        aCellOrdinal, aPrevLocalState, tYieldSurfaceNormal, tPenalizedHardeningModulusKinematic,
                        aLocalState);
                }
            });
    }
