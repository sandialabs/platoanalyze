...
#include "mesh/Plato_TopOptFunctors.hpp"
...
class J2PlasticityLocalResidual : public Plato::AbstractLocalVectorFunctionInc<EvaluationType>
{
   private:
    static constexpr auto mSpaceDim = EvaluationType::SpatialDim; /*!< spatial dimensions */

    static constexpr auto mNumDofsPerNode =
        SimplexPhysicsType::mNumDofsPerNode; /*!< number global degrees of freedom per node */
    ...
    static constexpr auto mNumLocalDofsPerCell =
        SimplexPhysicsType::mNumLocalDofsPerCell; /*!< number of local degrees of freedom */
    ...
    using GlobalStateT =
        typename EvaluationType::StateScalarType; /*!< global state variables automatic differentiation type */
    using PrevGlobalStateT =
        typename EvaluationType::PrevStateScalarType; /*!< global state variables automatic differentiation type */
    using LocalStateT =
        typename EvaluationType::LocalStateScalarType; /*!< local state variables automatic differentiation type */
    using PrevLocalStateT =
        typename EvaluationType::PrevLocalStateScalarType; /*!< local state variables automatic differentiation type */
    using ControlT =
        typename EvaluationType::ControlScalarType;            /*!< control variables automatic differentiation type */
    ...
    Plato::Scalar mAdditiveContinuationElasticProperties;    /*!< continuation parameter: multiplier on SIMP penalty for
                                                                elastic properties */
    Plato::Scalar mUpperBoundOnElasticPropertiesPenaltySIMP; /*!< continuation parameter: upper bound on SIMP penalty
    ...
    Plato::Scalar mAdditiveContinuationPlasticProperties;    /*!< continuation parameter: multiplier on SIMP penalty for
                                                                plastic properties */
    Plato::Scalar mUpperBoundOnPlasticPropertiesPenaltySIMP; /*!< continuation parameter: upper bound on SIMP penalty
                                                                for plastic properties */
    ...
    void initializeIsotropicElasticMaterial(Teuchos::ParameterList& aMaterialParams)
    {
        ...
        if (aMaterialParams.isSublist("Isotropic Linear Elastic"))
        {
            ...
            mElasticShearModulus = tElasticModulus / (static_cast<Plato::Scalar>(2.0) *
                                                      (static_cast<Plato::Scalar>(1.0) + tPoissonsRatio));
        }
        ...
        else
        {
            auto tMaterialName = mSpatialDomain.getMaterialName();
            std::stringstream ss;
            ss << "'Isotropic Linear Elastic' or 'Isotropic Linear Thermoelastic' sublist of '" << tMaterialName
               << "' does not exist.";
    ...
    void initializeJ2Plasticity(Teuchos::ParameterList& aInputParams)
    {
        ...
        if (mPlasticityParamList.isSublist("J2 Plasticity"))
        {
            ...
            mElasticPropertiesPenaltySIMP =
                tJ2PlasticitySubList.get<Plato::Scalar>("Elastic Properties Penalty Exponent", 1.0);
            mElasticPropertiesMinErsatzSIMP =
                tJ2PlasticitySubList.get<Plato::Scalar>("Elastic Properties Minimum Ersatz", 1e-9);
            mAdditiveContinuationElasticProperties =
                tJ2PlasticitySubList.get<Plato::Scalar>("Elastic Properties Additive Continuation", 0.1);
            mUpperBoundOnElasticPropertiesPenaltySIMP =
                tJ2PlasticitySubList.get<Plato::Scalar>("Elastic Properties Penalty Exponent Upper Bound", 4.0);

            mPlasticPropertiesPenaltySIMP =
                tJ2PlasticitySubList.get<Plato::Scalar>("Plastic Properties Penalty Exponent", 0.5);
            mPlasticPropertiesMinErsatzSIMP =
                tJ2PlasticitySubList.get<Plato::Scalar>("Plastic Properties Minimum Ersatz", 1e-4);
            mAdditiveContinuationPlasticProperties =
                tJ2PlasticitySubList.get<Plato::Scalar>("Plastic Properties Additive Continuation", 0.1);
            mUpperBoundOnPlasticPropertiesPenaltySIMP =
                tJ2PlasticitySubList.get<Plato::Scalar>("Plastic Properties Penalty Exponent Upper Bound", 3.5);
        }
        else
        {
            ANALYZE_THROWERR(
                "'J2 Plasticity' sublist of 'Material Model' does not exist. Needed for J2Plasticity Implementation.")
        }
    }
    ...
    void checkJ2PlasticityInputs(Teuchos::ParameterList& aInputParams)
    {
        const bool tRequriedInputParamsAreDefined = aInputParams.isParameter("Hardening Modulus Isotropic") &&
                                                    aInputParams.isParameter("Hardening Modulus Kinematic") &&
                                                    aInputParams.isParameter("Initial Yield Stress");
        if (tRequriedInputParamsAreDefined == false)
        {
            std::string tError =
                std::string(
                    "Required input parameters, 'Hardening Modulus Isotropic', 'Hardening Modulus Kinematic', ") +
    ...
    void updateElasticPropertiesPenaltyModel()
    {
        ...
        auto tSuggestedElasticPropertiesPenalty =
            tPreviousElasticPropertiesPenalty + mAdditiveContinuationElasticProperties;
        mElasticPropertiesPenaltySIMP = tSuggestedElasticPropertiesPenalty >= mUpperBoundOnElasticPropertiesPenaltySIMP
                                            ? mUpperBoundOnElasticPropertiesPenaltySIMP
                                            : tSuggestedElasticPropertiesPenalty;

        std::ostringstream tMsg;
        tMsg << "J2 Plasticity Local Residual: New penalty parameter for the elastic properties is set to '"
             << mElasticPropertiesPenaltySIMP << "'. Previous penalty parameter was '"
             << tPreviousElasticPropertiesPenalty << "'.\n";
    ...
    void updatePlasticPropertiesPenaltyModel()
    {
        ...
        auto tSuggestedPlasticPropertiesPenalty =
            tPreviousPlasticPropertiesPenalty + mAdditiveContinuationPlasticProperties;
        mPlasticPropertiesPenaltySIMP = tSuggestedPlasticPropertiesPenalty >= mUpperBoundOnPlasticPropertiesPenaltySIMP
                                            ? mUpperBoundOnPlasticPropertiesPenaltySIMP
                                            : tSuggestedPlasticPropertiesPenalty;

        std::ostringstream tMsg;
        tMsg << "J2 Plasticity Local Residual: New penalty parameter for the plastic properties is set to '"
             << mPlasticPropertiesPenaltySIMP << "'. Previous penalty parameter was '"
             << tPreviousPlasticPropertiesPenalty << "'.\n";
        REPORT(tMsg.str().c_str())
    }
   ...
   public:
     ...
     ******************************************************************************/
    J2PlasticityLocalResidual(const Plato::SpatialDomain& aSpatialDomain,
     ...
     ******************************************************************************/
    virtual void evaluate(const Plato::ScalarMultiVectorT<GlobalStateT>& aGlobalState,
                          ...
                          const Plato::TimeData& aTimeData) const
    {
        ...
        // ThermoPlasticity Utility Functions Object (for computing elastic strain and potentially temperature-dependent
        // material properties)
        Plato::ThermoPlasticityUtilities<mSpaceDim, SimplexPhysicsType> tThermoPlasticityUtils(
            mThermalExpansionCoefficient, mReferenceTemperature, mTemperatureScaling);
        ...
        Plato::ScalarVectorT<ControlT> tPenalizedHardeningModulusKinematic("PenalizedHardeningModulusKinematic",
                                                                           tNumCells);
        ...
        // go out of scope and are de-referenced immediately.
        {
            ...
            Plato::ScalarVectorT<ControlT> tPenalizedHardeningModulusIsotropic("PenalizedHardeningModulusIsotropic",
                                                                               tNumCells);
            ...
            // out of scope and are de-referenced immediately.
            {
                ...
                Kokkos::parallel_for(
                    "Compute cell local residuals - part 1", Kokkos::RangePolicy<>(0, tNumCells),
                    KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                        ...
                        tComputeDeviatoricStress(aCellOrdinal, tPenalizedShearModulus, tElasticStrain,
                                                 tDeviatoricStress);
                        ...
                        tJ2PlasticityUtils.computeDeviatoricStressMinusBackstressNormalized(
                            aCellOrdinal, tDeviatoricStress, aLocalState, tYieldSurfaceNormal,
                            tDevStressMinusBackstressNorm);
                        ...
                        tPenalizedHardeningModulusIsotropic(aCellOrdinal) =
                            tPlasticParamsPenalty * tHardeningModulusIsotropic;
                        tPenalizedHardeningModulusKinematic(aCellOrdinal) =
                            tPlasticParamsPenalty * tHardeningModulusKinematic;
                    });
            }

            // compute yield stress - separate loop in the functor.
            tComputeYieldStress(tYieldStress, aLocalState, tPenalizedInitialYieldStress,
                                tPenalizedHardeningModulusIsotropic);
        }

        // Third parallel_for loop.
        Kokkos::parallel_for(
            "Compute cell local residuals - part 2", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                // ### ELASTIC STEP ###
                ...
                tJ2PlasticityUtils.fillPlasticStrainTensorResidualElasticStep(aCellOrdinal, aLocalState,
                                                                              aPrevLocalState, aResult);

                // Residual: Backstress, DOF: Backstress
                tJ2PlasticityUtils.fillBackstressTensorResidualElasticStep(aCellOrdinal, aLocalState, aPrevLocalState,
                                                                           aResult);

                if (aLocalState(aCellOrdinal, 1) /*Current Plastic Multiplier Increment*/ >
                    0.0)  // -> yielding (assumes local state already updated)
                {
                    // Residual: Accumulated Plastic Strain, DOF: Accumulated Plastic Strain
                    ...
                    aResult(aCellOrdinal, 1) =
                        tSqrt3Over2 * tDevStressMinusBackstressNorm(aCellOrdinal) - tYieldStress(aCellOrdinal, 0);

                    // Residual: Plastic Strain Tensor, DOF: Plastic Strain Tensor
                    tJ2PlasticityUtils.fillPlasticStrainTensorResidualPlasticStep(
                        aCellOrdinal, aLocalState, aPrevLocalState, tYieldSurfaceNormal, aResult);

                    // Residual: Backstress, DOF: Backstress
                    tJ2PlasticityUtils.fillBackstressTensorResidualPlasticStep(
                        aCellOrdinal, tPenalizedHardeningModulusKinematic(aCellOrdinal), aLocalState, aPrevLocalState,
     ...
     * \param [in]  aConfig configuration parameters
     ******************************************************************************/
    virtual void updateLocalState(const Plato::ScalarMultiVector& aGlobalState,
                                  ...
                                  const Plato::TimeData& aTimeData) const
    {
        ...
        // The yield stress requires a factory.
        Plato::YieldStressFactory<Plato::ResidualTypes<SimplexPhysicsType> > tYieldStressFactory;
        auto pComputeYieldStress = tYieldStressFactory.create(mPlasticityParamList);
        auto& tComputeYieldStress = *pComputeYieldStress;
        ...
        // go out of scope and are de-referenced immediately.
        {
            ...
            // out of scope and are de-referenced immediately.
            {
                ...
                // First parallel_for loop.
                Kokkos::parallel_for(
                    "Update local state dofs - part 1", Kokkos::RangePolicy<>(0, tNumCells),
                    KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                        tComputeGradient(aCellOrdinal, tGradient, aConfig, tCellVolume);
                        ...
                        tJ2PlasticityUtils.updatePlasticStrainAndBackstressElasticStep(aCellOrdinal, aPrevLocalState,
                                                                                       aLocalState);
                        ...
                        // compute deviatoric stress
                        tComputeDeviatoricStress(aCellOrdinal, tPenalizedShearModulus(aCellOrdinal), tElasticStrain,
                                                 tDeviatoricStress);
        ...
        // Third parallel_for loop.
        Kokkos::parallel_for(
            "Update local state dofs - part 2", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                // compute the yield function at the trial state
                Plato::Scalar tTrialStateYieldFunction =
                    tSqrt3Over2 * tDevStressMinusBackstressNorm(aCellOrdinal) - tYieldStress(aCellOrdinal, 0);

                if (tTrialStateYieldFunction > static_cast<Plato::Scalar>(1.0e-10))  // plastic step
                {
                    // Plastic Multiplier Increment (for J2 w/ linear isotropic/kinematic hardening -> analytical return
                    // mapping)
                    aLocalState(aCellOrdinal, 1) =
                        tTrialStateYieldFunction /
