...
class ElasticWorkCriterion : public Plato::AbstractLocalScalarFunctionInc<EvaluationType>
{
   ...
   public:
     ...
     *******************************************************************************/
    ElasticWorkCriterion(const Plato::SpatialDomain& aSpatialDomain,
     ...
     * \param [in] aName        scalar function name
     *******************************************************************************/
    ElasticWorkCriterion(const Plato::SpatialDomain& aSpatialDomain, Plato::DataMap& aDataMap, std::string aName = "")
        : FunctionBaseType(aSpatialDomain, aDataMap, aName),
          mBulkModulus(1.0),
          mShearModulus(1.0),
          mPenaltySIMP(3),
                  ...
                  const Plato::TimeData& aTimeData)
    {
        ...
        using PreviousElasticStrainT =
            typename Plato::fad_type_t<SimplexPhysicsType, PrevLocalStateT, ConfigT, PrevGlobalStateT>;
        ...
        Plato::ScalarMultiVectorT<PreviousTotalStrainT> tPreviousTotalStrain("previous total strain", tNumCells,
                                                                             mNumStressTerms);
        Plato::ScalarMultiVectorT<PreviousElasticStrainT> tPreviousElasticStrain("previous elastic strain", tNumCells,
                                                                                 mNumStressTerms);
        Plato::ScalarMultiVectorT<ResultT> tPreviousDeviatoricStress("previous deviatoric stress", tNumCells,
                                                                     mNumStressTerms);
        ...
        Plato::ScalarMultiVectorT<ElasticStrainT> tCurrentElasticStrain("current elastic strain", tNumCells,
                                                                        mNumStressTerms);
        Plato::ScalarMultiVectorT<ResultT> tCurrentDeviatoricStress("current deviatoric stress", tNumCells,
                                                                    mNumStressTerms);
        ...
        Kokkos::parallel_for(
            "elastic work criterion", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                tThermoPlasticityUtils.computeElasticStrain(aCellOrdinal, aPreviousGlobalState, aPreviousLocalState,
                                                            tBasisFunctions, tPreviousTotalStrain,
                                                            tPreviousElasticStrain);
                ...
                tThermoPlasticityUtils.computeElasticStrain(aCellOrdinal, aCurrentGlobalState, aCurrentLocalState,
                                                            tBasisFunctions, tCurrentTotalStrain,
                                                            tCurrentElasticStrain);

                for (Plato::OrdinalType tIndex = 0; tIndex < tNumStressTerms; ++tIndex)
                    tElasticStrainMisfit(aCellOrdinal, tIndex) =
                        tCurrentElasticStrain(aCellOrdinal, tIndex) - tPreviousElasticStrain(aCellOrdinal, tIndex);
                ...
                tComputeDeviatoricStress(aCellOrdinal, tPenalizedShearModulus, tPreviousElasticStrain,
                                         tPreviousDeviatoricStress);
                tComputeDeviatoricStress(aCellOrdinal, tPenalizedShearModulus, tCurrentElasticStrain,
                                         tCurrentDeviatoricStress);
                ...
                for (Plato::OrdinalType tIndex = 0; tIndex < tNumStressTerms; ++tIndex)
                    tAverageCauchyStress(aCellOrdinal, tIndex) =
                        tOneHalf *
                ...
                for (Plato::OrdinalType tIndex = 0; tIndex < tNumStressTerms; ++tIndex)
                    aResult(aCellOrdinal) +=
                        tAverageCauchyStress(aCellOrdinal, tIndex) * tElasticStrainMisfit(aCellOrdinal, tIndex);
   ...
   private:
    ...
    void parsePenaltyModelParams(Teuchos::ParameterList& aInputParams)
    {
        ...
        {
            const auto tError = std::string("UNKNOWN USER DEFINED SCALAR FUNCTION SUBLIST '") + tFunctionName +
                                "'. USER DEFINED SCALAR FUNCTION SUBLIST '" + tFunctionName +
    ...
    void parseIsotropicMaterialProperties(Teuchos::ParameterList& aMaterialParams)
    {
        ...
        {
            ANALYZE_THROWERR(
                "'Isotropic Linear Elastic' or 'Isotropic Linear Thermoelastic' sublist of 'Material Model' is not "
                "defined.")
        }
    }
};
