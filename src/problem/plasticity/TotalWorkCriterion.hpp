...
class TotalWorkCriterion : public Plato::AbstractLocalScalarFunctionInc<EvaluationType>
{
   ...
   public:
     ...
     *******************************************************************************/
    TotalWorkCriterion(const Plato::SpatialDomain& aSpatialDomain,
     ...
     * \param [in] aName        scalar function name
     *******************************************************************************/
    TotalWorkCriterion(const Plato::SpatialDomain& aSpatialDomain, Plato::DataMap& aDataMap, std::string aName = "")
        : FunctionBaseType(aSpatialDomain, aDataMap, aName),
          mBulkModulus(1.0),
          mShearModulus(1.0),
          mPenaltySIMP(3),
                  ...
                  const Plato::TimeData& aTimeData)
    {
        ...
        Kokkos::parallel_for(
            "total work criterion", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                for (Plato::OrdinalType tIndex = 0; tIndex < tNumStressTerms; ++tIndex)
                    tTotalStrainMisfit(aCellOrdinal, tIndex) =
                        tCurrentTotalStrain(aCellOrdinal, tIndex) - tPreviousTotalStrain(aCellOrdinal, tIndex);
                ...
                for (Plato::OrdinalType tIndex = 0; tIndex < tNumStressTerms; ++tIndex)
                    aResult(aCellOrdinal) +=
                        tAverageCauchyStress(aCellOrdinal, tIndex) * tTotalStrainMisfit(aCellOrdinal, tIndex);
