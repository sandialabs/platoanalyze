...
class ThermoplasticityThermalEnergyCriterion : public Plato::AbstractLocalScalarFunctionInc<EvaluationType>
{
   ...
   private:
    ...
    static constexpr auto mTemperatureDofOffset =
        SimplexPhysicsType::mTemperatureDofOffset; /*!< temperature dof offset */
   ...
   public:
     ...
     *******************************************************************************/
    ThermoplasticityThermalEnergyCriterion(const Plato::SpatialDomain& aSpatialDomain,
                  ...
                  const Plato::TimeData& aTimeData)
    {
        ...
        Kokkos::parallel_for(
            "thermal energy criterion", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                for (Plato::OrdinalType tIndex = 0; tIndex < tSpaceDim; ++tIndex)
                    aResult(aCellOrdinal) +=
                        tTemperatureGrad(aCellOrdinal, tIndex) * tTemperatureGrad(aCellOrdinal, tIndex);
                aResult(aCellOrdinal) *= tPenalizedThermalConductivityCoefficient *
                                         (tCellVolume(aCellOrdinal) * tTemperatureScalingSquared * tOneHalf);
            });
    }
