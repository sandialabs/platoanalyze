...
class PlasticityProblem : public Plato::AbstractProblem
{
   ...
   private:
    ...
    static constexpr auto mTemperatureDofOffset =
        PhysicsT::mTemperatureDofOffset; /*!< number of temperature dofs offset*/
    ...
    static constexpr auto mNumGlobalDofsPerCell =
        PhysicsT::mNumDofsPerCell; /*!< number of global degrees of freedom per cell (i.e. element)*/
    static constexpr auto mNumLocalDofsPerCell =
        PhysicsT::mNumLocalDofsPerCell; /*!< number of local degrees of freedom per cell (i.e. element)*/
    static constexpr auto mNumPressGradDofsPerCell =
        PhysicsT::mNumNodeStatePerCell; /*!< number of projected pressure gradient degrees of freedom per cell (i.e.
                                           element)*/
    static constexpr auto mNumPressGradDofsPerNode =
        PhysicsT::mNumNodeStatePerNode; /*!< number of projected pressure gradient degrees of freedom per node*/
    ...
    std::shared_ptr<Plato::LocalVectorFunctionInc<PlasticityT>>
    ...
    Plato::ScalarMultiVector
        mProjectedPressGrad; /*!< projected pressure gradient (# Time Steps, # Projected Pressure Gradient dofs) */

    std::shared_ptr<Plato::EssentialBCs<PhysicsT>> mEssentialBCs; /*!< essential boundary conditions shared pointer */
    Plato::ScalarVector mPreviousStepDirichletValues; /*!< previous time step values associated with the Dirichlet
                                                         boundary conditions */
    Plato::ScalarVector mDirichletValues;             /*!< values associated with the Dirichlet boundary conditions */
    Plato::OrdinalVector
        mDirichletDofs; /*!< list of degrees of freedom associated with the Dirichlet boundary conditions */
    ...
    std::shared_ptr<Plato::PathDependentAdjointSolver<PhysicsT>>
        mAdjointSolver; /*!< Path-dependent adjoint solver interface */

    bool mStopOptimization; /*!< stops optimization problem if Newton-Raphson solver fails to converge during an
                               optimization run */
   ...
   public:
     ...
     *******************************************************************************/
    PlasticityProblem(Plato::Mesh aMesh, Teuchos::ParameterList& aInputs, Comm::Machine& aMachine)
        : mSpatialModel(aMesh, aInputs),
          mLocalEquation(
              std::make_shared<Plato::LocalVectorFunctionInc<PlasticityT>>(mSpatialModel, mDataMap, aInputs)),
          mGlobalEquation(std::make_shared<Plato::GlobalVectorFunctionInc<PhysicsT>>(
              mSpatialModel, mDataMap, aInputs, aInputs.get<std::string>("PDE Constraint"))),
          mProjectionEquation(std::make_shared<Plato::VectorFunctionVMS<ProjectorT>>(
              mSpatialModel, mDataMap, aInputs, std::string("State Gradient Projection"))),
    ...
    void readEssentialBoundaryConditions(Teuchos::ParameterList& aInputs)
    {
        ...
        mEssentialBCs = std::make_shared<Plato::EssentialBCs<PhysicsT>>(
     ...
     *******************************************************************************/
    void setEssentialBoundaryConditions(const Plato::OrdinalVector& aDirichletDofs,
                                        const Plato::ScalarVector& aDirichletValues)
    {
        if (aDirichletDofs.size() != aDirichletValues.size())
        {
            std::ostringstream tError;
            tError << "PLASTICITY PROBLEM: DIMENSION MISMATCH: THE NUMBER OF ELEMENTS IN INPUT DOFS AND VALUES ARRAY "
                      "DO NOT MATCH."
    ...
    void computeReactionForce(const Plato::ScalarVector& aControl, Plato::CurrentStates& aStates)
    {
        ...
        Kokkos::parallel_for(
            "reaction force", Kokkos::RangePolicy<>(0, tNumNodes), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
                               ...
                               const Plato::ScalarVector& aPreviousGlobalState) const
    {
        ...
        {
            Kokkos::parallel_for(
                "set temperature to reference", Kokkos::RangePolicy<>(0, tNumVerts),
   ...
   private:
    ...
    bool solveForwardProblem(const Plato::ScalarVector& aControls)
    {
        ...
        for (Plato::OrdinalType tCurrentStepIndex = 0; tCurrentStepIndex < mTimeData->mNumTimeSteps;
             tCurrentStepIndex++)
        {
            ...
            if (tNewtonRaphsonConverged == false)
            {
                ...
                tMsg << "**** Newton-Raphson Solver did not converge at time step #"
                     << mTimeData->getTimeStepIndexPlusOne() << ".  Number of pseudo time steps will be increased to '"
                     << static_cast<Plato::OrdinalType>(mTimeData->mNumTimeSteps *
                                                        mTimeData->mTimeStepExpansionMultiplier)
                     << "'. ****\n\n";
                mNewtonSolver->appendOutputMessage(tMsg);
    ...
    void updateDispAndLoadControlMultipliers(const Plato::OrdinalType& aInput)
    {
        ...
        Plato::blas1::axpy(static_cast<Plato::Scalar>(-1.0), mPreviousStepDirichletValues,
                           tNewtonUpdateDirichletValues);
     ...
     *******************************************************************************/
    void updateProjectedPressureGradient(const Plato::ScalarVector& aControls, Plato::CurrentStates& aStateData)
    {
        ...
        auto tProjResidual =
            mProjectionEquation->value(tNextProjectedPressureGradient, mPressure, aControls, tNextStepIndex);
        auto tProjJacobian =
            mProjectionEquation->gradient_u(tNextProjectedPressureGradient, mPressure, aControls, tNextStepIndex);
        Plato::blas1::scale(-1.0, tProjResidual);
        Plato::Solve::RowSummed<PhysicsT::mNumSpatialDims>(tProjJacobian, tNextProjectedPressureGradient,
                                                           tProjResidual);
    }
                                    ...
                                    const Plato::ScalarVector& aControls)
    {
             ...
             tCurrentStepIndex++)
        {
            ...
            mTimeData->updateTimeData(tCurrentStepIndex);
            tOutput += aCriterion.value(tCurrentGlobalState, tPreviousGlobalState, tCurrentLocalState,
                                        tPreviousLocalState, aControls, *mTimeData);
        }
                                        ...
                                        Plato::ScalarVector& aTotalGradient)
    {
             ...
             tCurrentStepIndex++)
        {
            ...
            mTimeData->updateTimeData(tCurrentStepIndex);
            auto tDfDz = aCriterion.gradient_z(tCurrentGlobalState, tPreviousGlobalState, tCurrentLocalState,
                                               tPreviousLocalState, aControls, *mTimeData);
                                        ...
                                        Plato::ScalarVector& aTotalGradient)
    {
             ...
             tCurrentStepIndex++)
        {
            ...
            mTimeData->updateTimeData(tCurrentStepIndex);
            auto tDfDX = aCriterion.gradient_x(tCurrentGlobalState, tPreviousGlobalState, tCurrentLocalState,
                                               tPreviousLocalState, aControls, *mTimeData);
                                 ...
                                 Plato::ScalarVector aTotalDerivative)
    {
        ...
        Plato::AdjointStates tAdjointStates(mGlobalEquation->size(), mLocalEquation->size(),
                                            mProjectionEquation->size());
        tAdjointStates.mInvLocalJacT =
            ScalarArray3D("Inv(DhDc)^T", tNumCells, mNumLocalDofsPerCell, mNumLocalDofsPerCell);
        ...
        for (tCurrentStates.mCurrentStepIndex = tLastStepIndex; tCurrentStates.mCurrentStepIndex >= 0;
             tCurrentStates.mCurrentStepIndex--)
        {
    ...
    void cacheStateData(Plato::CurrentStates& aStateData)
    {
        ...
        aStateData.mProjectedPressGrad =
            Kokkos::subview(mProjectedPressGrad, aStateData.mCurrentStepIndex, Kokkos::ALL());
    ...
    void updateForwardState(Plato::ForwardStates& aStateData)
    {
        ...
        Plato::blas1::extract<mNumGlobalDofsPerNode, mPressureDofOffset>(aStateData.mCurrentGlobalState,
                                                                         aStateData.mPressure);
    ...
    void updateAdjointState(Plato::AdjointStates& aAdjointStates)
    {
        ...
        Plato::blas1::update(tAlpha, aAdjointStates.mCurrentGlobalAdjoint, tBeta,
                             aAdjointStates.mPreviousGlobalAdjoint);
        Plato::blas1::update(tAlpha, aAdjointStates.mProjPressGradAdjoint, tBeta,
                             aAdjointStates.mPreviousProjPressGradAdjoint);
    }
    ...
    void allocateCriteria(Teuchos::ParameterList& aInputParams)
    {
        if (aInputParams.isSublist("Criteria"))
        {
            ...
            for (Teuchos::ParameterList::ConstIterator tIndex = tCriteriaParams.begin();
                 tIndex != tCriteriaParams.end(); ++tIndex)
            {
