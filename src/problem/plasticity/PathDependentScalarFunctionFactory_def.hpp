...
std::shared_ptr<Plato::LocalScalarFunctionInc> PathDependentScalarFunctionFactory<PhysicsT>::create(
    ...
    const std::string& aFunctionName)
{
    ...
    {
        return (std::make_shared<Plato::BasicLocalScalarFunction<PhysicsT>>(aSpatialModel, aDataMap, aInputParams,
                                                                            aFunctionName));
    }
    else if (tFunctionType == "Weighted Sum")
    {
        return (std::make_shared<Plato::WeightedLocalScalarFunction<PhysicsT>>(aSpatialModel, aDataMap, aInputParams,
                                                                               aFunctionName));
    }
    else
    {
