...
Teuchos::RCP<Teuchos::ParameterList> tGenericParamList_Two = Teuchos::getParametersFromXmlString(
    "<ParameterList name='Plato Problem'>                                                        \n"
    "  <ParameterList name='Spatial Model'>                                                      \n"
    "    <ParameterList name='Domains'>                                                          \n"
    "      <ParameterList name='Design Volume'>                                                  \n"
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_GetLocalStateData_2D)
{
    ...
    Kokkos::parallel_for(
        "GetLocalStateData Unit Test", Kokkos::RangePolicy<>(0, tNumCells),
    ...
    for (Plato::OrdinalType tCellIndex = 0; tCellIndex < tNumCells; ++tCellIndex)
    {
        ...
        {
            TEST_FLOATING_EQUALITY(tHostBackStress(tCellIndex, tDofIndex), tBackStressGold[tCellIndex][tDofIndex],
                                   tTolerance);
            // printf( "HostBackStress(%d,%d) = %f\n", tCellIndex, tDofIndex, tHostBackStress(tCellIndex, tDofIndex) );
            TEST_FLOATING_EQUALITY(tHostPlasticStrain(tCellIndex, tDofIndex), tPlasticStrainGold[tCellIndex][tDofIndex],
                                   tTolerance);
            // printf( "HostPlasticStrain(%d,%d) = %f\n", tCellIndex, tDofIndex, tHostPlasticStrain(tCellIndex,
            // tDofIndex) );
        }
    }
}
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_GetLocalStateData_3D)
{
    ...
    std::vector<Plato::Scalar> tPlasticMultiplierGold = {2.0, 4.0};
    std::vector<Plato::Scalar> tAccumPlasticStrainGold = {1.0, 2.0};
    std::vector<std::vector<Plato::Scalar>> tPlasticStrainGold = {{3.0, 4.0, 5.0, 6.0, 7.0, 8.0},
                                                                  {6.0, 8.0, 10.0, 12.0, 14.0, 16.0}};
    std::vector<std::vector<Plato::Scalar>> tBackStressGold = {{9.0, 10.0, 11.0, 12.0, 13.0, 14.0},
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& tCellOrdinal) {
            tJ2PlasticityUtils.updatePlasticStrainAndBackstressPlasticStep(
                tCellOrdinal, tPrevLocalState, tYieldSurfaceNormal, tPenalizedHardeningModulusKinematic, tLocalState);
        });

    constexpr Plato::Scalar tTolerance = 1e-4;
    std::vector<std::vector<Plato::Scalar>> tGold = {{tHostPrevLocalState(0, 0), tHostPrevLocalState(0, 1), 3.9798,
                                                      5.4697, 8.91918, 8.44949, 8.95959, 10.9394, 12.9192, 14.899},
                                                     {tHostPrevLocalState(1, 0), tHostPrevLocalState(1, 1), 9.9192,
                                                      13.8788, 25.6767, 21.798, 21.8384, 27.7576, 33.6767, 39.5959}};
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_UpdatePlasticStep3D)
{
    ...
    std::vector<std::vector<Plato::Scalar>> tGold = {
        {tHostPrevLocalState(0, 0), tHostPrevLocalState(0, 1), 3.9798, 5.4697, 6.9596, 10.899, 12.8788, 14.8586,
         10.9596, 12.9394, 14.9192, 16.899, 18.8788, 20.8586},
        {tHostPrevLocalState(1, 0), tHostPrevLocalState(1, 1), 9.9192, 13.8788, 17.8384, 31.5959, 37.5151, 43.4343,
         25.8384, 31.7576, 37.6767, 43.5959, 49.5151, 55.4343}};
    auto tHostLocalState = Kokkos::create_mirror(tLocalState);
    Kokkos::deep_copy(tHostLocalState, tLocalState);
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_UpdateElasticStep2D)
{
    ...
    for (Plato::OrdinalType tCellIndex = 0; tCellIndex < tNumCells; ++tCellIndex)
        for (Plato::OrdinalType tDofIndex = 2; tDofIndex < tNumLocalDofsPerCell; ++tDofIndex)
            TEST_FLOATING_EQUALITY(tHostLocalState(tCellIndex, tDofIndex), tHostPrevLocalState(tCellIndex, tDofIndex),
                                   tTolerance);
}
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& tCellOrdinal) {
            tJ2PlasticityUtils.computeDeviatoricStressMinusBackstressNormalized(
                tCellOrdinal, tDeviatoricStress, tLocalState, tYieldSurfaceNormal, tDevStressMinusBackstressNorm);
        });
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_PlasticStrainResidualPlasticStep2D)
{
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& tCellOrdinal) {
            tJ2PlasticityUtils.fillPlasticStrainTensorResidualPlasticStep(tCellOrdinal, tLocalState, tPrevLocalState,
                                                                          tYieldSurfaceNormal, tResult);
        });
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& tCellOrdinal) {
            tJ2PlasticityUtils.fillBackstressTensorResidualPlasticStep(
                tCellOrdinal, tHardeningModulusKinematic, tLocalState, tPrevLocalState, tYieldSurfaceNormal, tResult);
        });
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_PlasticStrainResidualElasticStep2D)
{
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& tCellOrdinal) {
            tJ2PlasticityUtils.fillPlasticStrainTensorResidualElasticStep(tCellOrdinal, tLocalState, tPrevLocalState,
                                                                          tResult);
        });
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2PlasticityUtils_BackstressResidualElasticStep2D)
{
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& tCellOrdinal) {
            tJ2PlasticityUtils.fillBackstressTensorResidualElasticStep(tCellOrdinal, tLocalState, tPrevLocalState,
                                                                       tResult);
        });
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, ThermoPlasticityUtils_ElasticStrainWithThermo2D)
{
    ...
    using ElasticStrainT = typename Plato::fad_type_t<PhysicsT, LocalStateT, GlobalStateT, ConfigT, ControlT>;
    ...
    Plato::ScalarMultiVectorT<ElasticStrainT> tElasticStrain("elastic strain output", tNumCells, tNumStressTerms);
    ...
    Plato::ThermoPlasticityUtilities<tSpaceDim, PhysicsT> tThermoPlasticityUtils(tThermalExpansionCoefficient,
    ...
    Kokkos::parallel_for(
        "Unit Test", Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            tComputeGradient(aCellOrdinal, tGradient, tConfigWS, tCellVolume);
            tComputeTotalStrain(aCellOrdinal, tTotalStrain, tGlobalStateWS, tGradient);
            tThermoPlasticityUtils.computeElasticStrain(aCellOrdinal, tGlobalStateWS, tLocalStateWS, tBasisFunctions,
                                                        tTotalStrain, tElasticStrain);
        });

    constexpr Plato::Scalar tTolerance = 1e-4;
    std::vector<std::vector<Plato::Scalar>> tGold = {{-1.0, -2.0, -3.0, -4.0}, {-1.0, -2.0, -3.0, -4.0}};
    ...
    for (Plato::OrdinalType tCellIndex = 0; tCellIndex < tNumCells; tCellIndex++)
        for (Plato::OrdinalType tStressIndex = 0; tStressIndex < tNumStressTerms; tStressIndex++)
            TEST_FLOATING_EQUALITY(tHostElasticStrain(tCellIndex, tStressIndex), tGold[tCellIndex][tStressIndex],
                                   tTolerance);
}
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, ThermoPlasticityUtils_ElasticStrainWithThermo3D)
{
    ...
    constexpr Plato::Scalar tTolerance = 1e-4;
    std::vector<std::vector<Plato::Scalar>> tGold = {
        {-1.0, -2.0, -3.0, -3.0, -3.0, -3.0}, {-1.0, -2.0, -3.0, -3.0, -3.0, -3.0},
        {-1.0, -2.0, -3.0, -3.0, -3.0, -3.0}, {-1.0, -2.0, -3.0, -3.0, -3.0, -3.0},
        {-1.0, -2.0, -3.0, -3.0, -3.0, -3.0}, {-1.0, -2.0, -3.0, -3.0, -3.0, -3.0}};
    auto tHostElasticStrain = Kokkos::create_mirror(tElasticStrain);
    Kokkos::deep_copy(tHostElasticStrain, tElasticStrain);
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2Plasticity_Evaluate3D)
{
    ...
    Plato::ScalarVector tLocalResidual =
        tLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);
    ...
    std::vector<Plato::Scalar> tGold = {-0.400000, 26.941399, -0.480125,  0.111393,  0.368732,  0.0443036, 0.0443036,
                                        0.0443036, 7.078994,  -44.680887, 37.601894, -0.829778, -0.829778, -0.829778};
    for (Plato::OrdinalType tIndex = 0; tIndex < tNumLocalDofs; ++tIndex)
        TEST_FLOATING_EQUALITY(tHostLocalResidual(tIndex), tGold[tIndex % tNumLocalDofsPerCell], tTolerance);
}
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2Plasticity_Evaluate2D)
{
    ...
    std::vector<Plato::Scalar> tGold = {-0.400000, 9.72218, -0.435375, 0.457842,  0.0901547,
                                        0.438152,  7.42286, -42.0187,  -0.653611, 3.3669};
    for (Plato::OrdinalType tIndex = 0; tIndex < tNumLocalDofs; ++tIndex)
        TEST_FLOATING_EQUALITY(tHostLocalResidual(tIndex), tGold[tIndex % tNumLocalDofsPerCell], tTolerance);
}
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2Plasticity_UpdateLocalState3D)
{
    ...
    Teuchos::RCP<Teuchos::ParameterList> tInputs = Teuchos::getParametersFromXmlString(
        ...
        "</ParameterList>                                                            \n");
    Plato::TimeData tTimeData(*tInputs);
    tLocalVectorFuncInc.updateLocalState(tGlobalState, tPrevGlobalState, tLocalState, tPrevLocalState, tControl,
                                         tTimeData);
    ...
    std::vector<Plato::Scalar> tGold = {0.3755345,        0.1755345,        -0.00606135,      -0.144478,  0.15053935,
                                        2.0 * 0.18231319, 2.0 * 0.18231319, 2.0 * 0.18231319, 1.95342253, 40.42664966,
                                        -42.38007219,     2.63252209,       2.63252209,       2.63252209};
    for (Plato::OrdinalType tIndex = 0; tIndex < tNumLocalDofs; ++tIndex)
        TEST_FLOATING_EQUALITY(tHostLocalState(tIndex), tGold[tIndex % tNumLocalDofsPerCell], tTolerance);
}
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, J2Plasticity_UpdateLocalState2D)
{
    ...
    std::vector<Plato::Scalar> tGold = {0.314575,   0.114575, -0.0657574, -0.206138, 0.359376,
                                        -0.0612751, 1.4947,   39.9528,    2.61235,   -0.414015};
