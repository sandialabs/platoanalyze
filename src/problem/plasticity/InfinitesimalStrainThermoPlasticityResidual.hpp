...
class InfinitesimalStrainThermoPlasticityResidual : public Plato::AbstractGlobalVectorFunctionInc<EvaluationType>
{
   ...
   private:
    ...
    static constexpr auto mNumStressTerms =
        SimplexPhysicsType::mNumStressTerms; /*!< number of stress/strain components */
    static constexpr auto mNumDofsPerCell =
        SimplexPhysicsType::mNumDofsPerCell; /*!< number of degrees of freedom (dofs) per cell */
    ...
    static constexpr auto mNumGlobalDofsPerNode =
        SimplexPhysicsType::mNumDofsPerNode; /*!< number of global dofs per node */

    static constexpr auto mNumDisplacementDims = mSpaceDim; /*!< number of displacement degrees of freedom */
    static constexpr auto mDisplacementDofOffset =
        SimplexPhysicsType::mDisplacementDofOffset;          /*!< displacement degrees of freedom offset */
    static constexpr Plato::OrdinalType mNumThermalDims = 1; /*!< number of thermal degrees of freedom */
    static constexpr auto mTemperatureDofOffset =
        SimplexPhysicsType::mTemperatureDofOffset; /*!< temperature dofs offset */
    ...
    Plato::Scalar
        mUpperBoundOnPenaltySIMP; /*!< continuation parameter: upper bound on SIMP penalty for elastic properties */
    Plato::Scalar
        mAdditiveContinuationParam; /*!< continuation parameter: multiplier on SIMP penalty for elastic properties */
   ...
   private:
    ...
    void parseOutputDataNames(Teuchos::ParameterList& aProblemParams)
    {
        ...
        {
            ANALYZE_THROWERR(
                "Infinitesimal Strain Thermoplasticity Residual: 'Elliptic' sublist is not defined in XML input file.")
        }
    }
    ...
    void parseExternalForces(Teuchos::ParameterList& aProblemParams)
    {
        ...
        {
            mBodyLoads = std::make_shared<Plato::BodyLoads<EvaluationType, SimplexPhysicsType>>(
        ...
        if (aProblemParams.isSublist("Natural Boundary Conditions"))
        {
            ...
            {
                mNeumannMechanicalLoads = std::make_shared<
                    Plato::NaturalBCs<mSpaceDim, mNumDisplacementDims, mNumGlobalDofsPerNode, mDisplacementDofOffset>>(
            ...
            {
                mNeumannThermalLoads = std::make_shared<
                    Plato::NaturalBCs<mSpaceDim, mNumThermalDims, mNumGlobalDofsPerNode, mTemperatureDofOffset>>(
    ...
    void parseMaterialProperties(Teuchos::ParameterList& aProblemParams)
    {
        ...
        {
            ANALYZE_THROWERR(
                "Infinitesimal Strain Thermoplasticity Residual: 'Material Models' sublist is not defined.")
        }
    }
    ...
    void parseIsotropicMaterialProperties(Teuchos::ParameterList& aProblemParams)
    {
        ...
        {
            std::stringstream ss;
            ss << "Infinitesimal Strain Thermoplasticity Residual: 'Isotropic Linear Thermoelastic' sublist of '"
     ...
     ****************************************************************************/
    void addExternalForces(const Plato::SpatialModel& aSpatialModel,
                           ...
                           const Plato::ScalarMultiVectorT<ResultT>& aResult)
    {
        ...
        {
            mNeumannMechanicalLoads->get(aSpatialModel, aGlobalState, aControl, aConfig, aResult, tMinusOne,
                                         tCurrentTime);
        }
     ...
     * \param [in/out] aResult      residual evaluation
     ****************************************************************************/
    void addBodyForces(const Plato::ScalarMultiVectorT<GlobalStateT>& aGlobalState,
                       const Plato::ScalarMultiVectorT<ControlT>& aControl,
                       const Plato::ScalarArray3DT<ConfigT>& aConfig,
                       const Plato::ScalarMultiVectorT<ResultT>& aResult)
    {
        ...
        {
            ANALYZE_THROWERR(
                "Infinitesimal Strain Thermoplasticity Residual: 'Load Control Constant' is NOT defined in data map.")
        }
   ...
   public:
     ...
     *******************************************************************************/
    InfinitesimalStrainThermoPlasticityResidual(const Plato::SpatialDomain& aSpatialDomain,
     ...
     *
     * \param [in]     aCurrentGlobalState    current global state workset ( i.e. state at the n-th time interval (\f$
     *t^{n} \f$) ) \param [in]     aPrevGlobalState       previous global state workset ( i.e. state at the n-th minus
     *one time interval (\f$ t^{n-1} \f$) ) \param [in]     aCurrentLocalState     current local state workset ( i.e.
     *state at the n-th time interval (\f$ t^{n} \f$) ) \param [in]     aPrevLocalState        previous local state
     *workset ( i.e. state at the n-th minus one time interval (\f$ t^{n-1} \f$) ) \param [in] aProjectedPressureGrad
     *current pressure gradient workset ( i.e. state at the n-th time interval (\f$ t^{n} \f$) ) \param [in] aControls
     *design variables workset \param [in]     aConfig                configuration workset \param [in/out] aResult
     *residual workset \param [in]     aTimeData              current time data
     *
     ****************************************************************************/
                  ...
                  const Plato::TimeData& aTimeData) override
    {
        ...
        using GState_Config_LState_T =
            typename Plato::fad_type_t<SimplexPhysicsType, LocalStateT, ConfigT, GlobalStateT>;
        ...
        Plato::InterpolateFromNodal<mSpaceDim, mSpaceDim, mDisplacementDofOffset, mSpaceDim>
            tInterpolatePressGradFromNodal;
        Plato::InterpolateFromNodal<mSpaceDim, mNumGlobalDofsPerNode, mTemperatureDofOffset>
            tInterpolateTemperatureFromNodal;
        Plato::InterpolateGradientFromScalarNodal<mSpaceDim, mNumGlobalDofsPerNode, mTemperatureDofOffset>
            tInterpolateTemperatureGradFromNodal;
        ...
        Plato::ScalarArray3DT<ConfigT> tConfigurationGradient("configuration gradient", tNumCells, mNumNodesPerCell,
                                                              mSpaceDim);
        Plato::ScalarMultiVectorT<NodeStateT> tProjectedPressureGradGP("projected pressure gradient", tNumCells,
                                                                       mSpaceDim);
        ...
        Kokkos::parallel_for(
            "stabilized infinitesimal strain thermoplasticity residual", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                tInterpolateTemperatureGradFromNodal(aCellOrdinal, tConfigurationGradient, aCurrentGlobalState,
                                                     tTemperatureGrad);
                // Trace of the isotropic thermal strain tensor which for 2D plane strain and 3D is always
                // 3*thermal_strain
                tThermalVolumetricStrain(aCellOrdinal) =
                    static_cast<GlobalStateT>(3.0) * tThermalExpansionCoefficient *
                ...
                tComputeScalarGrad(aCellOrdinal, tNumDofsPerNode, tPressureDofOffset, aCurrentGlobalState,
                                   tConfigurationGradient, tPressureGrad);
                ...
                tInterpolatePressGradFromNodal(aCellOrdinal, tBasisFunctions, aProjectedPressureGrad,
                                               tProjectedPressureGradGP);
                ...
                tComputeStrainDivergence(aCellOrdinal, tTotalStrain,
                                         tStrainDivergence); /*This is actually displacement divergence*/

                // compute volume difference
                ControlT tPenalizedBulkModulus = tElasticPropertiesPenalty * tElasticBulkModulus;
                tPressure(aCellOrdinal) *= tPressureScaling;
                tVolumeStrain(aCellOrdinal) =
                    tPressureScaling * (tStrainDivergence(aCellOrdinal) - tThermalVolumetricStrain(aCellOrdinal) -
                                        tPressure(aCellOrdinal) / tPenalizedBulkModulus);

                // compute cell stabilization term
                tComputeStabilization(aCellOrdinal, tCellVolume, tPressureGrad, tProjectedPressureGradGP,
                                      tStabilization);
                // Plato::apply_penalty<mSpaceDim>(aCellOrdinal, tElasticPropertiesPenalty, tStabilization);

                // compute the thermal flux
                ControlT tPenalizedThermalConductivityCoefficient =
                    tElasticPropertiesPenalty * tThermalConductivityCoefficient;
                for (Plato::OrdinalType tDimIndex = 0; tDimIndex < tSpaceDim; ++tDimIndex)
                    tThermalFlux(aCellOrdinal, tDimIndex) =
                        static_cast<GState_Config_Control_T>(-1.0) * tPenalizedThermalConductivityCoefficient *
                       ...
                       const Plato::TimeData& aTimeData) override
    {
        ...
        mPenaltySIMP =
            tSuggestedPenaltySIMP >= mUpperBoundOnPenaltySIMP ? mUpperBoundOnPenaltySIMP : tSuggestedPenaltySIMP;
