...
#include "solver/nonlinear_solvers/NewtonRaphsonUtilities.hpp"
...
class NewtonRaphsonSolver
{
   ...
   private:
    ...
    static constexpr auto mNumGlobalDofsPerCell =
        PhysicsT::mNumDofsPerCell; /*!< number of global degrees of freedom per node*/
    ...
    Plato::Scalar
        mCurrentResidualNormTolerance; /*!< current residual norm stopping tolerance - avoids unnecessary solves */
   ...
   private:
    ...
    void applyConstraints(const Teuchos::RCP<Plato::CrsMatrixType>& aMatrix, const Plato::ScalarVector& aResidual)
    {
        ...
        {
            Plato::blas1::update(static_cast<Plato::Scalar>(1.), mDirichletValues, static_cast<Plato::Scalar>(0.),
                                 tDispControlledDirichletValues);
        }
        ...
        {
            Plato::applyBlockConstraints<mNumGlobalDofsPerNode>(aMatrix, aResidual, mDirichletDofs,
                                                                tDispControlledDirichletValues);
        }
        else
        {
            Plato::applyConstraints<mNumGlobalDofsPerNode>(aMatrix, aResidual, mDirichletDofs,
                                                           tDispControlledDirichletValues);
        }
    }
                            ...
                            Plato::CurrentStates& aStates)
    {
        ...
        if (mDebugFlag == true)
        {
            // std::string tFilename = std::string("matrix_newton_iteration_") + std::to_string(mCurrentSolverIter) +
            // ".txt"; Plato::print_sparse_matrix_to_file(aMatrix, tFilename);
                                                             ...
                                                             const Plato::ScalarArray3D& aInvLocalJacobianT)
    {
        ...
        Plato::assemble_jacobian(tNumCells, mNumGlobalDofsPerCell, mNumGlobalDofsPerCell, tGlobalJacEntryOrdinal, tDrDu,
                                 tJacEntries);
    ...
    Plato::ScalarVector assembleResidual(const Plato::ScalarVector& aControls,
                                         ...
                                         const Plato::ScalarArray3D& aInvLocalJacobianT)
    {
        auto tGlobalResidual = mGlobalEquation->value(aStates.mCurrentGlobalState, aStates.mPreviousGlobalState,
                                                      aStates.mCurrentLocalState, aStates.mPreviousLocalState,
                                                      aStates.mProjectedPressGrad, aControls, *(aStates.mTimeData));

        // compute local residual workset (WS)
        auto tLocalResidualWS = mLocalEquation->valueWorkSet(aStates.mCurrentGlobalState, aStates.mPreviousGlobalState,
                                                             aStates.mCurrentLocalState, aStates.mPreviousLocalState,
        ...
        Plato::blas2::matrix_times_vector("N", tAlpha, aInvLocalJacobianT, tLocalResidualWS, tBeta,
                                          tInvLocalJacTimesLocalRes);
    ...
    void initialize(Teuchos::ParameterList& aInputs)
    {
        ...
        auto tStopMeasure = Plato::ParseTools::getSubParam<std::string>(aInputs, "Newton-Raphson", "Stopping Measure",
   ...
   public:
     ...
     *******************************************************************************/
    NewtonRaphsonSolver(Plato::Mesh aMesh,
                        Teuchos::ParameterList& aInputs,
                        std::shared_ptr<Plato::AbstractSolver>& aLinearSolver)
        : mWorksetBase(aMesh),
          mStoppingTolerance(
              Plato::ParseTools::getSubParam<Plato::Scalar>(aInputs, "Newton-Raphson", "Stopping Tolerance", 1e-8)),
          mCurrentResidualNormTolerance(Plato::ParseTools::getSubParam<Plato::Scalar>(
              aInputs, "Newton-Raphson", "Current Residual Norm Stopping Tolerance", 1e-8)),
          mMaxNumSolverIter(Plato::ParseTools::getSubParam<Plato::OrdinalType>(
              aInputs, "Newton-Raphson", "Maximum Number Iterations", 10)),
          mCurrentSolverIter(0),
     ...
     * \brief Destructor
     *******************************************************************************/
    ~NewtonRaphsonSolver() { this->closeDiagnosticsFile(); }
     ...
     *******************************************************************************/
    void debug(const bool& aInput) { mDebugFlag = aInput; }
     ...
     *******************************************************************************/
    void appendDirichletValues(const Plato::ScalarVector& aInput) { mDirichletValues = aInput; }
     ...
     *******************************************************************************/
    void appendOutputMessage(const std::stringstream& aInput) { mSolverDiagnosticsFile << aInput.str().c_str(); }
    ...
    bool solve(const Plato::ScalarVector& aControls, Plato::CurrentStates& aStates)
    {
        ...
        auto tNumCells = mLocalEquation->numCells();
        Plato::ScalarArray3D tInvLocalJacobianT("Inverse Transpose DhDc", tNumCells, mNumLocalDofsPerCell,
