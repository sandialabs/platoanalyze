...
struct FunctionFactory
{
    ...
    std::shared_ptr<Plato::AbstractLocalVectorFunctionInc<EvaluationType>> createLocalVectorFunctionInc(
        const Plato::SpatialDomain& aSpatialDomain, Plato::DataMap& aDataMap, Teuchos::ParameterList& aInputParams)
    {
        ...
        if (tPlasticityParamList.isSublist("J2 Plasticity"))
        {
            ...
            return std::make_shared<J2PlasticityLocalResidual<EvaluationType, Plato::SimplexPlasticity<tSpaceDim>>>(
                aSpatialDomain, aDataMap, aInputParams);
        }
        else
        {
            ANALYZE_THROWERR(
                "Unknown Plasticity Model.  Options are: J2 Plasticity.  User is advised to select one of the "
                "available options.")
        }
    }
};  // struct FunctionFactory
