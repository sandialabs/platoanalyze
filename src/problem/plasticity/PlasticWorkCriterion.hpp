...
class PlasticWorkCriterion : public Plato::AbstractLocalScalarFunctionInc<EvaluationType>
{
   ...
   public:
     ...
     *******************************************************************************/
    PlasticWorkCriterion(const Plato::SpatialDomain& aSpatialDomain,
     ...
     * \param [in] aName        scalar function name
     *******************************************************************************/
    PlasticWorkCriterion(const Plato::SpatialDomain& aSpatialDomain, Plato::DataMap& aDataMap, std::string aName = "")
        : FunctionBaseType(aSpatialDomain, aDataMap, aName),
          mBulkModulus(1.0),
          mShearModulus(1.0),
          mThermalExpansionCoefficient(0.0),
                  ...
                  const Plato::TimeData& aTimeData)
    {
        ...
        Kokkos::parallel_for(
            "plastic work criterion", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                tJ2PlasticityUtils.computePlasticStrainMisfit(aCellOrdinal, aCurrentLocalState, aPreviousLocalState,
                                                              tPlasticStrainMisfit);
                ...
                for (Plato::OrdinalType tIndex = 0; tIndex < tNumStressTerms; ++tIndex)
                    aResult(aCellOrdinal) +=
                        tAverageCauchyStress(aCellOrdinal, tIndex) * tPlasticStrainMisfit(aCellOrdinal, tIndex);
