...
/******************************************************************************/
{
   ...
   public:
    ...
    void constrainInactiveNodes(const Teuchos::RCP<Plato::CrsMatrixType>& aMatrix, Plato::ScalarVector aVector) const
    {
        auto tNodes = mMask->getInactiveNodes();

        if (aMatrix->isBlockMatrix())
        {
            Plato::applyBlockConstraints<mNumDofsPerNode>(aMatrix, aVector, tNodes);
        }
        else
        {
            Plato::applyConstraints<mNumDofsPerNode>(aMatrix, aVector, tNodes);
        }
    }
