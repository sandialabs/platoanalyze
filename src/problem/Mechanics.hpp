...
#include "problem/AbstractLocalMeasure.hpp"
#include "problem/elliptic/AbstractScalarFunction.hpp"
#include "problem/elliptic/EffectiveEnergy.hpp"
#include "problem/elliptic/ElastostaticResidual.hpp"
#include "problem/elliptic/IntermediateDensityPenalty.hpp"
#include "problem/elliptic/InternalElasticEnergy.hpp"
...
#include "problem/elliptic/StressPNorm.hpp"
#include "problem/elliptic/TensileEnergyDensityLocalMeasure.hpp"
#include "problem/elliptic/VolAvgStressPNormDenominator.hpp"
#include "problem/elliptic/Volume.hpp"
#include "problem/elliptic/VolumeAverageCriterionDenominator.hpp"
#include "problem/elliptic/VolumeIntegralCriterion.hpp"
    ...
    const std::string& aFuncName)
{
    ...
    {
        return std::make_shared<VonMisesLocalMeasure<EvaluationType>>(aSpatialDomain, aDataMap, aProblemParams,
                                                                      "VonMises");
    }
 ...
 **********************************************************************************/
template <typename EvaluationType>
inline std::shared_ptr<Plato::Elliptic::AbstractScalarFunction<EvaluationType>> stress_constraint_linear(
    const plato::domain::SpatialDomain& aSpatialDomain,
    Plato::DataMap& aDataMap,
    Teuchos::ParameterList& aProblemParams,
    const std::string& aFuncName)
{
    ...
    tOutput = std::make_shared<Plato::AugLagStressCriterion<EvaluationType>>(aSpatialDomain, aDataMap, aProblemParams,
                                                                             aFuncName);
    return (tOutput);
}
 ...
 **********************************************************************************/
template <typename EvaluationType>
inline std::shared_ptr<Plato::Elliptic::AbstractScalarFunction<EvaluationType>> stress_constraint_general(
    const plato::domain::SpatialDomain& aSpatialDomain,
    Plato::DataMap& aDataMap,
    Teuchos::ParameterList& aProblemParams,
    const std::string& aFuncName)
{
    std::shared_ptr<Plato::Elliptic::AbstractScalarFunction<EvaluationType>> tOutput;
    tOutput = std::make_shared<Plato::AugLagStressCriterionGeneral<EvaluationType>>(aSpatialDomain, aDataMap,
...
inline std::shared_ptr<Plato::Elliptic::AbstractScalarFunction<EvaluationType>> stress_constraint_quadratic(
    ...
    const std::string& aFuncName)
{
    auto EvalMeasure = Plato::MechanicsFactory::create_local_measure<EvaluationType>(aSpatialDomain, aDataMap,
                                                                                     aProblemParams, aFuncName);
    using Residual = typename Plato::Elliptic::ResidualTypes<typename EvaluationType::ElementType>;
    auto PODMeasure =
        Plato::MechanicsFactory::create_local_measure<Residual>(aSpatialDomain, aDataMap, aProblemParams, aFuncName);

    std::shared_ptr<Plato::AugLagStressCriterionQuadratic<EvaluationType>> tOutput;
    tOutput = std::make_shared<Plato::AugLagStressCriterionQuadratic<EvaluationType>>(aSpatialDomain, aDataMap,
                                                                                      aProblemParams, aFuncName);
...
volume_integral_criterion_for_volume_average(const plato::domain::SpatialDomain& aSpatialDomain,
                                             Plato::DataMap& aDataMap,
                                             Teuchos::ParameterList& aProblemParams,
                                             const std::string& aFuncName)
{
    auto tLocalMeasure = Plato::MechanicsFactory::create_local_measure<EvaluationType>(aSpatialDomain, aDataMap,
                                                                                       aProblemParams, aFuncName);

    std::shared_ptr<Plato::Elliptic::VolumeIntegralCriterion<EvaluationType>> tOutput;
    tOutput = std::make_shared<Plato::Elliptic::VolumeIntegralCriterion<EvaluationType>>(aSpatialDomain, aDataMap,
                                                                                         aProblemParams, aFuncName);
 ...
 **********************************************************************************/
template <typename EvaluationType>
inline std::shared_ptr<Plato::Elliptic::AbstractScalarFunction<EvaluationType>> vol_avg_criterion_denominator(
    const plato::domain::SpatialDomain& aSpatialDomain,
    Plato::DataMap& aDataMap,
    Teuchos::ParameterList& aProblemParams,
...
struct FunctionFactory
{
        ...
        std::string aFuncName)
    {
        ...
        {
            return Plato::makeScalarFunction<EvaluationType, Plato::Elliptic::StressPNorm>(aSpatialDomain, aDataMap,
                                                                                           aProblemParams, aFuncName);
        }
        ...
        {
            return Plato::makeScalarFunction<EvaluationType, Plato::Elliptic::Volume>(aSpatialDomain, aDataMap,
                                                                                      aProblemParams, aFuncName);
        }
        ...
        {
            return Plato::MechanicsFactory::vol_avg_criterion_denominator<EvaluationType>(aSpatialDomain, aDataMap,
                                                                                          aProblemParams, aFuncName);
        }
        ...
        {
            return Plato::MechanicsFactory::stress_constraint_linear<EvaluationType>(aSpatialDomain, aDataMap,
                                                                                     aProblemParams, aFuncName);
        }
        else if (tLowerFuncType == "stress constraint general")
        {
            return Plato::MechanicsFactory::stress_constraint_general<EvaluationType>(aSpatialDomain, aDataMap,
                                                                                      aProblemParams, aFuncName);
        }
        else if (tLowerFuncType == "stress constraint quadratic")
        {
            return Plato::MechanicsFactory::stress_constraint_quadratic<EvaluationType>(aSpatialDomain, aDataMap,
                                                                                        aProblemParams, aFuncName);
        }
        else if (tLowerFuncType == "density penalty")
        {
            return std::make_shared<Plato::IntermediateDensityPenalty<EvaluationType>>(aSpatialDomain, aDataMap,
                                                                                       aProblemParams, aFuncName);
        }
        else
        {
            const std::string tErrorString =
                std::string("Function '") + tLowerFuncType + "' not implemented yet in steady state mechanics.";
