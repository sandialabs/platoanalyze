...
void PhysicsScalarFunction<PhysicsType>::initialize(Teuchos::ParameterList& aInputParams)
{
    ...
    for (const auto& tDomain : mSpatialModel.mDomains)
    {
        ...
        mValueFunctions[tName] = tFactory.template createScalarFunction<Residual>(tDomain, mDataMap, aInputParams,
                                                                                  tFunctionType, mFunctionName);

        mGradientUFunctions[tName] = tFactory.template createScalarFunction<GradientU>(tDomain, mDataMap, aInputParams,
                                                                                       tFunctionType, mFunctionName);

        mGradientVFunctions[tName] = tFactory.template createScalarFunction<GradientV>(tDomain, mDataMap, aInputParams,
                                                                                       tFunctionType, mFunctionName);

        mGradientAFunctions[tName] = tFactory.template createScalarFunction<GradientA>(tDomain, mDataMap, aInputParams,
                                                                                       tFunctionType, mFunctionName);

        mGradientXFunctions[tName] = tFactory.template createScalarFunction<GradientX>(tDomain, mDataMap, aInputParams,
                                                                                       tFunctionType, mFunctionName);

        mGradientZFunctions[tName] = tFactory.template createScalarFunction<GradientZ>(tDomain, mDataMap, aInputParams,
                                                                                       tFunctionType, mFunctionName);
    }
}
                                                        ...
                                                        Plato::Scalar aTimeStep) const
{
    ...
    for (const auto& tDomain : mSpatialModel.mDomains)
    {
        ...
        Plato::ScalarMultiVectorT<StateDotDotScalar> tStateDotDotWS("state dot dot workset", tNumCells,
                                                                    mNumDofsPerCell);
        ...
        for (decltype(tNumSteps) tStepIndex = 1; tStepIndex < tNumSteps; ++tStepIndex)
        {
            ...
            mValueFunctions.at(tName)->evaluate(tStateWS, tStateDotWS, tStateDotDotWS, tControlWS, tConfigWS, tResult,
                                                aTimeStep);
    ...
    for (decltype(tNumSteps) tStepIndex = 1; tStepIndex < tNumSteps; ++tStepIndex)
    {
        auto tName = mSpatialModel.mDomains.front().domainName();
                                                                   ...
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    for (const auto& tDomain : mSpatialModel.mDomains)
    {
        ...
        // evaluate function
        //
        mGradientUFunctions.at(tName)->evaluate(tStateWS, tStateDotWS, tStateDotDotWS, tControlWS, tConfigWS, tResult,
                                                aTimeStep);
                                                                   ...
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    for (const auto& tDomain : mSpatialModel.mDomains)
    {
        ...
        // evaluate function
        //
        mGradientVFunctions.at(tName)->evaluate(tStateWS, tStateDotWS, tStateDotDotWS, tControlWS, tConfigWS, tResult,
                                                aTimeStep);

        Plato::assemble_vector_gradient_fad<mNumNodesPerCell, mNumDofsPerNode>(tDomain, mGlobalStateEntryOrdinal,
                                                                               tResult, tObjGradientV);
 ...
 **********************************************************************************/
template <typename PhysicsType>
Plato::ScalarVector PhysicsScalarFunction<PhysicsType>::gradient_a(const Plato::Solutions& aSolution,
                                                                   const Plato::ScalarVector& aControl,
                                                                   Plato::OrdinalType aStepIndex,
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    for (const auto& tDomain : mSpatialModel.mDomains)
    {
        ...
        // evaluate function
        //
        mGradientAFunctions.at(tName)->evaluate(tStateWS, tStateDotWS, tStateDotDotWS, tControlWS, tConfigWS, tResult,
                                                aTimeStep);

        Plato::assemble_vector_gradient_fad<mNumNodesPerCell, mNumDofsPerNode>(tDomain, mGlobalStateEntryOrdinal,
                                                                               tResult, tObjGradientA);
