...
#include "problem/hyperbolic/ElastomechanicsResidual.hpp"
#include "problem/hyperbolic/InternalElasticEnergy.hpp"
#include "problem/hyperbolic/StressPNorm.hpp"
...
struct FunctionFactory
{
        ...
        std::string aFuncName)
    {
        ...
        {
            return Plato::makeScalarFunction<EvaluationType, Plato::Hyperbolic::StressPNorm>(aSpatialDomain, aDataMap,
                                                                                             aProblemParams, aFuncName);
        }
        else
        {
            const std::string tErrorString =
                std::string("Function '") + tLowerFuncType + "' not implemented yet in transient mechanics.";
