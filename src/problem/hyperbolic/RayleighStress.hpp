...
class RayleighStress : public ElementType
{
   ...
   public:
     ...
     **********************************************************************************/
    RayleighStress(const Plato::Matrix<mNumVoigtTerms, mNumVoigtTerms> aCellStiffness) : mCellStiffness(aCellStiffness)
    {
    ...
    KOKKOS_INLINE_FUNCTION void operator()(Plato::Array<mNumVoigtTerms, StressScalarType>& aCauchyStress,
                                           const Plato::Array<mNumVoigtTerms, StrainScalarType>& aSmallStrain,
                                           const Plato::Array<mNumVoigtTerms, VelGradScalarType>& aVelGrad) const
    {
        for (Plato::OrdinalType iVoigt = 0; iVoigt < mNumVoigtTerms; iVoigt++)
        {
            ...
            {
                aCauchyStress(iVoigt) +=
                    (aSmallStrain(jVoigt) - mReferenceStrain(jVoigt)) * mCellStiffness(iVoigt, jVoigt) +
