 ...
 *******************************************************************************/
template <typename ElementType, Plato::OrdinalType DofOffset = 0>
class FullStressDivergence : public ElementType
{
   private:
    ...
    Plato::OrdinalType mVoigtMap[mNumSpatialDims][mNumSpatialDims]; /*!< matrix with indices to stress tensor entries in
                                                                       Voigt notation */
    ...
    Plato::Scalar mSkewScale[mNumSpatialDims][mNumSpatialDims]; /*!< matrix with indices to scalr factors for skew terms
                                                                   with Voigt storage */
   ...
   public:
    ...
    {
        this->addSymmetricStressDivergence(aCellOrdinal, aGpOrdinal, aOutput, aSymmetricStress, aGradient, aVolume,
                                           aScale);
        this->addSkewStressDivergence(aCellOrdinal, aGpOrdinal, aOutput, aSkewStress, aGradient, aVolume, aScale);
    }

   private:
    inline void initializeVoigtMap()
    {
    ...
    inline void initializeSkewScale()
    {
        for (Plato::OrdinalType tDofIndex = 0; tDofIndex < mNumSpatialDims; tDofIndex++)
        {
    ...
    template <typename ForcingScalarType,
              ...
              typename GradientScalarType,
              typename VolumeScalarType>
    KOKKOS_INLINE_FUNCTION void addSymmetricStressDivergence(Plato::OrdinalType aCellOrdinal,
                                                             Plato::OrdinalType aGpOrdinal,
                                                             Plato::ScalarMultiVectorT<ForcingScalarType> aOutput,
    ...
    template <typename ForcingScalarType,
              ...
              typename GradientScalarType,
              typename VolumeScalarType>
    KOKKOS_INLINE_FUNCTION void addSkewStressDivergence(Plato::OrdinalType aCellOrdinal,
                                                        Plato::OrdinalType aGpOrdinal,
                                                        Plato::ScalarMultiVectorT<ForcingScalarType> aOutput,
                                                        const Plato::ScalarArray3DT<StressScalarType>& aStress,
                                                        const Plato::ScalarArray4DT<GradientScalarType>& aGradient,
                                                        ...
                                                        Plato::Scalar aScale) const
    {
        for (Plato::OrdinalType tDimIndexI = 0; tDimIndexI < mNumSpatialDims; tDimIndexI++)
        {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < mNumNodesPerCell; tNodeIndex++)
            {
                ...
                {
                    Kokkos::atomic_add(&aOutput(aCellOrdinal, tLocalOrdinal),
                                       aScale * mSkewScale[tDimIndexI][tDimIndexJ] * aVolume(aCellOrdinal, aGpOrdinal) *
                                           aStress(aCellOrdinal, aGpOrdinal, mVoigtMap[tDimIndexI][tDimIndexJ]) *
                                           aGradient(aCellOrdinal, aGpOrdinal, tNodeIndex, tDimIndexJ));
                }
            }
