...
class LeastSquaresScalarFunction : public Plato::Fluids::CriterionBase
{
   private:
    ...
    static constexpr auto mNumControlDofsPerNode =
        PhysicsT::SimplexT::mNumControlDofsPerNode; /*!< number of control dofs per node */
    static constexpr auto mNumConfigDofsPerNode =
        PhysicsT::SimplexT::mNumConfigDofsPerNode; /*!< number of configuration dofs per node */
   ...
   public:
     ...
     **********************************************************************************/
    LeastSquaresScalarFunction(const Plato::SpatialModel& aModel,
                               Plato::DataMap& aDataMap,
                               Teuchos::ParameterList& aInputs,
                               std::string& aTag)
        : mDiagnostics(false), mDataMap(aDataMap), mSpatialModel(aModel), mFuncName(aTag)
    {
                                       ...
                                       const Plato::Primal& aPrimal) const override
    {
        ...
        for (auto& tCriterion : mCriteria)
        {
            ...
            auto tMultiplier =
                (static_cast<Plato::Scalar>(2.0) * tWeight * tMisfit) / (tNormalization * tNormalization);
   ...
   private:
    ...
    void checkInputs()
    {
        ...
        {
            ANALYZE_THROWERR(
                std::string("Dimensions mismatch.  Number of 'Functions' and 'Normalizations' do not match. ") +
                "Check inputs for scalar function with name '" + mFuncName + "'.")
        }
        ...
        {
            ANALYZE_THROWERR(
                std::string("Dimensions mismatch.  Number of 'Functions' and 'Gold/Target' values do not match. ") +
                "Check inputs for scalar function with name '" + mFuncName + "'.")
        }
    }
    ...
    void initialize(Teuchos::ParameterList& aInputs)
    {
        ...
        {
            ANALYZE_THROWERR(std::string("Scalar function with tag '") + mFuncName +
    ...
    void parseTargets(Teuchos::ParameterList& aInputs)
    {
        ...
        {
            ANALYZE_THROWERR(std::string("'Targets' keyword was not defined in function block with name '") +
                             mFuncName +
    ...
    void parseWeights(Teuchos::ParameterList& aInputs)
    {
        ...
        if (mCriterionWeights.empty())
        {
            ...
            {
                ANALYZE_THROWERR(
                    "Criterion names have not been parsed. Users must define the 'Functions' keyword to use the 'Least "
                    "Squares' criterion.")
            }
