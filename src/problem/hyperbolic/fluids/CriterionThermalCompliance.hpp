...
#include "problem/hyperbolic/fluids/FluidsThermalSources.hpp"
...
class CriterionThermalCompliance : public Plato::Fluids::AbstractScalarFunction<PhysicsT, EvaluationT>
{
   private:
    ...
    static constexpr auto mNumTempDofsPerNode =
        PhysicsT::mNumEnergyDofsPerNode; /*!< number of degrees of freedom per node */
    ...
    std::shared_ptr<Plato::NaturalBCs<mNumSpatialDims, mNumTempDofsPerNode>>
        mHeatFlux; /*!< natural boundary condition evaluator */
   ...
   public:
     ...
     ******************************************************************************/
    CriterionThermalCompliance(const std::string& aName,
                               const Plato::SpatialDomain& aDomain,
                               Plato::DataMap& aDataMap,
                               Teuchos::ParameterList& aInputs)
        : mDataMap(aDataMap), mSpatialDomain(aDomain), mFuncName(aName)
    {
     ...
     ******************************************************************************/
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarVectorT<ResultT>& aResultWS) const
    {
        if (mThermalSources.empty())
        {
            return;
        }
    ...
    void evaluateBoundary(const Plato::SpatialModel& aSpatialModel,
                          ...
                          Plato::ScalarVectorT<ResultT>& aResultWS) const override
    {
        if (mHeatFlux != nullptr)
        {
            ...
            auto tCurTempWS =
                Plato::metadata<Plato::ScalarMultiVectorT<CurTempT>>(aWorkSets.get("current temperature"));
...
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionThermalCompliance, Plato::IncompressibleFluids, Plato::SimplexFluids, 1, 1)
#endif

#ifdef PLATOANALYZE_2D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionThermalCompliance, Plato::IncompressibleFluids, Plato::SimplexFluids, 2, 1)
#endif

#ifdef PLATOANALYZE_3D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionThermalCompliance, Plato::IncompressibleFluids, Plato::SimplexFluids, 3, 1)
#endif
