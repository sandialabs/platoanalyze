...
class CriterionMeanSurfacePressure : public Plato::Fluids::AbstractScalarFunction<PhysicsT, EvaluationT>
{
   ...
   public:
     ...
     ******************************************************************************/
    CriterionMeanSurfacePressure(const std::string& aName,
                          ...
                          Plato::ScalarVectorT<ResultT>& aResult) const override
    {
        ...
        for (auto& tName : mSideSets)
        {
            ...
            Kokkos::parallel_for(
                "integrate surface pressure", Kokkos::RangePolicy<>(0, tNumFaces),
                KOKKOS_LAMBDA(const Plato::OrdinalType& aSideOrdinal) {
                    ...
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        ...
                        tCurrentPressGP(tElementOrdinal) +=
                            tBasisFunctions(tNode) * tCurrentPressWS(tElementOrdinal, tLocalElementNode);
                    }
                    ...
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        tResult(tElementOrdinal) +=
                            tBasisFunctions(tNode) * tCurrentPressGP(tElementOrdinal) * tSurfaceArea(aSideOrdinal);
                    }
                });

            Kokkos::parallel_for(
                "calculate mean surface pressure", Kokkos::RangePolicy<>(0, tNumCells),
...
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionMeanSurfacePressure, Plato::IncompressibleFluids, Plato::SimplexFluids, 1, 1)
#endif

#ifdef PLATOANALYZE_2D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionMeanSurfacePressure, Plato::IncompressibleFluids, Plato::SimplexFluids, 2, 1)
#endif

#ifdef PLATOANALYZE_3D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionMeanSurfacePressure, Plato::IncompressibleFluids, Plato::SimplexFluids, 3, 1)
#endif
