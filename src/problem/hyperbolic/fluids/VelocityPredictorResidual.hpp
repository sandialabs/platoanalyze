...
#include "problem/hyperbolic/fluids/BrinkmanForces.hpp"
#include "problem/hyperbolic/fluids/FluidsUtils.hpp"
#include "problem/hyperbolic/fluids/MomentumConservationUtils.hpp"
#include "problem/hyperbolic/fluids/SimplexFluids.hpp"
#include "problem/hyperbolic/fluids/SimplexFluidsFadTypes.hpp"
#include "problem/hyperbolic/fluids/ThermalBuoyancy.hpp"
#include "problem/hyperbolic/fluids/WorkSets.hpp"
...
class VelocityPredictorResidual : public Plato::Fluids::AbstractVectorFunction<PhysicsT, EvaluationT>
{
   private:
    ...
    using AdvectionT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT, PrevVelT, ConfigT>; /*!< advection force FAD
                                                                                               type */
    using PredStrainT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT, PredVelT, ConfigT>; /*!< predicted strain rate
                                                                                               FAD type */
    using PrevStrainT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT, PrevVelT, ConfigT>; /*!< previous strain rate
                                                                                               FAD type */
    ...
    std::shared_ptr<Plato::NaturalBCs<mNumSpatialDims, mNumDofsPerNode>>
        mPrescribedBCs; /*!< prescribed boundary conditions, e.g. tractions */
    std::shared_ptr<Plato::Fluids::BrinkmanForces<PhysicsT, EvaluationT>>
        mBrinkmanForces; /*!< Brinkman force evaluator */
    std::shared_ptr<Plato::Fluids::ThermalBuoyancy<PhysicsT, EvaluationT>>
        mThermalBuoyancy; /*!< thermal buoyancy force evaluator */
    ...
    bool mCalculateBrinkmanForces =
        false; /*!< indicator to determine if Brinkman forces will be considered in calculations */
    bool mCalculateThermalBuoyancyForces =
        false; /*!< indicator to determine if thermal buoyancy forces will be considered in calculations */
   ...
   public:
     ...
     ******************************************************************************/
    VelocityPredictorResidual(const Plato::SpatialDomain& aDomain,
     ...
     ******************************************************************************/
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarMultiVectorT<ResultT>& aResultWS) const override
    {
        ...
        if (tNumCells != static_cast<Plato::OrdinalType>(aResultWS.extent(0)))
        {
            ANALYZE_THROWERR(std::string("Number of elements mismatch. Spatial domain and output/result workset ") +
                             ...
                             "' elements and output workset has '" + std::to_string(aResultWS.extent(0)) +
                             "' elements.")
        }
        ...
        Plato::ScalarArray3DT<PredStrainT> tPredStrainRate("cell strain rate", tNumCells, mNumSpatialDims,
                                                           mNumSpatialDims);
        Plato::ScalarArray3DT<PrevStrainT> tPrevStrainRate("cell strain rate", tNumCells, mNumSpatialDims,
                                                           mNumSpatialDims);
        ...
        Plato::InterpolateFromNodal<mNumSpatialDims, mNumVelDofsPerNode, 0 /*offset*/, mNumSpatialDims>
            tIntrplVectorField;
        ...
        Kokkos::parallel_for(
            "quasi-implicit predicted velocity residual", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                Plato::Fluids::strain_rate<mNumNodesPerCell, mNumSpatialDims>(aCellOrdinal, tPredVelWS, tGradient,
                                                                              tPredStrainRate);
                ...
                Plato::Fluids::strain_rate<mNumNodesPerCell, mNumSpatialDims>(aCellOrdinal, tPrevVelWS, tGradient,
                                                                              tPrevStrainRate);
                Plato::Fluids::integrate_viscous_forces<mNumNodesPerCell, mNumSpatialDims>(
                ...
                tMultiplier =
                    tStabilization * static_cast<Plato::Scalar>(0.5) * tCriticalTimeStep(0) * tCriticalTimeStep(0);
     ...
     * \param [in/out] aResultWS result/output workset
     ******************************************************************************/
    void evaluateBoundary(const Plato::SpatialModel& aSpatialModel,
     ...
     * \param [in/out] aResultWS result/output workset
     ******************************************************************************/
    void evaluatePrescribed(const Plato::SpatialModel& aSpatialModel,
                            const Plato::WorkSets& aWorkSets,
                            Plato::ScalarMultiVectorT<ResultT>& aResultWS) const override
    {
        if (mPrescribedBCs != nullptr)
        {
            ...
            auto tCriticalTimeStep = Plato::metadata<Plato::ScalarVector>(aWorkSets.get("critical time step"));
            Kokkos::parallel_for(
                "traction force", Kokkos::RangePolicy<>(0, tNumCells),
                KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
   ...
   private:
     ...
     * \param [in] aInputs  input file metadata
     ******************************************************************************/
    void initialize(const Plato::SpatialDomain& aDomain, Plato::DataMap& aDataMap, Teuchos::ParameterList& aInputs)
    {
     ...
     * \param [in] aInputs  input file metadata
     ******************************************************************************/
    void setBrinkmanForces(const Plato::SpatialDomain& aDomain,
                           Plato::DataMap& aDataMap,
                           Teuchos::ParameterList& aInputs)
    {
     ...
     * \param [in] aInputs  input file metadata
     ******************************************************************************/
    void setThermalBuoyancyForces(const Plato::SpatialDomain& aDomain,
                                  Plato::DataMap& aDataMap,
                                  Teuchos::ParameterList& aInputs)
    {
        auto tHeatTransferTag = Plato::Fluids::heat_transfer_tag(aInputs);
        mCalculateThermalBuoyancyForces =
            ((tHeatTransferTag == "natural") || (tHeatTransferTag == "mixed")) ? true : false;
        if (mCalculateThermalBuoyancyForces)
        {
            mThermalBuoyancy =
