...
#include "parsing/UtilsTeuchos.hpp"
#include "problem/hyperbolic/fluids/AbstractScalarFunction.hpp"
#include "problem/hyperbolic/fluids/FluidsCriterionBase.hpp"
...
class ScalarFunction : public Plato::Fluids::CriterionBase
{
   private:
    ...
    using GradConfigEvalT =
        typename Plato::Fluids::Evaluation<typename PhysicsT::SimplexT>::GradConfig; /*!< partial wrt configuration FAD
                                                                                        evaluation type */
    using GradControlEvalT =
        typename Plato::Fluids::Evaluation<typename PhysicsT::SimplexT>::GradControl; /*!< partial wrt control FAD
                                                                                         evaluation type */
    using GradCurVelEvalT = typename Plato::Fluids::Evaluation<
        typename PhysicsT::SimplexT>::GradCurMomentum; /*!< partial wrt current velocity state FAD evaluation type */
    using GradCurTempEvalT = typename Plato::Fluids::Evaluation<
        typename PhysicsT::SimplexT>::GradCurEnergy; /*!< partial wrt current temperature state FAD evaluation type */
    using GradCurPressEvalT =
        typename Plato::Fluids::Evaluation<typename PhysicsT::SimplexT>::GradCurMass; /*!< partial wrt current pressure
                                                                                         state FAD evaluation type */

    // element scalar functions types
    using ValueFunc =
        std::shared_ptr<Plato::Fluids::AbstractScalarFunction<PhysicsT, ResidualEvalT>>; /*!< short name/notation for a
                                                                                            scalar function of residual
                                                                                            FAD evaluation type */
    using GradConfigFunc =
        std::shared_ptr<Plato::Fluids::AbstractScalarFunction<PhysicsT, GradConfigEvalT>>; /*!< short name/notation for
                                                                                              a scalar function of
                                                                                              partial wrt configuration
                                                                                              FAD evaluation type */
    using GradControlFunc =
        std::shared_ptr<Plato::Fluids::AbstractScalarFunction<PhysicsT, GradControlEvalT>>; /*!< short name/notation for
                                                                                               a scalar function of
                                                                                               partial wrt control FAD
                                                                                               evaluation type */
    using GradCurVelFunc = std::shared_ptr<
        Plato::Fluids::AbstractScalarFunction<PhysicsT, GradCurVelEvalT>>; /*!< short name/notation for a scalar
                                                                              function of partial wrt current velocity
                                                                              state FAD evaluation type */
    using GradCurTempFunc = std::shared_ptr<
        Plato::Fluids::AbstractScalarFunction<PhysicsT, GradCurTempEvalT>>; /*!< short name/notation for a scalar
                                                                               function of partial wrt current
                                                                               temperature state FAD evaluation type */
    using GradCurPressFunc = std::shared_ptr<
        Plato::Fluids::AbstractScalarFunction<PhysicsT, GradCurPressEvalT>>; /*!< short name/notation for a scalar
                                                                                function of partial wrt current pressure
                                                                                state FAD evaluation type */

    // element scalar functions per element block, i.e. domain
    std::unordered_map<std::string, ValueFunc>
        mValueFuncs; /*!< map from domain (i.e. element block) to scalar function of residual FAD evaluation type */
    std::unordered_map<std::string, GradConfigFunc>
        mGradConfigFuncs; /*!< map from domain (i.e. element block) to scalar function of partial wrt configuration FAD
                             evaluation type */
    std::unordered_map<std::string, GradControlFunc>
        mGradControlFuncs; /*!< map from domain (i.e. element block) to scalar function of partial wrt control FAD
                              evaluation type */
    std::unordered_map<std::string, GradCurVelFunc>
        mGradCurrentVelocityFuncs; /*!< map from domain (i.e. element block) to scalar function of partial wrt current
                                      velocity state FAD evaluation type */
    std::unordered_map<std::string, GradCurPressFunc>
        mGradCurrentPressureFuncs; /*!< map from domain (i.e. element block) to scalar function of partial wrt current
                                      pressure state FAD evaluation type */
    std::unordered_map<std::string, GradCurTempFunc>
        mGradCurrentTemperatureFuncs; /*!< map from domain (i.e. element block) to scalar function of partial wrt
                                         current temperature state FAD evaluation type */

    Plato::DataMap& mDataMap;                 /*!< holds output metadata */
    const Plato::SpatialModel& mSpatialModel; /*!< holds mesh and entity sets metadata */
    Plato::LocalOrdinalMaps<PhysicsT>
        mLocalOrdinalMaps; /*!< holds maps from element to local state degree of freedom */
   ...
   public:
     ...
     ******************************************************************************/
    ScalarFunction(const Plato::SpatialModel& aModel,
                   Plato::DataMap& aDataMap,
                   Teuchos::ParameterList& aInputs,
                   std::string& aName)
        : mFuncName(aName), mSpatialModel(aModel), mDataMap(aDataMap), mLocalOrdinalMaps(aModel.Mesh)
    {
     ...
     ******************************************************************************/
    Plato::Scalar value(const Plato::ScalarVector& aControls, const Plato::Primal& aVariables) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            Plato::Fluids::build_scalar_function_worksets<ResidualEvalT>(tDomain, aControls, aVariables,
        ...
        // evaluate boundary
        {
            ...
            Plato::Fluids::build_scalar_function_worksets<ResidualEvalT>(tNumCells, aControls, aVariables,
                                                                         mLocalOrdinalMaps, tInputWorkSets);
     ...
     ******************************************************************************/
    Plato::ScalarVector gradientConfig(const Plato::ScalarVector& aControls, const Plato::Primal& aVariables) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = tDomain.numCells();
            Plato::Fluids::build_scalar_function_worksets<GradConfigEvalT>(tDomain, aControls, aVariables,
                                                                           mLocalOrdinalMaps, tInputWorkSets);
        ...
        // evaluate boundary
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = mSpatialModel.Mesh->NumElements();
            Plato::Fluids::build_scalar_function_worksets<GradConfigEvalT>(tNumCells, aControls, aVariables,
                                                                           mLocalOrdinalMaps, tInputWorkSets);
     ...
     ******************************************************************************/
    Plato::ScalarVector gradientControl(const Plato::ScalarVector& aControls, const Plato::Primal& aVariables) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = tDomain.numCells();
            Plato::Fluids::build_scalar_function_worksets<GradControlEvalT>(tDomain, aControls, aVariables,
                                                                            mLocalOrdinalMaps, tInputWorkSets);
        ...
        // evaluate boundary
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = mSpatialModel.Mesh->NumElements();
            Plato::Fluids::build_scalar_function_worksets<GradControlEvalT>(tNumCells, aControls, aVariables,
                                                                            mLocalOrdinalMaps, tInputWorkSets);
                                             ...
                                             const Plato::Primal& aVariables) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = tDomain.numCells();
            Plato::Fluids::build_scalar_function_worksets<GradCurPressEvalT>(tDomain, aControls, aVariables,
                                                                             mLocalOrdinalMaps, tInputWorkSets);
        ...
        // evaluate boundary
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = mSpatialModel.Mesh->NumElements();
            Plato::Fluids::build_scalar_function_worksets<GradCurPressEvalT>(tNumCells, aControls, aVariables,
                                                                             mLocalOrdinalMaps, tInputWorkSets);
     ...
     ******************************************************************************/
    Plato::ScalarVector gradientCurrentTemp(const Plato::ScalarVector& aControls, const Plato::Primal& aVariables) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = tDomain.numCells();
            Plato::Fluids::build_scalar_function_worksets<GradCurTempEvalT>(tDomain, aControls, aVariables,
                                                                            mLocalOrdinalMaps, tInputWorkSets);
        ...
        // evaluate boundary
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = mSpatialModel.Mesh->NumElements();
            Plato::Fluids::build_scalar_function_worksets<GradCurTempEvalT>(tNumCells, aControls, aVariables,
                                                                            mLocalOrdinalMaps, tInputWorkSets);
     ...
     ******************************************************************************/
    Plato::ScalarVector gradientCurrentVel(const Plato::ScalarVector& aControls, const Plato::Primal& aVariables) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = tDomain.numCells();
            Plato::Fluids::build_scalar_function_worksets<GradCurVelEvalT>(tDomain, aControls, aVariables,
                                                                           mLocalOrdinalMaps, tInputWorkSets);
        ...
        // evaluate boundary
        {
            Plato::WorkSets tInputWorkSets;
            auto tNumCells = mSpatialModel.Mesh->NumElements();
            Plato::Fluids::build_scalar_function_worksets<GradCurVelEvalT>(tNumCells, aControls, aVariables,
                                                                           mLocalOrdinalMaps, tInputWorkSets);
   ...
   private:
    ...
    void initialize(Teuchos::ParameterList& aInputs)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            mValueFuncs[tName] = tScalarFuncFactory.template createScalarFunction<PhysicsT, ResidualEvalT>(
                mFuncName, tDomain, mDataMap, aInputs);

            mGradConfigFuncs[tName] = tScalarFuncFactory.template createScalarFunction<PhysicsT, GradConfigEvalT>(
                mFuncName, tDomain, mDataMap, aInputs);

            mGradControlFuncs[tName] = tScalarFuncFactory.template createScalarFunction<PhysicsT, GradControlEvalT>(
                mFuncName, tDomain, mDataMap, aInputs);

            mGradCurrentPressureFuncs[tName] =
                tScalarFuncFactory.template createScalarFunction<PhysicsT, GradCurPressEvalT>(mFuncName, tDomain,
                                                                                              mDataMap, aInputs);

            mGradCurrentTemperatureFuncs[tName] =
                tScalarFuncFactory.template createScalarFunction<PhysicsT, GradCurTempEvalT>(mFuncName, tDomain,
                                                                                             mDataMap, aInputs);

            mGradCurrentVelocityFuncs[tName] =
                tScalarFuncFactory.template createScalarFunction<PhysicsT, GradCurVelEvalT>(mFuncName, tDomain,
                                                                                            mDataMap, aInputs);
        }
    }
};
