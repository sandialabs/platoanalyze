...
class CriterionFlowRate : public Plato::Fluids::AbstractScalarFunction<PhysicsT, EvaluationT>
{
   ...
   public:
     ...
     ******************************************************************************/
    CriterionFlowRate(const std::string& aName,
                          ...
                          Plato::ScalarVectorT<ResultT>& aResult) const override
    {
        ...
        auto tCurrentVelocityWS =
            Plato::metadata<Plato::ScalarMultiVectorT<CurVelT>>(aWorkSets.get("current velocity"));
        ...
        for (auto& tName : mSideSets)
        {
            ...
            Plato::ScalarArray3DT<ConfigT> tJacobians("face Jacobians", tNumFaces, mNumSpatialDimsOnFace,
                                                      mNumSpatialDims);
            Plato::ScalarMultiVectorT<CurVelT> tCurVelGP("current velocity at Gauss points", tNumElements,
                                                         mNumSpatialDims);

            Kokkos::parallel_for(
                "flow rate", Kokkos::RangePolicy<>(0, tNumFaces),
                KOKKOS_LAMBDA(const Plato::OrdinalType& aSideOrdinal) {
                    ...
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        for (Plato::OrdinalType tDim = 0; tDim < mNumSpatialDims; tDim++)
                        {
                            ...
                            tCurVelGP(tElementOrdinal, tDim) +=
                                tBasisFunctions(tNode) * tCurrentVelocityWS(tElementOrdinal, tLocalElementDof);
                        }
                    }
                    ...
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        for (Plato::OrdinalType tDim = 0; tDim < mNumSpatialDims; tDim++)
                        {
                            aResult(tElementOrdinal) += tBasisFunctions(tNode) * tCurVelGP(tElementOrdinal, tDim) *
                                                        tUnitNormalVec(tDim) * tSurfaceAreaTimesCubWeight;
                        }
                    }
                });
        }
