...
class ThermalBuoyancy
{
   private:
    ...
    Plato::ScalarVector mNaturalConvectionNum; /*!< dimensionless natural convection number (either Rayleigh or Grashof
                                                  - depends on user's input) */
   ...
   public:
     ...
     ******************************************************************************/
    ThermalBuoyancy(const Plato::SpatialDomain& aDomain, Plato::DataMap& aDataMap, Teuchos::ParameterList& aInputs)
        : mDataMap(aDataMap),
          ...
          mCubatureRule(Plato::LinearTetCubRuleDegreeOne<mNumSpatialDims>())
    {
        ...
        mNaturalConvectionNum =
            Plato::Fluids::parse_natural_convection_number<mNumSpatialDims>(tMyMaterialName, aInputs);
    }
    ...
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarMultiVectorT<ResultT>& aResultWS) const
    {
        ...
        Kokkos::parallel_for(
            "add contribution from thermal buoyancy forces to residual", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                tComputeGradient(aCellOrdinal, tGradient, tConfigWS, tCellVolume);
                tCellVolume(aCellOrdinal) *= tCubWeight;

                // 1. add previous buoyancy force to residual, i.e. R += (\Delta{t}*Bu*Gr_i) M T_n, where Bu is the
                // buoyancy constant
                ...
                tMultiplier = tStabilization * static_cast<Plato::Scalar>(0.5) * tBuoyancyDamping *
