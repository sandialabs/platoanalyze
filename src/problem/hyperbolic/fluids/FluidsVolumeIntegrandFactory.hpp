...
#include "problem/hyperbolic/fluids/InternalThermalForces.hpp"
...
struct VolumeIntegrandFactory
{
    ...
    inline std::shared_ptr<Plato::AbstractVolumeIntegrand<PhysicsT, EvaluationT>> createInternalThermalForces(
        const Plato::SpatialDomain& aDomain, Plato::DataMap& aDataMap, Teuchos::ParameterList& aInputs)
    {
        ...
        {
            return (std::make_shared<Plato::Fluids::SIMP::InternalThermalForces<PhysicsT, EvaluationT>>(
                aDomain, aDataMap, aInputs));
        }
        else if (tScenario == "density-based topology optimization" && tHeatTransfer == "natural")
        {
            return (std::make_shared<Plato::Fluids::InternalThermalForces<PhysicsT, EvaluationT>>(aDomain, aDataMap,
                                                                                                  aInputs));
        }
        ...
        {
            ANALYZE_THROWERR(std::string("Requested Use Case: Scenario '") + tScenario +
                             "' & Heat Transfer Mechanism '" + tHeatTransfer + "' is not supported. " +
                             "Supported use cases are: Scenario = 'Density-Based Topology Optimization' and Heat "
                             "Transfer = 'Natural or Forced', " +
