...
#include "problem/hyperbolic/fluids/MassConservationUtils.hpp"
#include "problem/hyperbolic/fluids/MomentumSurfaceForces.hpp"
 ...
 * \f[
 *   \mathcal{R}^n(v^h) =
 *       \Delta{t}\alpha_p\alpha_u\int_{\Omega_e}\frac{\partial v^h}{\partial x_i}\frac{\partial \Delta{p}}{\partial
 *x_i} d\Omega_e
...
class PressureResidual : public Plato::Fluids::AbstractVectorFunction<PhysicsT, EvaluationT>
{
   private:
    ...
    using CurPressGradT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT,
                                           CurPressT,
                                           ConfigT>; /*!< current pressure gradient FAD evaluation type */
    using PrevPressGradT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT,
                                           PrevPressT,
                                           ConfigT>; /*!< previous pressure gradient FAD evaluation type */
    ...
    // surface integral
    using MomentumForces =
        Plato::Fluids::MomentumSurfaceForces<PhysicsT, EvaluationT>; /*!< local surface momentum force type */
    std::unordered_map<std::string, std::shared_ptr<MomentumForces>>
        mMomentumBCs; /*!< list of surface momentum forces */
   ...
   public:
     ...
     ******************************************************************************/
    PressureResidual(const Plato::SpatialDomain& aDomain, Plato::DataMap& aDataMap, Teuchos::ParameterList& aInputs)
        : mDataMap(aDataMap),
     ...
     * \param [in] aDataMap output database metadata
     ******************************************************************************/
    PressureResidual(const Plato::SpatialDomain& aDomain, Plato::DataMap& aDataMap)
        : mDataMap(aDataMap),
          mSpatialDomain(aDomain),
          mCubatureRule(Plato::LinearTetCubRuleDegreeOne<mNumSpatialDims>())
    {
    ...
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarMultiVectorT<ResultT>& aResultWS) const override
    {
        ...
        Plato::ScalarMultiVectorT<CurPressGradT> tCurPressGradGP("current pressure gradient", tNumCells,
                                                                 mNumSpatialDims);
        Plato::ScalarMultiVectorT<PrevPressGradT> tPrevPressGradGP("previous pressure gradient", tNumCells,
                                                                   mNumSpatialDims);
        ...
        Kokkos::parallel_for(
            "calculate continuity residual", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                tComputeGradient(aCellOrdinal, tGradient, tConfigWS, tCellVolume);
                tCellVolume(aCellOrdinal) *= tCubWeight;

                // 1. add divergence of previous pressure gradient to residual, i.e. RHS += -1.0*\theta^{u}\Delta{t} L
                // p^{n}
                ...
                // 3. add divergence of delta predicted velocity to residual, RHS += D\Delta{\bar{u}}, where
                // \Delta{\bar{u}} = \bar{u} - u_n
                tIntrplVectorField(aCellOrdinal, tBasisFunctions, tPredVelWS, tPredVelGP);
                Plato::Fluids::integrate_divergence_operator<mNumNodesPerCell, mNumSpatialDims>(
                    aCellOrdinal, tBasisFunctions, tGradient, tCellVolume, tPredVelGP, tRightHandSide,
                    tMomentumDamping);
                Plato::Fluids::integrate_divergence_operator<mNumNodesPerCell, mNumSpatialDims>(
                    aCellOrdinal, tBasisFunctions, tGradient, tCellVolume, tPrevVelGP, tRightHandSide,
                    -tMomentumDamping);
   ...
   private:
    ...
    void setSurfaceBoundaryIntegrals(Teuchos::ParameterList& aInputs)
    {
        ...
        for (Teuchos::ParameterList::ConstIterator tItr = tSublist.begin(); tItr != tSublist.end(); ++tItr)
        {
            ...
            {
                ANALYZE_THROWERR(
                    std::string("Error reading 'Velocity Essential Boundary Conditions' block: Expects a parameter ") +
