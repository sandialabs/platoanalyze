...
#include "boundary_conditions/SurfaceIntegralUtilities.hpp"
...
class CriterionMeanSurfaceTemperature : public Plato::Fluids::AbstractScalarFunction<PhysicsT, EvaluationT>
{
   private:
    ...
    static constexpr auto mNumSpatialDimsOnFace =
        PhysicsT::SimplexT::mNumSpatialDimsOnFace; /*!< number of spatial dimensions on face */
    ...
    static constexpr auto mNumTempDofsPerNode =
        PhysicsT::SimplexT::mNumEnergyDofsPerNode; /*!< number of temperature dofs per node */
    ...
    // set local typenames
    using CubatureRule =
        Plato::LinearTetCubRuleDegreeOne<mNumSpatialDimsOnFace>; /*!< local short name for cubature rule class */
    ...
    std::vector<std::string>
        mWallSets; /*!< sideset names corresponding to the surfaces associated with the surface integral */
   ...
   public:
     ...
     ******************************************************************************/
    CriterionMeanSurfaceTemperature(const std::string& aName,
                                    const Plato::SpatialDomain& aDomain,
                                    Plato::DataMap& aDataMap,
                                    Teuchos::ParameterList& aInputs)
        : mDataMap(aDataMap), mSurfaceCubatureRule(CubatureRule()), mSpatialDomain(aDomain), mFuncName(aName)
    {
     ...
     ******************************************************************************/
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarVectorT<ResultT>& aResult) const override { return; }
                          ...
                          Plato::ScalarVectorT<ResultT>& aResult) const override
    {
        ...
        if (tNumCells != aResult.extent(0))
        {
            ANALYZE_THROWERR(
                std::string(
                    "Dimension mismatch. 'Result View' and 'Spatial Domain' cell/element number do not match. ") +
        ...
        auto tCurrentTempWS =
            Plato::metadata<Plato::ScalarMultiVectorT<CurrentTempT>>(aWorkSets.get("current temperature"));
        ...
        for (auto& tName : mWallSets)
        {
            ...
            Plato::ScalarArray3DT<ConfigT> tJacobians("face Jacobians", tNumFaces, mNumSpatialDimsOnFace,
                                                      mNumSpatialDims);
            ...
            Kokkos::parallel_for(
                "integrate surface temperature", Kokkos::RangePolicy<>(0, tNumFaces),
                KOKKOS_LAMBDA(const Plato::OrdinalType& aSideOrdinal) {
                    ...
                    tCalculateSurfaceJacobians(tElementOrdinal, aSideOrdinal, tLocalNodeOrdinals, tConfigWS,
                                               tJacobians);
                    ...
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        ...
                        tCurrentTempGP(tElementOrdinal) +=
                            tBasisFunctions(tNode) * tCurrentTempWS(tElementOrdinal, tLocalElementNode);
                    }

                    // calculate surface integral, which is defined as \int_{\Gamma_e}N_p^a T^h d\Gamma_e
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        tResult(tElementOrdinal) +=
                            tBasisFunctions(tNode) * tCurrentTempGP(tElementOrdinal) * tSurfaceArea(aSideOrdinal);
                    }
                });

            Kokkos::parallel_for(
                "calculate mean surface temperature", Kokkos::RangePolicy<>(0, tNumCells),
                KOKKOS_LAMBDA(const Plato::OrdinalType& aElementOrdinal) {
                    aResult(aElementOrdinal) =
                        (static_cast<Plato::Scalar>(1.0) / tSurfaceAreaSum(0)) * tResult(aElementOrdinal);
                });
        }
    }
};
...
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionMeanSurfaceTemperature, Plato::IncompressibleFluids, Plato::SimplexFluids, 1, 1)
#endif

#ifdef PLATOANALYZE_2D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionMeanSurfaceTemperature, Plato::IncompressibleFluids, Plato::SimplexFluids, 2, 1)
#endif

#ifdef PLATOANALYZE_3D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionMeanSurfaceTemperature, Plato::IncompressibleFluids, Plato::SimplexFluids, 3, 1)
#endif
