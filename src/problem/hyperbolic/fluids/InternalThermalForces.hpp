...
#include "problem/hyperbolic/fluids/AbstractVolumeIntegrand.hpp"
#include "problem/hyperbolic/fluids/EnergyConservationUtils.hpp"
...
class InternalThermalForces : public Plato::AbstractVolumeIntegrand<PhysicsT, EvaluationT>
{
   private:
    ...
    static constexpr auto mNumVelDofsPerCell =
        PhysicsT::SimplexT::mNumMomentumDofsPerCell; /*!< number of momentum degrees of freedom per cell */
    static constexpr auto mNumTempDofsPerCell =
        PhysicsT::SimplexT::mNumEnergyDofsPerCell; /*!< number of energy degrees of freedom per node */
    static constexpr auto mNumVelDofsPerNode =
        PhysicsT::SimplexT::mNumMomentumDofsPerNode; /*!< number of momentum degrees of freedom per node */
    static constexpr auto mNumTempDofsPerNode =
        PhysicsT::SimplexT::mNumEnergyDofsPerNode; /*!< number of energy degrees of freedom per node */
    ...
    using CurFluxT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT, CurTempT, ConfigT>; /*!< current flux FAD
                                                                                               evaluation type */
    using PrevFluxT =
        typename Plato::Fluids::fad_type_t<typename PhysicsT::SimplexT, PrevTempT, ConfigT>; /*!< previous flux FAD
                                                                                                evaluation type */
    using ConvectionT = typename Plato::Fluids::
        fad_type_t<typename PhysicsT::SimplexT, PrevTempT, CurVelT, ConfigT>; /*!< convection FAD evaluation type */

    Plato::Scalar mArtificialDamping =
        1.0; /*!< artificial temperature damping - damping is a byproduct from time integration scheme */
   ...
   public:
     ...
     ******************************************************************************/
    InternalThermalForces(const Plato::SpatialDomain& aDomain,
    ...
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarMultiVectorT<ResultT>& aResultWS) const override
    {
        ...
        Kokkos::parallel_for(
            "energy conservation residual", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                Plato::Fluids::calculate_flux<mNumNodesPerCell, mNumSpatialDims>(aCellOrdinal, tGradient, tPrevTempWS,
                                                                                 tPrevThermalFlux);
                ...
                Plato::Fluids::integrate_scalar_field<mNumTempDofsPerCell>(aCellOrdinal, tBasisFunctions, tCellVolume,
                                                                           tConvection, aResultWS, 1.0);

                // 3. add current diffusive force contribution to residual, i.e. R += \theta_3 K T^{n+1}
                Plato::Fluids::calculate_flux<mNumNodesPerCell, mNumSpatialDims>(aCellOrdinal, tGradient, tCurTempWS,
                                                                                 tCurThermalFlux);
                ...
                // 5. add stabilizing convective force contribution to residual, i.e. R += \alpha_{stab} * C_u(u^{n+1})
                // T^n
                tMultiplier = tStabilizationMultiplier * static_cast<Plato::Scalar>(0.5) * tCriticalTimeStep(0) *
                              tCriticalTimeStep(0);
                ...
                Plato::Fluids::integrate_scalar_field<mNumNodesPerCell>(aCellOrdinal, tBasisFunctions, tCellVolume,
                                                                        tPrevTempGP, aResultWS, -1.0);
   ...
   private:
     ...
     * \param [in] aInputs input file metadata.
     ******************************************************************************/
    void setAritificalDiffusiveDamping(Teuchos::ParameterList& aInputs)
    {
...
class InternalThermalForces : public Plato::AbstractVolumeIntegrand<PhysicsT, EvaluationT>
{
    ...
    void evaluate(const Plato::WorkSets& aWorkSets, Plato::ScalarMultiVectorT<ResultT>& aResultWS) const override
    {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                ...
                ControlT tPenalizedEffectiveThermalProperty =
                    Plato::Fluids::penalized_effective_thermal_property<mNumNodesPerCell>(
                        aCellOrdinal, tEffectiveThermalProperty, tDiffusiveTermPenaltyExponent, tControlWS);

                // 2. add current diffusive force contribution to residual, i.e. R += \theta_3 pi^{\alpha}(theta) K
                // T^{n+1},
                Plato::Fluids::calculate_flux<mNumNodesPerCell, mNumSpatialDims>(aCellOrdinal, tGradient, tCurTempWS,
                                                                                 tCurThermalFlux);
                ...
                // 3. add previous diffusive force contribution to residual, i.e. R -= (\theta_3-1) pi^{\alpha}(theta) K
                // T^n
                Plato::Fluids::calculate_flux<mNumNodesPerCell, mNumSpatialDims>(aCellOrdinal, tGradient, tPrevTempWS,
                                                                                 tPrevThermalFlux);
                ControlT tMultiplierControlTwoT =
                    (tArtificialDamping - static_cast<Plato::Scalar>(1.0)) * tPenalizedEffectiveThermalProperty;
                Plato::Fluids::calculate_flux_divergence<mNumNodesPerCell, mNumSpatialDims>(
                ...
                Plato::Scalar tScalar = tStabilizationMultiplier * static_cast<Plato::Scalar>(0.5) *
                                        tCriticalTimeStep(0) * tCriticalTimeStep(0);
                Plato::Fluids::integrate_stabilizing_scalar_forces<mNumNodesPerCell, mNumSpatialDims>(
   ...
   private:
     ...
     * \param [in] aInputs input file metadata.
     ******************************************************************************/
    void setPenaltyModelParameters(Teuchos::ParameterList& aInputs)
    {
        ...
        if (tHyperbolicParamList.isSublist("Energy Conservation"))
        {
            ...
            if (tEnergyParamList.isSublist("Penalty Function"))
            {
                ...
                mDiffusiveTermPenaltyExponent =
                    tPenaltyFuncList.get<Plato::Scalar>("Diffusive Term Penalty Exponent", 3.0);
            }
        }
    }
};
