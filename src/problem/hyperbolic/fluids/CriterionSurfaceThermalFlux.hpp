...
class CriterionSurfaceThermalFlux : public Plato::Fluids::AbstractScalarFunction<PhysicsT, EvaluationT>
{
   private:
    ...
    static constexpr auto mNumPressDofsPerNode =
        PhysicsT::SimplexT::mNumMassDofsPerNode; /*!< number of pressure dofs per node */
    ...
    const Plato::SpatialDomain&
        mSpatialDomain; /*!< holds mesh and entity sets metadata for a volume domain (i.e. element block) */

    // member parameters
    std::string mFuncName; /*!< scalar funciton name */
    std::vector<std::string>
        mSideSets; /*!< sideset names corresponding to the surfaces associated with the surface integral */
    std::vector<Plato::Scalar>
        mConductivityRatios; /*!< list with surface material over fluid conductivity ratios (k_{surface} / k_{fluid}) */
   ...
   public:
     ...
     ******************************************************************************/
    CriterionSurfaceThermalFlux(const std::string& aName,
                                const Plato::SpatialDomain& aDomain,
                                Plato::DataMap& aDataMap,
                                Teuchos::ParameterList& aInputs)
        : mDataMap(aDataMap), mSurfaceCubatureRule(SurfaceCubatureRule()), mSpatialDomain(aDomain), mFuncName(aName)
    {
                          ...
                          Plato::ScalarVectorT<ResultT>& aResult) const override
    {
        ...
        for (auto& tName : mSideSets)
        {
            ...
            Kokkos::parallel_for(
                "surface thermal flux", Kokkos::RangePolicy<>(0, tNumFaces),
                KOKKOS_LAMBDA(const Plato::OrdinalType& aSideOrdinal) {
                    ...
                    auto tUnitNormalVec =
                        Plato::omega_h::unit_normal_vector(tElementOrdinal, tElemFaceOrdinal, tCoords);

                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        for (Plato::OrdinalType tDim = 0; tDim < mNumSpatialDims; tDim++)
                        {
                            tTempGradGP(tElementOrdinal, tDim) +=
                                tGradient(tElementOrdinal, tNode, tDim) * tCurTempWS(tElementOrdinal, tNode);
                        }
                    }
                    ...
                    for (Plato::OrdinalType tNode = 0; tNode < mNumNodesPerFace; tNode++)
                    {
                        for (Plato::OrdinalType tDim = 0; tDim < mNumSpatialDims; tDim++)
                        {
                            aResult(tElementOrdinal) += static_cast<Plato::Scalar>(-1.0) * tBasisFunctions(tNode) *
                                                        tSurfaceAreaTimesCubWeight *
                                                        (tSurfaceMaterialOverFluidConductivity *
                                                         tTempGradGP(tElementOrdinal, tDim) * tUnitNormalVec(tDim));
                        }
                    }
                });
        }
...
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionSurfaceThermalFlux, Plato::IncompressibleFluids, Plato::SimplexFluids, 1, 1)
#endif

#ifdef PLATOANALYZE_2D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionSurfaceThermalFlux, Plato::IncompressibleFluids, Plato::SimplexFluids, 2, 1)
#endif

#ifdef PLATOANALYZE_3D
PLATO_EXPL_DEC_FLUIDS(
    Plato::Fluids::CriterionSurfaceThermalFlux, Plato::IncompressibleFluids, Plato::SimplexFluids, 3, 1)
#endif
