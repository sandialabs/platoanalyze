...
class InertialContent : public ElementType
{
   ...
   public:
    InertialContent(const Teuchos::RCP<Plato::LinearElasticMaterial<mNumSpatialDims>> aMaterialModel)
        : mCellDensity(aMaterialModel->getMassDensity()), mRayleighA(aMaterialModel->getRayleighA())
    {
    }

    InertialContent(const Teuchos::RCP<Plato::MaterialModel<mNumSpatialDims>> aMaterialModel)
        : mCellDensity(aMaterialModel->getScalarConstant("Mass Density")), mRayleighA(0.0)
    {
    }

    template <typename TScalarType, typename TContentScalarType>
    KOKKOS_INLINE_FUNCTION void operator()(Plato::Array<mNumSpatialDims, TContentScalarType>& aContent,
                                           ...
                                           const Plato::Array<mNumSpatialDims, TAccelerationType>& aAcceleration) const
    {
        ...
        {
            aContent(tDimIndex) =
                aAcceleration(tDimIndex) * mCellDensity + aVelocity(tDimIndex) * mCellDensity * mRayleighA;
        }
    }
};
