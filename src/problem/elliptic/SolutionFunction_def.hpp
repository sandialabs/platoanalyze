...
void SolutionFunction<PhysicsType>::initialize(Teuchos::ParameterList& aProblemParams)
{
    ...
    else if (mTargetMagnitudeSpecified)
    {
        ...
        {
            ANALYZE_THROWERR(
                "Parsing 'Solution' criterion:  'Normal' must be specified in addition to 'TargetMagnitude' in order "
                "to know what direction to consider.");
        }
    ...
    else if (mTargetSolutionSpecified)
    {
        ...
        {
            ANALYZE_THROWERR(
                "Parsing 'Solution' criterion:  'Normal' must be specified in addition to 'TargetSolution' in order to "
                "know what direction to consider.");
        }
    ...
    else if (mMagnitudeSpecified)
    {
        ...
        {
            ANALYZE_THROWERR(
                "Parsing 'Solution' criterion:  'Normal' must be specified in addition to 'Magnitude' in order to know "
                "what direction to consider.");
        }
    ...
    {
        ANALYZE_THROWERR(
            "Parsing 'Solution' criterion:  Could not determine solution type based on user-provided input.");
    }
    switch (mSolutionType)
    {
        case solution_type_t::DIFF_BETWEEN_SOLUTION_VECTOR_AND_TARGET_VECTOR:
        {
            ...
            std::stringstream ss;
            ss << "Solution Type: Measure the magnitude of the difference between the solution vector and the "
                  "user-specified target vector."
               << std::endl;
            ss << "Target Vector:";
        ...
        case solution_type_t::DIFF_BETWEEN_SOLUTION_MAG_IN_DIRECTION_AND_TARGET:
        {
            ...
            std::stringstream ss;
            ss << "Solution Type: Measure the difference between the solution magnitude in the specified direction and "
                  "the target magnitude in that direction."
               << std::endl;
            ss << "Normal Vector:";
            for (int h = 0; h < tNumDofsPerNode; ++h)
            {
...
template <typename PhysicsType>
void SolutionFunction<PhysicsType>::initialize_target_vector(Teuchos::ParameterList& aFunctionParams)
{
    ...
    else
    {
        ...
        else if (tTargetArray.size() < mNumDofsPerNode)
        {
            std::stringstream ss;
...
template <typename PhysicsType>
void SolutionFunction<PhysicsType>::initialize_normal_vector(Teuchos::ParameterList& aFunctionParams)
{
    ...
    else
    {
        ...
        else if (tNormalArray.size() < mNumDofsPerNode)
        {
            std::stringstream ss;
 ...
 **********************************************************************************/
template <typename PhysicsType>
Plato::Scalar SolutionFunction<PhysicsType>::value(const Plato::Solutions& aSolution,
                                                   ...
                                                   Plato::Scalar aTimeStep) const
{
    ...
    switch (mSolutionType)
    {
        ...
        case solution_type_t::DIFF_BETWEEN_SOLUTION_MAG_IN_DIRECTION_AND_TARGET:
        {
            ...
            std::stringstream ss;
            ss << "Absolute value of difference between actual magnitude and target magnitude: " << tReturnValue
               << std::endl;
            REPORT(ss.str());
        }
        break;
        ...
        case solution_type_t::DIFF_BETWEEN_SOLUTION_IN_DIRECTION_AND_TARGET_SOLUTION_IN_DIRECTION:
        {
            ...
            std::stringstream ss;
            ss << "Absolute value of difference between current solution in given direction and target solution in "
                  "given direction: "
               << tReturnValue << std::endl;
            REPORT(ss.str());
        }
 ...
 **********************************************************************************/
template <typename PhysicsType>
Plato::ScalarVector SolutionFunction<PhysicsType>::gradient_x(const Plato::Solutions& aSolution,
                                                              const Plato::ScalarVector& aControl,
                                                              Plato::Scalar aTimeStep) const
{
 ...
 **********************************************************************************/
template <typename PhysicsType>
Plato::ScalarVector SolutionFunction<PhysicsType>::gradient_u(const Plato::Solutions& aSolution,
                                                              const Plato::ScalarVector& aControl,
                                                              ...
                                                              Plato::Scalar aTimeStep) const
{
    ...
    switch (mSolutionType)
    {
        case solution_type_t::DIFF_BETWEEN_SOLUTION_VECTOR_AND_TARGET_VECTOR:
            Kokkos::parallel_for(
                "gradient_u", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumNodes),
                KOKKOS_LAMBDA(Plato::OrdinalType aNodeOrdinal) {
                    ...
                    for (int iDof = 0; iDof < tNumDofsPerNode; iDof++)
                    {
                        ...
                        if (ds != 0.0)
                        {
                            tGradientU(tNumDofsPerNode * tIndex + iDof) =
                                (dv - tTargetSolutionVector[iDof]) / (ds * tNumNodes);
                        }
                        else
                        {
            ...
            Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(Plato::OrdinalType aNodeOrdinal) {
                    ...
                    for (int iDof = 0; iDof < tNumDofsPerNode; iDof++)
                    {
                        ...
                        {
                            tGradientU(tNumDofsPerNode * tIndex + iDof) =
                                tSign * (tNormal[iDof] * (tNormal[iDof] * dv) / (ds * tNumNodes));
                        }
                        else
                        {
                            tGradientU(tNumDofsPerNode * tIndex + iDof) = 0.0;
            ...
            Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(Plato::OrdinalType aNodeOrdinal) {
                    ...
                    for (int iDof = 0; iDof < tNumDofsPerNode; iDof++)
                    {
                        ...
                        {
                            tGradientU(tNumDofsPerNode * tIndex + iDof) =
                                tNormal[iDof] * (tNormal[iDof] * dv) / (ds * tNumNodes);
                        }
                        else
                        {
                            tGradientU(tNumDofsPerNode * tIndex + iDof) = 0.0;
...
/**
    ...
    **********************************************************************************/
template <typename PhysicsType>
Plato::ScalarVector SolutionFunction<PhysicsType>::gradient_z(const Plato::Solutions& aSolution,
                                                              const Plato::ScalarVector& aControl,
                                                              Plato::Scalar aTimeStep) const
{
...
/**
    ...
    **********************************************************************************/
template <typename PhysicsType>
void SolutionFunction<PhysicsType>::updateProblem(const Plato::ScalarVector& aState,
                                                  const Plato::ScalarVector& aControl) const
{
}
