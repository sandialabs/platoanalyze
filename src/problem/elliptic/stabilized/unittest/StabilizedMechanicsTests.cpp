...
#include "problem/elliptic/stabilized/Kinematics.hpp"
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, StabilizedMechanics_Kinematics3D)
{
    ...
    std::vector<std::vector<std::vector<Plato::Scalar>>> tGold = {{{4e-7, 4e-7, 3e-7, 8.0e-7, 13e-7, 10e-7},
    ...
    for (Plato::OrdinalType tIndexI = 0; tIndexI < tDim0; tIndexI++)
    {
        for (Plato::OrdinalType tIndexJ = 0; tIndexJ < tDim1; tIndexJ++)
        {
            ...
            {
                TEST_FLOATING_EQUALITY(tHostStrains(tIndexI, tIndexJ, tIndexK), tGold[tIndexI][tIndexJ][tIndexK],
                                       tTolerance);
            }
        }
    }
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, StabilizedMechanics_Solution3D)
{
    ...
    Plato::Stabilized::Problem<Plato::Stabilized::Mechanics<Plato::Tet4>> tEllipticVMSProblem(tMesh, *tParamList,
                                                                                              tMachine);
    ...
    auto tDirichletIndicesBoundaryX0_Xdof =
        Plato::TestHelpers::get_dirichlet_indices_on_boundary_3D(tMesh, "x-", tNumDofsPerNode, tDispDofX);
    auto tDirichletIndicesBoundaryX0_Ydof =
        Plato::TestHelpers::get_dirichlet_indices_on_boundary_3D(tMesh, "x-", tNumDofsPerNode, tDispDofY);
    auto tDirichletIndicesBoundaryX0_Zdof =
        Plato::TestHelpers::get_dirichlet_indices_on_boundary_3D(tMesh, "x-", tNumDofsPerNode, tDispDofZ);
    auto tDirichletIndicesBoundaryX1_Ydof =
        Plato::TestHelpers::get_dirichlet_indices_on_boundary_3D(tMesh, "x+", tNumDofsPerNode, tDispDofY);
    ...
    Kokkos::parallel_for(
        "set dirichlet values and indices", Kokkos::RangePolicy<>(0, tDirichletIndicesBoundaryX0_Xdof.size()),
    ...
    Kokkos::parallel_for(
        "set dirichlet values and indices", Kokkos::RangePolicy<>(0, tDirichletIndicesBoundaryX0_Ydof.size()),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
    ...
    Kokkos::parallel_for(
        "set dirichlet values and indices", Kokkos::RangePolicy<>(0, tDirichletIndicesBoundaryX0_Zdof.size()),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
            auto tIndex = tOffset + aIndex;
            tDirichletValues(tIndex) = tValueToSet;
    ...
    Kokkos::parallel_for(
        "set dirichlet values and indices", Kokkos::RangePolicy<>(0, tDirichletIndicesBoundaryX1_Ydof.size()),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
            auto tIndex = tOffset + aIndex;
            tDirichletValues(tIndex) = tValueToSet;
    ...
    std::vector<std::vector<Plato::Scalar>> tGold = {
        ...
        {0.00000000e+00, 0.00000000e+00,  0.00000000e+00,  -2.38598212e-06, 0.00000000e+00,  0.00000000e+00,
         0.00000000e+00, -5.11915178e-05, 0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  1.55973168e-04,
         0.00000000e+00, 0.00000000e+00,  0.00000000e+00,  1.30090408e-04,  -1.60075585e-04, -1.00000000e-03,
         7.71928699e-05, -1.11939597e-04, -3.78526434e-04, -1.00000000e-03, 6.97421103e-05,  -1.71691407e-04,
         2.89133402e-04, -1.00000000e-03, 5.66369118e-05,  3.44430529e-05,  2.13123038e-04,  -1.00000000e-03,
         4.48534078e-05, -2.38598212e-06}};
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, StabilizedMechanics_Residual3D)
{
    ...
    Teuchos::RCP<Teuchos::ParameterList> tPDEInputs = Teuchos::getParametersFromXmlString(
        "<ParameterList name='Plato Problem'>                                           \n"
        "  <ParameterList name='Spatial Model'>                                         \n"
        "    <ParameterList name='Domains'>                                             \n"
        "      <ParameterList name='Design Volume'>                                     \n"
    ...
    Plato::ScalarArray3DT<EvalType::ConfigScalarType> tConfigWS(
        "configuration", tNumCells, ElementType::mNumNodesPerCell, ElementType::mNumSpatialDims);
    tWorksetBase.worksetConfig(tConfigWS);

    // 2.2 SET DESIGN VARIABLES
    Plato::ScalarMultiVectorT<EvalType::ControlScalarType> tControlWS("design variables", tNumCells,
    ...
    Plato::ScalarMultiVectorT<EvalType::StateScalarType> tStateWS("current global state", tNumCells,
    ...
    Plato::ScalarMultiVectorT<EvalType::NodeStateScalarType> tProjPressGradWS("projected pressure grad", tNumCells,
                                                                              ElementType::mNumNodeStatePerCell);
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < tNumNodesPerCell; tNodeIndex++)
            {
                ...
                {
                    tProjPressGradWS(aCellOrdinal, tNodeIndex * tNumSpatialDims + tDimIndex) =
    ...
    Plato::Stabilized::ElastostaticResidual<EvalType, Plato::MSIMP> tComputeResidual(tOnlyDomain, tDataMap, *tPDEInputs,
                                                                                     tPenaltyParams);
    Plato::ScalarMultiVectorT<EvalType::ResultScalarType> tResidualWS("residual", tNumCells,
                                                                      ElementType::mNumDofsPerCell);
    tComputeResidual.evaluate(tStateWS, tProjPressGradWS, tControlWS, tConfigWS, tResidualWS);

    // 5. TEST RESULTS
    std::vector<std::vector<Plato::Scalar>> tGold = {
        {-6172.8395061728, -28189.3004115227, -4938.2716049383, 2207.8695745833, 20164.6090534980, 1234.5679012346,
         -18930.0411522634, -6691.4339916548, -22016.4609053498, 22016.4609053498, -3086.4197530864, 1467.1288339326,
         8024.6913580247, 4938.2716049383, 26954.7325102881, -4390.9718242686},
        {-6172.8395061728, -22633.7448559671, -4938.2716049383, 3318.9806856343, -1851.8518518519, 17695.4732510288,
         -16460.9053497943, -1428.0088614559, -14609.0534979424, -1234.5679012346, 13374.4855967079, 3543.2858435667,
         22633.7448559671, 6172.8395061728, 8024.6913580247, -7286.1095195970},
        {-8024.6913580247, -4938.2716049383, -19547.3251028807, 2353.9347870512, -14609.0534979424, 14609.0534979424,
         -3086.4197530864, 2578.2399451038, 1851.8518518519, -15843.6213991770, 14609.0534979424, 1983.5644170414,
         20781.8930041153, 6172.8395061728, 8024.6913580247, -6915.7391491966},
        {-8024.6913580247, -4938.2716049383, -23251.0288065844, 1613.1940462204, -16460.9053497943, -1234.5679012346,
         15226.3374485597, 3913.6562140572, 18312.7572016461, -18312.7572016461, 3086.4197530864, -4244.9066118006,
         6172.8395061728, 24485.5967078190, 4938.2716049383, -4985.6473521808},
        {-30041.1522633745, -6172.8395061728, -8024.6913580247, 4508.3317417291, 1851.8518518519, -25102.8806584363,
         23868.3127572017, -1349.7689162318, 22016.4609053498, 1234.5679012346, -20781.8930041153, -6321.0636215248,
         6172.8395061728, 30041.1522633745, 4938.2716049383, -6096.7584632318},
        {-31893.0041152264, -6172.8395061728, -8024.6913580247, 4137.9613713286, 25720.1646090536, -25720.1646090536,
         3086.4197530864, -5356.0177229718, -1851.8518518519, 26954.7325102881, -25720.1646090536, -4761.3421949094,
