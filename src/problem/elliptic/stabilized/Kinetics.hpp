...
class Kinetics : public ElementType
{
   ...
   public:
    ...
    KOKKOS_INLINE_FUNCTION void operator()(VolumeScalarType const& aVolume,
                                           ...
                                           Plato::Array<mNumSpatialDims, KineticsScalarType>& aCellStabilization) const
    {
        ...
        for (Plato::OrdinalType tVoigtIndex_I = 0; tVoigtIndex_I < mNumVoigtTerms; tVoigtIndex_I++)
        {
            ...
            {
                aDevStress(tVoigtIndex_I) += aStrain(tVoigtIndex_J) * mCellStiffness(tVoigtIndex_I, tVoigtIndex_J);
            }
        }
        ...
        //
        KinematicsScalarType tTau =
            pow(aVolume, static_cast<Plato::Scalar>(2.0 / 3.0)) / (static_cast<Plato::Scalar>(2.0) * mShearModulus);
        for (Plato::OrdinalType tDimIndex = 0; tDimIndex < mNumSpatialDims; tDimIndex++)
        {
            aCellStabilization(tDimIndex) =
                mPressureScaling * tTau * (mPressureScaling * aPressureGrad(tDimIndex) - aProjectedPGrad(tDimIndex));
        }
    }
};
// class Kinetics
