...
Problem<PhysicsType>::Problem(Plato::Mesh aMesh, Teuchos::ParameterList& aInputParams, Comm::Machine aMachine)
 ...
 *******************************************************************************/
template <typename PhysicsType>
void Problem<PhysicsType>::readEssentialBoundaryConditions(Teuchos::ParameterList& aInputParams)
{
    ...
    Plato::EssentialBCs<ElementType> tEssentialBoundaryConditions(
        aInputParams.sublist("Essential Boundary Conditions", false), mSpatialModel.Mesh);
 ...
 **********************************************************************************/
template <typename PhysicsType>
void Problem<PhysicsType>::updateProblem(const Plato::ScalarVector& aControl, const Plato::Solutions& aGlobalSolution)
{
    return;
}
...
Plato::Solutions Problem<PhysicsType>::solution(const Plato::ScalarVector& aControl)
{
    ...
    for (Plato::OrdinalType tStepIndex = 1; tStepIndex < mNumSteps; tStepIndex++)
    {
        ...
        for (Plato::OrdinalType tNewtonIndex = 0; tNewtonIndex < mNumNewtonSteps; tNewtonIndex++)
        {
            ...
            Plato::blas1::update(static_cast<Plato::Scalar>(1.0), tStateIncrement, static_cast<Plato::Scalar>(1.0),
                                 tState);

            // copy projection state
            Plato::blas1::extract<ElementType::mNumDofsPerNode, ProjectorType::ElementType::mProjectionDof>(
                tState, mProjectState);
        }
                                                            ...
                                                            Criterion aCriterion)
{
    ...
    for (Plato::OrdinalType tStepIndex = tLastStepIndex; tStepIndex > 0; tStepIndex--)
    {
        ...
        Plato::blas1::extract<ElementType::mNumDofsPerNode, ProjectorType::ElementType::mProjectionDof>(tStateAtStepK,
                                                                                                        mProjectState);
        mProjResidual = mStateProjection.value(mProjPGrad, mProjectState, aControl);
        mProjJacobian = mStateProjection.gradient_u(mProjPGrad, mProjectState, aControl);
        Plato::blas1::scale(static_cast<Plato::Scalar>(-1.0), mProjResidual);
