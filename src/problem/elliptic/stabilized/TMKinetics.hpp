...
class TMKinetics : public ElementType
{
   ...
   public:
                                           ...
                                           Plato::Array<mNumSpatialDims, KineticsScalarType>& aCellStabilization) const
    {
        ...
        {
            tstrain[iDim] =
                mTemperatureScaling * mCellThermalExpansionCoef(iDim) * (aTemperature - mCellReferenceTemperature);
        ...
        for (int iDim = 0; iDim < mNumSpatialDims; iDim++)
        {
            aCellStabilization(iDim) =
                mPressureScaling * tTau * (mPressureScaling * aPressureGrad(iDim) - aProjectedPGrad(iDim));
        }
    }
};
}  // namespace Stabilized
