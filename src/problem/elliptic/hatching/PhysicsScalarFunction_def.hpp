...
#include "problem/elliptic/hatching/AbstractScalarFunction.hpp"
...
void PhysicsScalarFunction<PhysicsType>::initialize(Teuchos::ParameterList& aProblemParams)
{
 ...
 **********************************************************************************/
template <typename PhysicsType>
PhysicsScalarFunction<PhysicsType>::PhysicsScalarFunction(Plato::SpatialModel& aSpatialModel,
 ...
 **********************************************************************************/
template <typename PhysicsType>
void PhysicsScalarFunction<PhysicsType>::setEvaluator(const GradientCFunction& aInput, std::string aName)
{
    ...
    for (Plato::OrdinalType tStepIndex = 0; tStepIndex < tNumSequenceSteps; tStepIndex++)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            Plato::ScalarMultiVectorT<GlobalStateScalar> tGlobalStateWS("global state workset", tNumCells,
                                                                        mNumDofsPerCell);
            Plato::ScalarArray3DT<LocalStateScalar> tLocalStateWS("local state workset", tNumCells, mNumGaussPoints,
            ...
            {
                tLocalState =
                    Kokkos::subview(aLocalStates, tStepIndex - 1, Kokkos::ALL(), Kokkos::ALL(), Kokkos::ALL());
            }
            else
            {
                tLocalState = Plato::ScalarArray3D("initial local state", aLocalStates.extent(1),
                                                   aLocalStates.extent(2), aLocalStates.extent(3));
            }
            ...
            mValueFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResult,
                                                aTimeStep);
                                                                   ...
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    for (Plato::OrdinalType tStepIndex = 0; tStepIndex < tNumSequenceSteps; tStepIndex++)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            //
            Kokkos::deep_copy(tResult, 0.0);
            mGradientXFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResult,
                                                    aTimeStep);
                                                                   ...
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    for (const auto& tDomain : mSpatialModel.Domains)
    {
        ...
        {
            tLocalState = Plato::ScalarArray3D("initial local state", aLocalStates.extent(1), aLocalStates.extent(2),
                                               aLocalStates.extent(3));
        }
        Plato::ScalarArray3DT<LocalStateScalar> tLocalStateWS("sacado-ized state", tNumCells, mNumGaussPoints,
                                                              mNumLocalStatesPerGP);
        Plato::WorksetBase<ElementType>::worksetLocalState(tLocalState, tLocalStateWS, tDomain);
        ...
        // evaluate function
        //
        mGradientUFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResult,
                                                aTimeStep);

        Plato::assemble_vector_gradient_fad<mNumNodesPerCell, mNumDofsPerNode>(tDomain, mGlobalStateEntryOrdinal,
                                                                               tResult, tObjGradientU);
 ...
 **********************************************************************************/
template <typename PhysicsType>
Plato::ScalarVector PhysicsScalarFunction<PhysicsType>::gradient_c(const Plato::Solutions& aSolution,
                                                                   const Plato::ScalarArray4D& aLocalStates,
                                                                   const Plato::ScalarVector& aControl,
                                                                   Plato::OrdinalType aStepIndex,
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    if (aStepIndex != tLastStepIndex)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // workset global state
            //
            Plato::ScalarMultiVectorT<GlobalStateScalar> tGlobalStateWS("sacado-ized state", tNumCells,
                                                                        mNumDofsPerCell);
            Plato::WorksetBase<ElementType>::worksetState(tGlobalState, tGlobalStateWS, tDomain);
            ...
            // evaluate function
            //
            mGradientCFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResult,
                                                    aTimeStep);
                                                                   ...
                                                                   Plato::Scalar aTimeStep) const
{
    ...
    for (Plato::OrdinalType tStepIndex = 0; tStepIndex < tNumSequenceSteps; tStepIndex++)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            //
            Kokkos::deep_copy(tResult, 0.0);
            mGradientZFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResult,
                                                    aTimeStep);

            Plato::assemble_scalar_gradient_fad<mNumNodesPerCell>(tDomain, mControlEntryOrdinal, tResult,
                                                                  tObjGradientZ);
