...
#include "problem/elliptic/hatching/ScalarFunctionBaseFactory.hpp"
...
namespace
{
constexpr auto kHatchingGradientDisabledErrorMessage =
    "Hatching gradients are disabled. The HATCHING_GRADIENT build option must be enabled to use this feature.";
}
...
Problem<PhysicsType>::Problem(Plato::Mesh aMesh, Teuchos::ParameterList& aProblemParams, Comm::Machine aMachine)
    : AbstractProblem(aMesh, aProblemParams),
      ...
      mLocalStates("Local states",
                   mSequence.getNumSteps(),
                   aMesh->NumElements(),
                   ElementType::mNumGaussPoints,
...
Plato::Solutions Problem<PhysicsType>::solution(const Plato::ScalarVector& aControl)
{
    ...
    for (Plato::OrdinalType tStepIndex = 0; tStepIndex < tNumSequenceSteps; tStepIndex++)
    {
        ...
        else
        {
            ...
            tLocalState = Plato::ScalarArray3D("initial local state", numCells(), ElementType::mNumGaussPoints,
                                               ElementType::mNumVoigtTerms);
        }
        ...
        Plato::ScalarArray3D tUpdatedLocalState =
            Kokkos::subview(mLocalStates, tStepIndex, Kokkos::ALL(), Kokkos::ALL(), Kokkos::ALL());
        mDataMap.scalarMultiVectors["total strain"] =
            Plato::ScalarMultiVector("total strain", numCells(), ElementType::mNumVoigtTerms);
                                                            ...
                                                            Criterion aCriterion)
{
    ...
    for (Plato::OrdinalType tStepIndex = tLastStepIndex; tStepIndex >= 0; tStepIndex--)
    {
        ...
        {
            // kokkos initializes new views to zero.
            tC_prev = Plato::ScalarArray3D("initial local state", numCells(), ElementType::mNumGaussPoints,
                                           ElementType::mNumVoigtTerms);
        }
...
void Problem<PhysicsType>::initialize(Teuchos::ParameterList& aProblemParams)
{
    ...
    if (aProblemParams.isSublist("Criteria"))
    {
             ...
             ++tIndex)
        {
            ...
            {
                auto tCriterion =
                    tNonlinearFunctionBaseFactory.create(mSpatialModel, mSequence, mDataMap, aProblemParams, tName);
                if (tCriterion != nullptr)
                {
        ...
        {
            mGlobalAdjoints =
                Plato::ScalarMultiVector("Global Adjoint Variables", mSequence.getNumSteps(), mPDE->size());
            mLocalAdjoints =
                Plato::ScalarMultiVector("Local Adjoint Variables", mSequence.getNumSteps(), mPDE->stateSize());
        }
    }
}
