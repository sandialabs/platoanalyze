...
/******************************************************************************/
{
    ...
    class ResidualFunction
    {
       ...
       public:
                      ...
                      const Plato::ScalarArray3DT<ResultScalarType>& aResult)
        {
            ...
            Kokkos::parallel_for(
                "residual", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumGP}),
   ...
   public:
    /******************************************************************************/
    explicit StateUpdate(Plato::SpatialModel& aSpatialModel)
        : Plato::WorksetBase<ElementType>(aSpatialModel.Mesh), mSpatialModel(aSpatialModel)
    {
    }
    /******************************************************************************/

    /******************************************************************************/
    void operator()(Plato::DataMap const& aDataMap,
    ...
    /******************************************************************************/
    {
        ...
        Kokkos::parallel_for(
            "update local state", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumGP}),
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                for (ordT iTerm = 0; iTerm < tNumTerms; iTerm++)
                {
                    aUpdatedStrain(iCellOrdinal, iGpOrdinal, iTerm) =
                        aPreviousStrain(iCellOrdinal, iGpOrdinal, iTerm) + tStrainInc(iCellOrdinal, iGpOrdinal, iTerm);
                }
            });
        ...
        if (aDataMap.scalarMultiVectors.count("total strain"))
        {
            ...
            Kokkos::parallel_for(
                "Save total strain", Kokkos::RangePolicy<>(0, tNumCells),
    ...
    /**************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_u(
    ...
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // Workset local state
            //
            Plato::ScalarArray3DT<LocalStateScalar> tLocalStateWS("Local State", tNumCells, tNumGP,
            ...
            //
            Plato::ScalarArray3DT<LocalStateScalar> tPrevLocalStateWS("Previous Local State", tNumCells, tNumGP,
                                                                      mNumLocalStatesPerGP);
