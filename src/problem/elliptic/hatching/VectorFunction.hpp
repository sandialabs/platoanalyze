...
#include "problem/elliptic/hatching/AbstractVectorFunction.hpp"
...
class VectorFunction : public Plato::WorksetBase<typename PhysicsType::ElementType>
{
   ...
   public:
        ...
        : Plato::WorksetBase<ElementType>(aSpatialModel.Mesh), mSpatialModel(aSpatialModel), mDataMap(aDataMap)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            mJacobianFunctions[tName] = tFunctionFactory.template createVectorFunction<Jacobian>(
                tDomain, aDataMap, aProblemParams, aProblemType);
            mGradientCFunctions[tName] = tFunctionFactory.template createVectorFunction<GradientC>(
                tDomain, aDataMap, aProblemParams, aProblemType);
     ...
     ******************************************************************************/
    Plato::OrdinalType numNodes() const { return (mNumNodes); }
     ...
     ******************************************************************************/
    Plato::OrdinalType numCells() const { return (mNumCells); }
     ...
     ******************************************************************************/
    Plato::OrdinalType numDofsPerCell() const { return (mNumDofsPerCell); }
     ...
     ******************************************************************************/
    Plato::OrdinalType numNodesPerCell() const { return (mNumNodesPerCell); }
     ...
     ******************************************************************************/
    Plato::OrdinalType numDofsPerNode() const { return (mNumDofsPerNode); }
     ...
     ******************************************************************************/
    Plato::OrdinalType numControlsPerNode() const { return (mNumControl); }
     ...
     *
     ******************************************************************************/
    void setEvaluator(const ResidualFunction& aResidual, const JacobianFunction& aJacobian, std::string aName)
    {
     ...
     *
     ******************************************************************************/
    void setEvaluator(const GradientCFunction& aGradientC, std::string aName)
    {
     ...
     *
     ******************************************************************************/
    void setEvaluator(const GradientZFunction& aGradientZ, std::string aName)
    {
     ...
     *
     ******************************************************************************/
    void setEvaluator(const GradientXFunction& aGradientX, std::string aName)
    {
     ...
     *
     ******************************************************************************/
    Plato::OrdinalType stateSize() const { return mNumCells * mNumLocalDofsPerCell; }
    ...
    /**************************************************************************/
    Plato::ScalarVector value(const Plato::ScalarVector& aGlobalState,
    ...
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            //
            Plato::ScalarMultiVectorT<GlobalStateScalar> tGlobalStateWS("Global State Workset", tNumCells,
                                                                        mNumDofsPerCell);
            Plato::WorksetBase<ElementType>::worksetState(aGlobalState, tGlobalStateWS, tDomain);

            // Workset local state
            //
            Plato::ScalarArray3DT<LocalStateScalar> tLocalStateWS("Local State Workset", tNumCells, tNumGP,
                                                                  mNumLocalStatesPerGP);
            ...
            // evaluate function
            //
            mResidualFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResidual,
                                                   aTimeStep);
...

        {
            ...
            //
            auto tFirstBlockName = mSpatialModel.Domains.front().getDomainName();
            mResidualFunctions.at(tFirstBlockName)
                ->evaluate_boundary(mSpatialModel, tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tResidual,
                                    aTimeStep);
    ...
    /**************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_x(
    ...
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // evaluate function
            //
            mGradientXFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                                    aTimeStep);

            // assembly to return matrix
            Plato::BlockMatrixEntryOrdinal<mNumNodesPerCell, mNumSpatialDims, mNumDofsPerNode> tJacobianMatEntryOrdinal(
                tJacobianMat, tMesh);
            ...
            Plato::WorksetBase<ElementType>::assembleTransposeJacobian(mNumDofsPerCell, mNumConfigDofsPerCell,
                                                                       tJacobianMatEntryOrdinal, tJacobian,
...

        {
            ...
            // Workset global state
            //
            Plato::ScalarMultiVectorT<GlobalStateScalar> tGlobalStateWS("Global State Workset", mNumCells,
                                                                        mNumDofsPerCell);
            Plato::WorksetBase<ElementType>::worksetState(aGlobalState, tGlobalStateWS);

            // Workset local state
            //
            Plato::ScalarArray3DT<LocalStateScalar> tLocalStateWS("Local State Workset", mNumCells, tNumGP,
                                                                  mNumLocalStatesPerGP);
            Plato::WorksetBase<ElementType>::worksetLocalState(aLocalState, tLocalStateWS);
            ...
            //
            auto tFirstBlockName = mSpatialModel.Domains.front().getDomainName();
            mGradientXFunctions.at(tFirstBlockName)
                ->evaluate_boundary(mSpatialModel, tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                    aTimeStep);
    ...
    /**************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_u_T(
        const Plato::ScalarVector& aGlobalState,
        const Plato::ScalarArray3D& aLocalState,
        const Plato::ScalarVector& aControl,
        Plato::Scalar aTimeStep = 0.0
    ...
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // evaluate function
            //
            mJacobianFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                                   aTimeStep);
...

        {
            ...
            //
            auto tFirstBlockName = mSpatialModel.Domains.front().getDomainName();
            mJacobianFunctions.at(tFirstBlockName)
                ->evaluate_boundary(mSpatialModel, tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                    aTimeStep);
    ...
    /**************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_u(const Plato::ScalarVector& aGlobalState,
                                                  const Plato::ScalarArray3D& aLocalState,
                                                  const Plato::ScalarVector& aControl,
    ...
    /**************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_cp_T(const Plato::ScalarVector& aGlobalState,
                                                     const Plato::ScalarArray3D& aLocalState,
                                                     const Plato::ScalarVector& aControl,
                                                     Plato::Scalar aTimeStep = 0.0) const
    /**************************************************************************/
    {
        ...
        auto tMesh = mSpatialModel.Mesh;
        Teuchos::RCP<Plato::CrsMatrixType> tJacobianMat =
            Plato::CreateLocalByGlobalBlockMatrix<Plato::CrsMatrixType, mNumNodesPerCell, mNumLocalDofsPerCell,
                                                  mNumDofsPerNode>(tMesh);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // evaluate function
            //
            mGradientCFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                                    aTimeStep);
            ...
            auto tJacobianMatEntries = tJacobianMat->entries();
            Plato::WorksetBase<ElementType>::assembleTransposeJacobian(mNumDofsPerCell, mNumLocalDofsPerCell,
                                                                       tJacobianMatEntryOrdinal, tJacobian,
                                                                       tJacobianMatEntries, tDomain);
        }

        {
            ...
            //
            auto tFirstBlockName = mSpatialModel.Domains.front().getDomainName();
            mGradientCFunctions.at(tFirstBlockName)
                ->evaluate_boundary(mSpatialModel, tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                    aTimeStep);
    ...
    /**************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_z(
        const Plato::ScalarVector& aGlobalState,
        const Plato::ScalarArray3D& aLocalState,
        const Plato::ScalarVector& aControl,
        Plato::Scalar aTimeStep = 0.0
    ...
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // evaluate function
            //
            mGradientZFunctions.at(tName)->evaluate(tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                                    aTimeStep);
...

        {
            ...
            //
            auto tFirstBlockName = mSpatialModel.Domains.front().getDomainName();
            mGradientZFunctions.at(tFirstBlockName)
                ->evaluate_boundary(mSpatialModel, tGlobalStateWS, tLocalStateWS, tControlWS, tConfigWS, tJacobian,
                                    aTimeStep);
