...
# create_plato_analyze_header_library 
...
function( create_plato_analyze_header_library LIBRARY_NAME DIRECTORIES TARGET_LINK_LIST)
    set(LIB_TYPE INTERFACE)
    set(EXPORT_TYPE INTERFACE)
    create_plato_analyze_library_impl(${LIBRARY_NAME} "${DIRECTORIES}" "${TARGET_LINK_LIST}" ${LIB_TYPE} ${EXPORT_TYPE})
endfunction(create_plato_analyze_header_library)

# create_plato_analyze_library 
#   LIBRARY_NAME: The name of the library and CMake target that will be created.
#   DIRECTORIES: A list of directories containing the test source files.
#   TARGET_LINK_LIST: A list of targets to link against.
function( create_plato_analyze_library LIBRARY_NAME DIRECTORIES TARGET_LINK_LIST)
    set(LIB_TYPE SHARED)
    set(EXPORT_TYPE PUBLIC)
    create_plato_analyze_library_impl(${LIBRARY_NAME} "${DIRECTORIES}" "${TARGET_LINK_LIST}" ${LIB_TYPE} ${EXPORT_TYPE})
endfunction(create_plato_analyze_library)

# Implementation detail
function(create_plato_analyze_library_impl LIBRARY_NAME DIRECTORIES TARGET_LINK_LIST LIB_TYPE EXPORT_TYPE)
    ...
    target_include_directories(${LIBRARY_NAME} ${EXPORT_TYPE} $<BUILD_INTERFACE:${ANALYZE_BASE_DIR}>)
    ...
    install( TARGETS ${LIBRARY_NAME} EXPORT PlatoAnalyze
