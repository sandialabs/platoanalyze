...
Teuchos::RCP<MatrixType> CreateGlobalByLocalBlockMatrix(Plato::Mesh aMesh)
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumNodes), KOKKOS_LAMBDA(Plato::OrdinalType aNodeOrdinal) {
    ...
    auto retMatrix =
        Teuchos::rcp(new MatrixType(rowMap, columnIndices, entries, tNumNodes * ElementType::mNumDofsPerNode,
                                    tNumElems * ElementType::mNumGaussPoints * ElementType::mNumLocalStatesPerGP,
                                    ElementType::mNumDofsPerNode, ElementType::mNumLocalStatesPerGP));
    return retMatrix;
}
...
Teuchos::RCP<MatrixType> CreateLocalByGlobalBlockMatrix(Plato::Mesh aMesh)
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumElems), KOKKOS_LAMBDA(Plato::OrdinalType aCellOrdinal) {
    ...
    auto tNumNodes = aMesh->NumNodes();
    auto retMatrix = Teuchos::rcp(new MatrixType(rowMap, columnIndices, entries, tNumElems * DofsPerElem_I,
                                                 tNumNodes * DofsPerNode_J, DofsPerElem_I, DofsPerNode_J));
    return retMatrix;
}
...
/******************************************************************************/
{
    ...
    auto retMatrix = Teuchos::rcp(new MatrixType(tOffsetMap, tNodeOrds, entries, numRows * DofsPerNode_I,
                                                 numRows * DofsPerNode_J, DofsPerNode_I, DofsPerNode_J));
    return retMatrix;
}
...
/******************************************************************************/
{
    ...
    constexpr Plato::OrdinalType numBlockDofs = DofsPerNode_I * DofsPerNode_J;
    typename MatrixType::ScalarVectorT entries("matrix entries", nnz * numBlockDofs);
    auto retMatrix = Teuchos::rcp(new MatrixType(tOffsetMap, tNodeOrds, entries, numRows * DofsPerNode_J,
                                                 numRows * DofsPerNode_I, DofsPerNode_J, DofsPerNode_I));
    return retMatrix;
}
