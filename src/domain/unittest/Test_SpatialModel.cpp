...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, PlatoMask)
{
    ...
    //
    Teuchos::RCP<Teuchos::ParameterList> tMaskParams = Teuchos::getParametersFromXmlString(
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, PlatoModel)
{
    ...
    auto tMask = tMaskFactory.create(tSpatialModel.mMesh, *tMaskParams);

    tSpatialModel.applyMask(tMask);

    auto tOrdinals = tSpatialModel.mDomains[0].cellOrdinals();

    auto tOrdinals_host = Kokkos::create_mirror_view(tOrdinals);
    Kokkos::deep_copy(tOrdinals_host, tOrdinals);

    std::vector<int> tOrdinals_gold = {0,  1,  2,  3,  4,  5,  12, 13, 14, 15, 16, 17,
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, DefaultBlockNotMarkedAsFixed)
{
    ...
    auto tIsFixed = tSpatialModel.mDomains[0].fixedBlock();
...
TEUCHOS_UNIT_TEST(PlatoAnalyzeUnitTests, DefaultBlockWorksetControlUnchanged)
{
    ...
    tWorksetBase.worksetControl(tControl, tControlWS, tSpatialModel.mDomains[0]);
