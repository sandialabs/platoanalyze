...
inline Scalar local_result_sum(const Plato::OrdinalType& aNumCells, const Result& aResult)
{
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal, Scalar& aLocalResult) {
...
inline void flatten_vector_workset(const Plato::OrdinalType& aNumCells, AViewType& aInput, BViewType& aOutput)
{
    ...
    Kokkos::parallel_for(
        "flatten residual vector", Kokkos::RangePolicy<>(0, aNumCells),
...
inline void flatten_vector_workset(const Plato::SpatialDomain& aDomain, AViewType& aInput, BViewType& aOutput)
{
    ...
    Kokkos::parallel_for(
        "flatten residual vector", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
...
inline void assemble_vector_workset(const Plato::SpatialDomain& aDomain, ViewType& aInput, ViewType& aOutput)
{
    ...
    Kokkos::parallel_for(
        "combine residual vector", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerCell; tDofIndex++)
            {
                                            ...
                                            Plato::ScalarVector& aOutput)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "Convert AD Partial to POD type", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
                                            ...
                                            Plato::ScalarArray3D& aOutput)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "convert AD type to Scalar type", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
                                            ...
                                            Plato::ScalarArray3D& aOutput)
{
    ...
    Kokkos::parallel_for(
        "convert AD type to Scalar type", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < NumRowsPerCell; tRowIndex++)
            {
                for (Plato::OrdinalType tColumnIndex = 0; tColumnIndex < NumColsPerCell; tColumnIndex++)
...
Scalar assemble_scalar_func_value(const Plato::OrdinalType& aNumCells, const Result& aResult)
{
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal, Scalar& aLocalValue) {
...
{
    Kokkos::parallel_for(
        "Assemble - Vector Gradient Calculation", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                for (Plato::OrdinalType tDimIndex = 0; tDimIndex < NumDofsPerNode; tDimIndex++)
                {
                    ...
                    Kokkos::atomic_add(&aOutput(tEntryOrdinal),
                                       aGradient(aCellOrdinal, tNodeIndex * NumDofsPerNode + tDimIndex));
                }
            }
        });
}
        ...
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                for (Plato::OrdinalType tDimIndex = 0; tDimIndex < NumDofsPerNode; tDimIndex++)
                {
                    Plato::OrdinalType tEntryOrdinal = aEntryOrdinal(aCellOrdinal, tNodeIndex, tDimIndex);
                    Kokkos::atomic_add(&aOutput(tEntryOrdinal),
                                       aGradient(aCellOrdinal).dx(tNodeIndex * NumDofsPerNode + tDimIndex));
                }
            }
        });
}
                                         ...
                                         ReturnVal& aOutput)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "Assemble - Vector Gradient Calculation", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
...
{
    Kokkos::parallel_for(
        "Assemble - Scalar Gradient Calculation", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                                         ...
                                         ReturnVal& aOutput)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "Assemble - Scalar Gradient Calculation", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                                          ...
                                          ControlWS& aControlWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_control_scalar_scalar", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
...
{
    Kokkos::parallel_for(
        "workset_control_scalar_scalar", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                                       ...
                                       FadControlWS& aFadControlWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_control_scalar_fad", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                Plato::OrdinalType tEntryOrdinal = aControlEntryOrdinal(tCellOrdinal, tNodeIndex);
                aFadControlWS(aCellOrdinal, tNodeIndex) =
                    ControlFad(NumNodesPerCell, tNodeIndex, aControl(tEntryOrdinal));
            }
        });
}
...
{
    Kokkos::parallel_for(
        "workset_control_scalar_fad", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                Plato::OrdinalType tEntryOrdinal = aControlEntryOrdinal(aCellOrdinal, tNodeIndex);
                                        ...
                                        StateWS& aStateWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_state_scalar_scalar", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
...
template <Plato::OrdinalType NumDofsPerNode,
          ...
          class State,
          class StateWS>
inline void workset_state_scalar_scalar(Plato::OrdinalType aNumCells,
                                        const StateEntryOrdinal& aStateEntryOrdinal,
                                        const State& aState,
                                        StateWS& aStateWS)
{
    Kokkos::parallel_for(
        "workset_state_scalar_scalar", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerNode; tDofIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                                     ...
                                     FadStateWS& aFadStateWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_state_scalar_fad", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerNode; tDofIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                {
                    Plato::OrdinalType tEntryOrdinal = aStateEntryOrdinal(tCellOrdinal, tNodeIndex, tDofIndex);
                    Plato::OrdinalType tLocalDof = tNodeIndex * NumDofsPerNode + tDofIndex;
                    aFadStateWS(aCellOrdinal, tLocalDof) =
                        StateFad(NumDofsPerNode * NumNodesPerCell, tLocalDof, aState(tEntryOrdinal));
                }
            }
        });
}
...
{
    Kokkos::parallel_for(
        "workset_state_scalar_fad", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerNode; tDofIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                                              ...
                                              StateWS& aStateWS)
{
    ...
    Kokkos::parallel_for(
        "workset_local_state_scalar_fad", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {aNumCells, tNumPoints}),
                                              ...
                                              StateWS& aStateWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_local_state_scalar_fad", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumPoints}),
        KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                                  ...
                                  ConfigWS& aConfigWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_config_scalar", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tDimIndex = 0; tDimIndex < SpaceDim; tDimIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                {
                    aConfigWS(aCellOrdinal, tNodeIndex, tDimIndex) =
                        aNodeCoordinate(tCellOrdinal, tNodeIndex, tDimIndex);
                }
            }
        });
}
 ...
 *******************************************************************************/
template <Plato::OrdinalType SpaceDim, Plato::OrdinalType NumNodesPerCell, class ConfigWS, class NodeCoordinates>
inline void workset_config_scalar(Plato::OrdinalType aNumCells,
                                  const NodeCoordinates& aNodeCoordinate,
                                  ConfigWS& aConfigWS)
{
    Kokkos::parallel_for(
        "workset_config_scalar", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tDimIndex = 0; tDimIndex < SpaceDim; tDimIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                {
                    aConfigWS(aCellOrdinal, tNodeIndex, tDimIndex) =
                        aNodeCoordinate(aCellOrdinal, tNodeIndex, tDimIndex);
                }
            }
        });
}
...
template <Plato::OrdinalType SpaceDim,
          Plato::OrdinalType NumNodesPerCell,
          Plato::OrdinalType numConfigDofsPerCell,
          class ConfigFad,
          class FadConfigWS,
                               ...
                               FadConfigWS& aFadConfigWS)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "workset_config_fad", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tDimIndex = 0; tDimIndex < SpaceDim; tDimIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                {
                    ...
                    aFadConfigWS(aCellOrdinal, tNodeIndex, tDimIndex) = ConfigFad(
                        numConfigDofsPerCell, tLocalDim, aNodeCoordinate(tCellOrdinal, tNodeIndex, tDimIndex));
                }
            }
        });
}
...
{
    Kokkos::parallel_for(
        "workset_config_fad", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tDimIndex = 0; tDimIndex < SpaceDim; tDimIndex++)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
                {
                    Plato::OrdinalType tLocalDim = tNodeIndex * SpaceDim + tDimIndex;
                    aFadConfigWS(aCellOrdinal, tNodeIndex, tDimIndex) = ConfigFad(
                        numConfigDofsPerCell, tLocalDim, aNodeCoordinate(aCellOrdinal, tNodeIndex, tDimIndex));
                }
            }
        });
}
                              ...
                              ReturnVal& aReturnValue)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "assemble_residual", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                ...
                {
                    Plato::OrdinalType tEntryOrdinal = aStateEntryOrdinal(tCellOrdinal, tNodeIndex, tDofIndex);
                    Kokkos::atomic_add(&aReturnValue(tEntryOrdinal),
                                       aResidual(aCellOrdinal, tNodeIndex * NumDofsPerNode + tDofIndex));
                }
            }
        });
}
...
template <Plato::OrdinalType NumNodesPerCell,
          ...
          class Residual,
          class ReturnVal>
inline void assemble_residual(const Plato::OrdinalType& aNumCells,
                              const StateEntryOrdinal& aStateEntryOrdinal,
                              const Residual& aResidual,
                              ReturnVal& aReturnValue)
{
    Kokkos::parallel_for(
        "assemble_residual", Kokkos::RangePolicy<Plato::OrdinalType>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < NumNodesPerCell; tNodeIndex++)
            {
                for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerNode; tDofIndex++)
...
{
    Kokkos::parallel_for(
        "assemble jacobian", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < aNumRowsPerCell; tRowIndex++)
            {
                for (Plato::OrdinalType tColumnIndex = 0; tColumnIndex < aNumColumnsPerCell; tColumnIndex++)
                {
                    ...
                    Kokkos::atomic_add(&aReturnValue(tEntryOrdinal),
                                       aJacobianWorkset(aCellOrdinal, tRowIndex, tColumnIndex));
                }
            }
        });
}
                                  ...
                                  ReturnVal& aReturnValue)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "assemble jacobian fad", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < aNumRowsPerCell; tRowIndex++)
            {
                for (Plato::OrdinalType tColumnIndex = 0; tColumnIndex < aNumColumnsPerCell; tColumnIndex++)
                {
                    ...
                    Kokkos::atomic_add(&aReturnValue(tEntryOrdinal),
                                       aJacobianWorkset(aCellOrdinal, tRowIndex).dx(tColumnIndex));
                }
            }
        });
}
 ...
 *******************************************************************************/
template <class MatrixEntriesOrdinal, class Jacobian, class ReturnVal>
inline void assemble_jacobian_fad(const Plato::OrdinalType& aNumCells,
                                  Plato::OrdinalType aNumRowsPerCell,
                                  Plato::OrdinalType aNumColumnsPerCell,
                                  const MatrixEntriesOrdinal& aMatrixEntryOrdinal,
                                  const Jacobian& aJacobianWorkset,
...
{
    Kokkos::parallel_for(
        "assemble jacobian fad", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < aNumRowsPerCell; tRowIndex++)
            {
                for (Plato::OrdinalType tColumnIndex = 0; tColumnIndex < aNumColumnsPerCell; tColumnIndex++)
 ...
 * \param [in]     aContributingNode ordinal of node on local face whose nonlocal cell contributions are being assembled
 *in \param [in]     aMatrixEntryOrdinal  matrix entries index map \param [in]     aJacobianWorkset     jacobian
 *workset, i.e. jacobian for each element/cell \param [in/out] aReturnValue         assembled Jacobian
 *
 *******************************************************************************/
template <class MatrixEntriesOrdinal, class Jacobian, class ReturnVal>
inline void assemble_jacobian_fad(Plato::OrdinalType aNumNodesPerFace,
        ...
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            ...
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < aNumNodesPerFace; tNodeIndex++)
            {
                ...
                for (Plato::OrdinalType tDofIndex = 0; tDofIndex < aNumDofsPerNode; tDofIndex++)
                {
                    ...
                    for (Plato::OrdinalType tColumnIndex = 0; tColumnIndex < aNumColumnsPerCell; tColumnIndex++)
                    {
                        Plato::OrdinalType tEntryOrdinal =
                            aMatrixEntryOrdinal(tLocalCell, tNonLocalCell, tRowIndex, tColumnIndex);
                        Kokkos::atomic_add(&aReturnValue(tEntryOrdinal),
                                           aJacobianWorkset(tLocalCell, tRowIndex).dx(tColumnIndex));
                    }
                }
            }
        });
                                        ...
                                        ReturnVal& aReturnValue)
{
    ...
    auto tCellOrdinals = aDomain.cellOrdinals();
    Kokkos::parallel_for(
        "assemble_transpose_jacobian", Kokkos::RangePolicy<>(0, tNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            auto tCellOrdinal = tCellOrdinals[aCellOrdinal];
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < aNumRowsPerCell; tRowIndex++)
            {
                                              ...
                                              ReturnVal& aReturnValue)
{
    ...
    Kokkos::parallel_for(
        "assemble state jacobian transpose", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumPoints}),
        KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
            auto tCellOrdinal = tCellOrdinals[iCellOrdinal];
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < aNumRowsPerCell; tRowIndex++)
            {
                ...
                {
                    Plato::OrdinalType tEntryOrdinal =
                        aMatrixEntryOrdinal(tCellOrdinal, iGpOrdinal, tColumnIndex, tRowIndex);
                    Kokkos::atomic_add(&aReturnValue(tEntryOrdinal),
                                       aJacobianWorkset(iCellOrdinal, iGpOrdinal, tRowIndex).dx(tColumnIndex));
                }
            }
        });
}
...
{
    Kokkos::parallel_for(
        "assemble_transpose_jacobian", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < aNumRowsPerCell; tRowIndex++)
            {
                for (Plato::OrdinalType tColumnIndex = 0; tColumnIndex < aNumColumnsPerCell; tColumnIndex++)
