...
class ProjectedSurfaceDisplacement : public AbstractSurfaceDisplacement<EvaluationType>
{
   private:
   ...
   public:
    ProjectedSurfaceDisplacement(const Plato::OrdinalVectorT<Plato::OrdinalType>& aParentElements,
                    ...
                    Plato::ScalarArray3DT<OutStateT>& aSurfaceDisp) const override
    {
        ...
        Plato::InterpolateFromNodal<ElementType, NumDofsPerNode, /*offset=*/0, ElementType::mNumSpatialDims>
            tInterpolateFromNodal;
        ...
        Kokkos::parallel_for(
            "projected surface displacement", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumFaces, tNumPoints}),
            KOKKOS_LAMBDA(const Plato::OrdinalType& iCellOrdinal, const Plato::OrdinalType& iGPOrdinal) {
                ...
                for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerNode; tDofIndex++)
                {
                    aSurfaceDisp(iCellOrdinal, iGPOrdinal, tDofIndex) =
                        tScale * tBasisValues(tChildNode) * tSurfaceDisp(tDofIndex);
                }
            });
    }

    void setChildNode(Plato::OrdinalType aChildNode) { mChildNode = aChildNode; }
