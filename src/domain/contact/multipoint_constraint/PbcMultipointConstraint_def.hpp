...
/****************************************************************************/
template <typename ElementT>
void Plato::PbcMultipointConstraint<ElementT>::get(OrdinalVector& aMpcChildNodes,
...
/****************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "parent nodes", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumberParentNodes),
        KOKKOS_LAMBDA(Plato::OrdinalType nodeOrdinal) {
    ...
    Kokkos::parallel_for(
        "child nodes, mpc matrix, and rhs values", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumberChildNodes),
        KOKKOS_LAMBDA(Plato::OrdinalType nodeOrdinal) {
            ...
            {
                tColumnIndices(aOffsetNnz + entryOrdinal) =
                    aOffsetParent + tMpcColumnIndices(entryOrdinal);              // column indices
...
/****************************************************************************/
template <typename ElementT>
void Plato::PbcMultipointConstraint<ElementT>::updateLengths(OrdinalType& lengthChild,
...
/****************************************************************************/
template <typename ElementT>
void Plato::PbcMultipointConstraint<ElementT>::updateNodesets(
    const OrdinalType& tNumberChildNodes, const Plato::OrdinalVectorT<const Plato::OrdinalType>& tChildNodeLids)
/****************************************************************************/
{
    auto tChildNodes = mChildNodes;
    Kokkos::parallel_for(
        "Child node IDs", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumberChildNodes),
        KOKKOS_LAMBDA(Plato::OrdinalType nodeOrdinal) {
...
/****************************************************************************/
template <typename ElementT>
void Plato::PbcMultipointConstraint<ElementT>::mapChildVertexLocations(
    Plato::Mesh aMesh,
...
/****************************************************************************/
{
    ...
    auto tChildNodes = mChildNodes;
    Kokkos::parallel_for(
        "get verts and apply map", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumberChildNodes),
        KOKKOS_LAMBDA(Plato::OrdinalType nodeOrdinal) {
            Plato::OrdinalType childNode = tChildNodes(nodeOrdinal);
            for (size_t iDim = 0; iDim < ElementT::mNumSpatialDims; ++iDim)
            {
                aMappedLocations(iDim, nodeOrdinal) =
                    tCoords[childNode * ElementT::mNumSpatialDims + iDim] + aTranslation(iDim);
            }
        });
}
...
/****************************************************************************/
template <typename ElementT>
void Plato::PbcMultipointConstraint<ElementT>::getUniqueParentNodes(Plato::Mesh aMesh,
...
/****************************************************************************/
{
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, tNumberParentElements),
    ...
    Kokkos::parallel_for(
        "mark 1 for parent element vertices", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumberParentElements),
        KOKKOS_LAMBDA(Plato::OrdinalType iElemOrdinal) {
    ...
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, tNVerts),
    ...
    Kokkos::parallel_for(
        "map from global vertex ID to local parent node ID", Kokkos::RangePolicy<Plato::OrdinalType>(0, tSum),
        KOKKOS_LAMBDA(Plato::OrdinalType parentOrdinal) {
...
/****************************************************************************/
template <typename ElementT>
void Plato::PbcMultipointConstraint<ElementT>::setMatrixValues(Plato::Mesh aMesh,
                                                               OrdinalVector& aParentElements,
                                                               Plato::ScalarMultiVector& aMappedLocations,
                                                               OrdinalVector& aParentGlobalLocalMap)
/****************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "nonzeros", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumChildNodes),
        KOKKOS_LAMBDA(Plato::OrdinalType iRowOrdinal) { tRowMap(iRowOrdinal) = ElementT::mNumNodesPerCell; });

    Plato::OrdinalType tNumEntries(0);
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumChildNodes + 1),
        KOKKOS_LAMBDA(const Plato::OrdinalType& iOrdinal, Plato::OrdinalType& aUpdate, const bool& tIsFinal) {
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Plato::OrdinalType iRowOrdinal) {
            ...
            {
                tColMap(iEntryOrdinal + iNode) =
                    aParentGlobalLocalMap(tCells2Nodes[iElemOrdinal * ElementT::mNumNodesPerCell + iNode]);
    ...
    // fill in mpc matrix
    mMpcMatrix =
        Teuchos::rcp(new Plato::CrsMatrixType(tRowMap, tColMap, tEntries, tNumChildNodes, tNumParentNodes, 1, 1));
}
