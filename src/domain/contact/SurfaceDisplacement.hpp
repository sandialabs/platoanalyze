...
template <typename EvaluationType, Plato::OrdinalType NumDofsPerNode = EvaluationType::ElementType::mNumSpatialDims>
class SurfaceDisplacement : public AbstractSurfaceDisplacement<EvaluationType>
{
   private:
   ...
   public:
    SurfaceDisplacement(const Plato::OrdinalVectorT<const Plato::OrdinalType>& aSideSetLocalNodes,
                        Plato::Scalar aScale = 1.0)
        : AbstractSurfaceDisplacement<EvaluationType>(aScale), mSideSetLocalNodes(aSideSetLocalNodes)
    {
    }

    void operator()(const Plato::OrdinalVectorT<const Plato::OrdinalType>& aElementOrds,
                    ...
                    Plato::ScalarArray3DT<OutStateT>& aSurfaceDisp) const override
    {
        ...
        Kokkos::parallel_for(
            "surface displacement", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumFaces, tNumPoints}),
            KOKKOS_LAMBDA(const Plato::OrdinalType& iCellOrdinal, const Plato::OrdinalType& iGPOrdinal) {
                ...
                for (Plato::OrdinalType tDofIndex = 0; tDofIndex < NumDofsPerNode; tDofIndex++)
                {
                    ...
                    {
                        Plato::OrdinalType tFaceNode =
                            tSideSetLocalNodes(iCellOrdinal * ElementType::mNumNodesPerFace + tNodeIndex);
                        Plato::OrdinalType tCellDofIndex = NumDofsPerNode * tFaceNode + tDofIndex;
                        aSurfaceDisp(iCellOrdinal, iGPOrdinal, tDofIndex) +=
                            tScale * tBasisValues(tNodeIndex) * aState(tGlobalCellOrdinal, tCellDofIndex);
                    }
                }
            });
    }
