...
ContactPair parse_contact_pair(const Teuchos::ParameterList& aParams, Plato::Mesh aMesh)
{
    ...
    if (aParams.isSublist("A Surface"))
    {
        ...
        ANALYZE_THROWERR(
            "Parsing 'Contact' parameter list 'Pairs' sublist. Required 'A Surface' parameter sublist not found");
    ...
    if (aParams.isSublist("B Surface"))
    {
        ...
        ANALYZE_THROWERR(
            "Parsing 'Contact' parameter list 'Pairs' sublist. Required 'B Surface' parameter sublist not found");
...
void parse_contact_penalty(const Teuchos::ParameterList& aParams, ContactPair& aContactPair)
{
    if (!aParams.isType<std::string>("Penalty Type"))
        ANALYZE_THROWERR(
            "Parsing 'Contact' parameter list 'Pairs' sublist. Required 'Penalty Type' parameter not found")
                                        ...
                                        const std::vector<plato::domain::SpatialDomain>& aDomains)
{
    ...
    std::string tMsg = "Block with name " + aDomainName +
                       " provided in get_domain does not correspond to an element block name in spatial model";
                                            ...
                                            const Teuchos::Array<Plato::Scalar>& aTranslation)
{
    ...
    if (aTranslation.size() != tSpaceDim || aLocations.extent(0) != tSpaceDim)
        ANALYZE_THROWERR(
            "In ContactUtils map_node_locations, an incorrect dimension is given. Only 3 dimensions are supported.")
    ...
    Kokkos::parallel_for(
        "map coords", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumNodes), KOKKOS_LAMBDA(int nodeOrdinal) {
                                                     ...
                                                     Plato::OrdinalType aNumMeshNodes)
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, aChildNodes.size()), KOKKOS_LAMBDA(int nodeOrdinal) {
    ...
    Plato::OrdinalType aNumNodesPerFace)
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<>(0, tNumChildElements), KOKKOS_LAMBDA(const Plato::OrdinalType& cellOrdinal) {
...
void check_for_missing_parent_elements(const Plato::OrdinalVector& aParentElements)
{
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, aParentElements.size()),
