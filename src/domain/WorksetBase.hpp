...
class WorksetBase : public ElementType
{
   protected:
    ...
    using ElementType::mNumLocalDofsPerCell; /*!< number of local degrees of freedom, e.g. plasticity variables, per
                                                element  */
    ...
    static constexpr Plato::OrdinalType mNumConfigDofsPerCell =
        mSpaceDim * mNumNodesPerCell; /*!< number of configuration degrees of freedom per element  */

    Plato::VectorEntryOrdinal<mSpaceDim, mNumDofsPerNode, mNumNodesPerCell>
        mGlobalStateEntryOrdinal; /*!< local-to-global ID map for global state */
    Plato::VectorEntryOrdinal<mSpaceDim, mNumNodeStatePerNode, mNumNodesPerCell>
        mNodeStateEntryOrdinal; /*!< local-to-global ID map for node state */
    Plato::VectorEntryOrdinal<mSpaceDim, mNumControl, mNumNodesPerCell>
        mControlEntryOrdinal; /*!< local-to-global ID map for control */
    Plato::VectorEntryOrdinal<mSpaceDim, mSpaceDim, mNumNodesPerCell>
        mConfigEntryOrdinal; /*!< local-to-global ID map for configuration */
   ...
   public:
                        ...
                        const plato::domain::SpatialDomain& aDomain) const
    {
        if (aDomain.fixedBlock())
        {
            ...
            Plato::workset_control_scalar_scalar<mNumNodesPerCell>(aDomain, mControlEntryOrdinal, tFixedControl,
                                                                   aControlWS);
        }
        else
        {
            Plato::workset_control_scalar_scalar<mNumNodesPerCell>(aDomain, mControlEntryOrdinal, aControl, aControlWS);
        }
    }
    ...
    {
        Plato::workset_control_scalar_scalar<mNumNodesPerCell>(mNumCells, mControlEntryOrdinal, aControl, aControlWS);
    }
                        ...
                        const plato::domain::SpatialDomain& aDomain) const
    {
        if (aDomain.fixedBlock())
        {
            Plato::ScalarVector tFixedControl("fixed control", aControl.size());
            Plato::blas1::fill(1.0, tFixedControl);
            Plato::workset_control_scalar_fad<mNumNodesPerCell, ControlFad>(aDomain, mControlEntryOrdinal,
                                                                            tFixedControl, aFadControlWS);
        }
        else
        {
            Plato::workset_control_scalar_fad<mNumNodesPerCell, ControlFad>(aDomain, mControlEntryOrdinal, aControl,
                                                                            aFadControlWS);
        }
    }
    ...
    {
        Plato::workset_control_scalar_fad<mNumNodesPerCell, ControlFad>(mNumCells, mControlEntryOrdinal, aControl,
                                                                        aFadControlWS);
    }
     ...
     **********************************************************************************/
    void worksetState(const Plato::ScalarVectorT<Plato::Scalar>& aState,
                      Plato::ScalarMultiVectorT<Plato::Scalar>& aStateWS) const
    {
        Plato::workset_state_scalar_scalar<mNumDofsPerNode, mNumNodesPerCell>(mNumCells, mGlobalStateEntryOrdinal,
                                                                              aState, aStateWS);
    }
     ...
     **********************************************************************************/
    void worksetState(const Plato::ScalarVectorT<Plato::Scalar>& aState,
                      Plato::ScalarMultiVectorT<StateFad>& aFadStateWS) const
    {
    ...
    void worksetLocalState(const Plato::ScalarArray3DT<Plato::Scalar>& aLocalState,
                           Plato::ScalarArray3DT<Plato::Scalar>& aLocalStateWS) const
    {
        Plato::workset_local_state_scalar_scalar<mNumLocalStatesPerGP>(mNumCells, aLocalState, aLocalStateWS);
    }
     ...
     **********************************************************************************/
    void worksetLocalState(const Plato::ScalarArray3DT<Plato::Scalar>& aLocalState,
                           Plato::ScalarArray3DT<LocalStateFad>& aFadLocalStateWS) const
    {
        Plato::workset_local_state_scalar_fad<mNumLocalDofsPerCell, LocalStateFad>(mNumCells, aLocalState,
                                                                                   aFadLocalStateWS);
    }
    ...
    {
        Plato::workset_state_scalar_scalar<mNumNodeStatePerNode, mNumNodesPerCell>(mNumCells, mNodeStateEntryOrdinal,
                                                                                   aState, aNodeStateWS);
    }
    ...
    {
        Plato::workset_config_scalar<mSpaceDim, mNumNodesPerCell>(mNumCells, mNodeCoordinate, aConfigWS);
    }
    ...
    {
        Plato::assemble_vector_gradient<mNumNodesPerCell, mNumDofsPerNode>(mNumCells, mGlobalStateEntryOrdinal,
                                                                           aWorkset, aOutput);
    }
    ...
    {
        Plato::assemble_vector_gradient_fad<mNumNodesPerCell, mNumDofsPerNode>(mNumCells, mGlobalStateEntryOrdinal,
                                                                               aWorkset, aOutput);
    }
    ...
    void assembleVectorGradientC(const WorksetType& aWorkset, OutType& aOutput) const
    {
        ...
        {
            ANALYZE_THROWERR(
                "Number of local degrees of freedom is set to zero. Local state variables are not defined for this "
                "application.");
        }
    ...
    {
        Plato::assemble_vector_gradient_fad<mNumNodesPerCell, mSpaceDim>(mNumCells, mConfigEntryOrdinal, aWorkset,
                                                                         aOutput);
    }
    ...
    {
        Plato::assemble_jacobian_fad(mNumCells, aNumRows, aNumColumns, aMatrixEntryOrdinal, aJacobianWorkset,
                                     aReturnValue);
    }
     ...
     *in \param [in] aMatrixEntryOrdinal container of Jacobian entry ordinal (local-to-global ID map) \param [in]
     *aJacobianWorkset Jacobian cell workset \param [in/out] aReturnValue assembled Jacobian
     **********************************************************************************/
    template <class MatrixEntriesOrdinalType, class JacobianWorksetType, class AssembledJacobianType>
    void assembleJacobianFad(Plato::OrdinalType aNumColumns,
                             ...
                             AssembledJacobianType& aReturnValue) const
    {
        Plato::assemble_jacobian_fad(mNumNodesPerFace, mNumDofsPerNode, aNumColumns, aLocalCells, aNonLocalCells,
                                     aNonLocalCellMap, aFaceLocalNodes, aContributingNode, aMatrixEntryOrdinal,
    ...
    void assembleJacobian(Plato::OrdinalType aNumRowsPerCell,
                          ...
                          AssembledJacobianType& aReturnValue) const
    {
        Plato::assemble_jacobian(mNumCells, aNumRowsPerCell, aNumColumnsPerCell, aMatrixEntryOrdinal, aJacobianWorkset,
                                 aReturnValue);
    }
    ...
    void assembleTransposeJacobian(Plato::OrdinalType aNumRowsPerCell,
                                   ...
                                   AssembledJacobianType& aReturnValue) const
    {
        Plato::assemble_transpose_jacobian(mNumCells, aNumRowsPerCell, aNumColumnsPerCell, aMatrixEntryOrdinal,
                                           aJacobianWorkset, aReturnValue);
    }
     ...
     **********************************************************************************/
    template <class MatrixEntriesOrdinalType, class JacobianWorksetType, class AssembledJacobianType>
    void assembleStateJacobianTranspose(Plato::OrdinalType aNumRowsPerCell,
                                        Plato::OrdinalType aNumColumnsPerCell,
                                        const MatrixEntriesOrdinalType& aMatrixEntryOrdinal,
                                        const JacobianWorksetType& aJacobianWorkset,
                                        AssembledJacobianType& aReturnValue,
    ...
    void assembleStateJacobianTranspose(Plato::OrdinalType aNumRowsPerCell,
                                        ...
                                        AssembledJacobianType& aReturnValue) const
    {
        Plato::assemble_state_jacobian_transpose(mNumCells, aNumRowsPerCell, aNumColumnsPerCell, aMatrixEntryOrdinal,
                                                 aJacobianWorkset, aReturnValue);
    }
};
// class WorksetBase
