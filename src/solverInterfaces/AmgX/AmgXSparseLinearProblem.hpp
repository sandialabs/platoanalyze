...
class AmgXSparseLinearProblem : public CrsLinearProblem<Ordinal>
{
   ...
   public:
    static std::string configurationString(ConfigurationOption configOption,
                                           double tol = DEFAULT_TOL,
                                           int maxIters = 10000,
                                           ToleranceNorm tolType = ABSOLUTE)
    {
        using namespace std;
...
";
   ...
   public:
    ...
    {
        // evidently, we should only call these once.  Probably best to separate them out into a discrete header, if
        // ever we decide we want to do other things with AmgX other than what's enabled by the present
   ...
   public:
    AmgXSparseLinearProblem(const Matrix A,
                            MultiVector x,
                            const MultiVector b,
                            std::string solverConfigString = configurationString(DEFAULT_CONFIG))
        : AmgXSparseLinearProblem(A,
                                  Vector(Kokkos::subview(x, 0, Kokkos::ALL())),
                                  Vector(Kokkos::subview(b, 0, Kokkos::ALL())),
                                  solverConfigString)
    {
        TEUCHOS_TEST_FOR_EXCEPTION(b.extent(0) != 1, std::invalid_argument,
                                   "We do not yet support having multiple RHSes");
        TEUCHOS_TEST_FOR_EXCEPTION(x.extent(0) != 1, std::invalid_argument,
                                   "We do not yet support having multiple RHSes");
    }
        ...
        : CrsLinearProblem<Ordinal>(A, x, b)
    {
        ...
        AMGX_resources_create(&_rsrc, _config, &mpi_comm, ndevices, devices);
    ...
    void setInitialGuess(const Vector x) { AMGX_vector_upload(_lhs, x.size() / BlockSize, BlockSize, x.data()); }
