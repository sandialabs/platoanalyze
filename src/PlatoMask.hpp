...
struct BoxLimits
{
    ...
    BoxLimits(const Teuchos::ParameterList& aParams)
        : mMaxKeywords{"Maximum X", "Maximum Y", "Maximum Z"}, mMinKeywords{"Minimum X", "Minimum Y", "Minimum Z"}
    {
...
/******************************************************************************/
{
   ...
   public:
     ...
     **********************************************************************************/
    void computeNodeMask(Plato::Mesh aMesh)
    {
        ...
        Kokkos::parallel_for(
            "compute node mask", Kokkos::RangePolicy<>(0, tNumCells),
     ...
     * \param [in] aMesh Plato abstract mesh
     **********************************************************************************/
    Plato::ScalarMultiVector getCellCenters(Plato::Mesh aMesh)
    {
        ...
        Kokkos::parallel_for(
            "get cell centers", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                for (Plato::OrdinalType tNode = 0; tNode < mSpaceDim + 1; tNode++)
                {
                    for (Plato::OrdinalType tDim = 0; tDim < mSpaceDim; tDim++)
    ...
    Mask(const Plato::Mesh aMesh)
        : mCellMask("cell mask", aMesh->NumElements()), mNodeMask("node mask", aMesh->NumNodes())
    {
    }
     ...
     * \param [in] aMesh Plato abstract mesh
     **********************************************************************************/
    OrdinalVector getInactiveNodes() const
    {
...
class BrickMask : public Plato::Mask<mSpaceDim>
{
   ...
   public:
     ...
     **********************************************************************************/
    BrickMask(Plato::Mesh aMesh, const Teuchos::ParameterList& aInputParams)
        : Plato::Mask<mSpaceDim>(aMesh), mLimits(aInputParams)
    {
        initialize(aMesh, aInputParams);
    }

    void initialize(Plato::Mesh aMesh, const Teuchos::ParameterList& aInputParams)
    {
...
class ConstructiveMask : public Plato::Mask<mSpaceDim>
{
   ...
   public:
     ...
     * \param [in] aInputParams Mask definition
     **********************************************************************************/
    ConstructiveMask(Plato::Mesh aMesh, const Teuchos::ParameterList& aInputParams)
        : Plato::Mask<mSpaceDim>(aMesh), mLimits(aInputParams)
    {
        initialize(aMesh, aInputParams);
    }
...
{
   public:
    std::shared_ptr<Plato::Mask<mSpaceDim>> create(Plato::Mesh aMesh, const Teuchos::ParameterList& aInputParams)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (tMaskType == "Constructive")
                {
