...
namespace Plato
{
void readCSVData(std::string const &aFileName,
...
void normalizeVector(Plato::ScalarVector aX, Plato::ScalarVector aY, Plato::ScalarVector aZ)
{
    Plato::OrdinalType tNumErr = 0;
...
void normalizeVector(Plato::ScalarVector aX, Plato::ScalarVector aY)
{
    Plato::OrdinalType tNumErr = 0;
    auto tLength = aX.extent(0);
    Kokkos::parallel_reduce(
...
Plato::ScalarArray3D createOrthonormalTensor2D(Teuchos::ParameterList &tTransformParams, Plato::DataMap &aDataMap)
{
    ...
    else
    {
        ...
        Kokkos::parallel_for(
            "perpendicular", Kokkos::RangePolicy<>(0, tLength), KOKKOS_LAMBDA(const Plato::OrdinalType &tDof) {
                tY0(tDof) = -tX1(tDof);
                tY1(tDof) = tX0(tDof);
            });
    }
    ...
    Kokkos::parallel_for(
        "create tensor", Kokkos::RangePolicy<>(0, tLength), KOKKOS_LAMBDA(const Plato::OrdinalType &tDof) {
...
Plato::ScalarArray3D createOrthonormalTensor3D(Teuchos::ParameterList &tTransformParams, Plato::DataMap &aDataMap)
{
    auto tXNames = Plato::ParseTools::getParam<Teuchos::Array<std::string>>(tTransformParams, "X");
    ...
    else
    {
        ...
        Kokkos::parallel_for(
            "cross product", Kokkos::RangePolicy<>(0, tLength), KOKKOS_LAMBDA(const Plato::OrdinalType &tDof) {
...
Plato::ScalarArray3D createOrthonormalTensor(Teuchos::ParameterList &tTransformParams,
                                             Plato::DataMap &aDataMap,
