...
/******************************************************************************/
template <int SpatialDim>
LinearThermoelasticMaterial<SpatialDim>::LinearThermoelasticMaterial(const Teuchos::ParameterList& paramList)
/******************************************************************************/
{
    for (int i = 0; i < mNumVoigtTerms; i++)
        for (int j = 0; j < mNumVoigtTerms; j++) mCellStiffness(i, j) = 0.0;

    for (int i = 0; i < SpatialDim; i++) mCellThermalExpansionCoef(i) = 0.0;

    for (int i = 0; i < SpatialDim; i++)
        for (int j = 0; j < SpatialDim; j++) mCellThermalConductivity(i, j) = 0.0;
...
/******************************************************************************/
template <int SpatialDim>
Teuchos::RCP<LinearThermoelasticMaterial<SpatialDim>> LinearThermoelasticModelFactory<SpatialDim>::create(
...
/******************************************************************************/
{
    ...
    else
    {
        ...
        {
            return Teuchos::rcp(new Plato::IsotropicLinearThermoelasticMaterial<SpatialDim>(
                tModelParamList.sublist("Isotropic Linear Thermoelastic")));
        }
        else if (tModelParamList.isSublist("Cubic Linear Thermoelastic"))
        {
            return Teuchos::rcp(new Plato::CubicLinearThermoelasticMaterial<SpatialDim>(
                tModelParamList.sublist("Cubic Linear Thermoelastic")));
        }
        return Teuchos::RCP<Plato::LinearThermoelasticMaterial<SpatialDim>>(nullptr);
    }
}
