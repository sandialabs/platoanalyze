...
class Quad4
{
   public:
    ...
    {
        return Plato::Array<mNumGaussPoints>{Plato::Scalar(1.0), Plato::Scalar(1.0), Plato::Scalar(1.0),
                                             Plato::Scalar(1.0)};
    }
    ...
    [[nodiscard]] static constexpr Plato::Matrix<mNumGaussPoints, mNumSpatialDims> getCubPoints()
    {
        ...
        return Plato::Matrix<mNumGaussPoints, mNumSpatialDims>{-sqt, -sqt, sqt, -sqt, sqt, sqt, -sqt, sqt};
    }
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint) -> Plato::Array<mNumNodesPerCell>
    {
        ...
        return Plato::Array<mNumNodesPerCell>{(1 - x) * (1 - y) / 4.0, (1 + x) * (1 - y) / 4.0, (1 + x) * (1 + y) / 4.0,
                                              (1 - x) * (1 + y) / 4.0};
    }
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint) -> Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>
    {
        ...
        return Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>{-(1 - y) / 4.0, -(1 - x) / 4.0, (1 - y) / 4.0,
                                                                -(1 + x) / 4.0, (1 + y) / 4.0,  (1 + x) / 4.0,
                                                                -(1 + y) / 4.0, (1 - x) / 4.0};
    }

    template <typename ScalarType>
    [[nodiscard]] constexpr KOKKOS_INLINE_FUNCTION static auto differentialMeasure(
    ...
    template <typename ScalarType>
    [[nodiscard]] constexpr KOKKOS_INLINE_FUNCTION static auto differentialVector(
