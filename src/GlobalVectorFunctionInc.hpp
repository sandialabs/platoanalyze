...
class GlobalVectorFunctionInc
{
   ...
   private:
    using Residual = typename Plato::Evaluation<typename PhysicsT::SimplexT>::Residual; /*!< automatic differentiation
                                                                                           (AD) type for the residual */
    using GradientX =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::GradientX; /*!< AD type for the configuration */
    using GradientZ =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::GradientZ; /*!< AD type for the controls */
    using JacobianPgrad =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::JacobianN; /*!< AD type for the projected pressure
                                                                               gradient */
    using LocalJacobianC =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::LocalJacobian; /*!< AD type for the current local
                                                                                   states */
    using LocalJacobianP =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::LocalJacobianP; /*!< AD type for the previous local
                                                                                    states */
    using GlobalJacobianC =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::Jacobian; /*!< AD type for the current global states */
    using GlobalJacobianP =
        typename Plato::Evaluation<typename PhysicsT::SimplexT>::JacobianP; /*!< AD type for the previous global states
                                                                             */

    static constexpr auto mNumControl =
        PhysicsT::SimplexT::mNumControl; /*!< number of control fields, i.e. vectors, number of materials */
    static constexpr auto mNumSpatialDims = PhysicsT::SimplexT::mNumSpatialDims; /*!< number of spatial dimensions */
    static constexpr auto mNumNodesPerCell =
        PhysicsT::SimplexT::mNumNodesPerCell; /*!< number of nodes per cell (i.e. element) */
    static constexpr auto mNumGlobalDofsPerNode =
        PhysicsT::SimplexT::mNumDofsPerNode; /*!< number of global degrees of freedom per node */
    static constexpr auto mNumGlobalDofsPerCell =
        PhysicsT::SimplexT::mNumDofsPerCell; /*!< number of global degrees of freedom per cell (i.e. element) */
    static constexpr auto mNumLocalDofsPerCell =
        PhysicsT::SimplexT::mNumLocalDofsPerCell; /*!< number of local degrees of freedom per cell (i.e. element) */
    static constexpr auto mNumNodeStatePerNode =
        PhysicsT::SimplexT::mNumNodeStatePerNode; /*!< number of pressure gradient degrees of freedom per node */
    static constexpr auto mNumNodeStatePerCell =
        PhysicsT::SimplexT::mNumNodeStatePerCell; /*!< number of pressure gradient degrees of freedom per cell (i.e.
                                                     element) */
    static constexpr auto mNumConfigDofsPerCell =
        mNumSpatialDims *
        mNumNodesPerCell; /*!< number of configuration (i.e. coordinates) degrees of freedom per cell (i.e. element) */
    ...
    std::map<std::string, JacobianCCFunction>
        mJacobianCCFunctions; /*!< global Jacobian with respect to current local states */
    std::map<std::string, JacobianPCFunction>
        mJacobianPCFunctions; /*!< global Jacobian with respect to previous local states */
    std::map<std::string, JacobianCUFunction>
        mJacobianCUFunctions; /*!< global Jacobian with respect to current global states */
    std::map<std::string, JacobianPUFunction>
        mJacobianPUFunctions; /*!< global Jacobian with respect to previous global states */
    std::map<std::string, JacobianPGFunction>
        mJacobianPGFunctions; /*!< global Jacobian with respect to projected pressure gradient */
   ...
   private:
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename Residual::ResultScalarType> residualBoundaryWorkset(
        ...
        const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3DT<ConfigScalar> tConfigWS("Configuration Workset", mNumCells, mNumNodesPerCell,
                                                      mNumSpatialDims);
        ...
        Plato::ScalarMultiVectorT<GlobalStateScalar> tCurrentGlobalStateWS("Current Global State Workset", mNumCells,
        ...
        Plato::ScalarMultiVectorT<PrevGlobalStateScalar> tPrevGlobalStateWS("Previous Global State Workset", mNumCells,
                                                                            mNumGlobalDofsPerCell);
        ...
        Plato::ScalarMultiVectorT<LocalStateScalar> tCurrentLocalStateWS("Current Local State Workset", mNumCells,
                                                                         mNumLocalDofsPerCell);
        ...
        Plato::ScalarMultiVectorT<PrevLocalStateScalar> tPrevLocalStateWS("Previous Local State Workset", mNumCells,
                                                                          mNumLocalDofsPerCell);
        ...
        Plato::ScalarMultiVectorT<NodeStateScalar> tProjPressGradWS("Projected Pressure Gradient Workset", mNumCells,
        ...
        mBoundaryLoadsResidualFunction->evaluate_boundary(mSpatialModel, tCurrentGlobalStateWS, tPrevGlobalStateWS,
                                                          tCurrentLocalStateWS, tPrevLocalStateWS, tProjPressGradWS,
     ...
     * \return residual workset, i.e. residual for each cell
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename Residual::ResultScalarType> residualWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        // Workset config
        using ConfigScalar = typename Residual::ConfigScalarType;
        Plato::ScalarArray3DT<ConfigScalar> tConfigWS("Configuration Workset", tNumCells, mNumNodesPerCell,
                                                      mNumSpatialDims);
        mWorksetBase.worksetConfig(tConfigWS, aDomain);

        // Workset current global state
        using GlobalStateScalar = typename Residual::StateScalarType;
        Plato::ScalarMultiVectorT<GlobalStateScalar> tCurrentGlobalStateWS("Current Global State Workset", tNumCells,
                                                                           mNumGlobalDofsPerCell);
        mWorksetBase.worksetState(aCurrentGlobalState, tCurrentGlobalStateWS, aDomain);

        // Workset previous global state
        using PrevGlobalStateScalar = typename Residual::PrevStateScalarType;
        Plato::ScalarMultiVectorT<PrevGlobalStateScalar> tPrevGlobalStateWS("Previous Global State Workset", tNumCells,
                                                                            mNumGlobalDofsPerCell);
        mWorksetBase.worksetState(aPrevGlobalState, tPrevGlobalStateWS, aDomain);

        // Workset local state
        using LocalStateScalar = typename Residual::LocalStateScalarType;
        Plato::ScalarMultiVectorT<LocalStateScalar> tCurrentLocalStateWS("Current Local State Workset", tNumCells,
                                                                         mNumLocalDofsPerCell);
        mWorksetBase.worksetLocalState(aCurrentLocalState, tCurrentLocalStateWS, aDomain);

        // Workset previous local state
        using PrevLocalStateScalar = typename Residual::PrevLocalStateScalarType;
        Plato::ScalarMultiVectorT<PrevLocalStateScalar> tPrevLocalStateWS("Previous Local State Workset", tNumCells,
                                                                          mNumLocalDofsPerCell);
        mWorksetBase.worksetLocalState(aPrevLocalState, tPrevLocalStateWS, aDomain);

        // Workset node state, i.e. projected pressure gradient
        using NodeStateScalar = typename Residual::NodeStateScalarType;
        Plato::ScalarMultiVectorT<NodeStateScalar> tProjPressGradWS("Projected Pressure Gradient Workset", tNumCells,
                                                                    mNumNodeStatePerCell);
        ...
        mResidualFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                               tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS, tResidualWS,
                                               aTimeData);
        return (tResidualWS);
    }
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename GradientZ::ResultScalarType> jacobianControlWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Control Workset", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt controls
        auto tName = aDomain.getDomainName();
        mJacobianZFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS, tJacobianWS,
                                                aTimeData);
        return (tJacobianWS);
    }
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename GradientX::ResultScalarType> jacobianConfigurationWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Configuration", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt configuration
        auto tName = aDomain.getDomainName();
        mJacobianXFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS, tJacobianWS,
                                                aTimeData);
        return (tJacobianWS);
    }
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename GlobalJacobianC::ResultScalarType> jacobianCurrentGlobalStateWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Current Global State", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt the current global states
        auto tName = aDomain.getDomainName();
        mJacobianCUFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                 tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS,
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename GlobalJacobianP::ResultScalarType> jacobianPreviousGlobalStateWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Previous Global State", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt the previous global states
        auto tName = aDomain.getDomainName();
        mJacobianPUFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                 tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS,
                                                 tJacobianWS, aTimeData);
        return (tJacobianWS);
    }
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename LocalJacobianC::ResultScalarType> jacobianCurrentLocalStateWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Local State Workset", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt the current local states
        auto tName = aDomain.getDomainName();
        mJacobianCCFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                 tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS,
                                                 tJacobianWS, aTimeData);
        return (tJacobianWS);
    }
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename LocalJacobianP::ResultScalarType> jacobianPreviousLocalStateWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Previous Local State Workset", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt the previous local states
        auto tName = aDomain.getDomainName();
        mJacobianPCFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                 tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS,
                                                 tJacobianWS, aTimeData);
        return (tJacobianWS);
    }
     ...
     *******************************************************************************/
    Plato::ScalarMultiVectorT<typename JacobianPgrad::ResultScalarType> jacobianProjPressGradWorkset(
        const Plato::ScalarVector& aCurrentGlobalState,
        const Plato::ScalarVector& aPrevGlobalState,
        const Plato::ScalarVector& aCurrentLocalState,
        const Plato::ScalarVector& aPrevLocalState,
        ...
        const Plato::SpatialDomain& aDomain) const
    {
        ...
        Plato::ScalarMultiVectorT<JacobianScalar> tJacobianWS("Jacobian Projected Pressure Gradient Workset", tNumCells,
                                                              mNumGlobalDofsPerCell);

        // Call evaluate function - compute Jacobian wrt pressure gradient
        auto tName = aDomain.getDomainName();
        mJacobianPGFunctions.at(tName)->evaluate(tCurrentGlobalStateWS, tPrevGlobalStateWS, tCurrentLocalStateWS,
                                                 tPrevLocalStateWS, tProjPressGradWS, tControlWS, tConfigWS,
                                                 tJacobianWS, aTimeData);
        return (tJacobianWS);
    }
   ...
   public:
     ...
     ***************************************************************************/
    GlobalVectorFunctionInc(const Plato::SpatialModel& aSpatialModel,
          ...
          mWorksetBase(aSpatialModel.Mesh)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            mResidualFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<Residual>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianCUFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<GlobalJacobianC>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianPUFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<GlobalJacobianP>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianCCFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<LocalJacobianC>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianPCFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<LocalJacobianP>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianZFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<GradientZ>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianXFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<GradientX>(
                tDomain, aDataMap, aParamList, aFuncType);
            mJacobianPGFunctions[tName] = tFunctionFactory.template createGlobalVectorFunctionInc<JacobianPgrad>(
                tDomain, aDataMap, aParamList, aFuncType);
        }
     ...
     *******************************************************************************/
    Plato::Mesh getSpatialModel() const { return mSpatialModel; }

    /***********************************************************************/
    /**
     * \brief Return total number of degrees of freedom
     ***************************************************************************/
    decltype(mNumNodes) size() const { return mNumNodes * mNumGlobalDofsPerNode; }
     ...
     ***************************************************************************/
    decltype(mNumNodes) numNodes() const { return mNumNodes; }
     ...
     ***************************************************************************/
    decltype(mNumNodesPerCell) numNodesPerCell() const { return mNumNodesPerCell; }
     ...
     ***************************************************************************/
    decltype(mNumCells) numCells() const { return mNumCells; }
     ...
     ***************************************************************************/
    decltype(mNumSpatialDims) numSpatialDims() const { return mNumSpatialDims; }
     ...
     ***************************************************************************/
    decltype(mNumGlobalDofsPerNode) numGlobalDofsPerNode() const { return mNumGlobalDofsPerNode; }
     ...
     ***************************************************************************/
    decltype(mNumGlobalDofsPerCell) numGlobalDofsPerCell() const { return mNumGlobalDofsPerCell; }
     ...
     ***************************************************************************/
    decltype(mNumLocalDofsPerCell) numLocalDofsPerCell() const { return mNumLocalDofsPerCell; }
     ...
     ***************************************************************************/
    decltype(mNumNodeStatePerNode) numNodeStatePerNode() const { return mNumNodeStatePerNode; }
     ...
     ***************************************************************************/
    decltype(mNumNodeStatePerCell) numNodeStatePerCell() const { return mNumNodeStatePerCell; }
     ...
     ***************************************************************************/
    Plato::ScalarVector value(const Plato::ScalarVector& aCurrentGlobalState,
                              const Plato::ScalarVector& aPrevGlobalState,
                              const Plato::ScalarVector& aCurrentLocalState,
                              const Plato::ScalarVector& aPrevLocalState,
                              const Plato::ScalarVector& aProjPressGrad,
                              ...
                              const Plato::TimeData& aTimeData) const
    {
        ...
        Kokkos::View<Plato::Scalar*, Plato::Layout, Plato::MemSpace> tAssembledResidual("Assembled Residual",
                                                                                        tTotalNumDofs);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            auto tResidualWS = this->residualWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                     aPrevLocalState, aProjPressGrad, aControls, aTimeData, tDomain);
            mWorksetBase.assembleResidual(tResidualWS, tAssembledResidual, tDomain);
        }

        auto tResidualWS = this->residualBoundaryWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                         aPrevLocalState, aProjPressGrad, aControls, aTimeData);
     ...
     ***************************************************************************/
    Plato::ScalarArray3D gradient_z(const Plato::ScalarVector& aCurrentGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aCurrentLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aProjPressGrad,
                                    ...
                                    const Plato::TimeData& aTimeData) const
    {
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian WRT Control", mNumCells, mNumGlobalDofsPerCell,
                                             mNumNodesPerCell);

        for (const auto& tDomain : mSpatialModel.Domains)
        {
            auto tJacobianWS =
                this->jacobianControlWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState, aPrevLocalState,
                                             aProjPressGrad, aControls, aTimeData, tDomain);
            Plato::transform_ad_type_to_pod_3Dview<mNumGlobalDofsPerCell, mNumNodesPerCell>(tDomain, tJacobianWS,
                                                                                            tOutputJacobian);
        }
     ...
     ***************************************************************************/
    Plato::ScalarArray3D gradient_x(const Plato::ScalarVector& aCurrentGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aCurrentLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aProjPressGrad,
                                    ...
                                    const Plato::TimeData& aTimeData) const
    {
        Plato::ScalarArray3D tOutputJacobian("Jacobian WRT Configuration", mNumCells, mNumGlobalDofsPerCell,
                                             mNumConfigDofsPerCell);

        for (const auto& tDomain : mSpatialModel.Domains)
        {
            auto tJacobianWS =
                this->jacobianConfigurationWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                   aPrevLocalState, aProjPressGrad, aControls, aTimeData, tDomain);
            Plato::transform_ad_type_to_pod_3Dview<mNumGlobalDofsPerCell, mNumConfigDofsPerCell>(tDomain, tJacobianWS,
                                                                                                 tOutputJacobian);
        }
        return tOutputJacobian;
    }
     ...
     ***************************************************************************/
    Plato::ScalarArray3D gradient_u(const Plato::ScalarVector& aCurrentGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aCurrentLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aProjPressGrad,
                                    ...
                                    const Plato::TimeData& aTimeData) const
    {
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Current State", mNumCells, mNumGlobalDofsPerCell,
                                             mNumGlobalDofsPerCell);

        for (const auto& tDomain : mSpatialModel.Domains)
        {
            auto tJacobianWS =
                this->jacobianCurrentGlobalStateWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                        aPrevLocalState, aProjPressGrad, aControls, aTimeData, tDomain);
            Plato::transform_ad_type_to_pod_3Dview<mNumGlobalDofsPerCell, mNumGlobalDofsPerCell>(tDomain, tJacobianWS,
                                                                                                 tOutputJacobian);
        }
        return tOutputJacobian;
    }
     ...
     ***************************************************************************/
    Plato::ScalarArray3D gradient_up(const Plato::ScalarVector& aCurrentGlobalState,
                                     const Plato::ScalarVector& aPrevGlobalState,
                                     const Plato::ScalarVector& aCurrentLocalState,
                                     const Plato::ScalarVector& aPrevLocalState,
                                     const Plato::ScalarVector& aProjPressGrad,
                                     ...
                                     const Plato::TimeData& aTimeData) const
    {
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Previous Global State", mNumCells, mNumGlobalDofsPerCell,
                                             mNumGlobalDofsPerCell);
     ...
     ***************************************************************************/
    Plato::ScalarArray3D gradient_c(const Plato::ScalarVector& aCurrentGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aCurrentLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aProjPressGrad,
                                    ...
                                    const Plato::TimeData& aTimeData) const
    {
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Current Local State", mNumCells, mNumGlobalDofsPerCell,
                                             mNumLocalDofsPerCell);
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            auto tJacobianWS =
                this->jacobianCurrentLocalStateWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                       aPrevLocalState, aProjPressGrad, aControls, aTimeData, tDomain);
            Plato::transform_ad_type_to_pod_3Dview<mNumGlobalDofsPerCell, mNumLocalDofsPerCell>(tDomain, tJacobianWS,
                                                                                                tOutputJacobian);
        }
        return tOutputJacobian;
    }
     ...
     ***************************************************************************/
    Plato::ScalarArray3D gradient_cp(const Plato::ScalarVector& aCurrentGlobalState,
                                     const Plato::ScalarVector& aPrevGlobalState,
                                     const Plato::ScalarVector& aCurrentLocalState,
                                     const Plato::ScalarVector& aPrevLocalState,
                                     const Plato::ScalarVector& aProjPressGrad,
                                     ...
                                     const Plato::TimeData& aTimeData) const
    {
        Plato::ScalarArray3D tOutputJacobian("Jacobian Previous Local State", mNumCells, mNumGlobalDofsPerCell,
                                             mNumLocalDofsPerCell);

        for (const auto& tDomain : mSpatialModel.Domains)
        {
            auto tJacobianWS =
                this->jacobianPreviousLocalStateWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                        aPrevLocalState, aProjPressGrad, aControls, aTimeData, tDomain);
            Plato::transform_ad_type_to_pod_3Dview<mNumGlobalDofsPerCell, mNumLocalDofsPerCell>(tDomain, tJacobianWS,
                                                                                                tOutputJacobian);
        }
     ...
     ***************************************************************************/
    Teuchos::RCP<Plato::CrsMatrixType> gradient_n_T_assembled(const Plato::ScalarVector& aCurrentGlobalState,
                                                              const Plato::ScalarVector& aPrevGlobalState,
                                                              const Plato::ScalarVector& aCurrentLocalState,
                                                              const Plato::ScalarVector& aPrevLocalState,
                                                              const Plato::ScalarVector& aProjPressGrad,
                                                              ...
                                                              const Plato::TimeData& aTimeData) const
    {
        ...
        //   mNumDofsPerNode: Nd
        //
        // Note that the second two template parameters must match the block shape of the destination matrix,
        // tJacobianMat
        //
        ...
        {
            auto tJacobianWS =
                this->jacobianProjPressGradWorkset(aCurrentGlobalState, aPrevGlobalState, aCurrentLocalState,
                                                   aPrevLocalState, aProjPressGrad, aControls, aTimeData, tDomain);

            // Assemble from the AD-typed result, tJacobianWS, into the POD-typed global matrix,
            // tAssembledTransposeJacobian
            //
            ...
            mWorksetBase.assembleTransposeJacobian(mNumGlobalDofsPerCell, mNumNodeStatePerCell,
                                                   tJacobianMatEntryOrdinal, tJacobianWS, tJacobianMatEntries, tDomain);
        }
