...
**********************************************************************************/
void addUnique(strmap& aMap, const string& aStrKey, const string& aStrVal, const string& aStrContext)
{
    if (aStrKey.empty())
    {
 ...
 * \param [in] aRequired If true, and 'Argument' not found, throw.
   ...
   If \p aRequired, and 'Argument' matching \p aStrArgument is not found,
      throw a parsing exception.
**********************************************************************************/
std::string getArgumentName(Plato::InputData& aOpNode,
...
/******************************************************************************/
{
    ...
    for (auto tESPInput = tESPInputs.begin(); tESPInput != tESPInputs.end(); ++tESPInput)
    {
        ...
        throw Plato::ParsingException(
            "PlatoApp was not compiled with ESP support.  Turn on 'PLATO_ESP' option and rebuild.");
#endif  // PLATO_ESP
    }
}
...
MPMD_App::MPMD_App(int aArgc, char** aArgv, MPI_Comm& aLocalComm)
    : mDebugAnalyzeApp(false), mMachine(aLocalComm), mNumSpatialDims(0), mMesh(nullptr)
/******************************************************************************/
{
    // parse app file
    //
...
/******************************************************************************/
void MPMD_App::createProblem(ProblemDefinition& aDefinition)
/******************************************************************************/
{
...
void MPMD_App::loadESPInterface()
{
...
/******************************************************************************/
void MPMD_App::mapToParameters(std::shared_ptr<ESPType> aESP,
...
/******************************************************************************/
MPMD_App::LocalOp* MPMD_App::getOperation(const std::string& aOperationName)
/******************************************************************************/
{
    if (mDebugAnalyzeApp == true)
    {
...
/******************************************************************************/
void MPMD_App::compute(const std::string& aOperationName)
/******************************************************************************/
{
    if (mDebugAnalyzeApp == true)
    {
...
/******************************************************************************/
MPMD_App::LocalOp::LocalOp(MPMD_App* aMyApp, Plato::InputData& aOperationNode, Teuchos::RCP<ProblemDefinition> aOpDef)
    : mMyApp(aMyApp), mDef(aOpDef)
/******************************************************************************/
{
    // parse parameters
...
/******************************************************************************/
MPMD_App::CriterionOp::CriterionOp(MPMD_App* aMyApp, Plato::InputData& aOpNode)
/******************************************************************************/
{
...
/******************************************************************************/
MPMD_App::OnChangeOp::OnChangeOp(MPMD_App* aMyApp, Plato::InputData& aNode)
    : mStrParameters("Parameters"), mConditional(false)
/******************************************************************************/
{
...
/******************************************************************************/
MPMD_App::ESP_Op::ESP_Op(MPMD_App* aMyApp, Plato::InputData& aNode)
/******************************************************************************/
{
...
/******************************************************************************/
void MPMD_App::LocalOp::updateParameters(const std::string& aName, const Plato::Scalar aValue)
/******************************************************************************/
{
...
/******************************************************************************/
MPMD_App::ComputeCriterion::ComputeCriterion(MPMD_App* aMyApp,
...
/******************************************************************************/
MPMD_App::ComputeCriterionX::ComputeCriterionX(MPMD_App* aMyApp,
                                               Plato::InputData& aOpNode,
                                               Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
      CriterionOp(aMyApp, aOpNode),
...
/******************************************************************************/
{
    ...
    if (aMyApp->mCriterionGradientsX.count(mStrCriterion) == 0)
    {
        ...
        aMyApp->mCriterionGradientsX[mStrCriterion] =
            Plato::ScalarVector("gradient_x", tNumSpatialDims * tNumLocalVals);
    }
...
/******************************************************************************/
{
    ...
    if (!mOutputFile.empty())
    {
        ...
        for (int i = 0; i < mMyApp->mNumSpatialDims; i++)
        {
            ...
            // write to hdf5
            herr_t tErrCode;
            Plato::save_scalar_vector_to_hdf5_file(tFileId, "data" + std::to_string(i), tScalarFieldHostMirror,
                                                   tErrCode);
        }
...
/******************************************************************************/
MPMD_App::ComputeCriterionP::ComputeCriterionP(MPMD_App* aMyApp,
                                               Plato::InputData& aOpNode,
                                               Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
...
/******************************************************************************/
MPMD_App::ComputeCriterionValue::ComputeCriterionValue(MPMD_App* aMyApp,
                                                       Plato::InputData& aOpNode,
                                                       Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
      CriterionOp(aMyApp, aOpNode),
...
/******************************************************************************/
MPMD_App::ComputeCriterionGradient::ComputeCriterionGradient(MPMD_App* aMyApp,
                                                             Plato::InputData& aOpNode,
                                                             Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
      CriterionOp(aMyApp, aOpNode),
...
/******************************************************************************/
MPMD_App::ComputeCriterionGradientX::ComputeCriterionGradientX(MPMD_App* aMyApp,
                                                               Plato::InputData& aOpNode,
                                                               Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
      CriterionOp(aMyApp, aOpNode),
...
/******************************************************************************/
MPMD_App::MapCriterionGradientX::MapCriterionGradientX(MPMD_App* aMyApp,
                                                       Plato::InputData& aOpNode,
                                                       Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef), CriterionOp(aMyApp, aOpNode), mStrOutputName("Criterion Sensitivity")
{
...
/******************************************************************************/
MPMD_App::ComputeCriterionGradientP::ComputeCriterionGradientP(MPMD_App* aMyApp,
                                                               Plato::InputData& aOpNode,
                                                               Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
      ESP_Op(aMyApp, aOpNode),
...
/******************************************************************************/
MPMD_App::ComputeSolution::ComputeSolution(MPMD_App* aMyApp,
                                           Plato::InputData& aOpNode,
                                           Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef), mWriteNativeOutput(false), mVizFilePath("")
{
...
/******************************************************************************/
MPMD_App::Reinitialize::Reinitialize(MPMD_App* aMyApp,
                                     Plato::InputData& aOpNode,
                                     Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef), OnChangeOp(aMyApp, aOpNode)
{
}
/******************************************************************************/
...
/******************************************************************************/
MPMD_App::ReinitializeESP::ReinitializeESP(MPMD_App* aMyApp,
                                           Plato::InputData& aOpNode,
                                           Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef), ESP_Op(aMyApp, aOpNode), OnChangeOp(aMyApp, aOpNode)
{
}
/******************************************************************************/
...
/******************************************************************************/
MPMD_App::UpdateProblem::UpdateProblem(MPMD_App* aMyApp,
                                       Plato::InputData& aOpNode,
                                       Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef)
{
...
/******************************************************************************/
MPMD_App::WriteOutput::WriteOutput(MPMD_App* aMyApp, Plato::InputData& aOpNode, Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef)
{
}
/******************************************************************************/
...
/******************************************************************************/
MPMD_App::ComputeFiniteDifference::ComputeFiniteDifference(MPMD_App* aMyApp,
                                                           Plato::InputData& aOpNode,
                                                           Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
...
MPMD_App::ReloadMesh::ReloadMesh(MPMD_App* aMyApp, Plato::InputData& aNode, Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aNode, aOpDef), m_reloadMeshFile("")
/******************************************************************************/
{
...
/******************************************************************************/
void MPMD_App::OutputToHDF5::operator()()
/******************************************************************************/
{
    // create file
...
/******************************************************************************/
MPMD_App::Visualization::Visualization(MPMD_App* aMyApp,
...
void MPMD_App::Visualization::operator()()
{
    ...
    {
        ANALYZE_THROWERR(std::string("Visualization operation failed to open file with path '") + mVizDirectory +
                         "/steps.pvd" + "'.")
    }
    ...
    for (decltype(mOptimizationIterationCounter) tItr = 0; tItr <= mOptimizationIterationCounter; tItr++)
    {
        ...
        tOuptutFile << "<DataSet timestep="
                    << "\"" << std::to_string(tItr) << "\"  part=\"0\" file=\"" << tSubDirectory << "/steps/"
                    << tSolutionAtThisTimeStepDirectory << "/pieces.pvtu\"/>\n";
    }
    tOuptutFile << "</Collection>\n";
    tOuptutFile << "</VTKFile>";
    tOuptutFile.close();
...
#ifdef PLATO_HELMHOLTZ
/******************************************************************************/
MPMD_App::ApplyHelmholtz::ApplyHelmholtz(MPMD_App* aMyApp,
                                         Plato::InputData& aOpNode,
                                         Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef),
...
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "apply bounds", Kokkos::RangePolicy<>(0, tNumEntries), KOKKOS_LAMBDA(const Plato::OrdinalType& tIndex) {
...
/******************************************************************************/
MPMD_App::ApplyHelmholtzGradient::ApplyHelmholtzGradient(MPMD_App* aMyApp,
                                                         Plato::InputData& aOpNode,
                                                         Teuchos::RCP<ProblemDefinition> aOpDef)
    : LocalOp(aMyApp, aOpNode, aOpDef), mWriteNativeOutput(false), mVizFilePath("")
{
...
/******************************************************************************/
{
    ...
    {
        REPORT(
            "Analyze Application - Apply Helmholtz Gradient Operation - Print Partial of Filtered Controls wrt "
            "Unfiltered Controls.\n");
...
/******************************************************************************/
{
    ...
    {
        Plato::ParsingException tParsingException(
            "analyze_MPMD currently only supports SCALAR_FIELD and ELEMENT_FIELD data layout");
        throw tParsingException;
    }
}

/******************************************************************************/
void MPMD_App::getScalarFieldHostMirror(const std::string& aName, typename Plato::ScalarVector::HostMirror& aHostMirror)
/******************************************************************************/
{
    Plato::ScalarVector tDeviceData;
    ...
    {
        auto tStrCriterion = mGradientZNameToCriterionName[tFieldName];
        tDeviceData = Plato::get_vector_component(mCriterionGradientsX[tStrCriterion], /*component=*/tFieldIndex,
                                                  /*stride=*/mNumSpatialDims);
    }
...
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "get coordinates", Kokkos::RangePolicy<>(0, tNumVerts), KOKKOS_LAMBDA(const Plato::OrdinalType& tVertOrdinal) {
