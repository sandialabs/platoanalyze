...
{
[[nodiscard]]
Teuchos::ParameterList create_problem(const std::string_view aInputFileName)
{
  ...
  Plato::Comm::Machine tMachine = create_machine();
  return Plato::input_file_parsing(tArgc, tArgv.data(), tMachine);
}

[[nodiscard]]
std::string first_criterion_name(const Teuchos::ParameterList& aProblem)
{
...
[[nodiscard]]
std::string first_file_name(const std::vector<std::string>& aFileNames)
{
  if(aFileNames.empty())
  {
...
std::vector<double> AnalyzeCriterionInterface::gradient(const MeshProxy& aMeshProxy) const
{
  ...
  const std::string tCriterionName = first_criterion_name(mFunctionalInterface.parameterList());
  const Plato::ScalarVector tGradient = aMeshProxy.mNodalDensities.empty() ? 
  ...
  return to_std_vector(tGradient);
}
