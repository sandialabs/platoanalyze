    ...
    const Plato::Functional::FilterParameters& aFilterParameters, const std::string_view aMeshName)
{
  ...
  parameters_sublist(tParameterList).set("Surface Length Scale", aFilterParameters.mBoundaryStickingPenalty.value_or(-1.0));
...
std::vector<double> to_std_vector(const Plato::ScalarVector aScalarVector)
{
  ...
  std::copy(tScalarVectorOnHost.data(), tScalarVectorOnHost.data() + tScalarVectorOnHost.size(), std::back_inserter(tResult));
  return tResult;
}
