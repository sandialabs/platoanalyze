...
class PressureGradient : public ElementType
{
   private:
    ...
    Plato::Scalar mPressureScaling; /*!< Pressure scaling term used to improve condition number, which may be high due
                                       to poor scaling */
   ...
   public:
     ...
     **********************************************************************************/
    PressureGradient(Plato::Scalar aInput = 1.0) : mPressureScaling(aInput) {}
        ...
        Plato::Matrix<mNumNodesPerCell, mNumSpatialDims, GradientScalarType> const& aGradient) const
    {
        for (Plato::OrdinalType tDimIndex = 0; tDimIndex < mNumSpatialDims; tDimIndex++)
        {
            ...
            {
                aPressureGrad(tDimIndex) +=
                    mPressureScaling * aPressure(aCellOrdinal, tNodeIndex) * aGradient(tNodeIndex, tDimIndex);
            }
        }
    }
};
