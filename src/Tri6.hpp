...
class Tri6
{
   public:
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint)
    {
        ...
        return Plato::Array<mNumNodesPerCell>{1 - 3 * (x + y) + 2 * (x2 + y2) + 4 * xy,
                                              2 * x2 - x,
                                              2 * y2 - y,
                                              4 * (x - xy - x2),
                                              4 * xy,
    ...
    [[nodiscard]] constexpr KOKKOS_INLINE_FUNCTION static Plato::Matrix<mNumNodesPerCell, mNumSpatialDims> basisGrads(
        const Plato::Array<mNumSpatialDims>& aCubPoint)
    {
        const auto x = aCubPoint(0);
        const auto y = aCubPoint(1);
        ...
        return Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>{
            4 * (x + y) - 3,   4 * (x + y) - 3, 4 * x - 1, 0,     0,      4 * y - 1,
            4 - 8 * x - 4 * y, -4 * x,          4 * y,     4 * x, -4 * y, 4 - 4 * x - 8 * y};
    }

    template <typename ScalarType>
    [[nodiscard]] constexpr KOKKOS_INLINE_FUNCTION static ScalarType differentialMeasure(
    ...
    template <typename ScalarType>
    [[nodiscard]] constexpr KOKKOS_INLINE_FUNCTION static Plato::Array<mNumSpatialDims + 1, ScalarType>
    differentialVector(const Plato::Matrix<mNumSpatialDims, mNumSpatialDims + 1, ScalarType>& aJacobian)
    {
