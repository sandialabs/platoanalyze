...
void affirmParameterSize(const Teuchos::ParameterList& aSublist,
...
class UniformNaturalBCData : public NaturalBCData<NumDofs>
{
   public:
    ...
    explicit UniformNaturalBCData(const std::array<Scalar, kNumComponents> aValues) : mValues(std::move(aValues)) {}
    ...
    BCDataType getDataType() const override { return DataType; }
...
class TimeVaryingNaturalBCData : public NaturalBCData<NumDofs>
{
   public:
    ...
    explicit TimeVaryingNaturalBCData(const std::array<std::unique_ptr<Plato::MathExpr>, kNumComponents>& aExprs)
    {
...
class SpatiallyVaryingNaturalBCData : public NaturalBCData<NumDofs>
{
   public:
    ...
    explicit SpatiallyVaryingNaturalBCData(const Teuchos::ParameterList& aSublist)
    {
        ...
        detail::affirmParameterSize<kNumComponents, std::string>(aSublist, kVariableParameterName,
                                                                 kVariablesParameterName);
    ...
    NaturalBCVectorData<NumDofs> getVectorData(const Plato::Mesh& aMesh, Scalar /*aCurrentTime*/) const override
    {
        ...
        {
            Kokkos::deep_copy(Kokkos::subview(tOutData, Kokkos::ALL(), i),
                              detail::getNodalData(tReader, mVariableNames[i]));
        }
        return NaturalBCVectorData<NumDofs>{std::move(tOutData)};
    }
...
std::unique_ptr<NaturalBCData<NumDofs>> makeNaturalBCData(const Teuchos::ParameterList& aSublist)
{
    ...
    switch (naturalBoundaryCondition(aSublist.get<std::string>("Type")))
    {
        case Neumann::UNIFORM_LOAD:
            ...
            {
                ANALYZE_THROWERR(
                    R"(Expected "Values" field of type array of double or string in uniform natural boundary condition.)");
            }
            break;
        case Neumann::VARIABLE_LOAD:
            ...
            {
                ANALYZE_THROWERR(
                    R"(Expected "Variable" or "Variables" field of type string in variable load natural boundary condition.)");
            }
            break;
        case Neumann::UNIFORM_PRESSURE:
            ...
            {
                ANALYZE_THROWERR(
                    R"(Expected "Value" field of type double or string in uniform pressure natural boundary condition.)");
            }
            break;
        case Neumann::VARIABLE_PRESSURE:
            ...
            {
                ANALYZE_THROWERR(
                    R"(Expected "Variable" field of type string in variable pressure natural boundary condition.)");
            }
            break;
                         ...
                         const std::string& aParameterPlural)
{
    ...
    const std::string tBaseMessage =
        "Natural boundary condition degrees of freedom does not"
    ...
    if (aSublist.isType<T>(aParameterSingular) && ExpectedSize != 1)
    {
        const std::string tErrorMessage =
            tBaseMessage + aParameterSingular + ". Required: " + std::to_string(ExpectedSize) + ", received: 1";
        ANALYZE_THROWERR(tErrorMessage);
    }
    else if (aSublist.isType<ArrayType>(aParameterPlural) &&
             ExpectedSize != aSublist.get<ArrayType>(aParameterPlural).size())
    {
...
/// data types.
template <OrdinalType NumDofs>
KOKKOS_INLINE_FUNCTION Array<NumDofs> vectorBoundaryDataAtIndex(const NaturalBCVectorData<NumDofs>& aBoundaryData,
                                                                const OrdinalType aIndex)
{
