...
#include "material/ExpressionTMKinetics.hpp"
#include "material/LinearTMKinetics.hpp"
#include "material/NonLinearTMKinetics.hpp"
...
class TMKineticsFactory
{
   public:
        ...
        const Plato::DataMap& aDataMap)
    {
        ...
        {
            return Teuchos::rcp(
                new Plato::NonLinearTMKinetics<EvaluationType, ElementType>(aMaterialModel, aSpatialDomain, aDataMap));
        }
        else if (tModelType == Plato::MaterialModelType::Linear)
        {
            return Teuchos::rcp(
                new Plato::LinearTMKinetics<EvaluationType, ElementType>(aMaterialModel, aSpatialDomain, aDataMap));
        }
        else if (tModelType == Plato::MaterialModelType::Expression)
        {
            return Teuchos::rcp(
                new Plato::ExpressionTMKinetics<EvaluationType, ElementType>(aMaterialModel, aSpatialDomain, aDataMap));
        }
        else
        {
