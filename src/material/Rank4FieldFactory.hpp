...
class Rank4FieldFactory
{
   public:
    ...
    Rank4FieldFactory(const Teuchos::ParameterList& aParams) : mParams(aParams) {}

    template <typename EvaluationType>
    std::shared_ptr<Rank4Field<EvaluationType>> create() const
    {
        ...
        else
        {
            const std::string tErrMessage =
                "Attempted to create material with non-supported symmetry " + tSymmetry + "\n";
