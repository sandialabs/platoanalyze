...
class UniformMaterialBasis
{
   ...
   public:
    UniformMaterialBasis(Plato::Matrix<SpatialDim, SpatialDim> const& aBasis) : mBasis(aBasis) {}
     ...
     ******************************************************************************/
    template <typename T>
    void VoigtTensorToMaterialBasis(Plato::ScalarArray3DT<T> aVoigtTensor, Plato::Scalar aShearFactor = 1.0)
    {
        auto tNumCells = aVoigtTensor.extent(0);
        auto tNumPoints = aVoigtTensor.extent(1);
        auto& tBasis = mBasis;
        Kokkos::parallel_for(
            "to material basis", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumPoints}),
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                Plato::Matrix<SpatialDim, SpatialDim, T> tTensor =
                    Plato::FromVoigt<SpatialDim, T>(aVoigtTensor, iCellOrdinal, iGpOrdinal, aShearFactor);
    ...
    template <typename T>
    void VoigtTensorFromMaterialBasis(Plato::ScalarArray3DT<T> aVoigtTensor, Plato::Scalar aShearFactor = 1.0)
    {
        auto tNumCells = aVoigtTensor.extent(0);
        auto tNumPoints = aVoigtTensor.extent(1);
        auto& tBasis = mBasis;
        Kokkos::parallel_for(
            "from material basis", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumPoints}),
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                Plato::Matrix<SpatialDim, SpatialDim, T> tTensor =
                    Plato::FromVoigt<SpatialDim, T>(aVoigtTensor, iCellOrdinal, iGpOrdinal, aShearFactor);
     ...
     ******************************************************************************/
    template <typename T>
    void VectorToMaterialBasis(Plato::ScalarArray3DT<T> aVector)
    {
    ...
    template <typename T>
    void VectorFromMaterialBasis(Plato::ScalarArray3DT<T> aVector)
    {
        auto tNumCells = aVector.extent(0);
        auto tNumPoints = aVector.extent(1);
        auto& tBasis = mBasis;
...
class VaryingMaterialBasis
{
   ...
   public:
    VaryingMaterialBasis(Plato::ScalarArray3D aBasis) : mBasis(aBasis) {}
                ...
                for (int i = 0; i < SpatialDim; i++)
                {
                    for (int j = 0; j < SpatialDim; j++)
                    {
                        for (int k = 0; k < SpatialDim; k++)
                        {
                            for (int l = 0; l < SpatialDim; l++)
                            {
                                tToTensor(i, l) += tBasis(iCellOrdinal, j, i) * tTensor(j, k) *
                                                   tBasis(iCellOrdinal, k, l);  // S' = B^T S B
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < SpatialDim; i++)
                {
                    for (int j = 0; j < SpatialDim; j++)
                    {
                        for (int k = 0; k < SpatialDim; k++)
                        {
                            for (int l = 0; l < SpatialDim; l++)
                            {
                                tFromTensor(i, l) += tBasis(iCellOrdinal, i, j) * tTensor(j, k) *
                                                     tBasis(iCellOrdinal, l, k);  // S = B S' B^T
                            }
                        }
                    }
                }
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                ...
                for (int i = 0; i < SpatialDim; i++)
                {
                    ...
                    {
                        tInMatBasis(i) +=
                            tBasis(iCellOrdinal, j, i) * aVector(iCellOrdinal, iGpOrdinal, j);  // v' = B^T v
                    }
                }
                for (int i = 0; i < SpatialDim; i++)
                {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                ...
                for (int i = 0; i < SpatialDim; i++)
                {
                    ...
                    {
                        tInGlobalBasis(i) +=
                            tBasis(iCellOrdinal, i, j) * aVector(iCellOrdinal, iGpOrdinal, j);  // v = B v'
                    }
                }
                for (int i = 0; i < SpatialDim; i++)
                {
...
class UniformMaterialBasisFactory
{
   public:
        ...
        plato::domain::SpatialDomain const& aSpatialDomain)
    {
        ...
        else if (tHasMaterialBasis)
        {
            tUniformBasis = aMaterialModel->getCartesianBasis();
        }
        else if (tHasBlockBasis)
        {
