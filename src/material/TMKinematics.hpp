...
class TMKinematics : ElementType
{
   ...
   public:
    ...
    KOKKOS_INLINE_FUNCTION void operator()(Plato::OrdinalType aCellOrdinal,
                                           ...
                                           Plato::ScalarArray4DT<GradientScalarType> aGradient) const
    {
        // compute strain
        //
        Plato::OrdinalType tVoigtTerm = 0;
        ...
        for (Plato::OrdinalType tDofIndexJ = mNumSpatialDims - 1; tDofIndexJ >= 1; tDofIndexJ--)
        {
            for (Plato::OrdinalType tDofIndexI = tDofIndexJ - 1; tDofIndexI >= 0; tDofIndexI--)
            {
                for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < mNumNodesPerCell; tNodeIndex++)
                {
                    ...
                    Plato::OrdinalType tLocalOrdinalJ = tNodeIndex * mNumDofsPerNode + tDofIndexJ;
                    aStrain(aCellOrdinal, aGpOrdinal, tVoigtTerm) +=
                        (aState(aCellOrdinal, tLocalOrdinalJ) *
                             aGradient(aCellOrdinal, aGpOrdinal, tNodeIndex, tDofIndexI) +
                         aState(aCellOrdinal, tLocalOrdinalI) *
                             aGradient(aCellOrdinal, aGpOrdinal, tNodeIndex, tDofIndexJ));
                }
                tVoigtTerm++;
            }
        }
