...
#include "material/OrthotropicLinearElasticMaterial.hpp"
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<3>::checkOrthoMaterialStability(const Teuchos::ParameterList& aParamList)
{
    ...
    if (fabs(tPoissonRatioXY) > tSqrtYoungsModulusXOverYoungsModulusY)
    {
        ...
        tMsg << "OrthotropicLinearElasticMaterial 3D: Condition abs(Poisson's Ratio XY) < sqrt(Young's Modulus X / "
                "Young's Modulus Y) "
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 3D: Condition abs(Poisson's Ratio YZ) < sqrt(Young's Modulus Y / "
                "Young's Modulus Z) "
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 3D: Condition abs(Poisson's Ratio XZ) < sqrt(Young's Modulus X / "
                "Young's Modulus Z) "
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 3D: Condition abs(Poisson's Ratio YX) < sqrt(Young's Modulus Y / "
                "Young's Modulus X) "
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 3D: Condition abs(Poisson's Ratio ZY) < sqrt(Young's Modulus Z / "
                "Young's Modulus Y) "
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 3D: Condition abs(Poisson's Ratio ZX) < sqrt(Young's Modulus Z / "
                "Young's Modulus X) "
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<3>::checkOrthoMaterialInputs(const Teuchos::ParameterList& aParamList)
{
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<3>::setOrthoMaterialModel(const Teuchos::ParameterList& aParamList)
{
    ...
    auto tPoissonRatioZX = tPoissonRatioXZ * (tYoungsModulusZ / tYoungsModulusX);
    auto tPoissonRatioZY = tPoissonRatioYZ * (tYoungsModulusZ / tYoungsModulusY);
    auto tDeterminantComplianceMat =
        static_cast<Plato::Scalar>(1.0) - (tPoissonRatioXY * tPoissonRatioYX) - (tPoissonRatioYZ * tPoissonRatioZY) -
        (tPoissonRatioXZ * tPoissonRatioZX) -
        static_cast<Plato::Scalar>(2.0) * (tPoissonRatioYX * tPoissonRatioZY * tPoissonRatioXZ);
 ...
 **********************************************************************************/
template <>
Plato::OrthotropicLinearElasticMaterial<3>::OrthotropicLinearElasticMaterial(const Teuchos::ParameterList& aParamList)
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<3>::setMaterialModel(const Teuchos::ParameterList& aParamList)
{
    this->checkOrthoMaterialInputs(aParamList);
    this->checkOrthoMaterialStability(aParamList);
    this->setOrthoMaterialModel(aParamList);
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<2>::checkOrthoMaterialStability(const Teuchos::ParameterList& aParamList)
{
    // Stability Check 1: Positive material properties
    auto tYoungsModulusX = aParamList.get<Plato::Scalar>("Youngs Modulus X");
    if (tYoungsModulusX < static_cast<Plato::Scalar>(0))
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 2D: Condition abs(Poisson's Ratio XY) < sqrt(Young's Modulus X / "
                "Young's Modulus Y) "
             << "was not met.  The value of abs(Poisson's Ratio XY) = " << fabs(tPoissonRatioXY) << " and the value of "
             << "sqrt(Young's Modulus X / Young's Modulus Y) = " << tSqrtYoungsModulusXOverYoungsModulusY << ".";
        ANALYZE_THROWERR(tMsg.str().c_str())
    ...
    {
        std::stringstream tMsg;
        tMsg << "OrthotropicLinearElasticMaterial 2D: Condition abs(Poisson's Ratio YX) < sqrt(Young's Modulus Y / "
                "Young's Modulus X) "
             << "was not met.  The value of abs(Poisson's Ratio YX) = " << fabs(tPoissonRatioYX) << " and the value of "
             << "sqrt(Young's Modulus Y / Young's Modulus X) = " << tSqrtYoungsModulusYOverYoungsModulusX << ".";
        ANALYZE_THROWERR(tMsg.str().c_str())
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<2>::checkOrthoMaterialInputs(const Teuchos::ParameterList& aParamList)
{
    if (aParamList.isParameter("Youngs Modulus X") == false)
    {
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<2>::setOrthoMaterialModel(const Teuchos::ParameterList& aParamList)
{
    mCellDensity = aParamList.isParameter("Mass Density") ? aParamList.get<Plato::Scalar>("Mass Density") : 1.0;
    ...
    mCellStiffness(0, 1) =
        (tPoissonRatioXY * tYoungsModulusY) / (static_cast<Plato::Scalar>(1.0) - (tPoissonRatioXY * tPoissonRatioYX));
    mCellStiffness(1, 0) =
        (tPoissonRatioXY * tYoungsModulusY) / (static_cast<Plato::Scalar>(1.0) - (tPoissonRatioXY * tPoissonRatioYX));
 ...
 **********************************************************************************/
template <>
Plato::OrthotropicLinearElasticMaterial<2>::OrthotropicLinearElasticMaterial(const Teuchos::ParameterList& aParamList)
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<2>::setMaterialModel(const Teuchos::ParameterList& aParamList)
{
    this->checkOrthoMaterialInputs(aParamList);
    this->checkOrthoMaterialStability(aParamList);
    this->setOrthoMaterialModel(aParamList);
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<1>::checkOrthoMaterialStability(const Teuchos::ParameterList& aParamList)
{
    return;
}
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<1>::checkOrthoMaterialInputs(const Teuchos::ParameterList& aParamList)
{
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<1>::setOrthoMaterialModel(const Teuchos::ParameterList& aParamList)
{
    mCellDensity = aParamList.isParameter("Mass Density") ? aParamList.get<Plato::Scalar>("Mass Density") : 1.0;
    ...
    auto tStiffCoeff =
        tYoungsModulus / ((static_cast<Plato::Scalar>(1.0) + tPoissonRatio) *
 ...
 **********************************************************************************/
template <>
Plato::OrthotropicLinearElasticMaterial<1>::OrthotropicLinearElasticMaterial(const Teuchos::ParameterList& aParamList)
 ...
 **********************************************************************************/
template <>
void Plato::OrthotropicLinearElasticMaterial<1>::setMaterialModel(const Teuchos::ParameterList& aParamList)
{
    this->checkOrthoMaterialInputs(aParamList);
    this->checkOrthoMaterialStability(aParamList);
    this->setOrthoMaterialModel(aParamList);
