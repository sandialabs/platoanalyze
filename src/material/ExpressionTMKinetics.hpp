...
#include "material/AbstractTMKinetics.hpp"
...
class ExpressionTMKinetics : public Plato::AbstractTMKinetics<EvaluationType, ElementType>
{
   protected:
   ...
   public:
    ...
    void setLocalControl(const Plato::ScalarMultiVectorT<ControlScalarType>& aControl,
                         Plato::ScalarMultiVectorT<ControlScalarType>& aLocalControl) const
    {
        ...
        if (mControlValue != -1.0)
        {
            ...
            Kokkos::parallel_for(
                "Compute local control", Kokkos::RangePolicy<>(0, aControl.extent(0)),
                KOKKOS_LAMBDA(Plato::OrdinalType i) {
    ...
    void computeThermalStrainStressAndFlux(Plato::OrdinalType const& aNumCells,
                                           ...
                                           Plato::ScalarArray3DT<KinematicsScalarType> const& aTGrad) const
    {
        ...
        Kokkos::parallel_for(
            "compute element kinematics", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {aNumCells, tNumPoints}),
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                ...
                for (int iVoigt = 0; iVoigt < mNumVoigtTerms; iVoigt++)
                {
                    ...
                    for (int jVoigt = 0; jVoigt < mNumVoigtTerms; jVoigt++)
                    {
                        aStress(iCellOrdinal, iGpOrdinal, iVoigt) +=
                            (aStrain(iCellOrdinal, iGpOrdinal, jVoigt) - tstrain[jVoigt]) *
                            tStiffness(iCellOrdinal, iGpOrdinal, iVoigt, jVoigt);
                    }
                }
                ...
                for (int iDim = 0; iDim < EvaluationType::SpatialDim; iDim++)
                {
                    ...
                    {
                        aFlux(iCellOrdinal, iGpOrdinal, iDim) += tScaling2 * aTGrad(iCellOrdinal, iGpOrdinal, jDim) *
                                                                 tThermalConductivityConstant(iDim, jDim);
                    }
                }
            });
    }
     ...
     **********************************************************************************/
    void compute(Plato::ScalarArray3DT<KineticsScalarType> const& aStress,
                 Plato::ScalarArray3DT<KineticsScalarType> const& aFlux,
                 Plato::ScalarArray3DT<KinematicsScalarType> const& aStrain,
                 ...
                 Plato::ScalarMultiVectorT<ControlScalarType> const& aControl) const
    {
        ...
        Plato::ScalarMultiVectorT<ControlScalarType> tLocalControl("Local Control", aControl.extent(0),
                                                                   aControl.extent(1));
