...
/******************************************************************************/
{
   ...
   public:
    ...
    Plato::Matrix<SpatialDim, SpatialDim> getCartesianBasis() const { return mCartesianBasis; }
    ...
    Plato::Scalar getScalarConstant(std::string aConstantName) { return mScalarConstantsMap[aConstantName]; }
    ...
    // scalar functor
    Plato::ScalarFunctor getScalarFunctor(std::string aFunctorName) { return mScalarFunctorsMap[aFunctorName]; }
    ...
    */
    ...
    void parseScalar(std::string aName, const Teuchos::ParameterList& aParamList)
    {
        ...
        else if (aParamList.isSublist(aName))
        {
        ...
        {
            std::stringstream err;
            err << "Required input missing. '" << aName << "' must be provided as either a Parameter or ParameterList";
            ANALYZE_THROWERR(err.str());
        }
    }
    /******************************************************************************/
    ...
    */
    /******************************************************************************/
    void parseScalarConstant(std::string aName, const Teuchos::ParameterList& aParamList, Plato::Scalar aDefaultValue)
    {
        if (aParamList.isType<Plato::Scalar>(aName))
        {
            auto tValue = aParamList.get<Plato::Scalar>(aName);
    ...
    */
    ...
    void parseScalarConstant(std::string aName, const Teuchos::ParameterList& aParamList)
    {
        ...
        {
            std::stringstream err;
            err << "Required input missing. '" << aName << "' must be provided as a Parameter of type 'double'";
            ANALYZE_THROWERR(err.str());
        }
    }
    ...
    */
    ...
    void parseRank4Voigt(std::string aName, const Teuchos::ParameterList& aParamList)
    {
        ...
        {
            std::stringstream err;
            err << "Required input missing. '" << aName << "' must be provided as a ParameterList";
            ANALYZE_THROWERR(err.str());
        }
    }
    ...
    */
    /******************************************************************************/
    void parseRank4Field(const std::string& aListName,
    ...
    {
        if (mRank4VoigtConstantsMap.count(aConstantName) == 0) return false;
        return true;
    }

    bool hasRank4Field(const std::string& aFieldName)
    {
        if (mRank4FieldFactoryMap.count(aFieldName) == 0) return false;
        return true;
    }
    ...
    void setModelType()
    {
        ...
        else if (mRank4FieldFactoryMap.size() > 0)
        {
            ...
            {
                ANALYZE_THROWERR(
                    "MaterialModel has Expression rank4 tensors and Linear rank4 tensors. Only one kind should be "
                    "used.");
            }
            if (mRank4VoigtFunctorsMap.size() > 0)
            {
                ANALYZE_THROWERR(
                    "MaterialModel has Expression rank4 tensors and Nonlinear rank4 tensors. Only one kind should be "
                    "used.");
            }
        }
    }
