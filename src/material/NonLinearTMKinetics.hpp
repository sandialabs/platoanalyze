...
class NonLinearTMKinetics : public Plato::AbstractTMKinetics<EvaluationType, ElementType>
{
   protected:
                 ...
                 Plato::ScalarMultiVectorT<ControlScalarType> const& aControl) const
    {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const Plato::OrdinalType iCellOrdinal, const Plato::OrdinalType iGpOrdinal) {
                ...
                {
                    tstrain[iDim] = tScaling *
                                    tThermalExpansivityFunctor(tTemperature, tVoigtMap.I[iDim], tVoigtMap.J[iDim]) *
                ...
                for (int iDim = 0; iDim < EvaluationType::SpatialDim; iDim++)
                {
                    ...
                    for (int jDim = 0; jDim < EvaluationType::SpatialDim; jDim++)
                    {
                        aFlux(iCellOrdinal, iGpOrdinal, iDim) += tScaling2 * aTGrad(iCellOrdinal, iGpOrdinal, jDim) *
                                                                 tThermalConductivityFunctor(tTemperature, iDim, jDim);
                    }
                }
            });
    }
