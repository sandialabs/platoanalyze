...
class Rank4Field
{
   ...
   public:
    ...
    Rank4Field& operator=(const Rank4Field& aField) = delete;

    Rank4Field& operator=(Rank4Field&& aField) = delete;

    Plato::ScalarExpression<EvaluationType> getTensorProperty(const std::string& aName) const
    {
    ...
    virtual Plato::ScalarArray4DT<KineticsScalarType> operator()(
                                      ...
                                      Plato::ScalarVectorT<ControlScalarType>& aIndependentVariable)
    {
        ...
        Kokkos::parallel_for(
            "compute independent variable", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumPoints}),
