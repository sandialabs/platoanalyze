...
class TetragonalSkewRank4Field : public Rank4Field<EvaluationType>
{
   ...
   public:
    TetragonalSkewRank4Field(const Teuchos::ParameterList& aParams) : Rank4Field<EvaluationType>()
    {
        ...
        const Plato::ScalarMultiVectorT<ControlScalarType>& aLocalControl) override
    {
        ...
        Plato::ScalarArray4DT<KineticsScalarType> tStiffness("stiffness", tNumCells, tNumPoints, mNumSkwTerms,
                                                             mNumSkwTerms);
        Kokkos::parallel_for(
   ...
   private:
    static constexpr auto mNumSkwTerms =
        (ElementType::mNumSpatialDims == 3)
            ? 3
            : ((ElementType::mNumSpatialDims == 2) ? 1 : (((ElementType::mNumSpatialDims == 1) ? 1 : 0)));
};
