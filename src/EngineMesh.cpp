...
namespace Plato
{
EngineMesh::EngineMesh(std::string aInputMeshName)
...
EngineMesh::~EngineMesh() { closeMesh(); }

void EngineMesh::initialize()
{
    openMesh();

    auto tFaceGraph =
        mMeshIO->getFaceGraph(/*block id=*/0);  // EngineMesh requires all element blocks to have same element type
...
void EngineMesh::closeMesh() { mMeshIO = nullptr; }

void EngineMesh::openMesh()
{
...
void EngineMesh::loadConnectivity()
{
...
void EngineMesh::loadCoordinates()
{
...
void EngineMesh::createNodeElementGraph()
{
    auto tNumNodes = mMeshIO->getNumNodes();
    ...
    Kokkos::parallel_for(
        "count connections", Kokkos::RangePolicy<>(0, tNumElems), KOKKOS_LAMBDA(Plato::OrdinalType aElemOrdinal) {
            for (decltype(tNumNodesPerElement) tElemLocalNodeOrd = 0; tElemLocalNodeOrd < tNumNodesPerElement;
                 tElemLocalNodeOrd++)
            {
    ...
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<>(0, tNumNodes + 1),
    ...
    Kokkos::parallel_for(
        "element ordinals", Kokkos::RangePolicy<>(0, tNumElems), KOKKOS_LAMBDA(Plato::OrdinalType aElemOrdinal) {
            for (decltype(tNumNodesPerElement) tElemLocalNodeOrd = 0; tElemLocalNodeOrd < tNumNodesPerElement;
                 tElemLocalNodeOrd++)
            {
                auto tProcLocalNodeOrd = tConnectivity(aElemOrdinal * tNumNodesPerElement + tElemLocalNodeOrd);
...
void EngineMesh::createNodeNodeGraph()
{
    auto& tNodeElementGraph_offsets = mNodeElementGraph_offsets;
    auto& tNodeElementGraph_ordinals = mNodeElementGraph_ordinals;
    auto& tConnectivity = mConnectivity;
    ...
    Kokkos::parallel_for(
        "node ordinals", Kokkos::RangePolicy<>(0, tNumNodes), KOKKOS_LAMBDA(Plato::OrdinalType aNodeOrdinal) {
            ...
            for (decltype(tFrom) tEntryIndex = tFrom; tEntryIndex < tTo; tEntryIndex++)
            {
                ...
                for (decltype(tElemOrd) tElemLocalNodeOrd = 0; tElemLocalNodeOrd < tNumNodesPerElement;
                     tElemLocalNodeOrd++)
                {
...
void EngineMesh::createElementBlocks()
{
    auto tNumBlocks = mMeshIO->getNumElemBlks();
    auto tNumElems = mMeshIO->getNumElems();
    ...
    for (int tBlockIndex = 0; tBlockIndex < tNumBlocks; tBlockIndex++)
    {
        ...
        Kokkos::parallel_for(
            "element ordinals", Kokkos::RangePolicy<>(0, tNumElemInBlk),
            KOKKOS_LAMBDA(Plato::OrdinalType aElemOrdinal) {
...
void EngineMesh::loadNodeSets()
{
    ...
    for (decltype(tNumNodeSets) tNodeSetIndex = 0; tNodeSetIndex < tNumNodeSets; tNodeSetIndex++)
    {
        ...
        Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(tNodeOrdinals.data(),
                                                                                                tNumNodesThisSet);
...
void EngineMesh::loadSideSets()
{
    ...
    for (decltype(tNumSideSets) tSideSetIndex = 0; tSideSetIndex < tNumSideSets; tSideSetIndex++)
    {
        ...
        {
            Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(
                tElementOrdinals.data(), tNumSidesThisSet);
        ...
        {
            Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(
                tFaceOrdinals.data(), tNumSidesThisSet);
        ...
        {
            Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(
                tUniqueNodesVector.data(), tUniqueNodesVector.size());
        ...
        {
            Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(
                tFaceNodeOrdinals.data(), tNumFaceNodes);
        ...
        Kokkos::parallel_for(
            "local node numbers", Kokkos::RangePolicy<>(0, tNumSidesThisSet),
            KOKKOS_LAMBDA(Plato::OrdinalType aSideOrdinal) {
                ...
                for (decltype(tNumNodesPerFace) tNodeI = 0; tNodeI < tNumNodesPerFace; tNodeI++)
                {
                    ...
                    {
                        if (tNodeOrds(aSideOrdinal * tNumNodesPerFace + tNodeI) ==
                            tConnectivity(tElementOrd * tNumNodesPerElement + tNodeJ))
                        {
...
Plato::ScalarVectorT<const Plato::OrdinalType> EngineMesh::GetLocalElementIDs(std::string aBlockName) const
{
    return mBlockElementOrdinals.at(aBlockName);
}

std::vector<std::string> EngineMesh::GetElementBlockNames() const
{
...
std::vector<std::string> EngineMesh::GetNodeSetNames() const
{
    std::vector<std::string> tRetVal;
...
std::vector<std::string> EngineMesh::GetSideSetNames() const
{
    std::vector<std::string> tRetVal;
...
std::string EngineMesh::FileName() const { return mFileName; }

std::string EngineMesh::ElementType() const { return mElementType; }

Plato::OrdinalType EngineMesh::NumNodes() const { return mNumNodes; }

Plato::OrdinalType EngineMesh::NumNodesPerElement() const { return mNumNodesPerElement; }

Plato::OrdinalType EngineMesh::NumElements() const { return mNumElements; }

Plato::OrdinalType EngineMesh::NumDimensions() const { return mNumDimensions; }

Plato::ScalarVectorT<const Plato::Scalar> EngineMesh::Coordinates() const { return mCoordinates; }

void EngineMesh::SetCoordinates(Plato::ScalarVector aCoordinates)
{
...
Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::Connectivity() { return mConnectivity; }

void EngineMesh::NodeNodeGraph(Plato::OrdinalVectorT<Plato::OrdinalType>& aOffsetMap,
...
void EngineMesh::NodeElementGraph(Plato::OrdinalVectorT<const Plato::OrdinalType>& aOffsetMap,
...
Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetSideSetFaces(std::string aSideSetName) const
{
    return mSideSetFaceOrdinals.at(aSideSetName);
}

Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetSideSetElements(std::string aSideSetName) const
{
    return mSideSetElementOrdinals.at(aSideSetName);
}

/* element-local indexing (not global) */
Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetSideSetLocalNodes(std::string aSideSetName) const
{
    return mSideSetLocalNodeOrdinals.at(aSideSetName);
}

Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetSideSetElementsComplement(
    std::vector<std::string> aExcludeNames)
{
...
Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetSideSetFacesComplement(
    std::vector<std::string> aExcludeNames)
{
    createFullSurfaceSideSet();
...
/* element-local indexing (not global) */
Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetSideSetLocalNodesComplement(
    std::vector<std::string> aExcludeNames)
{
    createFullSurfaceSideSet();
...
void EngineMesh::createComplement(std::vector<std::string> aExcludeNames)
{
    ...
    Kokkos::parallel_for(
        "create indexable array", Kokkos::RangePolicy<>(0, tNumFaces), KOKKOS_LAMBDA(Plato::OrdinalType aFaceOrdinal) {
    ...
    for (const auto& tExcludeName : aExcludeNames)
    {
        ...
        auto tNumFaces = tFaces.size();
        Kokkos::parallel_for(
            "subtract", Kokkos::RangePolicy<>(0, tNumFaces), KOKKOS_LAMBDA(Plato::OrdinalType aFaceOrdinal) {
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, tNumTotalFaces),
    ...
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumTotalFaces),
        KOKKOS_LAMBDA(const Plato::OrdinalType& iOrdinal, Plato::OrdinalType& aUpdate, const bool& tIsFinal) {
    ...
    Kokkos::parallel_for(
        "local nodes", Kokkos::RangePolicy<>(0, tNumRemainingFaces), KOKKOS_LAMBDA(Plato::OrdinalType aFaceOrdinal) {
...
void EngineMesh::createFullSurfaceSideSet()
{
    ...
    {
        Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(
            tSideSetElements.data(), tSideSetElements.size());
    ...
    {
        Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(tSideSetFaces.data(),
                                                                                                tSideSetFaces.size());
    ...
    {
        Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(tSideSetNodes.data(),
                                                                                                tSideSetNodes.size());
...
Plato::OrdinalVectorT<const Plato::OrdinalType> EngineMesh::GetNodeSetNodes(std::string aNodeSetName) const
{
    return mNodeSetOrdinals.at(aNodeSetName);
}

void EngineMesh::CreateNodeSet(std::string aNodeSetName, std::initializer_list<Plato::OrdinalType> aNodes)
{
    ...
    Kokkos::View<Plato::OrdinalType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(tNodesHost.data(),
                                                                                            tNumNodes);
    auto tDeviceView = Kokkos::create_mirror_view(tDeviceData);
    Kokkos::deep_copy(tDeviceView, tHostView);
    Kokkos::deep_copy(tDeviceData, tDeviceView);
