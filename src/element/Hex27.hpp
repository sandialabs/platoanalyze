...
class Hex27
{
   public:
    ...
    [[nodiscard]] static constexpr Plato::Array<mNumGaussPoints> getCubWeights()
    {
        ...
        return Plato::Array<mNumGaussPoints>{wc, we, wc, we, wf, we, wc, we, wc, we, wf, we, wf, wb,
                                             wf, we, wf, we, wc, we, wc, we, wf, we, wc, we, wc};
    }
    ...
    [[nodiscard]] static constexpr Plato::Matrix<mNumGaussPoints, mNumSpatialDims> getCubPoints()
    {
        ...
        return Plato::Matrix<mNumGaussPoints, mNumSpatialDims>{
            -p, -p, -p, 0, -p, -p, p, -p, -p, -p, 0, -p, 0, 0, -p, p, 0, -p, -p, p, -p, 0, p, -p, p, p, -p,
            -p, -p, 0,  0, -p, 0,  p, -p, 0,  -p, 0, 0,  0, 0, 0,  p, 0, 0,  -p, p, 0,  0, p, 0,  p, p, 0,
            -p, -p, p,  0, -p, p,  p, -p, p,  -p, 0, p,  0, 0, p,  p, 0, p,  -p, p, p,  0, p, p,  p, p, p};
    }
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint) -> Plato::Array<mNumNodesPerCell>
    {
        ...
        return Plato::Array<mNumNodesPerCell>{-(1 - x) * (1 - y) * (1 - z) * xyz / 8.0,
                                              (1 + x) * (1 - y) * (1 - z) * xyz / 8.0,
                                              -(1 + x) * (1 + y) * (1 - z) * xyz / 8.0,
                                              (1 - x) * (1 + y) * (1 - z) * xyz / 8.0,
                                              (1 - x) * (1 - y) * (1 + z) * xyz / 8.0,
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint) -> Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>
    {
        ...
        return Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>{
            -(1 - 2 * x) * (1 - y) * (1 - z) * yz / 8.0,
            -(1 - x) * (1 - 2 * y) * (1 - z) * xz / 8.0,
            -(1 - x) * (1 - y) * (1 - 2 * z) * xy / 8.0,
            (1 + 2 * x) * (1 - y) * (1 - z) * yz / 8.0,
            (1 + x) * (1 - 2 * y) * (1 - z) * xz / 8.0,
            (1 + x) * (1 - y) * (1 - 2 * z) * xy / 8.0,
            -(1 + 2 * x) * (1 + y) * (1 - z) * yz / 8.0,
            ...
            -(1 - x) * (1 + y) * (1 + 2 * z) * xy / 8.0,
            -(1 - y) * (1 - z) * xyz / 2.0,
            (1 - x) * (1 + x) * (1 - 2 * y) * (1 - z) * z / 4.0,
            (1 - x) * (1 + x) * (1 - y) * (1 - 2 * z) * y / 4.0,
            -(1 + 2 * x) * (1 - y) * (1 + y) * (1 - z) * z / 4.0,
            (1 + x) * (1 - z) * xyz / 2.0,
            -(1 + x) * (1 - y) * (1 + y) * (1 - 2 * z) * x / 4.0,
            (1 + y) * (1 - z) * xyz / 2.0,
            -(1 - x) * (1 + x) * (1 + 2 * y) * (1 - z) * z / 4.0,
            -(1 - x) * (1 + x) * (1 + y) * (1 - 2 * z) * y / 4.0,
            (1 - 2 * x) * (1 - y) * (1 + y) * (1 - z) * z / 4.0,
            -(1 - x) * (1 - z) * xyz / 2.0,
            (1 - x) * (1 - y) * (1 + y) * (1 - 2 * z) * x / 4.0,
            (1 - 2 * x) * (1 - y) * (1 - z) * (1 + z) * y / 4.0,
            (1 - x) * (1 - 2 * y) * (1 - z) * (1 + z) * x / 4.0,
            -(1 - x) * (1 - y) * xyz / 2.0,
            -(1 + 2 * x) * (1 - y) * (1 - z) * (1 + z) * y / 4.0,
            -(1 + x) * (1 - 2 * y) * (1 - z) * (1 + z) * x / 4.0,
            (1 + x) * (1 - y) * xyz / 2.0,
            ...
            (1 - y) * (1 + y) * (1 - z) * xz,
            (1 - x) * (1 + x) * (1 - z) * yz,
            -(1 - x) * (1 + x) * (1 - y) * (1 + y) * (1 - 2 * z) / 2.0,
            -(1 - y) * (1 + y) * (1 + z) * xz,
            -(1 - x) * (1 + x) * (1 + z) * yz,
            (1 - x) * (1 + x) * (1 - y) * (1 + y) * (1 + 2 * z) / 2.0,
            -(1 - 2 * x) * (1 - y) * (1 + y) * (1 - z) * (1 + z) / 2.0,
            (1 - x) * (1 - z) * (1 + z) * xy,
            (1 - x) * (1 - y) * (1 + y) * xz,
            (1 + 2 * x) * (1 - y) * (1 + y) * (1 - z) * (1 + z) / 2.0,
            -(1 + x) * (1 - z) * (1 + z) * xy,
