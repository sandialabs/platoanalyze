...
class ElementBase
{
   public:
    ...
    KOKKOS_INLINE_FUNCTION static Plato::Matrix<mNumSpatialDims, mNumSpatialDims, ScalarType> jacobian(
    ...
    KOKKOS_INLINE_FUNCTION static Plato::Matrix<mNumSpatialDims, mNumSpatialDims, ScalarType> jacobian(
        const Plato::Array<mNumSpatialDims>& aCubPoint,
        Plato::Matrix<mNumNodesPerCell, mNumSpatialDims, ScalarType>& aNodeLocations)
    {
        Plato::Matrix<mNumSpatialDims, mNumSpatialDims, ScalarType> tJacobian;
        auto tBasisGrads = ElementType::basisGrads(aCubPoint);
        for (int i = 0; i < mNumSpatialDims; i++)
    ...
    template <typename ScalarType>
    KOKKOS_INLINE_FUNCTION static void computeGradientMatrix(
        const Plato::Array<mNumSpatialDims>& aCubPoint,
