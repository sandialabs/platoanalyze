...
class GeneralStressDivergence : public ElementType
{
   private:
    ...
    Plato::OrdinalType mVoigt[mNumSpatialDims]
                             [mNumSpatialDims]; /*!< matrix with indices to stress tensor entries in Voigt notation */
            ...
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < mNumNodesPerCell; tNodeIndex++)
            {
                ...
                for (Plato::OrdinalType tDimIndexJ = 0; tDimIndexJ < mNumSpatialDims; tDimIndexJ++)
                {
                    Kokkos::atomic_add(&aOutput(aCellOrdinal, tLocalOrdinal),
                                       aScale * aCellVolume * aStress(mVoigt[tDimIndexI][tDimIndexJ]) *
                                           aGradient(tNodeIndex, tDimIndexJ));
                }
            }
        }
    }
