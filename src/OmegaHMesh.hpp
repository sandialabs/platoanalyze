...
class OmegaHMesh : public AbstractMesh
{
   ...
   public:
    ...
    OmegaHMesh(Omega_h::Mesh& aMesh, Omega_h::MeshSets& aMeshSets) : mMesh(aMesh), mMeshSets(aMeshSets)
    {
        initialize();
    }
    ...
    Plato::ScalarVectorT<const Plato::OrdinalType> GetLocalElementIDs(std::string aBlockName) const override;
    ...
    Plato::ScalarVectorT<const Plato::Scalar> Coordinates() const override;

    void SetCoordinates(Plato::ScalarVector) override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> Connectivity() override;

    void NodeNodeGraph(Plato::OrdinalVectorT<const Plato::OrdinalType>& aOffsetMap,
                       Plato::OrdinalVectorT<const Plato::OrdinalType>& aNodeOrds) override;

    void NodeElementGraph(Plato::OrdinalVectorT<const Plato::OrdinalType>& aOffsetMap,
                          Plato::OrdinalVectorT<const Plato::OrdinalType>& aElementOrds) override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetSideSetFaces(std::string aSideSetName) const override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetSideSetElements(std::string aSideSetName) const override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetSideSetLocalNodes(std::string aSideSetName) const override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetSideSetElementsComplement(
        std::vector<std::string> aExcludeNames) override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetSideSetFacesComplement(
        std::vector<std::string> aExcludeNames) override;

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetSideSetLocalNodesComplement(
        std::vector<std::string> aExcludeNames) override;

    void createComplement(std::vector<std::string> aExcludeNames);

    Plato::OrdinalVectorT<const Plato::OrdinalType> GetNodeSetNodes(std::string aNodeSetName) const override;

    void CreateNodeSet(std::string aNodeSetName, std::initializer_list<Plato::OrdinalType> aNodes) override;
    ...
    void InvertSideSet(const Omega_h::LOs& aFaceLids,
                       ...
                       Plato::OrdinalVector& aSideSetLocalNodes)
    {
        ...
        Kokkos::parallel_for(
            "invert sideset", Kokkos::RangePolicy<>(0, tNumFaces), KOKKOS_LAMBDA(const Plato::OrdinalType& aFaceI) {
                auto tFaceOrdinal = aFaceLids[aFaceI];
                for (Plato::OrdinalType tElem = tFace2Elems_map[tFaceOrdinal];
                     tElem < tFace2Elems_map[tFaceOrdinal + 1]; ++tElem)
                {
                    ...
                    for (Plato::OrdinalType tNodeI = 0; tNodeI < cNodesPerFace; tNodeI++)
                    {
                        ...
                        {
                            if (tFace2Verts[tFaceOrdinal * cNodesPerFace + tNodeI] ==
                                tCell2Verts[tCellOrdinal * cNodesPerCell + tNodeJ])
                            {
