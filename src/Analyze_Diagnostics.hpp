 ...
 **********************************************************************************/
template <typename EvaluationType, typename ElementType>
inline void test_partial_global_state(Plato::Mesh aMesh,
                                      Plato::LocalVectorFunctionInc<ElementType>& aLocalVectorFuncInc)
{
    using StateT = typename EvaluationType::StateScalarType;
    ...
    Plato::ScalarArray3D tPartialU = aLocalVectorFuncInc.gradient_u(tGlobalState, tPrevGlobalState, tLocalState,
                                                                    tPrevLocalState, tControl, tTimeData);
    ...
    Kokkos::deep_copy(tStep, tHostStep);
    Plato::ScalarVector tGradientDotStep = Plato::global_workset_matrix_vector_multiply(
        tPartialU, tStep, tEntryOrdinal, tNumNodesPerCell, tTotalNumLocalDofs);
    ...
    for (Plato::OrdinalType tIndex = tSuperscriptLowerBound; tIndex <= tSuperscriptUpperBound; tIndex++)
    {
        ...
        Plato::ScalarVector tVectorValueOne = aLocalVectorFuncInc.value(
            tTrialGlobalState, tPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tGlobalState, 0.0, tTrialGlobalState);
        Plato::blas1::update(-tEpsilon, tStep, 1.0, tTrialGlobalState);
        Plato::ScalarVector tVectorValueTwo = aLocalVectorFuncInc.value(
            tTrialGlobalState, tPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tGlobalState, 0.0, tTrialGlobalState);
        Plato::blas1::update(2.0 * tEpsilon, tStep, 1.0, tTrialGlobalState);
        Plato::ScalarVector tVectorValueThree = aLocalVectorFuncInc.value(
            tTrialGlobalState, tPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tGlobalState, 0.0, tTrialGlobalState);
        Plato::blas1::update(-2.0 * tEpsilon, tStep, 1.0, tTrialGlobalState);
        Plato::ScalarVector tVectorValueFour = aLocalVectorFuncInc.value(
            tTrialGlobalState, tPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);

        Kokkos::parallel_for(
            "compute error", Kokkos::RangePolicy<>(0, tTotalNumDofs),
 ...
 **********************************************************************************/
template <typename EvaluationType, typename ElementType>
inline void test_partial_prev_global_state(Plato::Mesh aMesh,
                                           Plato::LocalVectorFunctionInc<ElementType>& aLocalVectorFuncInc)
{
    using StateT = typename EvaluationType::StateScalarType;
    using LocalStateT = typename EvaluationType::LocalStateScalarType;
    ...
    Teuchos::RCP<Teuchos::ParameterList> tInputs = Teuchos::getParametersFromXmlString(
        ...
        "</ParameterList>                                                            \n");
    Plato::TimeData tTimeData(*tInputs);
    Plato::ScalarArray3D tPartialUP = aLocalVectorFuncInc.gradient_up(tGlobalState, tPrevGlobalState, tLocalState,
                                                                      tPrevLocalState, tControl, tTimeData);
    ...
    Kokkos::deep_copy(tStep, tHostStep);
    Plato::ScalarVector tGradientDotStep = Plato::global_workset_matrix_vector_multiply(
        tPartialUP, tStep, tEntryOrdinal, tNumNodesPerCell, tTotalNumLocalDofs);
    ...
    for (Plato::OrdinalType tIndex = tSuperscriptLowerBound; tIndex <= tSuperscriptUpperBound; tIndex++)
    {
        ...
        Plato::ScalarVector tVectorValueOne = aLocalVectorFuncInc.value(
            tGlobalState, tTrialPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tPrevGlobalState, 0.0, tTrialPrevGlobalState);
        Plato::blas1::update(-tEpsilon, tStep, 1.0, tTrialPrevGlobalState);
        Plato::ScalarVector tVectorValueTwo = aLocalVectorFuncInc.value(
            tGlobalState, tTrialPrevGlobalState, tLocalState, tPrevLocalState, tControl, tTimeData);
 ...
 **********************************************************************************/
template <typename EvaluationType, typename ElementType>
inline void test_partial_local_state(Plato::Mesh aMesh, Plato::LocalVectorFunctionInc<ElementType>& aLocalVectorFuncInc)
{
    using StateT = typename EvaluationType::StateScalarType;
    using LocalStateT = typename EvaluationType::LocalStateScalarType;
    using ConfigT = typename EvaluationType::ConfigScalarType;
    ...
    Teuchos::RCP<Teuchos::ParameterList> tInputs = Teuchos::getParametersFromXmlString(
        ...
        "</ParameterList>                                                            \n");
    Plato::TimeData tTimeData(*tInputs);
    Plato::ScalarArray3D tPartialC = aLocalVectorFuncInc.gradient_c(tGlobalState, tPrevGlobalState, tLocalState,
                                                                    tPrevLocalState, tControl, tTimeData);
    ...
    for (Plato::OrdinalType tIndex = tSuperscriptLowerBound; tIndex <= tSuperscriptUpperBound; tIndex++)
    {
        ...
        Plato::ScalarVector tVectorValueOne = aLocalVectorFuncInc.value(
            tGlobalState, tPrevGlobalState, tTrialLocalState, tPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tLocalState, 0.0, tTrialLocalState);
        Plato::blas1::update(-tEpsilon, tStep, 1.0, tTrialLocalState);
        Plato::ScalarVector tVectorValueTwo = aLocalVectorFuncInc.value(
            tGlobalState, tPrevGlobalState, tTrialLocalState, tPrevLocalState, tControl, tTimeData);
        ...
        Kokkos::parallel_for(
            "compute error", Kokkos::RangePolicy<>(0, tTotalNumLocalDofs),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aDofOrdinal) {
                Plato::Scalar tValuePlus1Eps = tVectorValueOne(aDofOrdinal);
                Plato::Scalar tValueMinus1Eps = tVectorValueTwo(aDofOrdinal);
                Plato::Scalar tValuePlus2Eps = tVectorValueThree(aDofOrdinal);
 ...
 **********************************************************************************/
template <typename EvaluationType, typename ElementType>
inline void test_partial_prev_local_state(Plato::Mesh aMesh,
                                          Plato::LocalVectorFunctionInc<ElementType>& aLocalVectorFuncInc)
{
    using StateT = typename EvaluationType::StateScalarType;
    using LocalStateT = typename EvaluationType::LocalStateScalarType;
    ...
    Teuchos::RCP<Teuchos::ParameterList> tInputs = Teuchos::getParametersFromXmlString(
        ...
        "</ParameterList>                                                            \n");
    Plato::TimeData tTimeData(*tInputs);
    Plato::ScalarArray3D tPartialCP = aLocalVectorFuncInc.gradient_cp(tGlobalState, tPrevGlobalState, tLocalState,
                                                                      tPrevLocalState, tControl, tTimeData);
    ...
    for (Plato::OrdinalType tIndex = tSuperscriptLowerBound; tIndex <= tSuperscriptUpperBound; tIndex++)
    {
        ...
        Plato::ScalarVector tVectorValueOne = aLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState,
                                                                        tTrialPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tPrevLocalState, 0.0, tTrialPrevLocalState);
        Plato::blas1::update(-tEpsilon, tStep, 1.0, tTrialPrevLocalState);
        Plato::ScalarVector tVectorValueTwo = aLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState,
                                                                        tTrialPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tPrevLocalState, 0.0, tTrialPrevLocalState);
        Plato::blas1::update(2.0 * tEpsilon, tStep, 1.0, tTrialPrevLocalState);
        Plato::ScalarVector tVectorValueThree = aLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState,
                                                                          tTrialPrevLocalState, tControl, tTimeData);
        Plato::blas1::update(1.0, tPrevLocalState, 0.0, tTrialPrevLocalState);
        Plato::blas1::update(-2.0 * tEpsilon, tStep, 1.0, tTrialPrevLocalState);
        Plato::ScalarVector tVectorValueFour = aLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState,
                                                                         tTrialPrevLocalState, tControl, tTimeData);
 ...
 **********************************************************************************/
template <typename EvaluationType, typename ElementType>
inline void test_partial_local_vect_func_inc_wrt_control(
    Plato::Mesh aMesh, Plato::LocalVectorFunctionInc<ElementType>& aLocalVectorFuncInc)
{
    using StateT = typename EvaluationType::StateScalarType;
    using LocalStateT = typename EvaluationType::LocalStateScalarType;
    using ConfigT = typename EvaluationType::ConfigScalarType;
    ...
    Teuchos::RCP<Teuchos::ParameterList> tInputs = Teuchos::getParametersFromXmlString(
        ...
        "</ParameterList>                                                            \n");
    Plato::TimeData tTimeData(*tInputs);
    Plato::ScalarArray3D tPartialZ = aLocalVectorFuncInc.gradient_z(tGlobalState, tPrevGlobalState, tLocalState,
                                                                    tPrevLocalState, tControl, tTimeData);
    ...
    for (Plato::OrdinalType tIndex = tSuperscriptLowerBound; tIndex <= tSuperscriptUpperBound; tIndex++)
    {
        ...
        Plato::blas1::update(tEpsilon, tStep, 1.0, tTrialControl);
        Plato::ScalarVector tVectorValueOne = aLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState,
                                                                        tPrevLocalState, tTrialControl, tTimeData);
        Plato::blas1::update(1.0, tControl, 0.0, tTrialControl);
        Plato::blas1::update(-tEpsilon, tStep, 1.0, tTrialControl);
        Plato::ScalarVector tVectorValueTwo = aLocalVectorFuncInc.value(tGlobalState, tPrevGlobalState, tLocalState,
                                                                        tPrevLocalState, tTrialControl, tTimeData);
