 ...
 **********************************************************************************/
template <Plato::OrdinalType Length, typename ScalarT, typename AViewTypeT, typename BViewTypeT>
KOKKOS_INLINE_FUNCTION void update(const Plato::OrdinalType& aCellOrdinal,
 ...
 **********************************************************************************/
template <Plato::OrdinalType Length, typename ScalarT, typename ResultT>
KOKKOS_INLINE_FUNCTION void scale(const Plato::OrdinalType& aCellOrdinal,
 ...
 **********************************************************************************/
template <Plato::OrdinalType Length, typename AViewType, typename BViewType, typename CViewType>
KOKKOS_INLINE_FUNCTION void dot(const Plato::OrdinalType& aCellOrdinal,
...
inline void extract(const Plato::ScalarMultiVector& aFrom, Plato::ScalarMultiVector& aTo)
{
    ...
    for (Plato::OrdinalType tIndexI = 0; tIndexI < tDim0; tIndexI++)
    {
        ...
        Kokkos::parallel_for(
            "blas2::extract", Kokkos::RangePolicy<>(0, tLength), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
 ...
 *
 * \tparam  NumStride   stride, e.g. number of degree of freedom per node
 * \tparam  NumDim      number of dimensions to extract from an ordinal, e.g. number of displacement components at a
 *vertex \tparam  NumOffset   offset, e.g. degree of freedom offset (default = 0)
 *
...
inline void extract(const Plato::OrdinalType& aNumOrdinal,
                    const Plato::ScalarMultiVector& aFrom,
                    Plato::ScalarMultiVector& aTo)
{
    auto tDim0 = aFrom.extent(0);
    for (Plato::OrdinalType tIndexI = 0; tIndexI < tDim0; tIndexI++)
    {
        ...
        auto tFromSubView = Kokkos::subview(aFrom, tIndexI, Kokkos::ALL());
        Kokkos::parallel_for(
            "blas2::extract", Kokkos::RangePolicy<>(0, aNumOrdinal), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
...
inline void fill(typename XViewType::const_value_type& aAlpha, XViewType& aXvec)
{
    ...
    Kokkos::parallel_for(
        "blas2::fill", Kokkos::RangePolicy<>(0, tNumEntriesDim0),
...
inline void scale(typename XViewType::const_value_type& aAlpha, XViewType& aXvec)
{
    ...
    const Plato::OrdinalType tNumEntriesDim1 = aXvec.extent(1);
    Kokkos::parallel_for(
        "blas2::scale", Kokkos::RangePolicy<>(0, tNumEntriesDim0),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tIndex = 0; tIndex < tNumEntriesDim1; tIndex++)
            {
                   ...
                   const YViewType& aYvec)
{
    ...
    Kokkos::parallel_for(
        "blas2::update", Kokkos::RangePolicy<>(0, tNumEntriesDim0),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tIndex = 0; tIndex < tNumEntriesDim1; tIndex++)
            {
                aYvec(aCellOrdinal, tIndex) =
                    aAlpha * aXvec(aCellOrdinal, tIndex) + aBeta * aYvec(aCellOrdinal, tIndex);
            }
        });
}
// function update
...
inline void axpy(const Plato::Scalar& aAlpha, const Plato::ScalarMultiVector& aIn, Plato::ScalarMultiVector& aOut)
{
    ...
    {
        std::stringstream tMsg;
        tMsg << "\nDIMENSION MISMATCH. X ARRAY DIM(0) = " << aOut.extent(0) << " AND Y ARRAY DIM(0) = " << aIn.extent(0)
             << ".\n";
        ANALYZE_THROWERR(tMsg.str().c_str());
    }
    ...
    Kokkos::parallel_for(
        "blas2::axpy", Kokkos::RangePolicy<>(0, tInputVecDim0), KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tInputVecIndex = 0; tInputVecIndex < tInputVecDim1; tInputVecIndex++)
            {
                ...
                aOut(aCellOrdinal, tOutputVecIndex) =
                    aAlpha * aOut(aCellOrdinal, tOutputVecIndex) + aIn(aCellOrdinal, tInputVecIndex);
            }
        });
}
// function axpy
                                ...
                                YViewType& aYvec)
{
    ...
    bool tValidTransA = (aTransA[0] == 'N') || (aTransA[0] == 'n') || (aTransA[0] == 'T') || (aTransA[0] == 't');

    if (!tValidTransA)
    {
        std::stringstream tMsg;
        tMsg << "\ntransA[0] = '" << aTransA[0]
             << "'. Valid values include 'N' or 'n' (No transpose) and 'T' or 't' (Transpose).\n";
    ...
    {
        Kokkos::parallel_for(
            "matrix vector multiplication - no transpose", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
    ...
    {
        Kokkos::parallel_for(
            "matrix vector multiplication - transpose", Kokkos::RangePolicy<>(0, tNumCells),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                for (Plato::OrdinalType tColIndex = 0; tColIndex < tNumCols; tColIndex++)
                {
