...
namespace Plato
{
OmegaHMeshIO::OmegaHMeshIO(std::string aFilePath, Plato::OmegaHMesh& aMesh, std::string aMode)
{
    ...
    else if (tModeLower == "read" || tModeLower == "r")
    {
        mMode = Mode::Read;
        mPaths = Plato::omega_h::read_pvtu_file_paths(aFilePath);
        mMesh = nullptr;
    }
    else if (tModeLower == "append" || tModeLower == "a")
    {
...
void OmegaHMeshIO::AddNodeData(std::string aName, Plato::ScalarVector aData) { AddNodeData(aName, aData, 1); }
void OmegaHMeshIO::AddNodeData(std::string aName, Plato::ScalarVector aData, std::vector<std::string> aDofNames)
{
...
void OmegaHMeshIO::AddNodeData(std::string aName, Plato::ScalarVector aData, Plato::OrdinalType aNumDofs)
{
...
void OmegaHMeshIO::AddElementData(std::string aName, Plato::ScalarVector aData)
{
...
void OmegaHMeshIO::AddElementData(std::string aName, Plato::ScalarMultiVector aData)
{
...
void OmegaHMeshIO::Write(Plato::OrdinalType aStepIndex, Plato::Scalar aTimeValue)
{
    Omega_h::TagSet tTagsOmegaH = Omega_h::vtk::get_all_vtk_tags(mMesh, mMesh->dim());
    mWriter->write(aStepIndex, aTimeValue, tTagsOmegaH);
}

Plato::OrdinalType OmegaHMeshIO::NumTimeSteps() { return mPaths.size(); }

Plato::ScalarVector OmegaHMeshIO::ReadNodeData(const std::string& aVariableName, Plato::OrdinalType aStepIndex)
{
