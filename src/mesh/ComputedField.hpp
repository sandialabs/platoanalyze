...
/******************************************************************************/
{
   ...
   public:
    ...
    /**************************************************************************/
    void initialize(const Plato::Mesh aMesh, const std::string& aName, const std::string& aFunction)
    /**************************************************************************/
    {
        ...
        Kokkos::parallel_for(
            "fill coords", Kokkos::RangePolicy<>(0, tNumPoints), KOKKOS_LAMBDA(Plato::OrdinalType aPointOrdinal) {
    ...
    /******************************************************************************/
    {
        TEUCHOS_TEST_FOR_EXCEPTION(mValues.extent(0) * aStride != aValues.extent(0), std::logic_error,
                                   "Size mismatch in field initialization:  Mod(view, stride) != 0");
        auto tFromValues = mValues;
        auto tToValues = aValues;
        Kokkos::parallel_for(
            "copy", Kokkos::RangePolicy<>(0, tFromValues.extent(0)), KOKKOS_LAMBDA(Plato::OrdinalType aPointOrdinal) {
    ...
    */
    ...
    /****************************************************************************/
    {
        for (Teuchos::ParameterList::ConstIterator i = params.begin(); i != params.end(); ++i)
        {
            ...
            TEUCHOS_TEST_FOR_EXCEPTION(!entry.isList(), std::logic_error,
                                       "Parameter in Computed Fields block not valid.  Expect lists only.");
