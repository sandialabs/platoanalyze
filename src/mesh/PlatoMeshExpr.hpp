...
void getFunctionValues(const Plato::ScalarArray3DT<ScalarType>& aPoints,
...
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "fill coords", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumCells, tNumPoints}),
...
void mapPoints(const Plato::ScalarArray3DT<ConfigScalarType>& aConfig,
            ...
            for (Plato::OrdinalType iNode = 0; iNode < ElementType::mNumNodesPerCell; iNode++)
            {
                for (Plato::OrdinalType iDim = 0; iDim < ElementType::mNumSpatialDims; iDim++)
                {
                    aMappedPoints(iCellOrdinal, iGpOrdinal, iDim) +=
                        tBasisValues(iNode) * aConfig(iCellOrdinal, iNode, iDim);
                }
            }
        });
}
        ...
        Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(Plato::OrdinalType aCellOrdinal) {
            ...
            for (Plato::OrdinalType ptOrdinal = 0; ptOrdinal < tNumPoints; ptOrdinal++)
            {
                ...
                for (tNodeOrdinal = 0; tNodeOrdinal < ElementType::mNumSpatialDims; tNodeOrdinal++)
                {
                    ...
                    {
                        aMappedPoints(aCellOrdinal, ptOrdinal, d) +=
                            tNodeValue * tNodeCoordinate(tCellOrdinal, tNodeOrdinal, d);
                    }
                }
                ...
                {
                    aMappedPoints(aCellOrdinal, ptOrdinal, d) +=
                        tFinalNodeValue * tNodeCoordinate(tCellOrdinal, tNodeOrdinal, d);
                }
            }
        });
}
        ...
        Kokkos::RangePolicy<>(0, tNumCells), KOKKOS_LAMBDA(Plato::OrdinalType aCellOrdinal) {
            for (Plato::OrdinalType ptOrdinal = 0; ptOrdinal < tNumPoints; ptOrdinal++)
            {
                ...
                for (tNodeOrdinal = 0; tNodeOrdinal < ElementType::mNumSpatialDims; tNodeOrdinal++)
                {
                    ...
                    {
                        aMappedPoints(aCellOrdinal, ptOrdinal, d) +=
                            tNodeValue * tNodeCoordinate(aCellOrdinal, tNodeOrdinal, d);
                    }
                }
                tNodeOrdinal = ElementType::mNumSpatialDims;
                for (Plato::OrdinalType d = 0; d < ElementType::mNumSpatialDims; d++)
                {
                    aMappedPoints(aCellOrdinal, ptOrdinal, d) +=
                        tFinalNodeValue * tNodeCoordinate(aCellOrdinal, tNodeOrdinal, d);
                }
            }
        });
}
                                                             ...
                                                             const std::string& aFunction)
{
    ...
    Plato::ScalarArray3DT<ConfigScalarType> tPhysicalPoints("physical points", tNumCells, tNumPoints,
