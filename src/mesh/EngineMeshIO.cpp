...
#include "domain/AnalyzeAppUtils.hpp"
...
EngineMeshIO::EngineMeshIO(std::string aName, Plato::EngineMesh& aMesh, std::string aMode)
    : mMesh(aMesh), mVariablesAreSet(false), mPlotIndex(0)
{
...
void EngineMeshIO::parseNodeVarNames()
{
    ...
    for (auto tIter = tBaseToExt.begin(), end = tBaseToExt.end(); tIter != end;
         tIter = tBaseToExt.upper_bound(tIter->first))
    {
...
void EngineMeshIO::setVariableTypeSuffixes()
{
    auto tNumSpaceDims = mMesh.NumDimensions();
...
void EngineMeshIO::AddNodeData(std::string aName, Plato::ScalarVector aData) { AddNodeData(aName, aData, {aName}); }
void EngineMeshIO::AddNodeData(std::string aName, Plato::ScalarVector aData, Plato::OrdinalType aNumDofs)
{
    ...
    else if (aNumDofs == tNumDims)  // vector
    {
        StrList tDofNames;
...
void EngineMeshIO::AddNodeData(std::string aName, Plato::ScalarVector aData, StrList aDofNames)
{
...
void EngineMeshIO::AddElementData(std::string aName, Plato::ScalarVector aData)
{
...
void EngineMeshIO::AddElementData(std::string aName, Plato::ScalarMultiVector aData)
{
    auto tNumElem = aData.extent(0);
    ...
    else if (tNumDofs == mVectorSuffixes.size())
    {
        tSuffixes = mVectorSuffixes;
    }
    else if (tNumDofs == mSymtensorSuffixes.size())
    {
...
void EngineMeshIO::extractColumn(const Plato::ScalarMultiVector& aFromVector,
                                 ...
                                 const Plato::ScalarVector& aToVector)
{
    ...
    Kokkos::parallel_for(
        "extract column", Kokkos::RangePolicy<>(0, tNumRows),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { aToVector(aOrdinal) = aFromVector(aOrdinal, aColIndex); });
}

void EngineMeshIO::AddElementData(std::string aName, Plato::ScalarMultiVector aData, StrList aNames)
{
...
void EngineMeshIO::Write(Plato::OrdinalType aStepIndex, Plato::Scalar aTimeValue)
{
    ...
    for (auto& tNodeVarPair : mNodeVars)
    {
        ...
        if (tNodeVar.isStale)
        {
            ...
            tMsg << "File output (step " << aStepIndex << ") -- Variable '" << tNodeVarPair.first
                 << "' not updated. Using previous step.";
            REPORT(tMsg.str());
        }
    ...
    for (auto& tElementVarPair : mElementVars)
    {
        ...
        {
            std::stringstream tMsg;
            tMsg << "File output (step " << aStepIndex << ") -- Variable " << tElementVarPair.first
                 << " not updated. using previous step.";
            REPORT(tMsg.str());
        }
...
Plato::OrdinalType EngineMeshIO::NumTimeSteps() { return mMeshIO->getNumSteps(); }

void EngineMeshIO::readNodeScalar(const std::string& aVariableName,
...
Plato::ScalarVector EngineMeshIO::ReadNodeData(const std::string& aVariableName, Plato::OrdinalType aStepIndex)
{
    auto tNumNodes = mMesh.NumNodes();
