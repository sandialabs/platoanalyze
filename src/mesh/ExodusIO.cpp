...
std::vector<std::vector<Int>> ExodusIO::getFaceGraph(Int aBlockIndex) const
{
    return getFaceGraph(mElementBlocks[aBlockIndex].elementType);
}

std::vector<std::vector<Int>> ExodusIO::getFaceGraph(std::string aElemType) const
{
    ...
    else if (tElemType == "tri3")
    {
        return {{0, 1}, {1, 2}, {2, 0}};
    }
    else if (tElemType == "tri" || tElemType == "tri3")
    {
        return {{0, 1}, {1, 2}, {2, 0}};
    }
    else if (tElemType == "quad" || tElemType == "quad4")
    {
        return {{0, 1}, {1, 2}, {2, 3}, {3, 0}};
    }
    else if (tElemType == "quad8" || tElemType == "quad9")
    {
        return {{0, 1, 4}, {1, 2, 5}, {2, 3, 6}, {3, 0, 7}};
    }
    else if (tElemType == "tetra" || tElemType == "tetra4" || tElemType == "tet4" || tElemType == "tet")
    {
        return {{0, 1, 3}, {1, 2, 3}, {2, 0, 3}, {0, 2, 1}};
    }
    else if (tElemType == "tetra10" || tElemType == "tet10")
    {
        return {{0, 1, 3, 4, 8, 7}, {1, 2, 3, 5, 9, 8}, {2, 0, 3, 6, 7, 9}, {0, 2, 1, 6, 5, 4}};
    }
    else if (tElemType == "hex" || tElemType == "hex8")
    {
        return {{0, 1, 5, 4}, {1, 2, 6, 5}, {2, 3, 7, 6}, {3, 0, 4, 7}, {3, 2, 1, 0}, {4, 5, 6, 7}};
    }
    else if (tElemType == "hex20" || tElemType == "hex21")
    {
        return {{0, 1, 5, 4, 8, 13, 16, 12},  {1, 2, 6, 5, 9, 14, 17, 13}, {2, 3, 7, 6, 10, 15, 18, 14},
                {3, 0, 4, 7, 11, 12, 19, 15}, {3, 2, 1, 0, 10, 9, 8, 11},  {4, 5, 6, 7, 16, 17, 18, 19}};
    }
    else if (tElemType == "hex27")
    {
        return {{0, 1, 5, 4, 8, 13, 16, 12, 25},  {1, 2, 6, 5, 9, 14, 17, 13, 24}, {2, 3, 7, 6, 10, 15, 18, 14, 26},
                {3, 0, 4, 7, 11, 12, 19, 15, 23}, {3, 2, 1, 0, 10, 9, 8, 11, 21},  {4, 5, 6, 7, 16, 17, 18, 19, 22}};
    }
    else
    {
...
void ExodusIO::readMesh(const std::string& aFileName, bool aIgnoreNodeMap, bool aIgnoreElemMap)
{
...
void ExodusIO::closeMesh()
{
    if (mFileID != -1)
    {
        ...
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Error closing exodus file. ex_close() failed.");
        }
...
void ExodusIO::readData()
{
    ...
    tErrorStatus =
        ex_get_init(mFileID, tTitle, &mNumDims, &mNumNodes, &mNumElems, &tNumBlocks, &tNumNodeSets, &tNumSideSets);
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to read exodus file. ex_get_init() failed.");
    }
...
void ExodusIO::readCoords()
{
    ...
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to read exodus file. ex_get_coord() failed.");
    }
}

void ExodusIO::readElemIds()
{
    if (mIgnoreElemMap)
    {
...
void ExodusIO::readElementBlocks()
{
    Int tErrorStatus;
    ...
    if (tNumElementBlocks)
    {
        ...
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to read exodus file. ex_get_ids() failed.");
        }

        char** tBlockNames = new char*[tNumElementBlocks];
        for (Int i = 0; i < tNumElementBlocks; i++) tBlockNames[i] = new char[256];

        tErrorStatus = ex_get_names(mFileID, EX_ELEM_BLOCK, tBlockNames);
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to read exodus file. ex_get_names() failed.");
        }
        ...
        for (Int i = 0; i < tNumElementBlocks; i++)
        {
            ...
            tErrorStatus = ex_get_block(mFileID, EX_ELEM_BLOCK, tBlockIds[i], tElemType, &tNumElemsInBlock,
                                        &tNumNodesPerElem, &tNumEdgesPerElem, &tNumFacesPerElem, &tNumAttr);
            if (tErrorStatus)
            {
                ANALYZE_THROWERR("Unable to read exodus file. ex_get_block() failed.");
            }
            ...
            if (tErrorStatus)
            {
                ANALYZE_THROWERR("Unable to read exodus file. ex_get_conn() failed.");
            }

            // from one-based to zero-based indices
            for (Int iConn = 0; iConn < tNumConn; iConn++) tConnect[iConn] = tConnect[iConn] - 1;
        }

        for (Int i = 0; i < tNumElementBlocks; i++) delete[] tBlockNames[i];
        delete[] tBlockNames;
    }
}

void ExodusIO::readSideSets()
{
    Int tErrorStatus;
    ...
    if (tNumSideSets)
    {
        ...
        for (Int i = 0; i < tNumSideSets; i++) tSideSetNames[i] = new char[256];
        ...
        for (Int i = 0; i < tNumSideSets; i++)
        {
            ...
            if (tErrorStatus)
            {
                ANALYZE_THROWERR("Unable to read exodus file. ex_get_set_param() failed.");
            }
            ...
            if (tErrorStatus)
            {
                ANALYZE_THROWERR("Unable to read exodus file. ex_get_side_set_node_list_len() failed.");
            }
            ...
            if (tNumSidesInSet)
            {
                ...
                if (tErrorStatus)
                {
                    ANALYZE_THROWERR("Unable to read exodus file. ex_get_set() failed.");
                }
                ...
                if (tErrorStatus)
                {
                    ANALYZE_THROWERR("Unable to read exodus file. ex_get_side_set_node_list() failed.");
                }
        ...
        for (Int i = 0; i < tNumSideSets; i++) delete[] tSideSetNames[i];
        delete[] tSideSetNames;
    }
}

void ExodusIO::readNodeSets()
{
    ...
    if (tNumNodeSets)
    {
        ...
        for (Int i = 0; i < tNumNodeSets; i++) tNodeSetNames[i] = new char[256];
        ...
        for (Int i = 0; i < tNumNodeSets; i++) delete[] tNodeSetNames[i];
        delete[] tNodeSetNames;
    }
}

void ExodusIO::readNodeIds()
{
    if (mIgnoreNodeMap)
    {
...
void ExodusIO::openMesh(std::string aFileName, const std::string& aMode)
{
    closeMesh();
    ...
    if (tMode == "write" || tMode == "w" || tMode == "append" || tMode == "a")
    {
        writeData();
    }
}

void ExodusIO::writeData()
{
    Int tErrorStatus;

    char tTitle[24] = "Analyze output";
    tErrorStatus = ex_put_init(mFileID, tTitle, mNumDims, mNumNodes, mNumElems, mElementBlocks.size(), mNodeSets.size(),
                               mSideSets.size());
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write exodus file. ex_put_init() failed.");
    }

    tErrorStatus = ex_put_id_map(mFileID, EX_NODE_MAP, mNodeGlobalIds.data());
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write exodus file. ex_put_id_map() failed.");
    }
    ...
    {
        tErrorStatus =
            ex_put_set_param(mFileID, EX_NODE_SET, tNodeSet.ID, tNodeSet.nodes.size(), /*num_dist_in_set=*/0);
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write exodus file. ex_put_set_param() failed.");
        }
        ...
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write exodus file. ex_put_set() failed.");
        }

        tErrorStatus = ex_put_name(mFileID, EX_NODE_SET, tNodeSet.ID, tNodeSet.name.c_str());
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write exodus file. ex_put_name() failed.");
        }
    }
    ...
    {
        tErrorStatus =
            ex_put_set_param(mFileID, EX_SIDE_SET, tSideSet.ID, tSideSet.elems.size(), /*num_dist_in_set=*/0);
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write exodus file. ex_put_set_param() failed.");
        }
    ...
    {
        tErrorStatus = ex_put_block(mFileID, EX_ELEM_BLOCK, tBlock.ID, tBlock.elementType.c_str(), tBlock.numElems,
                                    tBlock.numNPE, 0, 0, 0);
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write exodus file. ex_put_block() failed.");
        }
        ...
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write exodus file. ex_put_conn() failed.");
        }
    }
    ...
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write exodus file. ex_put_map_param() failed.");
    }
    ...
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write exodus file. ex_put_coords() failed.");
    }
}
...
const std::vector<Int>& ExodusIO::getElemToNodeConnInBlk(Int blk) const { return mElementBlocks[blk].connectivity; }

const std::vector<std::vector<Real>>& ExodusIO::getCoords() const { return mCoords; }

std::string ExodusIO::getElemTypeInBlk(Int blk) const { return mElementBlocks[blk].elementType; }
std::string ExodusIO::getBlockName(Int blk) const { return mElementBlocks[blk].name; }
...
const std::vector<Int>& ExodusIO::getNodeSetNodes(Int i) const { return mNodeSets[i].nodes; }
...
const std::vector<Int>& ExodusIO::getSideSetFaces(Int i) const { return mSideSets[i].sides; }
const std::vector<Int>& ExodusIO::getSideSetElems(Int i) const { return mSideSets[i].elems; }
const std::vector<Int>& ExodusIO::getSideSetNodes(Int i) const { return mSideSets[i].nodes; }

/******************************************************************************/
/**
 * \brief Get nodal variable names from the exodus database
 **********************************************************************************/
std::vector<std::string> ExodusIO::getNodeVarNames() const
{
    ...
    for (Int i = 0; i < tNumNodeVars; i++) tNames[i] = new char[MAX_STR_LENGTH + 1];
 ...
 **********************************************************************************/
void ExodusIO::writeTime(Int aTimeStep, Real aTimeValue) const
{
    auto tOneBasedTimeStep = aTimeStep + 1;
    Int tErrorStatus = ex_put_time(mFileID, tOneBasedTimeStep, &aTimeValue);
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write data. ex_put_time() failed.");
    }
}

void ExodusIO::initVars(std::string aCentering, Int aNumVars, std::vector<std::string> aNames) const
{
    Int tErrorStatus;
    ...
    if (aCentering == "node")
    {
        ...
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write data. ex_put_variable_param() failed.");
        }

        tErrorStatus = ex_put_variable_names(mFileID, EX_NODAL, aNumVars, tNames);
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write data. ex_put_variable_names() failed.");
        }
    }
    else if (aCentering == "element")
    {
 ...
 **********************************************************************************/
void ExodusIO::writeNodePlot(Real* aData, Int aVariableIndex, Int aStepIndex) const
{
    Int tErrorStatus;
    ...
    tErrorStatus =
        ex_put_var(mFileID, tOneBasedStepIndex, EX_NODAL, tOneBasedVariableIndex, /*obj_id=*/1, mNumNodes, aData);
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write data. ex_put_var() failed.");
    }

    tErrorStatus = ex_update(mFileID);
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to write data. ex_update() failed.");
    }
}
 ...
 * \param [in] aStepIndex zero-based index into time step array
 **********************************************************************************/
void ExodusIO::writeElemPlot(Real* aData, Int aVariableIndex, Int aStepIndex) const
{
    Int tErrorStatus;
    ...
    for (auto& tBlock : mElementBlocks)
    {
        ...
        tErrorStatus = ex_put_var(mFileID, tOneBasedStepIndex, EX_ELEM_BLOCK, tOneBasedVariableIndex, tBlock.ID,
                                  tBlock.numElems, tOffsetData);
        if (tErrorStatus)
        {
            ANALYZE_THROWERR("Unable to write data. ex_put_var() failed.");
        }
 ...
 **********************************************************************************/
Int ExodusIO::getNumSteps() const { return ex_inquire_int(mFileID, EX_INQ_TIME); }
 ...
 * \param [in] aStepIndex zero-based index into time step array
 **********************************************************************************/
void ExodusIO::readNodePlot(double* aData, std::string aVariableName, Int aStepIndex) const
{
    Int tErrorStatus;
    ...
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to read data. ex_inquire() failed.");
    }
    ...
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to read data. ex_get_variable_names() failed.");
    }
    ...
    if (tErrorStatus)
    {
        ANALYZE_THROWERR("Unable to read data. ex_get_var() failed.");
    }
}

const std::vector<Int>& ExodusIO::getNodeGlobalIds() const { return mNodeGlobalIds; }
