...
#include "boundary_conditions/NaturalBCData.hpp"
#include "domain/SpatialModel.hpp"
#include "domain/WeightedNormalVector.hpp"
...
SurfacePressureIntegral<ElementType, NumDofs, DofsPerNode, DofOffset>::SurfacePressureIntegral(
    const std::string aSidesetName, const Plato::Scalar aCurrentTime, std::unique_ptr<NaturalBCData<NumDofs>> aBCData)
    : mSidesetName(aSidesetName), mCurrentTime(aCurrentTime), mBCData(std::move(aBCData))
{
    assert(mBCData);
}
    ...
    Plato::Scalar aScale) const
{
    ...
    Kokkos::parallel_for(
        "surface pressure integral", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {tNumFaces, tNumPoints}),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aSideOrdinal, const Plato::OrdinalType& aPointOrdinal) {
            ...
            {
                const auto tGlobalNodeOrdinal =
                    tConnectivity(tElementOrdinal * ElementType::mNumNodesPerCell + tLocalNodeOrds(tNode));
                ...
                for (Plato::OrdinalType tDof = 0; tDof < NumDofs; tDof++)
                {
                    ...
                    const ResultScalarType tVal = tWeightedNormalVec(tDof) * tPressure * aScale * tCubatureWeight *
                                                  tNormalMultiplier * tBasisValues(tNode);
