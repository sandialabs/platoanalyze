...
void affirmOneValidInput(const std::string &aName, Teuchos::ParameterList &aSubList)
{
    ...
    if (std::count(std::begin(tAllOptions), std::end(tAllOptions), true) > 1)
    {
        ...
        tMsg << "Natural Boundary Condition: Expected one of 'Values', 'Value', 'Variable', or 'Variables' Parameter "
                "Keyword in "
    ...
    {
        std::stringstream tMsg;
        tMsg << "Natural Boundary Condition: Load Boundary Condition in Parameter Sublist: '" << aName.c_str()
