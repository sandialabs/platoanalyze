...
namespace
{
const EnumTable<Neumann> kNaturalBCTable({{Neumann::UNIFORM_LOAD, "uniform"},
...
Plato::Neumann naturalBoundaryCondition(const std::string& aTypeTag)
{
    ...
    if (!tType)
    {
        ANALYZE_THROWERR(std::string("Natural Boundary Condition: 'Type' Parameter Keyword: '") + tLowerTag +
                         "' is not supported.")
    }
    return *tType;
}
