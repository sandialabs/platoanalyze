...
/******************************************************************************/
{
   public:
     ...
     */
    EssentialBCs(Teuchos::ParameterList& aParams, const Plato::Mesh aMesh) : mMesh(aMesh), mBCs()
    {
    ...
    void constructEssentialBC(Teuchos::ParameterList& aEssentialBCParams,
                              ...
                              Plato::Scalar aScaleFactor = 1.0)
    {
        ...
        if ("Zero Value" == tType)
        {
            ...
            TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                                       " Boundary Condition type invalid: Not 'Zero Value' or 'Fixed Value'.");
        mBCs.push_back(tMyBC);
    }

    Plato::Scalar findScaleFactor(Teuchos::ParameterList& aEssentialBCParams,
                                  const std::map<Plato::OrdinalType, Plato::Scalar>& aDofOffsetToScaleFactor)
    {
        ...
        if (tSearch != aDofOffsetToScaleFactor.end()) return tSearch->second;
        return 1.0;
    }
     ...
     */
    void get(Plato::OrdinalVector& aBcDofs, Plato::ScalarVector& aBcValues, const Plato::Scalar aTime = 0.0)
    {
        this->sizeBcArrays(aBcDofs, aBcValues);
        this->fillBcData(aBcDofs, aBcValues, aTime);
    }

    void sizeBcArrays(Plato::OrdinalVector& aBcDofs, Plato::ScalarVector& aBcValues)
    {
    ...
    void fillBcData(Plato::OrdinalVector& aBcDofs, Plato::ScalarVector& aBcValues, const Plato::Scalar aTime = 0.0)
    {
    ...
    bool empty() const { return mBCs.empty(); }
