                                          ...
                                          const Plato::Array<NumIndices, unsigned int>& aIndices)
{
    ...
    Kokkos::parallel_for(
        aIndices.size(), KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
...
TEUCHOS_UNIT_TEST(NaturalBCDataTests, MeshInputVaryingElementIteration)
{
    ...
    Kokkos::parallel_for(
        tElementOrds.size(), KOKKOS_LAMBDA(const Plato::OrdinalType& aSideOrdinal) {
            ...
            for (Plato::OrdinalType tNode = 0; tNode < ElementType::mNumNodesPerFace; ++tNode)
            {
                ...
                const auto tGlobalNodeOrdinal =
                    tConnectivity(tElementOrdinal * ElementType::mNumNodesPerCell + tLocalNodeOrdinal);
                const Plato::Scalar tPressure = Plato::scalarBoundaryDataAtIndex(tBoundaryData, tGlobalNodeOrdinal);
                const Plato::OrdinalType tIndex =
                    std::abs(tCoordinates(3 * tGlobalNodeOrdinal + 1) + 5) * (kNumElemsPerEdge + 1) +
                    std::abs(tCoordinates(3 * tGlobalNodeOrdinal + 2) + 5);
                tSidesetPressures(tIndex) = tPressure;
                tExpectedValue(tIndex) =
                    tCoordinates(3 * tGlobalNodeOrdinal + 1) + tCoordinates(3 * tGlobalNodeOrdinal + 2) + 10;
            }
        });
