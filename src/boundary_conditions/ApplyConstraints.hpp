...
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "dof ids", Kokkos::RangePolicy<>(0, tNumNodes), KOKKOS_LAMBDA(const Plato::OrdinalType& aNodeOrdinal) {
...
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "Dirichlet BC imposition - First loop", Kokkos::RangePolicy<>(0, tNumBCs),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aBcOrdinal) {
            ...
            {
                for (OrdinalType tLocalColumnDofOrdinal = 0; tLocalColumnDofOrdinal < NumDofPerNode;
                     tLocalColumnDofOrdinal++)
                {
                    ...
                    else
                    {
                        ...
                        for (OrdinalType tColRowNodeOffset = tColRowStart; tColRowNodeOffset < tColRowEnd;
                             tColRowNodeOffset++)
                        {
    ...
    Kokkos::parallel_for(
        "Dirichlet BC imposition - Second loop", Kokkos::RangePolicy<int>(0, tNumBCs), KOKKOS_LAMBDA(int bcOrdinal) {
...
/******************************************************************************/
template <int NumDofPerNode>
void applyConstraints(Teuchos::RCP<Plato::CrsMatrixType> aMatrix, Plato::ScalarVector aRhs, Plato::OrdinalVector aNodes)
/******************************************************************************/
{
    Plato::OrdinalType tNumNodes = aNodes.size();
...
/******************************************************************************/
template <int NumDofPerNode>
void applyConstraints(Teuchos::RCP<Plato::CrsMatrixType> matrix,
...
/******************************************************************************/
{
    ...
    Kokkos::parallel_for(
        "BC imposition", Kokkos::RangePolicy<int>(0, numBCs), KOKKOS_LAMBDA(int bcOrdinal) {
            ...
            for (OrdinalType entryOrdinal = rowStart; entryOrdinal < rowEnd; entryOrdinal++)
            {
                ...
                else
                {
                    ...
                    for (OrdinalType colRowEntryOrdinal = colRowStart; colRowEntryOrdinal < colRowEnd;
                         colRowEntryOrdinal++)
                    {
...
inline void apply_constraints(const Plato::OrdinalVector& aBcDofs,
 ...
 *
 **********************************************************************************/
inline void enforce_boundary_condition(const Plato::OrdinalVector& aBcDofs,
                                       const Plato::ScalarVector& aBcValues,
                                       const Plato::ScalarVector& aState)
{
    auto tLength = aBcValues.size();
    Kokkos::parallel_for(
        "enforce boundary condition", Kokkos::RangePolicy<>(0, tLength),
 ...
 *
 **********************************************************************************/
inline void set_dofs_values(const Plato::OrdinalVector& aBcDofs,
                            Plato::ScalarVector& aOutput,
                            Plato::Scalar aValue = 0.0)
{
    Kokkos::parallel_for(
        "set values at bc dofs to zero", Kokkos::RangePolicy<>(0, aBcDofs.size()),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { aOutput(aBcDofs(aOrdinal)) = aValue; });
}
