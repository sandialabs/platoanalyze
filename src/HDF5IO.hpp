 ...
 */
std::string make_path_parallel(const std::string& aPath, int aParSize = 1, int aRank = 0)
{
    // test if running in parallel mode
    if (aParSize > 1)
    {
        // get file extesion
        auto tFileExt = aPath.substr(aPath.find_last_of("."), aPath.length());

        // get base path
        auto tBasePath = aPath.substr(0, aPath.find_last_of("."));

        // add proc number to path
        std::string aParallelPath = tBasePath + "_" + std::to_string(aParSize) + "." + std::to_string(aRank) + tFileExt;
 ...
 */
hid_t create_hdf5_file(const std::string& aPath)
{
    assert(aPath.size() > 0);
    return H5Fcreate(make_path_parallel(aPath).c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
}
 ...
 */
hid_t open_hdf5_file(const std::string& aPath)
{
    assert(aPath.size() > 0);
    ...
    return H5Fopen(tPath.c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
}
 ...
 */
herr_t close_hdf5_file(hid_t aFileID) { return H5Fclose(aFileID); }
 ...
 */
bool dataset_exists(hid_t aFileID, const std::string& aLabel)
{
    hid_t tDataSet = 0;
 ...
 */
template <typename T>
hid_t get_hdf5_datatype(const T& aSample)
{
    assert(false);
    return 0;
}
...
template <>
hid_t get_hdf5_datatype(const int& aSample)
{
...
template <>
hid_t get_hdf5_datatype(const long int& aSample)
{
...
template <>
hid_t get_hdf5_datatype(const unsigned int& aSample)
{
...
template <>
hid_t get_hdf5_datatype(const long unsigned int& aSample)
{
...
template <>
hid_t get_hdf5_datatype(const double& aSample)
{
...
template <>
hid_t get_hdf5_datatype(const long double& aSample)
{
...
template <>
hid_t get_hdf5_datatype(const bool& aSample)
{
 ...
 */
template <typename T>
bool test_size_of_datatype(const T aValue)
{
 ...
 */
template <typename T>
void save_vector_to_hdf5_file(hid_t& aFileID, const std::string& aLabel, const std::vector<T>& aVector, herr_t& aStatus)
{
    // check datatype
    ...
    if (tSize > 0)
    {
        ...
        aStatus = H5Dwrite(tDataSet, tDataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, aVector.data());
    }
 ...
 * https://support.hdfgroup.org/ftp/HDF5/examples/misc-examples/
 */
void save_scalar_vector_to_hdf5_file(hid_t& aFileID,
 ...
 */
template <typename T>
void load_vector_from_hdf5_file(hid_t& aFileID, const std::string& aLabel, std::vector<T>& aVector, herr_t& aStatus)
{
    // check datatype
    assert(test_size_of_datatype((T)0));
    ...
    {
        std::string tMessage = "ERROR in reading from file: vector " + aLabel + " has the wrong datatype.\n";
    ...
    {
        // read data from file
        aStatus = H5Dread(tDataSet, tDataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, aVector.data());
    }
    else if (aStatus == 2)
    {
...
void load_scalar_vector_from_hdf5_file(hid_t& aFileID,
                                       const std::string& aLabel,
 ...
 */
template <typename T>
void save_scalar_to_hdf5_file(hid_t& aFileID, const std::string& aLabel, const T& aValue, herr_t& aStatus)
{
    // test if dataset exists
    if (dataset_exists(aFileID, aLabel))
    {
    ...
    // create new dataset
    hid_t tDataSet = H5Dcreate(aFileID, aLabel.c_str(), tDataType, tDataSpace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

    // write data into dataset
    aStatus = H5Dwrite(tDataSet, tDataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, &aValue);
 ...
 */
template <typename T>
void load_scalar_from_hdf5_file(hid_t& aFileID, const std::string& aLabel, T& aValue, herr_t& aStatus)
{
    // test if dataset exists
    if (!dataset_exists(aFileID, aLabel))
    {
    ...
    // read data from file
    aStatus = H5Dread(tDataSet, tDataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, &aValue);
...
void save_string_to_hdf5_file(hid_t& aFileID, const std::string& aLabel, const std::string& aValue, herr_t& aStatus)
{
    // test if dataset exists
    if (dataset_exists(aFileID, aLabel))
    {
    ...
    // write data into dataset
    aStatus = H5Dwrite(tDataSet, tDataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, aValue.c_str());
...
void load_string_from_hdf5_file(hid_t& aFileID, const std::string& aLabel, std::string& aValue, herr_t& aStatus)
{
    // test if dataset exists
    if (!dataset_exists(aFileID, aLabel))
    {
    ...
    aStatus = H5Dread(tDataSet, tDataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, tBuffer);
