...
std::string getEquationParam(const Teuchos::ParameterList& aInputParams, std::string equationName)
{
    // Only passed a string, add the default index.
    return getEquationParam(aInputParams, (Plato::OrdinalType)-1, equationName);
}
    ...
    else if (aInputParams.isType<std::string>("BingoFile"))
    {
        ...
        if (infile.is_open())
        {
            ...
            while (std::getline(infile, line))
            {
                ...
                else if (line.find("FITNESS") != std::string::npos && line.find("COMPLEXITY") != std::string::npos &&
                         line.find("EQUATION") != std::string::npos)
                {
                    line.clear();

                    // Read the equation requested, default is the first.
                    while (std::getline(infile, line) && equationIndex > 0) --equationIndex;
