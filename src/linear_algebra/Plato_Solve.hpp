...
void RowSummed(Teuchos::RCP<Plato::CrsMatrixType> a_A, Plato::ScalarVector a_x, Plato::ScalarVector a_b)
{
    ...
    Kokkos::parallel_for(
        "row sum inverse", Kokkos::RangePolicy<>(0, tNumBlockRows),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aBlockRowOrdinal) {
            // compute row sum
