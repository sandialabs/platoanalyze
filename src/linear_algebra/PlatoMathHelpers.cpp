                                 ...
                                 const Plato::ScalarVectorT<ScalarT>& aOutput)
{
    if (aMatrix->numCols() != aInput.size())
    {
        ...
        tMsg << "DIMENSION MISMATCH.  INPUT VECTOR LENGTH DOES NOT MATCH THE NUMBER OF COLUMNS IN MATRIX A.  "
             ...
             << aMatrix->numCols() << "'. INPUT VECTOR LABEL IS '" << aInput.label() << "' AND OUTPUT VECTOR LABEL IS '"
             << aOutput.label() << "'.";
    ...
    if (aMatrix->isBlockMatrix())
    {
        ...
        Kokkos::parallel_for(
            "BlockMatrix * Vector_a + Vector_b", Kokkos::RangePolicy<>(0, tNumNodeRows),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aNodeRowOrdinal) {
    ...
    else
    {
        ...
        Kokkos::parallel_for(
            "Matrix * Vector_a + Vector_b", Kokkos::RangePolicy<>(0, tNumRows),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aRowOrdinal) {
...
Plato::Scalar diagonalAveAbs(Plato::CrsMatrixType const& aMatrix)
{
    if (aMatrix.isBlockMatrix())
    {
        auto tNodeRowMap = aMatrix.rowMap();
        ...
        Kokkos::parallel_reduce(
            Kokkos::RangePolicy<>(0, tNumNodeRows),
...
void shiftDiagonal(Plato::CrsMatrixType const& aMatrix, Plato::Scalar aShift)
{
    if (aMatrix.isBlockMatrix())
    {
        auto tNodeRowMap = aMatrix.rowMap();
        ...
        Kokkos::parallel_for(
            "shift diagonal", Kokkos::RangePolicy<>(0, tNumNodeRows),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aNodeRowOrdinal) {
                auto tRowStartIndex = tNodeRowMap(aNodeRowOrdinal);
                auto tRowEndIndex = tNodeRowMap(aNodeRowOrdinal + 1);
                for (auto tCrsIndex = tRowStartIndex; tCrsIndex < tRowEndIndex; tCrsIndex++)
...
template <typename ScalarT>
void VectorTimesMatrixPlusVector(const Plato::ScalarVectorT<ScalarT>& aInput,
                                 ...
                                 const Plato::ScalarVectorT<ScalarT>& aOutput)
{
    ...
    if (aMatrix->isBlockMatrix())
    {
        ...
        Kokkos::parallel_for(
            "Vector_a * BlockMatrix + Vector_b", Kokkos::RangePolicy<>(0, tNumNodeRows),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aNodeRowOrdinal) {
                auto tRowStartIndex = tNodeRowMap(aNodeRowOrdinal);
                auto tRowEndIndex = tNodeRowMap(aNodeRowOrdinal + 1);
                for (auto tCrsIndex = tRowStartIndex; tCrsIndex < tRowEndIndex; tCrsIndex++)
                {
                    ...
                    for (auto tDofRowIndex = tFromDofRowIndex; tDofRowIndex < tToDofRowIndex; tDofRowIndex++)
                    {
                        for (auto tDofColIndex = tFromDofColIndex; tDofColIndex < tToDofColIndex; tDofColIndex++)
                        {
                            Kokkos::atomic_add(&aOutput(tDofColIndex),
                                               tEntries(tMatrixEntryIndex) * aInput(tDofRowIndex));
                            tMatrixEntryIndex += 1;
                        }
                    }
                }
    ...
    else
    {
        ...
        Kokkos::parallel_for(
            "Vector_a * Matrix + Vector_b", Kokkos::RangePolicy<>(0, tNumRows),
            KOKKOS_LAMBDA(const Plato::OrdinalType& aRowOrdinal) {
                auto tRowStart = tRowMap(aRowOrdinal);
                auto tRowEnd = tRowMap(aRowOrdinal + 1);
                for (auto tEntryIndex = tRowStart; tEntryIndex < tRowEnd; tEntryIndex++)
...
void RowSummedInverseMultiply(const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixOne,
                              Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixTwo)
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<>(0, tNumBlockRows), KOKKOS_LAMBDA(const Plato::OrdinalType& tBlockRowOrdinal) {
            tRowSumFunctor(tBlockRowOrdinal, tRowSum);
            tDiagInverseMultiplyFunctor(tBlockRowOrdinal, tRowSum);
        });
}

void getDataAsNonBlock(const Teuchos::RCP<const Plato::CrsMatrixType>& aMatrix,
                       ...
                       int aRowOffset)
{
    ...
    {
        Kokkos::parallel_for(
            Kokkos::RangePolicy<>(0, tNumMatrixRows), KOKKOS_LAMBDA(const Plato::OrdinalType& tMatrixRowIndex) {
                ...
                aMatrixRowMap(tMatrixRowIndex + 1) =
                    tFrom * tBlockSize + (tLocalRowIndex + 1) * tBlockRowSize * tNumColsPerBlock;
            });
    }
    else
    {
        Kokkos::parallel_for(
            Kokkos::RangePolicy<>(0, tNumMatrixRows), KOKKOS_LAMBDA(const Plato::OrdinalType& tMatrixRowIndex) {
                ...
                auto tTo = tRowMap(tBlockRowIndex + 1);
                auto tBlockRowSize = tTo - tFrom;
                aMatrixRowMap(tMatrixRowIndex) =
                    tFrom * aRowStride * tNumColsPerBlock + tLocalRowIndex * tBlockRowSize * tNumColsPerBlock;
                aMatrixRowMap(tMatrixRowIndex + 1) =
                    tFrom * aRowStride * tNumColsPerBlock + (tLocalRowIndex + 1) * tBlockRowSize * tNumColsPerBlock;
            });
    }
            ...
            Kokkos::RangePolicy<>(0, tNumMatrixRows), KOKKOS_LAMBDA(const Plato::OrdinalType& tMatrixRowIndex) {
                ...
                for (auto tColMapIndex = tFrom; tColMapIndex < tTo; ++tColMapIndex)
                {
                    for (Plato::OrdinalType tBlockColOffset = 0; tBlockColOffset < tNumColsPerBlock; ++tBlockColOffset)
                    {
                        ...
                        auto tValIndex =
                            tColMapIndex * tBlockSize + tLocalRowIndex * tNumColsPerBlock + tBlockColOffset;
...
void sortColumnEntries(const Plato::ScalarVectorT<Plato::OrdinalType>& aMatrixRowMap,
                       Plato::ScalarVectorT<Plato::OrdinalType>& aMatrixColMap,
                       Plato::ScalarVectorT<Plato::Scalar>& aMatrixValues)
{
...
void setDataFromNonBlock(Teuchos::RCP<Plato::CrsMatrixType>& aMatrix,
                         ...
                         Plato::ScalarVectorT<Plato::Scalar>& aMatrixValues)
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<>(0, tNumMatrixRows + 1), KOKKOS_LAMBDA(const Plato::OrdinalType& tMatrixRowIndex) {
            auto tBlockRowIndex = tMatrixRowIndex / tNumRowsPerBlock;
            auto tLocalRowIndex = tMatrixRowIndex % tNumRowsPerBlock;
            if (tLocalRowIndex == 0) tRowMap(tBlockRowIndex) = aMatrixRowMap(tMatrixRowIndex) / tBlockSize;
        });
...
void MatrixMatrixMultiply(const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixOne,
                          ...
                          SPGEMMAlgorithm aAlgorithm)
{
    ...
    typedef KokkosKernels::Experimental::KokkosKernelsHandle<
        OrdinalType, OrdinalType, Scalar, typename Plato::ExecSpace, typename Plato::MemSpace, typename Plato::MemSpace>
        KernelHandle;
    ...
    if (tNumRowsTwo != tNumColsOne)
    {
        ANALYZE_THROWERR("input matrices have incompatible shapes");
    }

    // numRows(C)  === numRows(M1)
    if (tNumRowsOut != tNumRowsOne)
    {
        ANALYZE_THROWERR("output matrix has incorrect shape");
    }

    // numCols(C)  === numCols(M2)
    if (tNumColsOut != tNumColsTwo)
    {
        ANALYZE_THROWERR("output matrix has incorrect shape");
    }
...
void MatrixMinusMatrix(Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixOne,
                       const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixTwo,
                       Plato::OrdinalType aOffset)
{
    ...
    if (tNumColsOne != tNumColsTwo)
    {
        ANALYZE_THROWERR("matrices have incompatible shape");
    }
...
void MatrixTranspose(const Teuchos::RCP<Plato::CrsMatrixType>& aMatrix,
                     Teuchos::RCP<Plato::CrsMatrixType>& aMatrixTranspose)
{
    ...
    Kokkos::parallel_for(
        "nonzeros", Kokkos::RangePolicy<OrdinalType>(0, tNumRows), KOKKOS_LAMBDA(OrdinalType iRowOrdinal) {
    ...
    Kokkos::parallel_scan(
        Kokkos::RangePolicy<OrdinalType>(0, tNumRowsT + 1),
    ...
    Kokkos::parallel_for(
        "colmap and entries", Kokkos::RangePolicy<OrdinalType>(0, tNumRows), KOKKOS_LAMBDA(OrdinalType iRowOrdinal) {
            auto tRowStart = tRowMap(iRowOrdinal);
            auto tRowEnd = tRowMap(iRowOrdinal + 1);
...
template void Plato::MatrixTimesVectorPlusVector<double>(const Teuchos::RCP<Plato::CrsMatrixType>&,
                                                         const Plato::ScalarVectorT<double>&,
                                                         const Plato::ScalarVectorT<double>&);

template void Plato::VectorTimesMatrixPlusVector<double>(const Plato::ScalarVectorT<double>&,
