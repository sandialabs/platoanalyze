...
class Eigenvalues
{
   ...
   public:
     ...
     * \brief Constructor
     **********************************************************************************/
    Eigenvalues(Plato::OrdinalType aNumFixedIterations = 6) : mNumFixedIterations(aNumFixedIterations) {}
     ...
     **********************************************************************************/
    void setNumFixedIterations(const Plato::Scalar aNumFixedIterations) { mNumFixedIterations = aNumFixedIterations; }
    ...
    KOKKOS_INLINE_FUNCTION void operator()(const Plato::Array<NumVoigtTerms, ResultType>& aVoigtTensor,
...
template <>
template <typename ResultType>
KOKKOS_INLINE_FUNCTION void Eigenvalues<3, 6>::operator()(const Plato::Array<6, ResultType>& aVoigtTensor,
                                                          ...
                                                          const bool& aIsStrainType) const
{
    ...
    if (aIsStrainType) tTensor(0, 1) /= tTwoScalar;

    tTensor(0, 2) = aVoigtTensor(4);
    if (aIsStrainType) tTensor(0, 2) /= tTwoScalar;

    tTensor(1, 2) = aVoigtTensor(3);
    if (aIsStrainType) tTensor(1, 2) /= tTwoScalar;
    ...
    for (Plato::OrdinalType tIteration = 0; tIteration < mNumFixedIterations; ++tIteration)
    {
        ...
        tTau = tZeroScalar;
        if (tCondition6) tTau = (tTensor(q, q) - tTensor(p, p)) / (tTwoScalar * tTensor(p, q));

        tTangent = tMinusOneScalar / (sqrt(tOneScalar + tTau * tTau) - tTau);
        if (tTau >= tZeroScalar) tTangent = tOneScalar / (tTau + sqrt(tOneScalar + tTau * tTau));

        tCosine = tOneScalar;
        if (tCondition6) tCosine = tOneScalar / sqrt(tOneScalar + tTangent * tTangent);

        tSine = tZeroScalar;
        if (tCondition6) tSine = tTangent * tCosine;

        // ########## Apply similarity transform with Givens rotation ##########
        tGivensRotation(0, 0) = tOneScalar;
        tGivensRotation(0, 1) = tZeroScalar;
        tGivensRotation(0, 2) = tZeroScalar;
        tGivensRotation(1, 0) = tZeroScalar;
        tGivensRotation(1, 1) = tOneScalar;
        tGivensRotation(1, 2) = tZeroScalar;
        tGivensRotation(2, 0) = tZeroScalar;
        tGivensRotation(2, 1) = tZeroScalar;
        tGivensRotation(2, 2) = tOneScalar;

        tGivensRotation(p, p) = tCosine;
        tGivensRotation(p, q) = tSine;
        tGivensRotation(q, p) = -tSine;
        tGivensRotation(q, q) = tCosine;
        ...
        for (i = 0; i < 3; ++i)
            for (j = 0; j < 3; ++j) tTensor(i, j) = tTensorRotated(i, j);
    }
...
template <>
template <typename ResultType>
KOKKOS_INLINE_FUNCTION void Eigenvalues<2, 3>::operator()(const Plato::Array<3, ResultType>& aVoigtTensor,
                                                          ...
                                                          const bool& aIsStrainType) const
{
    ...
    if (aIsStrainType) tTensor12 /= tTwo;

    ResultType tTensor11 = aVoigtTensor(0);
    ResultType tTensor22 = aVoigtTensor(1);
    ResultType tTau = tZero;
    if (tTensor12 > tZero) tTau = (tTensor22 - tTensor11) / (tTwo * tTensor12);

    ResultType tTangent = tMinusOne / (sqrt(tOne + tTau * tTau) - tTau);
    if (tTau >= tZero) tTangent = tOne / (tTau + sqrt(tOne + tTau * tTau));

    ResultType tCosine = tOne;
    ResultType tSine = tZero;
    if (tTensor12 > tZero) tCosine = tOne / sqrt(tOne + tTangent * tTangent);
    if (tTensor12 > tZero) tSine = tTangent * tCosine;

    aEigenvalues(0) =
        tCosine * (tTensor11 * tCosine - tTensor12 * tSine) - tSine * (tTensor12 * tCosine - tTensor22 * tSine);
    aEigenvalues(1) =
        tCosine * (tTensor22 * tCosine + tTensor12 * tSine) + tSine * (tTensor12 * tCosine + tTensor11 * tSine);
}
...
template <>
template <typename ResultType>
KOKKOS_INLINE_FUNCTION void Eigenvalues<1, 1>::operator()(const Plato::Array<1, ResultType>& aVoigtTensor,
