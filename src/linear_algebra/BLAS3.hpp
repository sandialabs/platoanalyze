...
inline void identity(const Plato::OrdinalType& aNumCells, Plato::ScalarArray3D& aIdentity)
{
    ...
    {
        ANALYZE_THROWERR(
            "\nNumber of cell mismatch. Input array has different number of cells than input number of cell "
            "argument.\n")
    }

    Kokkos::parallel_for(
        "blas3::identity", Kokkos::RangePolicy<>(0, aNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
...
inline void inverse(const Plato::OrdinalType& aNumCells, AViewType& aA, BViewType& aInverse)
{
    ...
    Kokkos::parallel_for(
        "compute matrix inverse 3DView", Kokkos::RangePolicy<>(0, aNumCells),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            ...
            SerialTrsm<Side::Left, Uplo::Lower, Trans::NoTranspose, Diag::Unit, Algo::Trsm::Blocked>::invoke(tAlpha, tA,
                                                                                                             tAinv);
            SerialTrsm<Side::Left, Uplo::Upper, Trans::NoTranspose, Diag::NonUnit, Algo::Trsm::Blocked>::invoke(
                tAlpha, tA, tAinv);
        });
}
// function inverse
...
inline void fill(const Plato::OrdinalType& aNumCells, typename AViewType::const_value_type& aAlpha, AViewType& aOutput)
{
    ...
    Kokkos::parallel_for(
        "blas3::fill", Kokkos::RangePolicy<>(0, aNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            for (Plato::OrdinalType tRowIndex = 0; tRowIndex < NumRowsPerCell; tRowIndex++)
            {
                   ...
                   const BViewType& aB)
{
    ...
    Kokkos::parallel_for(
        "blas3::update", Kokkos::RangePolicy<>(0, aNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
                     ...
                     CViewType& aC)
{
    ...
    if (aA.extent(2) != aB.extent(1))
    {
        tError << "\nDimension mismatch: The number of columns in A matrix workset does not match "
               << "the number of rows in B matrix workset. "
               << "A has " << aA.extent(2) << " columns and B has " << aB.extent(1) << " rows.\n";
        ANALYZE_THROWERR(tError.str())
    }
    ...
    Kokkos::parallel_for(
        "blas3::multiply", Kokkos::RangePolicy<>(0, aNumCells), KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal) {
            ...
            for (Plato::OrdinalType tOutRowIndex = 0; tOutRowIndex < tNumOutRows; tOutRowIndex++)
            {
                for (Plato::OrdinalType tOutColIndex = 0; tOutColIndex < tNumOutCols; tOutColIndex++)
                {
                    ...
                    {
                        tValue += aAlpha * aA(aCellOrdinal, tOutRowIndex, tCommonIndex) *
                                  aB(aCellOrdinal, tCommonIndex, tOutColIndex);
                    }
