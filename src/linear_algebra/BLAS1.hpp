...
inline void abs(const Plato::ScalarVector& aVector)
{
    ...
    Kokkos::parallel_for(
        "calculate absolute value", Kokkos::RangePolicy<>(0, tLength),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { aVector(aOrdinal) = fabs(aVector(aOrdinal)); });
}
// function abs
...
inline Plato::Scalar dot(const VecOneT& aVec1, const VecTwoT& aVec2)
{
    if (aVec2.size() != aVec1.size())
    {
        ...
        tMsg << "BLAS1 DOT: DIMENSION MISMATCH. VECTOR ONE HAS SIZE = " << aVec1.size()
             << " AND VECTOR TWO HAS SIZE = " << aVec2.size() << ".VECTOR ONE HAS LABEL '" << aVec1.label()
             << "' AND VECTOR TWO HAS LABEL '" << aVec2.label() << "'.\n";
    ...
    Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex, Plato::Scalar& aSum) { aSum += aVec1(aIndex) * aVec2(aIndex); },
        tOutput);
    return (tOutput);
}
// function dot
...
inline void fill(const Plato::Scalar& aInput, const VectorT& aVector)
{
    ...
    Kokkos::Experimental::fill(Kokkos::DefaultExecutionSpace(), aVector, aInput);
}
// function fill
                 ...
                 const VectorT& aOutput)
{
    ...
    Kokkos::parallel_for(
        "fill vector", Kokkos::RangePolicy<>(0, tNumLocalVals), KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
...
inline void copy(const VecOneT& aInput, const VecTwoT& aOutput)
{
    ...
    Kokkos::parallel_for(
        "copy vector", Kokkos::RangePolicy<>(0, tNumLocalVals),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { aOutput(aOrdinal) = aInput(aOrdinal); });
}
// function copy
...
inline void axpy(const Plato::Scalar& aAlpha, const VecT& aInput, const VecT& aOutput)
{
    ...
    Plato::OrdinalType tNumLocalVals = aInput.size();
    Kokkos::parallel_for(
        "Plato::axpy", Kokkos::RangePolicy<>(0, tNumLocalVals),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) { aOutput(aOrdinal) += aAlpha * aInput(aOrdinal); });
}
// function axpy
...
void update(const Plato::Scalar& aAlpha, const VecT& aInput, const Plato::Scalar& aBeta, const VecT& aOutput)
{
    ...
    Plato::OrdinalType tNumLocalVals = aInput.size();
    Kokkos::parallel_for(
        "update vector", Kokkos::RangePolicy<>(0, tNumLocalVals), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
...
void local_sum(const VecT& aInput, ScalarT& aOutput)
{
    ...
    Kokkos::parallel_reduce(
        Kokkos::RangePolicy<>(0, tNumLocalElems),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aCellOrdinal, ScalarT& aLocalSum) {
...
void max(const VecT& aInput, ScalarT& aOutput)
{
    ...
    Kokkos::parallel_reduce(
        "KokkosReductionOperations::max", Kokkos::RangePolicy<>(0, tSize),
        KOKKOS_LAMBDA(const OrdinalType& aIndex, ScalarT& aValue) { tMaxReducer.join(aValue, aInput[aIndex]); },
        tMaxReducer);
}
// function max
...
void min(const VecT& aInput, ScalarT& aOutput)
{
    ...
    Kokkos::parallel_reduce(
        "KokkosReductionOperations::min", Kokkos::RangePolicy<>(0, tSize),
        KOKKOS_LAMBDA(const OrdinalType& aIndex, ScalarT& aValue) { tMinReducer.join(aValue, aInput[aIndex]); },
        tMinReducer);
}
// function min
...
inline void extract(const Plato::ScalarVector& aFromVector, Plato::ScalarVector& aToVector)
{
    ...
    Kokkos::parallel_for(
        "extract", Kokkos::RangePolicy<>(0, tNumRows), KOKKOS_LAMBDA(const Plato::OrdinalType& aOrdinal) {
 ...
 *
 **********************************************************************************/
inline Plato::Scalar inf_norm(const Plato::ScalarVector& aFieldOne, const Plato::ScalarVector& aFieldTwo)
{
 ...
 *
 **********************************************************************************/
inline Plato::Scalar norm(const Plato::ScalarVector& aFieldOne, const Plato::ScalarVector& aFieldTwo)
{
    Plato::ScalarVector tResidual("residual", aFieldOne.size());
    Plato::blas1::copy(aFieldTwo, tResidual);
    Plato::blas1::update(1.0, aFieldOne, -1.0, tResidual);
