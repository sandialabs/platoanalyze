...
#include "simplex/SimplexMechanics.hpp"
...
class StressDivergence : public Plato::SimplexMechanics<SpaceDim>
{
   private:
    ...
    Plato::OrdinalType mVoigt[SpaceDim]
                             [SpaceDim]; /*!< matrix with indices to stress tensor entries in Voigt notation */
                                           ...
                                           const Plato::Scalar aScale = 1.0) const
    {
        for (Plato::OrdinalType tDimIndexI = 0; tDimIndexI < SpaceDim; tDimIndexI++)
        {
            for (Plato::OrdinalType tNodeIndex = 0; tNodeIndex < mNumNodesPerCell; tNodeIndex++)
            {
                ...
                {
                    aOutput(aCellOrdinal, tLocalOrdinal) += aScale * aCellVolume(aCellOrdinal) *
