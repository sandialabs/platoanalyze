...
class VectorFunction : public Plato::WorksetBase<typename PhysicsType::ElementType>
{
   ...
   public:
    ...
    template <typename EvaluationType>
    Plato::ScalarMultiVectorT<typename EvaluationType::ResultScalarType> externalForceContribution(
        const EvaluationFunction<EvaluationType>& aFunction,
        const Plato::ScalarVector& aState,
        const Plato::ScalarVector& aControl,
        Plato::Scalar aTimeStep = 0.0) const
    ...
    Teuchos::RCP<Plato::CrsMatrixType> gradient_x(
        const Plato::ScalarVector& aState, const Plato::ScalarVector& aControl, Plato::Scalar aTimeStep = 0.0
        /**************************************************************************/
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            Plato::WorksetBase<ElementType>::assembleJacobianFad(mNumDofsPerCell, mNumConfigDofsPerCell,
                                                                 tGradientXMatEntryOrdinal, tInternalForceValues,
                                                                 tMatEntries, tDomain);
        }
    ...
    ) const
    {
        ...
        Plato::WorksetBase<ElementType>::assembleJacobianFad(mNumDofsPerCell, mNumDofsPerCell, tJacobianMatEntryOrdinal,
                                                             tExternalForceValues, tMatEntries);
    ...
    ) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            Plato::WorksetBase<ElementType>::assembleJacobianFad(mNumDofsPerCell, mNumNodesPerCell,
                                                                 tGradientZMatEntryOrdinal, tInternalForceValues,
                                                                 tMatEntries, tDomain);
        }
