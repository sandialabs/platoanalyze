...
{
   public:
    EssentialBC(const std::string& aName, Teuchos::ParameterList& aParam, Plato::Scalar aScaleFactor = 1.0)
        : mName(aName),
    ...
    void checkValidInputs(Teuchos::ParameterList& aParam)
    {
    ...
    void initializeValueExpression(Teuchos::ParameterList& aParam)
    {
     ...
     */
    void get(const Plato::Mesh& aMesh,
    ...
    Plato::OrdinalVectorT<const Plato::OrdinalType> parseConstrainedNodeSets(const Plato::Mesh aMesh)
    {
        auto tNodeLids = aMesh->GetNodeSetNodes(mNodeSetName);
        return tNodeLids;
    }

    Plato::Scalar get_value(const Plato::Scalar aTime = 0.0) const
    {
    ...
    void fillBcData(Plato::OrdinalVector& aBcDofs,
                    Plato::ScalarVector& aBcValues,
                    ...
                    const Plato::Scalar aValue)
    {
        ...
        Kokkos::parallel_for(
            "Dirichlet BC", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumberConstrainedNodes),
    ...
    Plato::OrdinalType get_length(const Plato::Mesh aMesh)
    {
        auto tNodeIds = this->parseConstrainedNodeSets(aMesh);
    ...
    void checkForZeroLength(const Plato::Scalar aLength)
    {
        if (aLength == static_cast<Plato::OrdinalType>(0))
        {
            const std::string tErrorMessage = std::string("The set '") + mNodeSetName +
                                              "' specified in Essential Boundary Conditions contains 0 nodes.";
            WARNING(tErrorMessage)
        }
    }

    std::string const& get_ns_name() const { return mNodeSetName; }
