...
class Quad9
{
   public:
    ...
    [[nodiscard]] static constexpr Plato::Array<mNumGaussPoints> getCubWeights()
    {
        ...
        return Plato::Array<mNumGaussPoints>{wc, we, wc, we, wf, we, wc, we, wc};
    }
    ...
    [[nodiscard]] static constexpr Plato::Matrix<mNumGaussPoints, mNumSpatialDims> getCubPoints()
    {
        ...
        return Plato::Matrix<mNumGaussPoints, mNumSpatialDims>{-p, -p, 0, -p, p, -p, -p, 0, 0,
                                                               0,  p,  0, -p, p, 0,  p,  p, p};
    }
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint) -> Plato::Array<mNumNodesPerCell>
    {
        ...
        return Plato::Array<mNumNodesPerCell>{
            (1 - x) * (1 - y) * xy / 4.0,           -(1 + x) * (1 - y) * xy / 4.0,
            (1 + x) * (1 + y) * xy / 4.0,           -(1 - x) * (1 + y) * xy / 4.0,
            -(1 - x) * (1 + x) * (1 - y) * y / 2.0, (1 + x) * (1 - y) * (1 + y) * x / 2.0,
            (1 - x) * (1 + x) * (1 + y) * y / 2.0,  -(1 - x) * (1 - y) * (1 + y) * x / 2.0,
            (1 - x) * (1 + x) * (1 - y) * (1 + y)};
    }
        ...
        const Plato::Array<mNumSpatialDims>& aCubPoint) -> Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>
    {
        ...
        return Plato::Matrix<mNumNodesPerCell, mNumSpatialDims>{(1 - 2 * x) * (1 - y) * y / 4.0,
                                                                (1 - x) * (1 - 2 * y) * x / 4.0,
                                                                -(1 + 2 * x) * (1 - y) * y / 4.0,
                                                                -(1 + x) * (1 - 2 * y) * x / 4.0,
                                                                (1 + 2 * x) * (1 + y) * y / 4.0,
