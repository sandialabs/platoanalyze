...
#define BAMG_COMPUTE(function)                                       \
    {                                                                \
        if (matches(aSpec.meshType, "hex8"))                         \
        {                                                            \
            return Hex8::function(aSpec);                            \
        }                                                            \
        else if (matches(aSpec.meshType, "hex27"))                   \
        {                                                            \
            return Hex27::function(aSpec);                           \
        }                                                            \
        else if (matches(aSpec.meshType, "tet4"))                    \
        {                                                            \
            return Tet4::function(aSpec);                            \
        }                                                            \
        else if (matches(aSpec.meshType, "tet10"))                   \
        {                                                            \
            return Tet10::function(aSpec);                           \
        }                                                            \
        else if (matches(aSpec.meshType, "quad4"))                   \
        {                                                            \
            return Quad4::function(aSpec);                           \
        }                                                            \
        else if (matches(aSpec.meshType, "tri3"))                    \
        {                                                            \
            return Tri3::function(aSpec);                            \
        }                                                            \
        else if (matches(aSpec.meshType, "bar2"))                    \
        {                                                            \
            return Bar2::function(aSpec);                            \
        }                                                            \
...
SideSet getSides(const MeshSpec& aSpec,
                 std::string aFace,
                 Uint aFromX,
                 Uint aFromY,
                 Uint aFromZ,
...
SideSet getSides(const MeshSpec& aSpec, std::string aFace, Uint aFromX, Uint aFromY, Uint aNumX, Uint aNumY);
