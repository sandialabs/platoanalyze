...
void setControlWS(std::vector<std::vector<Plato::Scalar>>& aValues, Plato::ScalarMultiVectorT<Plato::Scalar>& aControl)
{
    ...
    Kokkos::parallel_for(
        "fill control", Kokkos::RangePolicy<Plato::OrdinalType>(0, tNumCells), KOKKOS_LAMBDA(Plato::OrdinalType iCell) {
            for (int iNode = 0; iNode < tNumNodesPerCell; iNode++)
...
Plato::Mesh get_box_mesh(std::string aMeshType, Plato::OrdinalType aMeshIntervals, std::string aFileName)
{
                                            ...
                                            const Plato::Scalar& aSetValue)
{
    ...
    Kokkos::parallel_for(
        "fill vector boundary dofs", Kokkos::RangePolicy<>(0, tNumBoundaryNodes),
                                                          ...
                                                          const Plato::OrdinalType& aDofToSet)
{
    ...
    Kokkos::parallel_for(
        "fill dirichlet dof indices on boundary 2-D", Kokkos::RangePolicy<>(0, tNumBoundaryNodes),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
            Plato::OrdinalType tIndex = aDofStride * tBoundaryNodes[aIndex] + aDofToSet;
                                                          ...
                                                          const Plato::OrdinalType& aDofToSet)
{
    ...
    Kokkos::parallel_for(
        "fill dirichlet dof indices on boundary 3-D", Kokkos::RangePolicy<>(0, tNumBoundaryNodes),
        KOKKOS_LAMBDA(const Plato::OrdinalType& aIndex) {
            Plato::OrdinalType tIndex = aDofStride * tBoundaryNodes[aIndex] + aDofToSet;
            tDofIndices(aIndex) = tIndex;
        });
                             ...
                             const Plato::Scalar& aSetValue)
{
    ...
    Kokkos::parallel_for(
        "fill specific vector entry globally", Kokkos::RangePolicy<>(0, tRange),
...
std::vector<std::vector<Plato::Scalar>> to_full(Teuchos::RCP<Plato::CrsMatrixType> aInMatrix)
{
    ...
    std::vector<std::vector<Scalar>> retMatrix(aInMatrix->numRows(), std::vector<Scalar>(aInMatrix->numCols(), 0.0));
    ...
    for (OrdinalType iRowIndex = 0; iRowIndex < tNumRows; iRowIndex++)
    {
        ...
        for (auto iColMapEntryIndex = tFrom; iColMapEntryIndex < tTo; iColMapEntryIndex++)
        {
            ...
            for (OrdinalType iLocalRowIndex = 0; iLocalRowIndex < tNumRowsPerBlock; iLocalRowIndex++)
            {
                ...
                {
                    const auto tColIndex = tBlockColIndex * tNumColsPerBlock + iLocalColIndex;
                    const auto tSparseIndex =
