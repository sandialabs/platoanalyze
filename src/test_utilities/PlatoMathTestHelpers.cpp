...
void row_sum(const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrix, Plato::ScalarVector& aOutRowSum)
{
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const Plato::OrdinalType& tBlockRowOrdinal) { tRowSumFunctor(tBlockRowOrdinal, aOutRowSum); });
}
                                ...
                                const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixTwo)
{
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<>(0, tNumEntries), KOKKOS_LAMBDA(const Plato::OrdinalType& tEntryOrdinal) {
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<>(0, tNumBlockRows), KOKKOS_LAMBDA(const Plato::OrdinalType& tBlockRowOrdinal) {
            ...
            for (auto tColMapIndex = tFrom; tColMapIndex < tTo; ++tColMapIndex)
            {
                ...
                auto tToEntryOffset =
                    tColMapIndex * tToNumRowsPerBlock * tToNumColsPerBlock + aOffset * tToNumColsPerBlock;
                ;
...
bool is_same(const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixA, const Teuchos::RCP<Plato::CrsMatrixType>& aInMatrixB)
{
