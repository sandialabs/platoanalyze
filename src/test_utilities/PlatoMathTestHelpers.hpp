...
void set_view_from_vector(Plato::ScalarVectorT<DataType> aView, const std::vector<DataType>& aVector)
{
    Kokkos::View<const DataType*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged> tHostView(aVector.data(), aVector.size());
...
template <typename DataType>
auto is_near(const Plato::ScalarVectorT<DataType>& aView, const std::vector<DataType>& aVector, double aTolerance)
