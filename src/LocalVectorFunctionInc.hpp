...
class LocalVectorFunctionInc
{
   ...
   public:
     ...
     *
     ******************************************************************************/
    LocalVectorFunctionInc(const Plato::SpatialModel& aSpatialModel,
          ...
          mDataMap(aDataMap)
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            mResidualFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<Residual>(tDomain, aDataMap, aParamList);
            mJacobianUFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<GlobalJacobian>(tDomain, aDataMap, aParamList);
            mJacobianUPFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<GlobalJacobianP>(tDomain, aDataMap, aParamList);
            mJacobianCFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<LocalJacobian>(tDomain, aDataMap, aParamList);
            mJacobianCPFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<LocalJacobianP>(tDomain, aDataMap, aParamList);
            mJacobianXFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<GradientX>(tDomain, aDataMap, aParamList);
            mJacobianZFunctions[tName] =
                tFunctionFactory.template createLocalVectorFunctionInc<GradientZ>(tDomain, aDataMap, aParamList);
        }
    }
     ...
     *
     ******************************************************************************/
    LocalVectorFunctionInc(const Plato::SpatialModel& aSpatialModel, Plato::DataMap& aDataMap)
        : mWorksetBase(aSpatialModel.Mesh),
          mNumCells(aSpatialModel.Mesh->NumElements()),
          mNumNodes(aSpatialModel.Mesh->NumNodes()),
          mSpatialModel(aSpatialModel),
     ...
     *
     ******************************************************************************/
    Plato::OrdinalType size() const { return mNumCells * mNumLocalDofsPerCell; }
     ...
     ***************************************************************************/
    decltype(mNumDofsPerNode) numGlobalDofsPerNode() const { return mNumDofsPerNode; }
     ...
     ******************************************************************************/
    void updateLocalState(const Plato::ScalarVector& aGlobalState,
                          ...
                          const Plato::TimeData& aTimeData) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            //
            Plato::ScalarMultiVectorT<StateScalar> tGlobalStateWS("Global State Workset", tNumCells,
                                                                  mNumGlobalDofsPerCell);
            mWorksetBase.worksetState(aGlobalState, tGlobalStateWS, tDomain);

            // Workset prev global state
            //
            Plato::ScalarMultiVectorT<PrevStateScalar> tPrevGlobalStateWS("Prev Global State Workset", tNumCells,
                                                                          mNumGlobalDofsPerCell);
            mWorksetBase.worksetState(aPrevGlobalState, tPrevGlobalStateWS, tDomain);

            // Workset local state
            //
            Plato::ScalarMultiVectorT<LocalStateScalar> tLocalStateWS("Local State Workset", tNumCells,
                                                                      mNumLocalDofsPerCell);
            mWorksetBase.worksetLocalState(aLocalState, tLocalStateWS, tDomain);

            // Workset prev local state
            //
            Plato::ScalarMultiVectorT<PrevLocalStateScalar> tPrevLocalStateWS("Prev Local State Workset", tNumCells,
                                                                              mNumLocalDofsPerCell);
            ...
            //
            mResidualFunctions.at(tName)->updateLocalState(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                           tPrevLocalStateWS, tControlWS, tConfigWS, aTimeData);
     ...
     ******************************************************************************/
    Plato::ScalarVectorT<typename Residual::ResultScalarType> value(const Plato::ScalarVector& aGlobalState,
                                                                    const Plato::ScalarVector& aPrevGlobalState,
                                                                    const Plato::ScalarVector& aLocalState,
                                                                    const Plato::ScalarVector& aPrevLocalState,
                                                                    const Plato::ScalarVector& aControl,
                                                                    const Plato::TimeData& aTimeData) const
    {
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // Workset prev global state
            //
            Plato::ScalarMultiVectorT<PrevStateScalar> tPrevGlobalStateWS("Prev State Workset", tNumCells,
                                                                          mNumGlobalDofsPerCell);
            mWorksetBase.worksetState(aPrevGlobalState, tPrevGlobalStateWS, tDomain);
            ...
            // evaluate function
            //
            mResidualFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS, tPrevLocalStateWS,
     ...
     ******************************************************************************/
    Plato::ScalarMultiVectorT<typename Residual::ResultScalarType> valueWorkSet(
     ...
     ******************************************************************************/
    Plato::ScalarArray3D gradient_x(const Plato::ScalarVector& aGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aControl,
                                    const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Configuration", mNumCells, mNumLocalDofsPerCell,
                                             tNumConfigDofsPerCell);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // create return view
            //
            Plato::ScalarMultiVectorT<ResultScalar> tJacobianWS("Jacobian Configuration Workset", tNumCells,
                                                                mNumLocalDofsPerCell);

            // evaluate function
            //
            mJacobianXFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                    tPrevLocalStateWS, tControlWS, tConfigWS, tJacobianWS, aTimeData);

            Plato::transform_ad_type_to_pod_3Dview<mNumLocalDofsPerCell, tNumConfigDofsPerCell>(tDomain, tJacobianWS,
                                                                                                tOutputJacobian);
        }
     ...
     ******************************************************************************/
    ScalarArray3D gradient_u(const Plato::ScalarVector& aGlobalState,
                             const Plato::ScalarVector& aPrevGlobalState,
                             const Plato::ScalarVector& aLocalState,
                             const Plato::ScalarVector& aPrevLocalState,
                             const Plato::ScalarVector& aControl,
                             const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Current Global State", mNumCells, mNumLocalDofsPerCell,
                                             mNumGlobalDofsPerCell);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // create return view
            //
            Plato::ScalarMultiVectorT<ResultScalar> tJacobianWS("Jacobian Current Global State Workset", tNumCells,
                                                                mNumLocalDofsPerCell);

            // evaluate function
            //
            mJacobianUFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                    tPrevLocalStateWS, tControlWS, tConfigWS, tJacobianWS, aTimeData);

            Plato::transform_ad_type_to_pod_3Dview<mNumLocalDofsPerCell, mNumGlobalDofsPerCell>(tDomain, tJacobianWS,
                                                                                                tOutputJacobian);
        }
        return tOutputJacobian;
    }
     ...
     ******************************************************************************/
    Plato::ScalarArray3D gradient_up(const Plato::ScalarVector& aGlobalState,
                                     const Plato::ScalarVector& aPrevGlobalState,
                                     const Plato::ScalarVector& aLocalState,
                                     const Plato::ScalarVector& aPrevLocalState,
                                     const Plato::ScalarVector& aControl,
                                     const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Previous Global State", mNumCells, mNumLocalDofsPerCell,
                                             mNumGlobalDofsPerCell);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // create return view
            //
            Plato::ScalarMultiVectorT<ResultScalar> tJacobianWS("Jacobian Previous Global State Workset", tNumCells,
                                                                mNumLocalDofsPerCell);

            // evaluate function
            //
            mJacobianUPFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                     tPrevLocalStateWS, tControlWS, tConfigWS, tJacobianWS, aTimeData);
     ...
     ******************************************************************************/
    Plato::ScalarArray3D gradient_c(const Plato::ScalarVector& aGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aControl,
                                    const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Current Local State", mNumCells, mNumLocalDofsPerCell,
                                             mNumLocalDofsPerCell);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // create return view
            //
            Plato::ScalarMultiVectorT<ResultScalar> tJacobianWS("Jacobian Current Local State Workset", tNumCells,
                                                                mNumLocalDofsPerCell);

            // evaluate function
            //
            mJacobianCFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                    tPrevLocalStateWS, tControlWS, tConfigWS, tJacobianWS, aTimeData);

            Plato::transform_ad_type_to_pod_3Dview<mNumLocalDofsPerCell, mNumLocalDofsPerCell>(tDomain, tJacobianWS,
                                                                                               tOutputJacobian);
        }
        return tOutputJacobian;
    }
     ...
     ******************************************************************************/
    Plato::ScalarArray3D gradient_cp(const Plato::ScalarVector& aGlobalState,
                                     const Plato::ScalarVector& aPrevGlobalState,
                                     const Plato::ScalarVector& aLocalState,
                                     const Plato::ScalarVector& aPrevLocalState,
                                     const Plato::ScalarVector& aControl,
                                     const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Previous Local State", mNumCells, mNumLocalDofsPerCell,
                                             mNumLocalDofsPerCell);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // create return view
            //
            Plato::ScalarMultiVectorT<ResultScalar> tJacobianWS("Jacobian Previous Local State Workset", tNumCells,
                                                                mNumLocalDofsPerCell);

            // evaluate function
            //
            mJacobianCPFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                     tPrevLocalStateWS, tControlWS, tConfigWS, tJacobianWS, aTimeData);
     ...
     ******************************************************************************/
    Plato::ScalarArray3D gradient_z(const Plato::ScalarVector& aGlobalState,
                                    const Plato::ScalarVector& aPrevGlobalState,
                                    const Plato::ScalarVector& aLocalState,
                                    const Plato::ScalarVector& aPrevLocalState,
                                    const Plato::ScalarVector& aControl,
                                    const Plato::TimeData& aTimeData) const
    {
        ...
        Plato::ScalarArray3D tOutputJacobian("Output Jacobian Control", mNumCells, mNumLocalDofsPerCell,
                                             mNumNodesPerCell);
        ...
        for (const auto& tDomain : mSpatialModel.Domains)
        {
            ...
            // create result
            //
            Plato::ScalarMultiVectorT<ResultScalar> tJacobianWS("Jacobian Control Workset", tNumCells,
                                                                mNumLocalDofsPerCell);

            // evaluate function
            //
            mJacobianZFunctions.at(tName)->evaluate(tGlobalStateWS, tPrevGlobalStateWS, tLocalStateWS,
                                                    tPrevLocalStateWS, tControlWS, tConfigWS, tJacobianWS, aTimeData);

            Plato::transform_ad_type_to_pod_3Dview<mNumLocalDofsPerCell, mNumNodesPerCell>(mNumCells, tJacobianWS,
                                                                                           tOutputJacobian);
        }
        return tOutputJacobian;
    }
