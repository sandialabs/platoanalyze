...
class FauxSharedData
{
   public:
    ...
    void setData(const std::vector<Plato::Scalar>& aData) { m_data = aData; }
    void getData(std::vector<Plato::Scalar>& aData) const { aData = m_data; }
    int size() const { return m_data.size(); }

    std::string myName() const { return "Analyze Unit Test: FauxSharedData"; }

    std::string myContext() const { return m_context; }
    void setContext(std::string context) { m_context = context; }

    Plato::data::layout_t myLayout() const { return m_layout; }
...
class FauxParameter
{
   public:
    ...
    void setData(const std::vector<Plato::Scalar>& aData) { m_value = aData[0]; }
    ...
    std::string myName() const { return "Unit Test: Faux Parameter myName"; }

    std::string myContext() const { return m_context; }
    void setContext(std::string context) { m_context = context; }

    Plato::data::layout_t myLayout() const { return Plato::data::layout_t::SCALAR_PARAMETER; }
