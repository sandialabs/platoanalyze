...
TEUCHOS_UNIT_TEST(NaturalBCTests, SurfaceStateIntegral)
{
    ...
    Plato::SurfaceStateIntegral<ElementType, cNumDofs, cDofsPerNode, cDofOffset> tSurfaceStateIntegral(
        tOnFace, tFluxExpressions, tStateNames);
    ...
    for (int iElem = 0; iElem < tElementOrds_Host.size(); iElem++)
    {
        ...
        for (int iNode = 0; iNode < cNodesPerFace; iNode++)
        {
            ...
            TEST_FLOATING_EQUALITY(tResult_Host(tElemIndex, tElemLocalNodeIndex * cDofsPerNode + cTemperatureDofIndex),
                                   tNodeVal, 2.0 * DBL_EPSILON);
        }
    }
