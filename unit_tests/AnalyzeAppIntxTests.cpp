...
void objectiveFiniteDifferenceTest(std::shared_ptr<Plato::MPMD_App> aApp,
                                   Plato::Scalar& val1,
                                   Plato::Scalar& val2,
                                   Plato::Scalar tol);
void objectiveFiniteDifferenceTest(
    std::string inputFile, std::string appFile, Plato::Scalar& val1, Plato::Scalar& val2, Plato::Scalar tol);
...
TEUCHOS_UNIT_TEST(AnalyzeAppTests, MultipleProblemDefinitions)
{
    ...
    {
        if (fabs(stdStateOne[i]) > 1e-16) TEST_FLOATING_EQUALITY(stdStateOne[i], -stdStateTwo[i], 1e-10);
    }
}
...
TEUCHOS_UNIT_TEST(AnalyzeAppTests, OperationParameter)
{
    ...
    for (int i = 0; i < localIDs.size(); i++)
    {
        if (fabs(stdStateOne[i]) > 1e-16) TEST_FLOATING_EQUALITY(stdStateOne[i], -stdStateTwo[i], 1e-12);
    }
}
