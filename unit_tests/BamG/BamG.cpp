...
void writeMesh(MeshData& aMeshData, const MeshSpec& aMeshSpec)
{
    // create mesh file
    int CPU_word_size = 8;
    int IO_word_size = 8;
    ...
    for (const auto& tEntry : aMeshData.nodeSets)
    {
        ...
        for (auto& tNode : tNodesCopy)
        {
            tNode += 1;
        }  // exodus numbering is one-based
    ...
    for (const auto& tEntry : aMeshData.sideSets)
    {
        ...
        for (auto& tElement : tElementsCopy)
        {
            tElement += 1;
        }  // exodus numbering is one-based
    ...
    // put block data
    if (ex_put_block(tFileID, EX_ELEM_BLOCK, /*block_id=*/1, aMeshSpec.meshType.c_str(), tNumElems, aMeshData.numNPE, 0,
                     0, 0))
    {
    ...
    for (decltype(aSpec.numX) tIndexI = 0; tIndexI < aSpec.numX; tIndexI++)
        for (decltype(aSpec.numY) tIndexJ = 0; tIndexJ < aSpec.numY; tIndexJ++)
            for (decltype(aSpec.numZ) tIndexK = 0; tIndexK < aSpec.numZ; tIndexK++)
            {
                ...
                tConnect[tElIndex * cNumNPE + 0] =
                    indexMap(tIndexI, tIndexJ, tIndexK, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 1] =
                    indexMap(tIndexI + 1, tIndexJ, tIndexK, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 2] =
                    indexMap(tIndexI + 1, tIndexJ + 1, tIndexK, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 3] =
                    indexMap(tIndexI, tIndexJ + 1, tIndexK, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 4] =
                    indexMap(tIndexI, tIndexJ, tIndexK + 1, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 5] =
                    indexMap(tIndexI + 1, tIndexJ, tIndexK + 1, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 6] =
                    indexMap(tIndexI + 1, tIndexJ + 1, tIndexK + 1, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 7] =
                    indexMap(tIndexI, tIndexJ + 1, tIndexK + 1, aSpec.numX + 1, aSpec.numY + 1, aSpec.numZ + 1);
            }
    return tConnect;
}

IArray getNodes(const MeshSpec& aSpec, Uint aFromX, Uint aFromY, Uint aFromZ, Uint aToX, Uint aToY, Uint aToZ)
{
...
SideSet getSides(
    const MeshSpec& aSpec, std::string face, Uint aFromX, Uint aFromY, Uint aFromZ, Uint aNumX, Uint aNumY, Uint aNumZ)
{
    SideSet tSideSet;
...
IArray generateConnectivity(const MeshSpec& aSpec)
{
    ...
    for (decltype(aSpec.numX) tIndexI = 0; tIndexI < aSpec.numX; tIndexI++)
        for (decltype(aSpec.numY) tIndexJ = 0; tIndexJ < aSpec.numY; tIndexJ++)
            ...
            {
                int tElIndex = indexMap(tIndexI, tIndexJ, tIndexK, aSpec.numX, aSpec.numY, aSpec.numZ);
                tConnect[tElIndex * cNumNPE + 0] = indexMap(2 * tIndexI, 2 * tIndexJ, 2 * tIndexK, 2 * aSpec.numX + 1,
                                                            2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 1] = indexMap(2 * tIndexI + 2, 2 * tIndexJ, 2 * tIndexK,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 2] = indexMap(2 * tIndexI + 2, 2 * tIndexJ + 2, 2 * tIndexK,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 3] = indexMap(2 * tIndexI, 2 * tIndexJ + 2, 2 * tIndexK,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 4] = indexMap(2 * tIndexI, 2 * tIndexJ, 2 * tIndexK + 2,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 5] = indexMap(2 * tIndexI + 2, 2 * tIndexJ, 2 * tIndexK + 2,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 6] = indexMap(2 * tIndexI + 2, 2 * tIndexJ + 2, 2 * tIndexK + 2,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 7] = indexMap(2 * tIndexI, 2 * tIndexJ + 2, 2 * tIndexK + 2,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 8] = indexMap(2 * tIndexI + 1, 2 * tIndexJ, 2 * tIndexK,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 9] = indexMap(2 * tIndexI + 2, 2 * tIndexJ + 1, 2 * tIndexK,
                                                            2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 10] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ + 2, 2 * tIndexK, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 11] =
                    indexMap(2 * tIndexI, 2 * tIndexJ + 1, 2 * tIndexK, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 12] =
                    indexMap(2 * tIndexI, 2 * tIndexJ, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 13] =
                    indexMap(2 * tIndexI + 2, 2 * tIndexJ, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 14] =
                    indexMap(2 * tIndexI + 2, 2 * tIndexJ + 2, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 15] =
                    indexMap(2 * tIndexI, 2 * tIndexJ + 2, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 16] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ, 2 * tIndexK + 2, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 17] =
                    indexMap(2 * tIndexI + 2, 2 * tIndexJ + 1, 2 * tIndexK + 2, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 18] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ + 2, 2 * tIndexK + 2, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 19] =
                    indexMap(2 * tIndexI, 2 * tIndexJ + 1, 2 * tIndexK + 2, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 20] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ + 1, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 21] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ + 1, 2 * tIndexK, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 22] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ + 1, 2 * tIndexK + 2, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 23] =
                    indexMap(2 * tIndexI, 2 * tIndexJ + 1, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 24] =
                    indexMap(2 * tIndexI + 2, 2 * tIndexJ + 1, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);

                tConnect[tElIndex * cNumNPE + 25] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
                tConnect[tElIndex * cNumNPE + 26] =
                    indexMap(2 * tIndexI + 1, 2 * tIndexJ + 2, 2 * tIndexK + 1, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1,
                             2 * aSpec.numZ + 1);
            }
    return tConnect;
}
    ...
    for (Uint tIndexI = 2 * aFromX; tIndexI <= 2 * aToX; tIndexI++)
        for (Uint tIndexJ = 2 * aFromY; tIndexJ <= 2 * aToY; tIndexJ++)
            for (Uint tIndexK = 2 * aFromZ; tIndexK <= 2 * aToZ; tIndexK++)
                tNodes[tIndex++] =
                    indexMap(tIndexI, tIndexJ, tIndexK, 2 * aSpec.numX + 1, 2 * aSpec.numY + 1, 2 * aSpec.numZ + 1);
...
IArray getNodes(const MeshSpec& aSpec, Uint aFromX, Uint aFromY, Uint aToX, Uint aToY)
{
...
SideSet getSides(const MeshSpec& aSpec, std::string face, Uint aFromX, Uint aFromY, Uint aNumX, Uint aNumY)
{
    SideSet tSideSet;
...
IArray getNodes(const MeshSpec& aSpec, Uint aFromX, Uint aToX)
{
    ...
    Uint tIndex = 0;
    for (Uint tIndexI = aFromX; tIndexI <= aToX; tIndexI++) tNodes[tIndex++] = indexMap(tIndexI, aSpec.numX + 1);
...
Array2D generateCoords(const MeshSpec& aSpec) { return Hex8::generateCoords(aSpec); }
IArray generateConnectivity(const MeshSpec& aSpec)
{
...
IArray fromHex8(const IArray& aHex8Conn)
{
    ...
    for (decltype(tNumHex8Elems) iHex = 0; iHex < tNumHex8Elems; iHex++)
    {
        for (Uint iLocalTet = 0; iLocalTet < cTetPerHex; iLocalTet++)
        {
            ...
            {
                tTet4Conn[iTotalTets * Tet4::cNumNPE + iLocalVert] =
                    aHex8Conn[iHex * Hex8::cNumNPE + cH2T[iLocalTet][iLocalVert]];
            }
            iTotalTets++;
        }
    }
...
IArrayMap generateNodeSets(const MeshSpec& aSpec) { return Hex8::generateNodeSets(aSpec); }

SideSetMap fromHex8(const SideSetMap& aHex8SideSets)
{
    SideSetMap tSideSets;

    std::map<Uint, const IArray2D> cH2T = {{1, {{3, 4}, {4, 4}}}, {2, {{4, 2}, {5, 2}}}, {3, {{0, 2}, {1, 2}}},
                                           {4, {{1, 4}, {2, 4}}}, {5, {{0, 4}, {5, 4}}}, {6, {{2, 2}, {3, 2}}}};
...
Array2D generateCoords(const MeshSpec& aSpec) { return Hex27::generateCoords(aSpec); }
IArray generateConnectivity(const MeshSpec& aSpec)
{
...
IArray fromHex27(const IArray& aHex27Conn)
{
    const IArray2D cH2T = {{0, 2, 3, 6, 21, 10, 11, 20, 14, 26}, {0, 3, 7, 6, 11, 15, 23, 20, 26, 18},
                           {0, 7, 4, 6, 23, 19, 12, 20, 18, 22}, {0, 4, 5, 6, 12, 16, 25, 20, 22, 17},
                           {0, 5, 1, 6, 25, 13, 8, 20, 17, 24},  {0, 1, 2, 6, 8, 9, 21, 20, 24, 14}};
    ...
    for (decltype(tNumHex27Elems) iHex = 0; iHex < tNumHex27Elems; iHex++)
    {
        for (Uint iLocalTet = 0; iLocalTet < cTetPerHex; iLocalTet++)
        {
            ...
            {
                tTet10Conn[iTotalTets * Tet10::cNumNPE + iLocalVert] =
                    aHex27Conn[iHex * Hex27::cNumNPE + cH2T[iLocalTet][iLocalVert]];
            }
            iTotalTets++;
        }
    }
...
IArrayMap generateNodeSets(const MeshSpec& aSpec) { return Hex27::generateNodeSets(aSpec); }

SideSetMap fromHex(const SideSetMap& aHexSideSets)
{
    SideSetMap tSideSets;
...
Array2D generateCoords(const MeshSpec& aSpec) { return Quad4::generateCoords(aSpec); }
IArray generateConnectivity(const MeshSpec& aSpec)
{
...
IArray fromQuad4(const IArray& aQuad4Conn)
{
    ...
    for (decltype(tNumQuad4Elems) iHex = 0; iHex < tNumQuad4Elems; iHex++)
    {
        for (Uint iLocalTet = 0; iLocalTet < cTetPerHex; iLocalTet++)
        {
            ...
            {
                tTri3Conn[iTotalTets * Tri3::cNumNPE + iLocalVert] =
                    aQuad4Conn[iHex * Quad4::cNumNPE + cQ2T[iLocalTet][iLocalVert]];
            }
            iTotalTets++;
        }
    }
...
IArrayMap generateNodeSets(const MeshSpec& aSpec) { return Quad4::generateNodeSets(aSpec); }

SideSetMap fromQuad4(const SideSetMap& aQuad4SideSets)
{
    SideSetMap tSideSets;

    std::map<Uint, const IArray2D> cQ2T = {{1, {{0, 1}}}, {2, {{0, 2}}}, {3, {{1, 2}}}, {4, {{1, 3}}}};
...
Uint getNumNPE(const MeshSpec& aSpec) { BAMG_COMPUTE(getNumNPE); }

Array2D generateCoords(const MeshSpec& aSpec) { BAMG_COMPUTE(generateCoords); }

IArray generateConnectivity(const MeshSpec& aSpec) { BAMG_COMPUTE(generateConnectivity); }

IArrayMap generateNodeSets(const MeshSpec& aSpec) { BAMG_COMPUTE(generateNodeSets); }

SideSetMap generateSideSets(const MeshSpec& aSpec) { BAMG_COMPUTE(generateSideSets); }
